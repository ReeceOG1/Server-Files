...
$(document).ready(function() {
    window.addEventListener('message', function(event) {
        if (event.data.action == 'setVisible') {
            ...
            $('#' + event.data.tab).show()
            $.post('https://FDMBarbers/setCurrentTab', JSON.stringify({
                tab: event.data.tab,
            }));
        } 
...
function setView(event, view) {
    ...
    if (!wasActive) {
        $.post('https://FDMBarbers/setCameraView', JSON.stringify({
            view: view
        }));
        $.post('https://FDMBarbers/playSound', JSON.stringify({
...
$('#cameracontrol').on('mousemove', function(event) {
    if (moving == true) {
        ...
        lastOffsetX = offsetX;
        lastOffsetY = offsetY;
        $.post('https://FDMBarbers/updateCameraRotation', JSON.stringify({
            x: offsetX,
            y: offsetY,
        }));
    }
...
$('#cameracontrol').on('wheel', function(event) {
    ...
    $.post('https://FDMBarbers/updateCameraZoom', JSON.stringify({
        zoom: zoom,
    }));
});
...
function closeWindow(save) {
    $.post('https://FDMBarbers/close', JSON.stringify({
        save: save
    }));
}
...
$('.panelbottom button').on('click', function(evt) {
    evt.preventDefault();
    $.post('https://FDMBarbers/playSound', JSON.stringify({sound:'panelbuttonclick'}));
});
...
function updateGender(value) {
    $.post('https://FDMBarbers/updateGender', JSON.stringify({
        value: value,
    }));
}

function updateHeadBlend(key, value) {
    $.post('https://FDMBarbers/updateHeadBlend', JSON.stringify({
        key: key,
        value: value,
    }));
}
...
function updateFaceFeature(key, value, index) {
    $.post('https://FDMBarbers/updateFaceFeature', JSON.stringify({
        key: key,
        value: value,
        index: index,
    }));
...
function updateEyeColor(value) {
    $.post('https://FDMBarbers/updateEyeColor', JSON.stringify({
        value: value,
    }));
}

function updateHairColor(key, value, highlight) {
    $.post('https://FDMBarbers/updateHairColor', JSON.stringify({
        key: key,
        value: value,
        highlight: highlight,
    }));
...
function updateHeadOverlay(key, keyPaired, value, index, isOpacity) {
    $.post('https://FDMBarbers/updateHeadOverlay', JSON.stringify({
        key: key,
...
function updateHeadOverlayExtra(key, keyPaired, value, index, keyExtra, valueExtra, indexExtra, isOpacity) {
    $.post('https://FDMBarbers/updateHeadOverlayExtra', JSON.stringify({
        key: key,
        keyPaired: keyPaired,
        value: value,
        index: index,
...
function updateOverlayColor(key, value, index, colortype) {
    $.post('https://FDMBarbers/updateOverlayColor', JSON.stringify({
        key: key,
        value: value,
        index: index,
        colortype: colortype,
...
function updateComponent(drawable, dvalue, texture, tvalue, index) {
    $.post('https://FDMBarbers/updateComponent', JSON.stringify({
        drawable: drawable,
        dvalue: dvalue,
        texture: texture,
        tvalue: tvalue,
...
function updateApparelComponent(drwkey, drwval, texkey, texval, cmpid) {
    $.post('https://FDMBarbers/updateApparelComponent', JSON.stringify({
        drwkey: drwkey,
        drwval: drwval,
        texkey: texkey,
        texval: texval,
...
function updateApparelProp(drwkey, drwval, texkey, texval, propid) {
    $.post('https://FDMBarbers/updateApparelProp', JSON.stringify({
        drwkey: drwkey,
        drwval: drwval,
        texkey: texkey,
        texval: texval,
...
$(document).on('click', '.colors span', function(evt) {
    let skinColour = $(this).attr("id")
    if (skinColour == "black") {
        $.post('https://FDMBarbers/updateHeadBlend', JSON.stringify({value: 14}));
    }
    if (skinColour == "white") {
        $.post('https://FDMBarbers/updateHeadBlend', JSON.stringify({value: 12}));
    }
    if (skinColour == "lightskin") {
        $.post('https://FDMBarbers/updateHeadBlend', JSON.stringify({value: 19}));
    }
});
...
$(document).on('change', 'select.facepaintoverlay', function(evt) {
    ...
    if (prevtype != type) {
        $.post('https://FDMBarbers/syncFacepaintOpacity', JSON.stringify({
            prevtype: prevtype,
            currenttype: type,
        }));
        ...
        $.post('https://FDMBarbers/clearMakeup', JSON.stringify({
            clearopacity: false,
            clearblusher: true,
        }));
    }
...
$(document).on('change', 'select.makeuptype', function(evt) {
    ...
    $.post('https://FDMBarbers/updateMakeupType', JSON.stringify({type:value}));
});
