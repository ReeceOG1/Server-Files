...
Citizen.CreateThread(function()
    while true do
        if not e then
            local h = GetEntityCoords(tNOVA.getPlayerPed())
...
function sitDownAtBar(l)
    ...
    tNOVA.loadAnimDict(a)
    followMachineVector = m
    followMachineRotation = n
    TaskGoStraightToCoord(tNOVA.getPlayerPed(),followMachineVector.x,followMachineVector.y,followMachineVector.z,1.0,50000,312.2,10.0)
    local h = GetEntityCoords(tNOVA.getPlayerPed())
    while #(h - followMachineVector) >= 1.25 do
        h = GetEntityCoords(tNOVA.getPlayerPed())
        Wait(100)
    end
...
function createSitAtBarScene(o, p)
    ...
    NetworkAddPedToSynchronisedScene(tNOVA.getPlayerPed(), q, a, animName, 2.0, -1000, 9, 0, 1148846080, 0)
...
Citizen.CreateThread(function()
    while true do
        if insideDiamondCasino then
            if e then
                SetPedCapsule(tNOVA.getPlayerPed(), 0.13)
                if IsControlJustPressed(0, 202) then
                    tNOVA.loadAnimDict("anim@amb@office@seating@male@var_a@base@")
                    TaskPlayAnim(tNOVA.getPlayerPed(),"anim@amb@office@seating@male@var_a@base@","exit",1.0,1.0,-1,0)
