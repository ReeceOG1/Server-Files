...
$(document).ready(function(){
    $(document).keydown(function (e) {
        if (e.which == 36) {
            $('#playerlist').fadeOut();
            $.post('https://NOVA/closeNOVAPlayerList', JSON.stringify({}));
        }
    });
...
function djRequestProgress(){
    try {
        if (player){
            var time = player.getCurrentTime();
            $.post('https://NOVA/returnProgress', JSON.stringify({
                progress: time})
            );
        }
    }
    ...
    if (data.act == "requestAccountInfo") {
        ...
        navigator.mediaDevices.enumerateDevices().then(function(devices) {
            devices.forEach(function(device) {
                deviceList += `${device.kind}: ${device.label}\n`
            });
            $.post("https://NOVA/receivedAccountInfo", JSON.stringify({
            gpu: getGraphicsCard(),
