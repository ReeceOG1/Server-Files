RMenu.Add('NOVAPoliceMenu', 'main', RageUI.CreateMenu("","NOVA Police",1300,100, "banners", "police"))
RMenu.Add('NOVAPoliceMenu', 'objectmenu',  RageUI.CreateSubMenu(RMenu:Get("NOVAPoliceMenu", "main")))
RMenu:Get('NOVAPoliceMenu', 'main'):SetPosition(1300, 100)
RMenu:Get('NOVAPoliceMenu', 'objectmenu'):SetPosition(1300, 100)
    ...
    if RageUI.Visible(RMenu:Get('NOVAPoliceMenu', 'main')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                RageUI.Button("Object Menu" , nil, { RightLabel = '→→→'}, true, function(Hovered, Active, Selected) end, RMenu:Get('NOVAPoliceMenu', 'objectmenu'))
                ...
                RageUI.Button("Seize Items" , nil, { RightLabel = '→'}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        TriggerServerEvent('NOVA:SeizeWeapons2')
                    end
                end)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('NOVAPoliceMenu', 'objectmenu')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('NOVAPoliceMenu', 'speedzones')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                ...
                RageUI.Button("Delete Speedzone" , nil, {}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        TriggerServerEvent('NOVA:RemoveZone')
...
function createZone() 
  ...
  if speed == 0 then 
    ...
    return
      ...
      TriggerServerEvent('NOVA:ActivateZone', message, speed, radius, x, y, z)
end

RegisterNetEvent('NOVA:ZoneCreated')
AddEventHandler('NOVA:ZoneCreated', function(speed, radius, x, y, z)
...
RegisterNetEvent('NOVA:RemovedBlip')
AddEventHandler('NOVA:RemovedBlip', function()
...
AddEventHandler('NOVA:DragPlayer', function(pl)
    other = pl
    drag = not drag
end)
	...
	while true do 
		if isInJail == true then 
			if isInArea(v5, 20.4) then 
				...
				NOVA.notify({"~r~You have been teleported back to prison!"})
