...
local function DriveOutOfGarage(pos)
	Citizen.CreateThread(function()
		...
		local ok, veh_type, model = tNOVA.getNearestOwnedVehicle(1)
		myveh.model = model
...
AddEventHandler("playerSpawned",function(D, E)
    if E then
        ...
        for q,h in pairs(garages)do 
            tNOVA.addBlip(h.inside.position.x,h.inside.position.y,h.inside.position.z,72, nil, "LS Customs")
        end 
	end
end)
...
function tNOVA.isInsideLsCustoms()
    return inside
end
