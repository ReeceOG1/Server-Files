RMenu.Add('CARDEV', 'main', RageUI.CreateMenu("","~r~Car Dev",1250,100, "banners", "cardev"))
...
local m = module("nova-cars", "cfg/garages")
m=m.garage_types
                ...
                RageUI.ButtonWithStyle("Cycle through seats" , nil, { RightLabel = '→→→'}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        ...
                        if IsPedInAnyVehicle(p) then
                            ...
                            tNOVA.notify("~r~Not in a vehicle.")
                        end
                    end
                end)
...
function TakeVehScreenshot()
    ...
    Wait(3000)
    TriggerServerEvent('NOVA:takeCarScreenshot')
    Wait(5000)
