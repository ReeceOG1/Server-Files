...
function tNOVA.toggleHandcuff()
  ...
  if handcuffed then
    ...
    tNOVA.playAnim(true,{{"mp_arresting","idle",1}},true)
  else
    DeleteEntity(cuffs)
    tNOVA.stopAnim(true)
...
function tNOVA.setHandcuffed(flag)
  if handcuffed ~= flag then
    tNOVA.toggleHandcuff()
  end
end

function tNOVA.isHandcuffed()
  return handcuffed
end

-- (experimental, based on experimental getNearestVehicle)
function tNOVA.putInNearestVehicleAsPassenger(radius)
...
function tNOVA.putInNetVehicleAsPassenger(net_veh)
...
function tNOVA.putInVehiclePositionAsPassenger(x,y,z)
  local veh = tNOVA.getVehicleAtPosition(x,y,z)
  if IsEntityAVehicle(veh) then
    for i=1,GetVehicleMaxNumberOfPassengers(veh) do
      if IsVehicleSeatFree(veh,i) then
        SetPedIntoVehicle(GetPlayerPed(-1),veh,i)
...
function tNOVA.jail(x,y,z,radius)
  tNOVA.teleport(x,y,z) -- teleport to center
  jail = {x+0.0001,y+0.0001,z+0.0001,radius+0.0001}
end

-- unjail the player
function tNOVA.unjail()
  jail = nil
end

function tNOVA.isJailed()
  return jail ~= nil
end
