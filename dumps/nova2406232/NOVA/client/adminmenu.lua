NOVAclient = Tunnel.getInterface("NOVA","NOVA")
...
local p
function tNOVA.getShowHealthPercentageFlag()
    return h
end
function tNOVA.setShowHealthPercentageFlag(p)
            ...
            RageUI.Checkbox("Create Lightning","",U.lightning,{},function(a2, a4, a3, a8)
             ...
             RageUI.List("~y~Particles~w~",R,U.particle,"",{},true,function(a2, a3, a4, a5)
                ...
                if a3 and U.particle ~= a5 then
                   ...
                   if not tNOVA.isPlusClub() and not tNOVA.isPlatClub() then

                      tNOVA.notify("~y~You need to be a subscriber of NOVA Plus or NOVA Platinum to use this feature.")

                      tNOVA.notify("~y~Available @ store.NOVAstudios.uk")
             ...
             if GetGameTimer() - V > aj + 1000 then
                ...
                tNOVA.addKillEffect(PlayerPedId(), true)
    ...
    if RageUI.Visible(RMenu:Get('SettingsMenu', 'uisettings')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            if (Selected) then
                if Checked then
                    h = true
                	tNOVA.setShowHealthPercentageFlag(true)
                else
                    h = false
                	tNOVA.setShowHealthPercentageFlag(false)              
                end
            end
        end)
...
AddEventHandler("NOVA:SpawnWeaponC",function(spawncode)
    if Stafflevel >= 11 then
        NOVAclient.allowWeapon({spawncode, "-1"})
...
AddEventHandler('NOVA:Spectate', function(plr,tpcoords)
    ...
    if not Spectating then
        ...
        if targetPed == playerPed then tNOVA.notify('~r~I mean you cannot spectate yourself...') return end
        ...
        tNOVA.notify('~g~Spectating Player.')
        ...
        tNOVA.notify('~r~Stopped Spectating Player.')
    end 
end)
