...
local weapons = module("cfg/cfg_weapons")
local loadoutCoords = {
...
RMenu.Add("policeloadouts","main",RageUI.CreateMenu("","Please Select Division",1300,100,"banners","gunstore"))
RMenu.Add("policeloadouts","confirm",RageUI.CreateSubMenu(RMenu:Get("policeloadouts", "main"),"","Confirm Selection",1300,100))
    ...
    if RageUI.Visible(RMenu:Get('policeloadouts', 'confirm')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.Button("Confirm", nil, {RightBadge = "→→→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent('NOVA:selectLoadout', selectedLoadout)
...
RegisterNetEvent('NOVA:gotLoadouts')
AddEventHandler('NOVA:gotLoadouts', function(loadoutsTable)
    loadouts = loadoutsTable
end)

AddEventHandler("NOVA:onClientSpawn",function(p, q)
    if q then
        local r = function(s)
        end
        local t = function(s)
        ...
        local u = function(s)
            if IsControlJustPressed(1, 38) then
                TriggerServerEvent('NOVA:getPoliceLoadouts')
        ...
        for k,v in pairs(loadoutCoords) do
            tNOVA.createArea("police_loadouts_"..k, v, 1.5, 6, r, t, u, {locationId = k})
        end
    end
end)