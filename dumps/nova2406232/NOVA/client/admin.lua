...
Citizen.CreateThread(function()
    while true do
        ...
        if noclip then
            ...
            local dx, dy, dz = tNOVA.getCamDirection()
            ...
            DrawAdvancedText(0.320, 0.780, 0.025, 0.0048, 0.5, "NoClip is Enabled", 255, 255, 255, 200, 6, 0)
            DrawAdvancedText(0.320, 0.800, 0.025, 0.0048, 0.5, "Current Noclip Speed: "..currentSpeed, 255, 255, 255, 200, 6, 0)
            DrawAdvancedText(0.320, 0.820, 0.025, 0.0048, 0.5, "Faster: Left Shift", 255, 255, 255, 200, 6, 0)
            DrawAdvancedText(0.320, 0.840, 0.025, 0.0048, 0.5, "Slower: Left Alt", 255, 255, 255, 200, 6, 0)
        ...
        if EntityCleanupGun then 
            ...
            if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_STAFFGUN') then
                if IsPlayerFreeAiming(plr) then 
                    ...
                    if yes then 
                        tNOVA.notify('~g~Deleted Entity: ' .. GetEntityModel(entity))
...
local function teleportToWaypoint()
    ...
    Citizen.CreateThread(function()
        ...
        if success then
            ...
            DoScreenFadeIn(250)
            tNOVA.notify('~g~Teleported success!')
        end
    end)
end
...
AddEventHandler("NOVA:vehicleMenu",function(DioMode, isInTicket)
    if getStaffLevel() > 0 then
	OMioDioMode = DioMode
    ticketStatus = isInTicket
    TriggerEvent('greenzoneStaffMode', OMioDioMode)
	if not OMioDioMode then
		tNOVA.setCustomization(adminTicketSavedCustomization)
        ...
        adminTicketSavedCustomization = tNOVA.getCustomization()
        gender = getModelGender()
        if gender == "male" then
            z="mp_m_freemode_01"
            local A=loadModel(z)
            tNOVA.setCustomization({modelhash=A})
            Wait(100)
...
RegisterCommand("fix", function()
    if OMioDioMode or tNOVA.getUserId() == 2 or tNOVA.getUserId() == 1 then
...
AddEventHandler("NOVA:showBlips",function()
    blips = not blips
    if blips then
        tNOVA.notify("~g~Blips enabled")
    else
        tNOVA.notify("~r~Blips disabled")
...
RegisterNetEvent('NOVA:policeRemove')
AddEventHandler('NOVA:policeRemove', function()
    local source = source
