...
function tNOVA.removeBlipAtCoords(x,y,z) -- used to remove green house blips for everyone when purchased
...
Citizen.CreateThread(function()
    while true do
        q = tNOVA.getNearbyMarkers()
        Citizen.Wait(10000)
    end
end)
function tNOVA.getNearbyMarkers()
    local N = {}
    local O = getPlayerCoords()
    local P = 0
...
function tNOVA.setArea(l, a, b, c, T, U)
...
function tNOVA.createArea(l, W, T, U, X, Y, Z, _)
...
function tNOVA.removeArea(l)
    if Q[l] then
        Q[l] = nil
    end
end
function tNOVA.doesAreaExist(l)
    if Q[l] then
        return true
    end
    return false
