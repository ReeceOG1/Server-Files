...
function tNOVA.setRedzoneTimerDisabled(e)
    a = e
end
function tNOVA.isPlayerInRedZone()
    return globalInRedzone
end
local f = 0
function tNOVA.setPlayerCombatTimer(g, h)
    b = g
    if h then
        c = true
    end
...
function tNOVA.getPlayerCombatTimer()
    return b, c
end
...
Citizen.CreateThread(function()
    while true do
        if not a then
            local r = GetEntityCoords(tNOVA.getPlayerPed())
            globalInRedzone = false
            for s, k in pairs(d) do
                if i(r, k, false) then
                    globalInRedzone = true
                    local r = GetEntityCoords(tNOVA.getPlayerPed())
                    tNOVA.setPlayerCombatTimer(30, false)
                    local t
                    local u = false
                    while not u do
                        r = GetEntityCoords(tNOVA.getPlayerPed())
                        while i(r, k, true) do
                            r = GetEntityCoords(tNOVA.getPlayerPed())
                            t = r
                            if IsPedShooting(tNOVA.getPlayerPed()) and GetSelectedPedWeapon(tNOVA.getPlayerPed()) ~= GetHashKey("WEAPON_UNARMED") then
                                tNOVA.setPlayerCombatTimer(60, true)
                            end
                            if b == 0 then
                        ...
                        if b == 0 then
                            ...
                            local v = k.pos - GetEntityCoords(tNOVA.getPlayerPed())
                            t = t + v * 0.01
                            if GetVehiclePedIsIn(tNOVA.getPlayerPed(), false) == 0 then
                                TaskGoStraightToCoord(tNOVA.getPlayerPed(),t.x,t.y,t.z,8.0,1000,GetEntityHeading(tNOVA.getPlayerPed()),0.0)
                                local w = GetSoundId()
                                PlaySoundFrontend(w, "End_Zone_Flash", "DLC_BTL_RB_Remix_Sounds", true)
                                ReleaseSoundId(w)
                                tNOVA.announceMpBigMsg("~r~WARNING", "Get back in the redzone!", 2000)
                            else
                                SetEntityCoords(tNOVA.getPlayerPed(), t.x, t.y, t.z)
                            end
                            SetTimeout(1000,function()
                                ClearPedTasks(tNOVA.getPlayerPed())
                            end)
                        end
                        Wait(0)
                    end
...
Citizen.CreateThread(function()
    while true do
        if b > 0 then
            if a then
                tNOVA.setPlayerCombatTimer(0, false)
            else
                b = b - 1
                if b == 0 then
                    c = false
...
function tNOVA.isEmergencyService()
...
AddEventHandler("NOVA:startCombatTimer",function(h)
    if not tNOVA.isEmergencyService() then --and not tNOVA.isInPaintball() then
        tNOVA.setPlayerCombatTimer(60, h)
    end
end)
local function y()
    if not tNOVA.isEmergencyService() and not tNOVA.isInComa() then --and not tNOVA.isInPaintball() then
        local z = PlayerPedId()
        ...
        if IsPedShooting(z) and not x[A] then
            tNOVA.setPlayerCombatTimer(60, true)
        elseif GetPlayerTargetEntity(tNOVA.getPlayerId()) and IsControlPressed(0, 24) then
            tNOVA.setPlayerCombatTimer(60, true)
        end
    end
    if b > 0 then
...
local function B()
    ...
    if GetSelectedPedWeapon(z) == GetHashKey("WEAPON_MOLOTOV") then
        tNOVA.setWeapon(z, "WEAPON_UNARMED", true)
    end
end
local function C()
