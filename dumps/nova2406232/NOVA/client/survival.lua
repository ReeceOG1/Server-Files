        ...
        if in_coma then
			...
			if not calledNHS then
				...
				if IsControlJustPressed(1, 51) then
					...
					tNOVA.notify('~g~NHS called to your approximate location')
...
Citizen.CreateThread(function() -- coma thread
    ...
    while true do
        ...
        if IsEntityDead(GetPlayerPed(-1)) and not in_coma then --Wait for death check
            ...
            NOVAserver.StoreWeaponsDead()

            NOVAserver.Coma()

            TriggerEvent('NOVA:IsInMoneyComa', true)

            TriggerServerEvent('NOVA:InComa')

            NOVAserver.MoneyDrop()
            ...
            TriggerEvent("NOVA:PlaySound", "dead")
    ...
    Citizen.CreateThread(function()
        ...
        while true do 
            ...
            if resurrectspamm == true then
                ...
                tNOVA.stopScreenEffect(cfg.coma_effect)
...
RegisterNetEvent("NOVA:getNumberOfDocsOnline",function(I)
    ...
    CreateThread(function()
        ...
        while tNOVA.getKillerInfo().ready == nil do

            Wait(0)

        end

        local L = tNOVA.getKillerInfo()
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsEntityDead(PlayerPedId()) then
            ...
            local az = tNOVA.getPedServerId(PedKiller)
            ...
            L.user_id = tNOVA.getPedServerId(PedKiller)
            ...
            if not g and in_coma and killfeed then
                ...
                TriggerServerEvent("NOVA:onPlayerKilled", "finished off", tNOVA.getPedServerId(PedKiller), s)

            else

                TriggerServerEvent("NOVA:onPlayerKilled", "killed", tNOVA.getPedServerId(PedKiller), R, a6, distance)
...
function tNOVA.getPedServerId(a5)
...
function tNOVA.reviveComa()

    local ped = GetPlayerPed(-1)

    SetEntityInvincible(ped,false)

    tNOVA.setRagdoll(false)
...
function tNOVA.varyHealth(variation)
...
function tNOVA.setHealth(health)
...
function tNOVA.setArmour(armour)
...
function tNOVA.setFriendlyFire(flag)
