...
RegisterCommand("carry",function(source, args)
    if not carry.InProgress then
        ...
        if closestPlayer then
            ...
            if targetSrc ~= -1 then
                TriggerServerEvent("NOVA:CarryRequest", targetSrc, OMioDioMode)
            else
...
RegisterNetEvent("NOVAEXTRAS:StartCarry")
AddEventHandler("NOVAEXTRAS:StartCarry", function(targetSrc)
...
RegisterNetEvent("NOVAEXTRAS:CarryTargetAsk")
AddEventHandler("NOVAEXTRAS:CarryTargetAsk", function(senderSrc)
    carryrequest = true
    Citizen.CreateThread(function()
        while carryrequest do
            if IsControlJustPressed(1, 83) then
                TriggerServerEvent("NOVAEXTRAS:CarryAccepted",senderSrc)
                carryrequest = false
            elseif IsControlJustPressed(1, 84) then
                TriggerServerEvent("NOVAEXTRAS:CarryDeclined",senderSrc)
                carryrequest = false
            end
            Wait(0)
        end
