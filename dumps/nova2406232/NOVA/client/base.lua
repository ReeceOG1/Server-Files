...
cfgweapons = module("nova-weapons", "cfg/cfg_weaponNames")
local user_id = 0
tNOVA = {}
allowedWeapons = {}
...
Tunnel.bindInterface("NOVA",tNOVA)

-- get server interface
NOVAserver = Tunnel.getInterface("NOVA","NOVA")

-- add client proxy interface (same as tunnel interface)
Proxy.addInterface("NOVA",tNOVA)



-- functions
function tNOVA.teleport(x,y,z)
  tNOVA.unjail() -- force unjail before a teleportation
  SetEntityCoords(GetPlayerPed(-1), x+0.0001, y+0.0001, z+0.0001, 1,0,0,1)
  NOVAserver.updatePos({x,y,z})
end

-- return x,y,z
function tNOVA.getPosition()
  local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
  return x,y,z
end

--returns the distance between 2 coordinates (x,y,z) and (x2,y2,z2)
function tNOVA.getDistanceBetweenCoords(x,y,z,x2,y2,z2)
...
function tNOVA.isInside()
  local x,y,z = tNOVA.getPosition()
  return not (GetInteriorAtCoords(x,y,z) == 0)
end

-- return vx,vy,vz
function tNOVA.getSpeed()
  local vx,vy,vz = table.unpack(GetEntityVelocity(GetPlayerPed(-1)))
  return math.sqrt(vx*vx+vy*vy+vz*vz)
end

function tNOVA.getCamDirection()
...
function tNOVA.addPlayer(player)
  players[player] = true
end

function tNOVA.removePlayer(player)
  players[player] = nil
end

function tNOVA.getNearestPlayers(radius)
  local r = {}

  local ped = GetPlayerPed(i)
  local pid = PlayerId()
  local px,py,pz = tNOVA.getPosition()
...
function tNOVA.getNearestPlayer(radius)
  local p = nil

  local players = tNOVA.getNearestPlayers(radius)
...
function tNOVA.notify(msg)
  SetNotificationTextEntry("STRING")
  AddTextComponentString(msg)
  DrawNotification(true, false)
end

function tNOVA.notifyPicture(icon, type, sender, title, text)
    SetNotificationTextEntry("STRING")
...
function tNOVA.playScreenEffect(name, duration)
...
-- name, see https://wiki.fivem.net/wiki/Screen_Effects
function tNOVA.stopScreenEffect(name)
  StopScreenEffect(name)
end
...
function tNOVA.playAnim(upper, seq, looping)
  if seq.task ~= nil then -- is a task (cf https://github.com/ImagicTheCat/NOVA/pull/118)
    tNOVA.stopAnim(true)
...
function tNOVA.setRagdoll(flag)
  ragdoll = flag
end
...
function tNOVA.playSpatializedSound(dict,name,x,y,z,range)
  PlaySoundFromCoord(-1,name,x+0.0001,y+0.0001,z+0.0001,dict,0,range+0.0001,0)
end

-- play sound
function tNOVA.playSound(dict,name)
  PlaySound(-1,name,dict,0,0,1)
end

--[[
-- not working
function tNOVA.setMovement(dict)
  if dict then
...
AddEventHandler("playerSpawned",function()
  TriggerServerEvent("NOVAcli:playerSpawned")
  TriggerServerEvent("NOVA:gettingUserID")
...
AddEventHandler("onPlayerDied",function(player,reason)
  TriggerServerEvent("NOVAcli:playerDied")
end)
...
TriggerServerEvent('NOVA:CheckID')

RegisterNetEvent('NOVA:CheckIdRegister')
AddEventHandler('NOVA:CheckIdRegister', function()
    TriggerEvent('playerSpawned', GetEntityCoords(PlayerPedId()))
end)

RegisterNetEvent("NOVA:setUserID")
AddEventHandler("NOVA:setUserID", function(serverid)
  user_id = serverid
end)
...
function tNOVA.isGrinder(user_id) 
  return grind[user_id] or false
end

function tNOVA.isDeveloper(user_id) 
  return developers[user_id] or false
end

function tNOVA.isDevMode(user_id) 
  return devmode[user_id] or false
end

function tNOVA.getUserId(val)
  if val==nil then
    return user_id
  end
end

function tNOVA.announceMpBigMsg(I,J,K)
...
function tNOVA.generateUUID(ar, am, type)
    if aj[ar] == nil then
        aj[ar] = {}
    end
    if type == nil then
...
function tNOVA.loadAnimDict(dict)
...
function tNOVA.getPlayerPed()
    return a
end
function tNOVA.getPlayerVehicle()
    return b, e
end
function tNOVA.getPlayerId()
    return c
end
function tNOVA.getPlayerCoords()
    return d
end

createThreadOnTick(savePlayerInfo)

function tNOVA.getClosestVehicle(bm)
  local br = tNOVA.getPlayerCoords()
  local bs = 100
  local bt = 100
