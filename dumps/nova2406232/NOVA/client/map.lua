globalBlips = {}
function tNOVA.addBlip(a, b, c, d, e, f, g, h)
...
function tNOVA.removeBlip(j)
    RemoveBlip(j)
end
local k = {}
function tNOVA.setNamedBlip(l, a, b, c, d, e, f, g)
    tNOVA.removeNamedBlip(l)
    k[l] = tNOVA.addBlip(a, b, c, d, e, f, g)
    return k[l]
end
function tNOVA.removeNamedBlip(l)
    if k[l] ~= nil then
        tNOVA.removeBlip(k[l])
        k[l] = nil
    end
end
function tNOVA.setGPS(a, b)
    SetNewWaypoint(a + 0.0001, b + 0.0001)
end
function tNOVA.setBlipRoute(j)
    SetBlipRoute(j, true)
end
local m = {}
...
local p = {}
local q = {}
function tNOVA.addMarker(a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    local I = {
...
function tNOVA.setNamedMarker(l, a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    tNOVA.removeNamedMarker(l)
    o[l] = tNOVA.addMarker(a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    return o[l]
end
function tNOVA.removeNamedMarker(l)
    if o[l] ~= nil then
        tNOVA.removeMarker(o[l])
        o[l] = nil
    end
end
local J = {}
...
-- return marker id
function tNOVA.addMarker(x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
...
-- remove marker
function tNOVA.removeMarker(id)
...
-- return id
function tNOVA.setNamedMarker(name,x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
  tNOVA.removeNamedMarker(name) -- remove old marker

  named_markers[name] = tNOVA.addMarker(x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
  return named_markers[name]
end

function tNOVA.removeNamedMarker(name)
  if named_markers[name] ~= nil then
    tNOVA.removeMarker(named_markers[name])
...
function tNOVA.setArea(name,x,y,z,radius,height)
...
-- remove area
function tNOVA.removeArea(name)
...
-- locked: boolean
-- doorswing: -1 to 1
function tNOVA.setStateOfClosestDoor(doordef, locked, doorswing)
...
function tNOVA.openClosestDoor(doordef)
  tNOVA.setStateOfClosestDoor(doordef, false, 0)
end

function tNOVA.closeClosestDoor(doordef)
  tNOVA.setStateOfClosestDoor(doordef, true, 0)
end
