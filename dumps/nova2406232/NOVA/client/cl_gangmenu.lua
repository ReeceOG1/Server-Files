...
AddEventHandler("NOVA:GotGangData",function(i,j,k,PO)
    if i==nil then 
        PlayerIsInGang=false 
    else 
        PlayerIsInGang=true
...
RegisterNetEvent("NOVA:InviteRecieved")
AddEventHandler("NOVA:InviteRecieved",function(o,Q)
    NOVAGangInvites[NOVAGangInviteIndex]=Q
...
function func_drawGangUI()
    if showNoGangUI then 
        ...
        for p,q in pairs(NOVAGangInvites)do 
        ...
        if CursorInArea(0.435,0.51,0.415,0.46)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if selectedGangInvite~=nil then 
                    ...
                    TriggerServerEvent("NOVA:addUserToGang",selectedGangInvite,perm1s)
                    NOVAGangInvites={}
                    showNoGangUI=false
                    NOVAGangInviteIndex=0
                    showGangUI=true
    ...
    if showFundsGangUI then 
        ...
        DrawRect(0.501,0.374,0.421,0.047,0,168,255,248)
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if amount~=nil then 
                    if closeToBarclays()then 
                        TriggerServerEvent("NOVA:depositGangBalance",amount)
                    end 
                else 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if amount~=nil then 
                    if gangPermission>=3 then 
                        if closeToBarclays()then 
                            TriggerServerEvent("NOVA:withdrawGangBalance",amount)
                        end 
                    else 
    ...
    if showMembersGangUI then 
        ...
        DrawRect(0.501,0.308,0.421,0.047,0,168,255,248)
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if s~=nil then 
                    if gangPermission>=2 then 
                        TriggerServerEvent("NOVA:InviteUserToGang",gangID,s)
                    else 
    ...
    if showGangUI then 
        ...
        DrawRect(0.501,0.396,0.375,0.046,0,168,255,255)
...
RegisterNetEvent("NOVA:Notify")
AddEventHandler("NOVA:Notify",function(u)
    Notify(u)
end)
