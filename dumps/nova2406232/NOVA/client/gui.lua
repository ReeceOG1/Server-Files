...
-- pause
AddEventHandler("NOVA:pauseChange", function(paused)
  SendNUIMessage({act="pause_change", paused=paused})
end)

-- MENU

function tNOVA.openMenuData(menudata)
  SendNUIMessage({act="open_menu", menudata = menudata})
end

function tNOVA.closeMenu()
  SendNUIMessage({act="close_menu"})
end

-- PROMPT

function tNOVA.prompt(title,default_text)
...
function tNOVA.request(id,text,time)
...
RegisterNUICallback("menu",function(data,cb)
  if data.act == "close" then
    NOVAserver.closeMenu({data.id})
  elseif data.act == "valid" then
    NOVAserver.validMenuChoice({data.id,data.choice,data.mod})
  end
end)
...
RegisterNUICallback("prompt",function(data,cb)
  if data.act == "close" then
    SetNuiFocus(false)
    SetNuiFocus(false)
    NOVAserver.promptResult({data.result})
  end
end)
...
RegisterNUICallback("request",function(data,cb)
  if data.act == "response" then
    NOVAserver.requestResult({data.id,data.ok})
  end
end)
...
function tNOVA.announce(background,content)
...
-- try to fix missing cfg issue (cf: https://github.com/ImagicTheCat/NOVA/issues/89)
for i=1,5 do
...
function tNOVA.setProgressBar(name,anchor,text,r,g,b,value)
...
function tNOVA.setProgressBarValue(name,value)
  SendNUIMessage({act="set_pbar_val", name = name, value = value})
end

-- set progress bar text
function tNOVA.setProgressBarText(name,text)
  SendNUIMessage({act="set_pbar_text", name = name, text = text})
end

-- remove a progress bar
function tNOVA.removeProgressBar(name)
...
Citizen.CreateThread(function()
  while true do
    ...
    -- open general menu
    if IsControlJustPressed(table.unpack(cfg.controls.phone.open)) and (not tNOVA.isInComa() or not cfg.coma_disable_menu) and (not tNOVA.isHandcuffed() or not cfg.handcuff_disable_menu) and GetLastInputMethod(0) then 
      NOVAserver.openMainMenu({})
    end
    ...
    if pause_menu and not paused then
      paused = true
      TriggerEvent("NOVA:pauseChange", paused)
