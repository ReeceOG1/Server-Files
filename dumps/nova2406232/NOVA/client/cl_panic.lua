...
tNOVA.hasPoliceJob = function(bool)
    police = bool
end

tNOVA.PanicCL = function(player, s1, s2, name)
    if police then
...
tNOVA.BlipCL = function(tx, ty, tz)
    if police then
    ...
    while true do
        if showPanicBlip and police and not panic then
            ...
            if tostring(GetStreetNameFromHashKey(streetA)) and tostring(GetStreetNameFromHashKey(streetB)) then
                ...
                NOVAserver.PanicSV({player,  GetStreetNameFromHashKey(streetA), GetStreetNameFromHashKey(streetB)})
                NOVAserver.BlipSV({plyPos.x, plyPos.y, plyPos.z})
            else
                NOVAserver.PanicSV({player,  "No street found", ""})
                NOVAserver.BlipSV({plyPos.x, plyPos.y, plyPos.z})
            end
        end
