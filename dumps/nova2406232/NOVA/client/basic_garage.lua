...
function tNOVA.spawnGarageVehicle(vtype, name, pos, vehiclename, vehicleplate) -- vtype is the vehicle type (one vehicle per type allowed at the same time)
    ...
    if vehicle == nil then
        ...
        if HasModelLoaded(mhash) then
            ...
            TriggerServerEvent("NOVA:PayVehicleTax")
        ...
        tNOVA.notify("~r~Vehicle " ..vehiclename.. " is already out.")
    end
end

function tNOVA.despawnGarageVehicle(vtype, max_range)
    local Vehicle = tNOVA.getNearestVehicle(3)
...
function tNOVA.getNearestVehicl2()
    local ped = GetPlayerPed(-1)
    if IsPedSittingInAnyVehicle(ped) then
...
function tNOVA.fixeNearestVehicle(radius)
    local veh = tNOVA.getNearestVehicle(radius)
    if IsEntityAVehicle(veh) then
        SetVehicleFixed(veh)
    end
end

function tNOVA.replaceNearestVehicle(radius)
    local veh = tNOVA.getNearestVehicle(radius)
    if IsEntityAVehicle(veh) then
...
function tNOVA.getVehicleAtPosition(x, y, z)
    x = x + 0.0001
    y = y + 0.0001
    z = z + 0.0001
...
function tNOVA.getNearestOwnedVehicle(radius)
...
-- return ok,x,y,z
function tNOVA.getAnyOwnedVehiclePosition()
    for k, v in pairs(vehicles) do
...
-- return x,y,z
function tNOVA.getOwnedVehiclePosition(vtype)
...
function tNOVA.getOwnedVehicleId(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
...
-- vehicle commands
function tNOVA.vc_openDoor(hash, door_index)
...
function tNOVA.vc_closeDoor(hash, door_index)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tNOVA.vc_detachTrailer(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tNOVA.vc_detachTowTruck(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tNOVA.vc_detachCargobob(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tNOVA.vc_toggleEngine(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tNOVA.vc_toggleLock(hash)
    local vehicle = vehicles[hash]
    if vehicle then
        local veh = vehicle[3]
        ...
        if locked then -- unlock
            ...
            tNOVA.notify("~r~Vehicle unlocked.")
        else -- lock
            SetVehicleDoorsLocked(veh, 2)
            SetVehicleDoorsLockedForAllPlayers(veh, true)
            tNOVA.notify("~g~Vehicle locked.")
        end
    end
end
