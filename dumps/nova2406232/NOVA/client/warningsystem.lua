...
RegisterNetEvent("NOVA:showWarningsOfUser")
AddEventHandler("NOVA:showWarningsOfUser",function(NOVAwarningstables)
	showWarningSystem = true
	NOVAWarnings = NOVAwarningstables
end)

RegisterNetEvent("NOVA:recievedRefreshedWarningData")
AddEventHandler("NOVA:recievedRefreshedWarningData",function(NOVAwarningstables)
	NOVAWarnings = NOVAwarningstables
end)
...
RegisterCommand('warnings', function()
    ...
    if showWarningSystem then
        TriggerServerEvent("NOVA:refreshWarningSystem")
    end
end)
...
Citizen.CreateThread(function()
	while true do
		if showWarningSystem then
			...
			DrawAdvancedText(0.59, 0.198, 0.005, 0.0028, 0.619, 'NOVA Punishments', 255, 255, 255, 255, 7, 0)
			...
			for warningID,warningTable in pairs(NOVAWarnings) do
