local cfg = module("nova-cars", "cfg/cfg_speedcap")
...
legionJD = {x = 148.066, y = -1050.763, z = 29.34099}
...
viptp = {x = - 732.95123291016, y = 5812.35546875, z = 17.42693901062}
...
local blips = {
	...
	{title="Safe Zone", colour=2, id=1, pos=vector3(legionJD.x, legionJD.y, legionJD.z),dist=40,nonRP=false,setBit=false},
...
if not purge then
	...
	SetBlipColour(pos, 2)
	SetBlipAlpha(pos, 170)
	local pos = AddBlipForRadius(148.066, -1050.763, 29.34099, 40.0) -- jd green 
	SetBlipColour(pos, 2)
	SetBlipAlpha(pos, 170)
	...
	SetBlipColour(pos, 2)
	SetBlipAlpha(pos, 170)
	local pos = AddBlipForRadius(- 732.95123291016, 5812.35546875, 17.42693901062, 40.0) -- viptp
	SetBlipColour(pos, 2)
	SetBlipAlpha(pos, 170)
                ...
                if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then
					...
					then
                        ...
                        if GetVehicleClass(GetVehiclePedIsIn(GetPlayerPed(-1), true)) ~= 13 then
                            SetEntityMaxSpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false), cfg.maxSpeeds["100"])
                        end
                    end
                end
            end
