...
RegisterNetEvent('NOVA:load')
AddEventHandler('NOVA:load', function(list)
	doors = list
end)

RegisterNetEvent('NOVA:statusSend')
AddEventHandler('NOVA:statusSend', function(i, status)
	...
	while true do
		...
		if not IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
			if IsControlJustPressed(table.unpack(LockHotkey)) then
				...
				if id ~= 0 then
					TriggerServerEvent("NOVA:open", id)
				end
			end
		end
	end
