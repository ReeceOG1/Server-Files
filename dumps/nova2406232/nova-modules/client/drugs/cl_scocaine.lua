          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TaskStartScenarioInPlace(ped, 'WORLD_HUMAN_GARDENER_PLANT', false, true)
                ...
                NOVAcfgdrugsServer.CocaineGather()
                Action = false
              else
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              NOVAcfgdrugsServer.CocaineCanProcess({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  NOVAcfgdrugsServer.CocaineDoneProcessing()
                  NOVA.notify({"~w~Processed~g~ Cocaine~w~ 1."})
                  Action = false
                ...
                if not canProcess then
                  NOVA.notify({"~r~You do not have the required license."})
                    ...
                    if not ketAmount then
                      NOVA.notify({"~r~You do not have enough Cocaine."})
                    end
                end
              end)
...
function sellingCocaineUnits()
     NOVAcfgdrugsServer.SellCocaineJob({tonumber(1)})
    return false
end
