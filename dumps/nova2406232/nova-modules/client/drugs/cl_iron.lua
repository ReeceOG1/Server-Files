        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              NOVAcfgdrugsServer.IronCanProcess({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  NOVAcfgdrugsServer.IronDoneProcessing()
                  NOVA.notify({"~w~Processed~g~ Iron~w~ 1."})
                  Action = false
                ...
                if not canProcess then
                  NOVA.notify({"~r~You do not have the required license."})
                    ...
                    if not ketAmount then
                      NOVA.notify({"~r~You do not have enough Iron Ore."})
                    end
                end
              end)
...
function sellingIronUnits()
     NOVAcfgdrugsServer.SellIronJob({tonumber(1)})
    return false
end
