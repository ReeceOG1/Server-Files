          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                NOVAcfgdrugsServer.LSDGather()
                Action = false
              else
                NOVA.notify({"~r~You cant gather Lysergic Acid while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              NOVAcfgdrugsServer.CanProcessLSD({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  NOVAcfgdrugsServer.ProcessLSD()

                  NOVA.notify({"~w~Processed~g~ LSD~w~ 1."})
                  Action = false
                end
                if not canProcess then
                  NOVA.notify({"~r~You do not have the required license."})
                    ...
                    if not ketAmount then
                      NOVA.notify({"~r~You do not have enough Lysergic Acid."})
                    end
                end
              end)
...
function sellingLSDUnits()
    NOVAcfgdrugsServer.SellLSD({tonumber(1)})
    return false
end
