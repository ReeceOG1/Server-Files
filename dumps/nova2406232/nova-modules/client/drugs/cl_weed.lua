          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                NOVAcfgdrugsServer.WeedGather ()
                Action = false
              else
                NOVA.notify({"~r~You cant gather Weed while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              NOVAcfgdrugsServer.CanProcessWeed({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then

          


                  NOVAcfgdrugsServer.ProcessWeed()

                  NOVA.notify({"~w~Processed~g~ Weed~w~ 1."})
                  Action = false
                end
                if not canProcess then
                  NOVA.notify({"~r~You do not have the required license."})
                    ...
                    if not ketAmount then
                      NOVA.notify({"~r~You do not have enough Cannabis Sativa."})
                    end
                end
              end)
              ClearPedTasksImmediately(ped)
...
function sellingWeedUnits()
    NOVAcfgdrugsServer.SellWeed({tonumber(1)})
    return false
end
