          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                NOVAcfgdrugsServer.HeroinGather ()
                Action = false
              else
                NOVA.notify({"~r~You cant gather Heroin while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              NOVAcfgdrugsServer.CanProcessHeroin({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  NOVAcfgdrugsServer.ProcessHeroin()

                  NOVA.notify({"~w~Processed~g~ Heroin~w~ 1."})
                  Action = false
                end
                if not canProcess then
                  NOVA.notify({"~r~You do not have the required license."})
                    ...
                    if not ketAmount then
                      NOVA.notify({"~r~You do not have enough Opium."})
                    end
                end
              end)
...
function sellingHeroinUnits()
    NOVAcfgdrugsServer.SellHeroin({tonumber(1)})
    return false
end
