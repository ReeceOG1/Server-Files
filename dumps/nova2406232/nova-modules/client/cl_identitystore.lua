local cfg = module("nova-modules", "cfg/cfg_identitystore")
local inMenu = false

RMenu.Add("NOVA:Identity", "main", RageUI.CreateMenu("Identity Shop", "~b~Identity Shop", 1350, 50, "banners", "identity"))

RageUI.CreateWhile(1.0, RMenu:Get("NOVA:Identity", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("NOVA:Identity", "main"), true, false, true, function()
                            ...
                            if secondName ~= "" then  
                                ...
                                if (GetOnscreenKeyboardResult()) then
                                    ...
                                    if tonumber(age) and tonumber(age) < 100 then 
                                        TriggerServerEvent("NOVA:ChangeIdentity", firstName, secondName, tonumber(age))
                                    else
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if isInArea(cfg.location, 1.0) and inMenu == false then
            ...
            RageUI.Visible(RMenu:Get("NOVA:Identity", "main"), true)
            inMenu = true
            end
        end
        ...
        if isInArea(cfg.location, 1.0) == false and inMenu then
            inMenu = false
            RageUI.Visible(RMenu:Get("NOVA:Identity", "main"), false)
        end
    end
end)
