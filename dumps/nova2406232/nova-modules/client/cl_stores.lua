...
RMenu.Add("NOVA:Shops", "main", RageUI.CreateMenu(nil, "~b~Shop", 1350, 50, "banners", "shop"))
RMenu.Add("NOVA:Shops", "sub", RageUI.CreateSubMenu(RMenu:Get("NOVA:Shops", "main"), nil, "~b~Confirm Purchase", 1350, 50, "banners", "shop"))

RageUI.CreateWhile(1.0, RMenu:Get("NOVA:Shops", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("NOVA:Shops", "main"), true, false, true, function()
        for k, v in pairs(cfg.items) do
            if v.type == 'store' then
                ...
                end, RMenu:Get("NOVA:Shops", "sub", true))
            end
        end
    ...
    RageUI.IsVisible(RMenu:Get("NOVA:Shops", "sub"), true, false, true, function()
        ...
        RageUI.Button("~g~Confirm Purchase",nil,  {RightLabel = "£"..getMoneyStringFormatted(currentItemPrice * amount)}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("NOVA:BuyShopItem", currentItemID, amount)
            end
        end, RMenu:Get("NOVA:Shops", "main", true))
        ...
        for k, v in pairs(cfg.shops) do
            ...
            if isInArea(v, 1.0) and inMenu == false then
                ...
                if IsControlJustPressed(0, 51) then 
                    inMenu = true
                    currentShop = k
                    RageUI.Visible(RMenu:Get("NOVA:Shops", "main"), true)
                end
            end
