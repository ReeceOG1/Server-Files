...
RegisterCommand("purge", function(source,args,rawCommand)
    purge = not purge
    print(purge)
    TriggerServerEvent('NOVA:activatePurgeSV', purge)
end)

RegisterCommand("testgun", function(source,args,rawCommand)
    if purge then
        TriggerServerEvent('NOVA:purgeRespawn')
    end
end)
...
RegisterNetEvent("NOVA:activatePurge")
AddEventHandler("NOVA:activatePurge", function()
    purgeActivatingAnnounce = true
    TriggerEvent("NOVA:PlaySound", "purge")
    Wait(10000)
