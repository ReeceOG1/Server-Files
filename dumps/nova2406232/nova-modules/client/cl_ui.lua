    ...
    function()
        while true do
            if not a then
                ...
                if E then
                    DrawText(v.Left_x + v.Width / 2.0,v.Bottom_y - 0.0075,tostring(math.floor(w * 100)) .. "%",0.175,nil,nil,m)
                end
                ...
                elseif x > 0.0 then
                    ...
                    if E then
                        DrawText(v.Left_x + v.Width / 2.0,v.Bottom_y + 0.006,tostring(math.floor(x * 100)) .. "%",0.175,nil,nil,m)
                    end
                end
            end
            Wait(0)
...
RegisterCommand(
    ...
    function()
        a = true
        TriggerEvent("NOVA-Client:UI:Toggle", false)
    end
)
RegisterCommand(
    "showui",
    function()
        a = false
        TriggerEvent("NOVA-Client:UI:Toggle", true)
    end
)
RegisterNetEvent("NOVA:ToggleHud")
AddEventHandler(
    "NOVA:ToggleHud",
    function()
        a = not a
