...
RegisterNetEvent("NOVA:showHUD")
AddEventHandler("NOVA:showHUD",function(flag)
    hideHud = not flag
end)
                ...
                elseif EntityType == 3 then
                    ...
                    if `xs_prop_arena_bag_01` == entityModel then
                        ...
                        if IsControlJustReleased(1, 38) then
                            ...
                            TriggerEvent("NOVA:startCombatTimer")
                            TriggerServerEvent("NOVA:LootBag",id)
                            Wait(1000)
                        end
                    elseif `prop_poly_bag_money` == entityModel then
                        ...
                        if IsControlJustReleased(1, 38) then
                            local id = DecorGetInt(Entity,"lootid")
                            TriggerEvent("NOVA:startCombatTimer")
                            TriggerServerEvent("NOVA:Moneydrop",id)
                            Wait(1000)
                        end
                    elseif `prop_box_ammo03a` == entityModel then
                        ...
                        if IsControlJustReleased(1, 38) then
                            local id = DecorGetInt(Entity,"lootid")
                            TriggerEvent("NOVA:startCombatTimer")
                            TriggerServerEvent("NOVA:openCrate",id)
                            Wait(1000)
                        end
...
function lockpick(entityId)
    TriggerEvent("NOVA:LockPick",entityId)
end

function repairVehicle(entityId)
    TriggerServerEvent("NOVA:attemptRepairVehicle")
end
...
function searchVehicle(entityId)
    if globalOnPoliceDuty then
        TriggerEvent("NOVA:SearchPlr",entityId)
    end
end

function impoundVehicle(entityId)
    if globalOnPoliceDuty then
        local user_id = tonumber(DecorGetInt(entityId, "NOVA_owner"))
        if user_id > 0 then
            exports.NOVA:impound(user_id, GetEntityModel(entityId), VehToNet(entityId),entityId)
        else
            TriggerEvent("NOVA:Notify","~r~Vehicle is not owned by anyone")
        end
    end
end
...
function robPerson(entityId)
    ...
    if playerSrc > 0 then
        if GetSelectedPedWeapon(PlayerPedId()) ~= `WEAPON_UNARMED` then
            TriggerServerEvent("NOVA:robPlayer",playerSrc)
        else
            TriggerEvent("NOVA:Notify","~r~You need a weapon in your hands.")
        end
    end
end
...
function askId(entityId)
    ...
    local playerSrc = GetPlayerServerId(player)
    if playerSrc > 0 then
        TriggerServerEvent("NOVA:AskID",playerSrc)
    end
end
...
function giveCash(entityId)
    ...
    local playerSrc = GetPlayerServerId(player)
    if playerSrc > 0 then
        TriggerServerEvent("NOVA:GiveMoney",playerSrc)
    end
end
    ...
    if not globalOnPoliceDuty and not globalOnPrisonDuty then
        ...
        if ped ~= nil then
            if IsEntityPlayingAnim(ped, 'missminuteman_1ig_2', 'handsup_enter', 3) or IsEntityPlayingAnim(ped, "random@arrests", "idle_2_hands_up", 3) or IsEntityPlayingAnim(ped, "random@arrests@busted", "idle_a", 3) then
                local playerSrc = GetPlayerServerId(player)
                if playerSrc > 0 then
                    TriggerServerEvent("NOVA:searchPlayer",playerSrc)
                end
            else
                TriggerEvent("NOVA:Notify","~r~Player must have their hands up or be on their knees!")
            end
        end
    else
        local playerSrc = GetPlayerServerId(player)
        if playerSrc > 0 then
            TriggerServerEvent("NOVA:SearchPlr",playerSrc)
        end
    end
end
...
function revive(entityId)
    ...
    if playerSrc > 0 then
        if globalOnNHSDuty then
            TriggerServerEvent("NOVA:nhsRevive",playerSrc)
        else
            if not cpr_in_progress then
                TriggerServerEvent("NOVA:attemptCPR",playerSrc)
                ...
                TriggerEvent("NOVA:Notify","~r~CPR in progress")
            end
        end
    end
end
...
function handcuff(entityId)
    if globalOnPoliceDuty or globalOnPrisonDuty then
        ...
        local playerSrc = GetPlayerServerId(player)
        if playerSrc > 0 then
            TriggerServerEvent("NOVA:policeCuff",playerSrc)
        end
    end
end
...
function drag(entityId)
    if globalOnPoliceDuty or globalOnPrisonDuty then
        ...
        local playerSrc = GetPlayerServerId(player)
        if playerSrc > 0 then
            TriggerServerEvent("NOVA:Drag",playerSrc)
        end
    end
end
...
function putInCar(entityId)
    if globalOnPoliceDuty or globalOnPrisonDuty then
        ...
        local playerSrc = GetPlayerServerId(player)
        if playerSrc > 0 then
            TriggerServerEvent("NOVA:PutPlrInVeh",playerSrc)
        end
    end
end
...
local function gunshotTest(entityId)
    ...
    local playerSrc = GetPlayerServerId(player)
    if playerSrc > 0 then
        TriggerServerEvent("NOVA:gunshotTest", playerSrc)
    end
end
...
function jail(entityId)
    if globalOnPoliceDuty or globalOnPrisonDuty then
        ...
        local playerSrc = GetPlayerServerId(player)
        if playerSrc > 0 then
            TriggerServerEvent("NOVA:SeizeWeapons2",playerSrc)
            TriggerServerEvent("NOVA:JailPlayer",playerSrc)
        end
    end
end
...
function requestTransport(entityId)
    if globalOnPoliceDuty or globalOnPrisonDuty then
        ...
        local playerSrc = GetPlayerServerId(player)
        if playerSrc > 0 then
            TriggerServerEvent("NOVA:requestTransport",playerSrc)
        end
    end
end
...
RegisterNetEvent("NOVA:setPoliceOnDuty")
AddEventHandler("NOVA:setPoliceOnDuty", function(onduty)
	globalOnPoliceDuty = onduty
end)

RegisterNetEvent("NOVA:RecieveNHSOnDutyFlag")
AddEventHandler("NOVA:RecieveNHSOnDutyFlag", function(onduty)
	globalOnNHSDuty = onduty
end)

RegisterNetEvent("NOVA:setPrisonGuardOnDuty", function(status)
