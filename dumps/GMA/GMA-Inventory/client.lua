...
tGMA = Proxy.getInterface("GMA")

RegisterNetEvent('openBoot')
AddEventHandler('openBoot', function()
    nearestVeh = GMA.getNearestVehicle({3})
    local VehInRadius, VehType, NVeh = tGMA.getNearestOwnedVehicle({3.5})
    if VehInRadius and IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then 
        ...
        TriggerServerEvent('GMA:FetchTrunkInventory', NVeh, NetworkGetNetworkIdFromEntity(nearestVeh))
    else
...
RegisterCommand('inventory', function()
    if not tGMA.isInComa({}) then
        if not inventoryOpen then
            TriggerServerEvent('GMA:FetchPersonalInventory')
            inventoryOpen = true; 
            ...
            if IsLootBagOpening then
                ...
                TriggerServerEvent('GMA:CloseLootbag')
        ...
        tGMA.notify({'~r~You cannot open your inventory while dead.'})
    end
end)

RegisterNetEvent("GMA:OpenHomeStorage")
AddEventHandler("GMA:OpenHomeStorage", function(toggle , houseName)
    if toggle == true then
        TriggerServerEvent('GMA:FetchPersonalInventory')
        inventoryOpen = true; 
        SetNuiFocus(true, true)
        ...
        TriggerServerEvent('GMA:FetchHouseInventory', houseName)
    else
        inventoryOpen = false;
        SetNuiFocus(false, false)
        SetNuiFocusKeepInput(false)
...
RegisterNetEvent('GMA:InventoryOpen')
AddEventHandler('GMA:InventoryOpen', function(toggle, lootbag)
    ...
    if IsLootBagOpening then
        TriggerEvent("gma:PlaySound", "zipper")
...
RegisterNetEvent('GMA:ToggleNUIFocus')
AddEventHandler('GMA:ToggleNUIFocus', function(value)
    --print('focus', value)
    SetNuiFocus(value, value)
    SetNuiFocusKeepInput(value)
end)

RegisterNetEvent('GMA:SendSecondaryInventoryData')
AddEventHandler('GMA:SendSecondaryInventoryData', function(InventoryData, CurrentKG, MaxKg)
...
RegisterNetEvent('GMA:FetchPersonalInventory')
AddEventHandler('GMA:FetchPersonalInventory', function(table, CurrentKG, MaxKG)
...
RegisterNUICallback('UseBtn', function(data, cb)
    TriggerServerEvent('GMA:UseItem', data.itemId, data.invType)
    cb(true);
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)

RegisterNUICallback('TrashBtn', function(data, cb)
    TriggerServerEvent('GMA:TrashItem', data.itemId, data.invType)
    cb(true);
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)

RegisterNUICallback('GiveBtn', function(data, cb)
    TriggerServerEvent('GMA:GiveItem', data.itemId, data.invType)
    cb(true)
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)
...
RegisterNUICallback('MoveBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('GMA:MoveItem', data.invType, data.itemId, VehTypeA) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('GMA:MoveItem', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('GMA:MoveItem', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNUICallback('MoveXBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('GMA:MoveItemX', data.invType, data.itemId, VehTypeA) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('GMA:MoveItemX', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('GMA:MoveItemX', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNUICallback('MoveAllBtn', function(data, cb)
    if not IsLootBagOpening then
        local nearestVeh2 = GMA.getNearestVehicle({3})
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('GMA:MoveItemAll', data.invType, data.itemId, VehTypeA, NetworkGetNetworkIdFromEntity(nearestVeh2)) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('GMA:MoveItemAll', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('GMA:MoveItemAll', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
AddEventHandler('GMA:IsInComa', function(coma)
    PlayerInComa = coma;
    if coma then 
        LootBagCoords = false;
        NearLootBag = false; 
            ...
            if NearMoneyBag then
                if IsControlJustPressed(0, 38) then
                    if not IsPedSittingInAnyVehicle(Ped) then
                        TriggerServerEvent('GMA:Moneydrop', NearestMoneyNetID)
                    else
    ...
    while true do 
        ...
        if NearLootBag then
            if IsControlJustPressed(0, 38) then
                if not IsPedSittingInAnyVehicle(PlayerPedId()) then
                    LoadAnimDict('amb@medic@standing@kneel@base')
                    TriggerServerEvent('GMA:LootBag', LootBagIDNew)
                else
...
RegisterNetEvent('GMA:LockPick')
AddEventHandler('GMA:LockPick', function()
    TriggerServerEvent('GMA:LockPick')
end)

RegisterNetEvent('GMA:whatIsThis')
AddEventHandler('GMA:whatIsThis', function()
      local chance = math.random(1,3)
      local nearestVeh = GMA.getNearestVehicle({3})
        hasDoneIt = false
               ...
               TriggerEvent("gma:PlaySound", "caralarm") 
               TriggerEvent('omgLol')
               ...
               if chance == 3 then
                   ...
                   --
                       ...
                       VehTypeC = nearestVeh
                       inventoryType = 'CarBoot'
                       TriggerServerEvent('GMA:FetchTrunkInventory', GetEntityArchetypeName(nearestVeh), NetworkGetNetworkIdFromEntity(nearestVeh))                   
                   GMA.notify({"~g~You were succesful in picking this car."})
               else
                   GMA.notify({"~r~You were unsuccesful in picking this car."})
                end
