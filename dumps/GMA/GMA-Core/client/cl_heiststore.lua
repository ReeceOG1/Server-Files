local cfg = module("GMA-Core", "cfg/cfg_stores")
local inMenu = false
...
RMenu.Add("GMA:HeistShops", "main", RageUI.CreateMenu("Heist Store", "~b~Heist Store", 1350, 50))
RMenu.Add("GMA:HeistShops", "sub", RageUI.CreateSubMenu(RMenu:Get("GMA:HeistShops", "main"), "Heist Store", "~b~Confirm Purchase", 1350, 50))

RageUI.CreateWhile(1.0, RMenu:Get("GMA:HeistShops", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("GMA:HeistShops", "main"), true, false, true, function()
        for k, v in pairs(cfg.items) do
            if v.type == 'robbery' then
                ...
                end, RMenu:Get("GMA:HeistShops", "sub", true))
            end
        end
    end, function() 
    end)

    RageUI.IsVisible(RMenu:Get("GMA:HeistShops", "sub"), true, false, true, function()
        ...
        if currentItemName == 'Jewellery' then
            RageUI.Button("~g~Confirm Sale", nil, {RightLabel = "£"..getMoneyStringFormatted(currentItemPrice * amount)}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent("GMA:SellHeistItem", currentItemID, amount)
                end
            end, RMenu:Get("GMA:HeistShops", "main", true))
        else
            RageUI.Button("~g~Confirm Purchase", nil, {RightLabel = "£"..getMoneyStringFormatted(currentItemPrice * amount)}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent("GMA:BuyHeistItem", currentItemID, amount)
                end
            end, RMenu:Get("GMA:HeistShops", "main", true))
        end
    ...
    while true do
        ...
        if isInArea(v, 1.0) and inMenu == false then
            ...
            if IsControlJustPressed(0, 51) then 
                inMenu = true
                currentShop = k
                RageUI.Visible(RMenu:Get("GMA:HeistShops", "main"), true)
            end
        end
