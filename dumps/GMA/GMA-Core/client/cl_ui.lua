...
RegisterCommand(
    ...
    function()
        a = true
        TriggerEvent("GMA-Client:UI:Toggle", false)
    end
)
RegisterCommand(
    "showui",
    function()
        a = false
        TriggerEvent("GMA-Client:UI:Toggle", true)
    end
)
RegisterNetEvent("GMA:ToggleHud")
AddEventHandler(
    "GMA:ToggleHud",
    function()
        a = not a
...
local l = 10.0
local m = {0, 0, 0, 255}
function tGMA.setMaxUnderWaterUITimenewTime(n)
    l = n
end
local function o(p, q, r, s, t, u, v, w)
    DrawRect(p + r / 2, q + s / 2, r, s, t, u, v, w)
end
function tGMA.getCachedMinimapAnchor()
    return k
end
function tGMA.getCachedResolution()
    ...
    function()
        while true do
            if not a then
                    ...
                    local E = tGMA.getShowHealthPercentageFlag()
                    if C > 1.0 then
                        C = 1.0
                    end
