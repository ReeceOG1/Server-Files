...
RMenu.Add('GMAblackjack', 'instructions', RageUI.CreateMenu("", "test",0,100,"casinoui_cards_blackjack", "casinoui_cards_blackjack"))
RMenu:Get('GMAblackjack', 'instructions'):SetSubtitle("~g~BLACKJACK")
RMenu.Add('GMAblackjack_high', 'instructions', RageUI.CreateMenu("", "test",0,100,"casinoui_cards_blackjack_high", "casinoui_cards_blackjack_high"))
RMenu:Get('GMAblackjack_high', 'instructions'):SetSubtitle("~g~BLACKJACK")


RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('GMAblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            ...
            end, RMenu:Get('GMAblackjack', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('GMAblackjack_high', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "The aim of Blackjack is to beat the Dealer's hand without going over 21. This game uses four 52           card decks, which are shuffled at the start of every hand.                                                                                    The dealer will stand on soft 17.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
            ...
            end, RMenu:Get('GMAblackjack_high', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
function showHowToBlackjack(flag)
    if closestChair < 8 then 
        RageUI.Visible(RMenu:Get('GMAblackjack', 'instructions'), flag)
    else 
        RageUI.Visible(RMenu:Get('GMAblackjack_high', 'instructions'), flag)
    end
end
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    --print("[GMA Casino] start sit at blackjack seat") 
    ...
    --print("[GMA Casino] blackjack anims loaded") 
    ...
    --print("[GMA Casino] walking to seat, x: " .. tostring(walkToVector.x) .. " y: " .. tostring(walkToVector.y) .. " z: " .. tostring(walkToVector.z))
    ...
    --print("[GMA Casino] Blackjack sit at table net scene starting")
    --print("[GMA Casino] creating Scene at, x: " .. tostring(goToVector.x) .. " y: " .. tostring(goToVector.y) .. " z: " .. tostring(goToVector.z))
    ...
    --print("[GMA Casino] Blackjack sit at table net scene started")
...
function betChipsForNextHand(chipsAmount,chipsProp,something,chairID,someBool,zOffset)
    ...
    while not HasModelLoaded(chipsProp) do  
        Wait(0)
        --print("[GMA Casino] Stuck requesting model: " .. tostring(chipsProp))
        RequestModel(chipsProp)
    end
    vVar8 =  vector3(0.0, 0.0, getTableHeading(blackjack_func_368(chairID)))
    local tablePosX,tablePosY,tablePosZ = getTableCoords(blackjack_func_368(chairID))
