...
local m = module("GMA-Veh", "cfg/cfg_garages")
m=m.garage_types

RMenu.Add('PlateShop', 'main', RageUI.CreateMenu("Number Plate", "~g~Number Plate", 1350, 50, "banners", "settings"))
RMenu.Add("PlateShop", "sub", RageUI.CreateSubMenu(RMenu:Get("PlateShop", "main"), "Number Plate", "~g~Number Plate", 1350, 50, "banners", "settings"))
...
RageUI.CreateWhile(1.0, RMenu:Get("PlateShop", "main"), nil, function()
    ...
    RageUI.IsVisible(RMenu:Get("PlateShop", "sub"), true, false, true, function()
        RageUI.Button("Change Number Plate", "~g~Changing plate of "..selectedCarName, {RightLabel = "~g~£50,000"}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("GMA:ChangeNumberPlate", selectedCar)
            end
        end)
    ...
    while true do
        ...
        if isInArea(location, 1.4) and isInMenu == false then 
            ...
            if IsControlJustPressed(0, 51) then 
                TriggerServerEvent('GMA:getCars')
...
RegisterNetEvent("GMA:RecieveNumberPlate")
AddEventHandler("GMA:RecieveNumberPlate", function(numplate)
...
RegisterNetEvent("GMA:carsTable")
AddEventHandler("GMA:carsTable",function(cars)
    carstable = cars
end)
