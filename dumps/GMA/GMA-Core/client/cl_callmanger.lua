...
RMenu.Add('callmanager', 'main', RageUI.CreateMenu("", '~r~GMA Call Manager', 1300, 50, "banners", "callmanager"))
                    ...
                    if k == ticketID then
                        if v[2] == GetPlayerServerId(PlayerId()) then
                            ...
                            CallManagerServer.GetUpdatedCoords({v[2]}, function(targetCoords)
                                ...
                                TriggerServerEvent("GMA:GiveTicketMoney", v[1], v[2], v[3], true)
                            end)  
                            CallManagerServer.RemoveTicket({k, "admin"})
                            name = v[1]
                            ticketReason = v[3] -- Save the ticket reason here
                            isInTicket = true
                            TriggerServerEvent('GMA:getTempFromPerm',v[2])
                        end
                    end
                end
            end
...
AddEventHandler("staffon", function(isInTicket)
    ...
    TriggerEvent("GMA:vehicleMenu", true, isInTicket)
    if GetEntityHealth(GetPlayerPed(-1)) <= 103 then
    TriggerEvent('GMA:FixClient')
    end
end)
...
AddEventHandler("staffoff", function()
    isInTicket = false
    TriggerEvent("GMA:vehicleMenu", false, isInTicket) 
end)


RegisterNetEvent('GMA:AdminTicketCooldown')
AddEventHandler('GMA:AdminTicketCooldown', function(source, Reason)
    if AdminTicketCooldown == false then
        TriggerServerEvent('GMA:sendAdminTicket', source, Reason)
...
RegisterNetEvent('GMA:PDCallCooldown')
AddEventHandler('GMA:PDCallCooldown', function(source, Reason)
    if PDCallCooldown == false then
        TriggerServerEvent('GMA:sendPDCall', source, Reason)
        PDCallCooldown = true
        Wait(60000*2)
...
RegisterNetEvent('GMA:NHSCallCooldown')
AddEventHandler('GMA:NHSCallCooldown', function(source, Reason)
    if NHSCallCooldown == false then
        TriggerServerEvent('GMA:sendNHSCall', source, Reason)
...
RegisterNetEvent('GMA:PLAYTICKETRECIEVED')
AddEventHandler('GMA:PLAYTICKETRECIEVED', function(source)
    PlaySoundFrontend(-1, "Apt_Style_Purchase", "DLC_APT_Apartment_SoundSet", 0)
end)

RegisterNetEvent('GMA:sendPermID')
AddEventHandler('GMA:sendPermID', function(permid)
    permid = permid
    while isInTicket do
        inRedzone = false
        Wait(0)
