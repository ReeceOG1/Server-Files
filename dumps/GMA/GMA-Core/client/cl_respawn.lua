local Housing = module("GMA-Core", "cfg/cfg_homes")
...
RegisterNetEvent("GMA:PolicePerms")
AddEventHandler("GMA:PolicePerms",function(pd)
    hasPD = pd
end)
RegisterNetEvent("GMA:RebelPerms")
AddEventHandler("GMA:RebelPerms",function(rebel)
    hasRebel = rebel
end)
RegisterNetEvent("GMA:VIPPerms")
AddEventHandler("GMA:VIPPerms",function(vip)
    hasVIP = vip
end)
        ...
        for a , b in pairs(respawn.hospitals) do 
            RageUI.Button(a, nil, "", true, function(Hovered, Active, Selected)
                if Selected then
                    ...
                    TriggerEvent("gma:PlaySound", "gtaloadin") 
                end
            end)
        end
        if hasVIP then
...
RegisterNetEvent("GMA:StartRespawnCam")
AddEventHandler("GMA:StartRespawnCam",function()
    inRedZone = false
    inCam = true
    inMenu = true
    DoScreenFadeIn(500)
...
RegisterNetEvent("GMA:HousingTable")
AddEventHandler("GMA:HousingTable",function(houses)
    housetable = houses
end)