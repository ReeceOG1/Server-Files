                ...
                if targetPed ~= PlayerPedId() then
                    if playerDistances[id] then
                        if playerDistances[id] < disPlayerNames then
                            ...
                            -- Line-of-sight check
                            local rayHandle = StartShapeTestLosProbe(playerCoords, targetPedCords, 1, PlayerPedId(), 0)
                            local a, hit, hitPoint, surfaceNormal, entityHit = GetShapeTestResult(rayHandle)
                            if hit == 0 then  -- if there's no entity obstructing the view
...
RegisterCommand("farids", function()
    if faridsenabled then
        TriggerServerEvent("GMA:IDsAboveHead", true)
    else
        TriggerServerEvent("GMA:IDsAboveHead", false)
    end
end)
...
RegisterCommand("hidechat", function()
    TriggerEvent('GMA:toggleChat', true)
    notify('~g~Chat Hidden')
end)

RegisterCommand("showchat", function()
    TriggerEvent('GMA:toggleChat', false)
    notify('~g~Chat Shown')
end)

RegisterNetEvent("GMA:ChangeIDs")
AddEventHandler("GMA:ChangeIDs", function(status)
    if status then
        disPlayerNames = 10
