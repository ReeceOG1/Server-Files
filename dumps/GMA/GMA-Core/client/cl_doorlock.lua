...
RegisterNetEvent('gma_doors:load')
AddEventHandler('gma_doors:load', function(list)
    doors = list
end)
RegisterNetEvent('gma_doors:statusSend')
AddEventHandler('gma_doors:statusSend', function(id,status)
        ...
        if v.close ~= nil then
            ...
            if door1 < newrange then
                if door1 < GetDistanceBetweenCoords(x,y,z,doors[v.close].coords[1], doors[v.close].coords[2], doors[v.close].coords[3],true) then
                    TriggerServerEvent("gma_doors:status", k, not v.locked)
                else
                    TriggerServerEvent("gma_doors:status", v.close, not doors[v.close].locked)
                end
                break
            end
        else
