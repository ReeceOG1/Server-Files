...
RegisterNetEvent('gma:load')
AddEventHandler('gma:load', function(list)
	doors = list
end)

RegisterNetEvent('gma:statusSend')
AddEventHandler('gma:statusSend', function(i, status)
	...
	while true do
		...
		if not IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
			if IsControlJustPressed(table.unpack(LockHotkey)) then
				...
				if id ~= 0 then
					TriggerServerEvent("gma:open", id)
				end
			end
		end
	end
