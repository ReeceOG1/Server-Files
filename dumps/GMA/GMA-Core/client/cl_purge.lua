...
RegisterCommand("purge", function(source,args,rawCommand)
    purge = not purge
    print(purge)
    TriggerServerEvent('GMA:activatePurgeSV', purge)
end)

RegisterCommand("testgun", function(source,args,rawCommand)
    if purge then
        TriggerServerEvent('GMA:purgeRespawn')
    end
end)
...
RegisterNetEvent("GMA:activatePurge")
AddEventHandler("GMA:activatePurge", function()
    purgeActivatingAnnounce = true
    TriggerEvent("gma:PlaySound", "purge")
    Wait(10000)
