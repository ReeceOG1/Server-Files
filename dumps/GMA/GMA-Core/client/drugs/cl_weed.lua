          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                GMAcfgdrugsServer.WeedGather ()
                Action = false
              else
                GMA.notify({"~r~You cant gather Weed while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              GMAcfgdrugsServer.CanProcessWeed({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then

          


                  GMAcfgdrugsServer.ProcessWeed()

                  GMA.notify({"~w~Processed~g~ Weed~w~ 1."})
                  Action = false
                end
                if not canProcess then
                  GMA.notify({"~r~You do not have the required license."})
                    ...
                    if not ketAmount then
                      GMA.notify({"~r~You do not have enough Cannabis Sativa."})
                    end
                end
              end)
              ClearPedTasksImmediately(ped)
...
function sellingWeedUnits()
    GMAcfgdrugsServer.SellWeed({tonumber(1)})
    return false
end
