        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              GMAcfgdrugsServer.IronCanProcess({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  GMAcfgdrugsServer.IronDoneProcessing()
                  GMA.notify({"~w~Processed~g~ Iron~w~ 1."})
                  Action = false
                ...
                if not canProcess then
                  GMA.notify({"~r~You do not have the required license."})
                    ...
                    if not ketAmount then
                      GMA.notify({"~r~You do not have enough Iron Ore."})
                    end
                end
              end)
...
function sellingIronUnits()
     GMAcfgdrugsServer.SellIronJob({tonumber(1)})
    return false
end
