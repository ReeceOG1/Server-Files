          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                GMAcfgdrugsServer.LSDGather()
                Action = false
              else
                GMA.notify({"~r~You cant gather Lysergic Acid while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              GMAcfgdrugsServer.CanProcessLSD({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  GMAcfgdrugsServer.ProcessLSD()

                  GMA.notify({"~w~Processed~g~ LSD~w~ 1."})
                  Action = false
                end
                if not canProcess then
                  GMA.notify({"~r~You do not have the required license."})
                    ...
                    if not ketAmount then
                      GMA.notify({"~r~You do not have enough Lysergic Acid."})
                    end
                end
              end)
...
function sellingLSDUnits()
    GMAcfgdrugsServer.SellLSD({tonumber(1)})
    return false
end
