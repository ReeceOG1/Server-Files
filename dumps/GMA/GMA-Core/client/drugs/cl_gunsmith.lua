          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TaskStartScenarioInPlace(ped, 'WORLD_HUMAN_GARDENER_PLANT', false, true)
                ...
                GMAcfgdrugsServer.DiamondGather()
                Action = false
              else
                GMA.notify({"~r~How the fuck are you supposed to gather sitting in that thing?"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              GMAcfgdrugsServer.DiamondCanProcess({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  GMAcfgdrugsServer.DiamondDoneProcessing()
                  GMA.notify({"~w~Processed~g~ Diamond~w~ 1."})
                  Action = false

                end
                if not canProcess then
                  GMA.notify({"~r~You do not have the required license."})
                  else
                    if not ketAmount then
                      GMA.notify({"~r~You do not have enough Diamond Crystals."})
                    end
                end
              end)
...
function sellingDiamondUnits()
     GMAcfgdrugsServer.SellDiamondJob({tonumber(1)})
    return false
end
