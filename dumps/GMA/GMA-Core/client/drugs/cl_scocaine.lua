          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TaskStartScenarioInPlace(ped, 'WORLD_HUMAN_GARDENER_PLANT', false, true)
                ...
                GMAcfgdrugsServer.CocaineGather()
                Action = false
              else
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              GMAcfgdrugsServer.CocaineCanProcess({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  GMAcfgdrugsServer.CocaineDoneProcessing()
                  GMA.notify({"~w~Processed~g~ Cocaine~w~ 1."})
                  Action = false
                ...
                if not canProcess then
                  GMA.notify({"~r~You do not have the required license."})
                    ...
                    if not ketAmount then
                      GMA.notify({"~r~You do not have enough Cocaine."})
                    end
                end
              end)
...
function sellingCocaineUnits()
     GMAcfgdrugsServer.SellCocaineJob({tonumber(1)})
    return false
end
