...
RMenu.Add("GMA:Shops", "main", RageUI.CreateMenu("Shop", "~b~Shop", 1350, 50))
RMenu.Add("GMA:Shops", "sub", RageUI.CreateSubMenu(RMenu:Get("GMA:Shops", "main"), "Shop", "~b~Confirm Purchase", 1350, 50))

RageUI.CreateWhile(1.0, RMenu:Get("GMA:Shops", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("GMA:Shops", "main"), true, false, true, function()
        for k, v in pairs(cfg.items) do
            if v.type == 'store' then
                ...
                end, RMenu:Get("GMA:Shops", "sub", true))
            end
        end
    ...
    RageUI.IsVisible(RMenu:Get("GMA:Shops", "sub"), true, false, true, function()
        ...
        RageUI.Button("~g~Confirm Purchase",nil,  {RightLabel = "£"..getMoneyStringFormatted(currentItemPrice * amount)}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("GMA:BuyShopItem", currentItemID, amount)
            end
        end, RMenu:Get("GMA:Shops", "main", true))
        ...
        for k, v in pairs(cfg.shops) do
            ...
            if isInArea(v, 1.0) and inMenu == false then
                ...
                if IsControlJustPressed(0, 51) then 
                    inMenu = true
                    currentShop = k
                    RageUI.Visible(RMenu:Get("GMA:Shops", "main"), true)
                end
            end
