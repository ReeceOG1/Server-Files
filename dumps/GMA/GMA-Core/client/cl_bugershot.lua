GMACbc = {}
Tunnel.bindInterface("gma_chief",GMACbc)
Proxy.addInterface("gma_chief",GMACbc)
GMASbc = Tunnel.getInterface("gma_chief","gma_chief")
...
function GMACbc.startMissionbook()
    notputontable = true
...
function meat()
    GMA.playAnim({true, {task="PROP_HUMAN_BBQ"}, false})
    GMA.notify({"~w~You now preparing ~g~Meat"})
    Wait(30000)
    GMA.stopAnim({false})
    GMA.notify({"~w~Finishng to preparing ~g~Meat"})
    GMASbc.getfood({1})
end

function fries()
    GMA.playAnim({true, {task="PROP_HUMAN_BBQ"}, false})
    GMA.notify({"~w~You now preparing ~g~Fries"})
    Wait(30000)
    GMA.stopAnim({false})
    GMA.notify({"~w~Finishng to preparing ~g~Fries"})
    GMASbc.getfood({2})
end

function fruitsandvegetabels()
    GMA.playAnim({true, {task="PROP_HUMAN_PARKING_METER"}, false})
    GMA.notify({"~w~You now take ~g~Fruit and Vegetables"})
    Wait(3000)
    GMA.stopAnim({false})
    GMASbc.getfood({3})
end
...
function coke()
    GMA.playAnim({true, {task="PROP_HUMAN_PARKING_METER"}, false})
    GMA.notify({"~w~You take a ~g~Coke"})
    Wait(3000)
    GMA.stopAnim({false})
    GMASbc.getfood({4})
end
