...
RegisterNetEvent('GMA:isPoliceVar')
AddEventHandler('GMA:isPoliceVar', function(target)

end)

RegisterNetEvent('GMA:getTackled')
AddEventHandler('GMA:getTackled', function(target)
...
RegisterNetEvent('GMA:playTackle')
AddEventHandler('GMA:playTackle', function()
	isTackling = true
	local playerPed =PlayerPedId()
	...
	while true do
		...
		if IsControlPressed(0, Keys['LEFTSHIFT'], true) and IsControlPressed(0, Keys['G'], true) and not isTackling then
			...
			if distance ~= -1 and distance <= 3.0 and not isTackling and not isGettingTackled and not IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
				...
				TriggerServerEvent('GMA:tryTackle', closestPlayerId)
			end
		end
	end
end)
