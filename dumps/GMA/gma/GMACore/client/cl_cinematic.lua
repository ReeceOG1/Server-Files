...
RMenu.Add('CinematicMenu', 'MainMenu', RageUI.CreateMenu("", "GMA Cinematic", 1300,100, "banners", "settings")) 
    ...
    if RageUI.Visible(RMenu:Get('CinematicMenu', 'MainMenu')) then
        ...
        if HasCinematic then
            ...
            RageUI.Button("Get Permission", nil, {RightLabel = "→→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    SendNUIMessage({act="openurl",url="https://discord.gg/GMA"})
                end
            end)
        end
       end)
