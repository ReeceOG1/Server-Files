                ...
                RageUI.Button("Cycle through seats" , nil, { RightLabel = '→→→'}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        ...
                        if IsPedInAnyVehicle(p) then
                            ...
                            tGMA.notify("~r~Not in a vehicle.")
                        end
                    end
                end)
                ...
                if b then
                    RageUI.Checkbox("Return to normal Universe", "~r~Leave Car Dev Mode", b, {}, function(Hovered, Active, Selected, Checked)
                        if Selected then
                            ...
                            b=false
                            TriggerServerEvent("GMA:setCarDev",b)
                        end
                    end)
                else
...
RegisterCommand('cardev', function()
    TriggerServerEvent('GMA:openCarDev')
end)

function tGMA.openCarDev()
...
function TakeVehScreenshot()
    ...
    Wait(3000)
    TriggerServerEvent('GMA:takeCarScreenshot')
    Wait(5000)
...
local function a1()
    ...
    TriggerServerEvent('GMA:getCarDevDebug',{a2})
end
...
local function a8()
    ...
    if CursorInArea(0.25,0.31,0.12,0.15)and IsDisabledControlJustPressed(0,24)then 
        a1()
        TriggerEvent("gma:PlaySound", "apple") 
