...
Citizen.CreateThread(function()
    while true do
        ...
        if noclip then
            ...
            local dx, dy, dz = tGMA.getCamDirection()
        ...
        if EntityCleanupGun then 
            ...
            if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_STAFFGUN') then
                if IsPlayerFreeAiming(plr) then 
                    ...
                    if yes then 
                        tGMA.notify('~g~Deleted Entity: ' .. GetEntityModel(entity))
...
local function teleportToWaypoint()
    Citizen.CreateThread(function()
        ...
        if success then
            ...
            DoScreenFadeIn(250)
            tGMA.notify('~g~Teleported success!')
        end
    end)
end
...
RegisterNetEvent("GMA:vehicleMenu")
AddEventHandler("GMA:vehicleMenu",function(DioMode, isInTicket)
    if getStaffLevel() > 0 then
	OMioDioMode = DioMode
    ticketStatus = isInTicket
    TriggerEvent('greenzoneStaffMode', OMioDioMode)
	if not OMioDioMode then
		tGMA.setCustomization(adminTicketSavedCustomization)
        setRedzoneTimerDisabled(false)
        ...
        adminTicketSavedCustomization = tGMA.getCustomization()
        gender = getModelGender()
        if gender == "male" then
            z="mp_m_freemode_01"
            local A=loadModel(z)
            setRedzoneTimerDisabled(true)
            tGMA.setCustomization({modelhash=A})
            Wait(100)
...
RegisterCommand("fix", function()
    if OMioDioMode or tGMA.getUserId() == 2 or tGMA.getUserId() == 1 then
...
RegisterNetEvent("GMA:showBlips")
AddEventHandler("GMA:showBlips",function()
    blips = not blips
    if blips then
        tGMA.notify("~g~Blips enabled")
    else
        tGMA.notify("~r~Blips disabled")
