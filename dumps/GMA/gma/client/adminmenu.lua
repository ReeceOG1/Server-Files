GMAclient = Tunnel.getInterface("GMA","GMA")
...
AddEventHandler("playerSpawned",function()
    local h = true
    if h then 
        TriggerServerEvent("GMA:requestAdminPerks")
    end 
end)

RegisterNetEvent('GMA:SendAdminPerks', function(a)
    Stafflevel = a 
    if getStaffLevel() > 0 then 
        print('[GMA] Your staff level is: ' ..Stafflevel)
    end
end)
...
function tGMA.isAdmin()
    return isPlayerAdmin
end

function tGMA.setAdmin(bool_value)
    ...
    if RageUI.Visible(RMenu:Get('adminmenu', 'main')) then
        ...
        if admincfg.buttonsEnabled["adminMenu"][1] and buttons["adminMenu"] then
            RageUI.ButtonWithStyle("Nearby Players", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent("GMA:GetNearbyPlayers", 250)
                end
            ...
            for k, v in pairs(players) do
                RageUI.ButtonWithStyle(v[1] .." ["..v[3].."]", v[1] .. " ("..v[4].." hours) PermID: " .. v[3] .. " TempID: " .. v[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then
                        ...
                        SelectedPerm = v[3]
                        TriggerServerEvent("GMA:CheckPov",v[3])
                        TriggerServerEvent("GMA:GetPlayerGroups", v[3])
                    end
...
RegisterNetEvent("GMA:ReturnNearbyPlayers")
AddEventHandler("GMA:ReturnNearbyPlayers", function(table)
    playersNearby = table
end)

RegisterNetEvent('GMA:playerDamageDisplay')
AddEventHandler('GMA:playerDamageDisplay', function(target, damage)
    ...
    while true do
        ...
        if IsPedShooting(playerPed) then
            ...
            if IsPedAPlayer(target) then
                ...
                TriggerServerEvent('GMA:playerDamage', targetPlayerId, damage)
            end
        end
    end
end)
...
RegisterNetEvent('GMA:ToggleHealthPercentage')
AddEventHandler('GMA:ToggleHealthPercentage', function(show)
    if show then
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('SettingsMenu', 'MainMenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.Checkbox("Show Damage", nil, damageDisplay, {}, function(Hovered, Active, Selected, Checked)
                if (Selected) then
                    damageDisplay = not damageDisplay
                    TriggerEvent('GMA:ToggleDamageDisplay', damageDisplay)
                    if damageDisplay then
            ...
            RageUI.Checkbox("Show Health Percentage", nil, healthPercentageDisplay, {}, function(Hovered, Active, Selected, Checked)
                if (Selected) then
                    ...
                    if Checked then
                        ...
                        TriggerEvent('GMA:ToggleHealthPercentage', true)
                    else
                        notify("~r~Health Percentage Off!")
                        TriggerEvent('GMA:ToggleHealthPercentage', false)
                    end
                end
            end)
...
RegisterNetEvent('GMA:OpenSettingsMenu')
AddEventHandler('GMA:OpenSettingsMenu', function()
...
RegisterNetEvent('GMA:ToggleDamageDisplay')
AddEventHandler('GMA:ToggleDamageDisplay', function(enabled)
...
RegisterCommand('settings',function()
    TriggerServerEvent('GMA:OpenSettings')
end)
    ...
    if RageUI.Visible(RMenu:Get('adminmenu', 'functions')) then
       ...
       if admincfg.buttonsEnabled["kick"][1] and buttons["kick"] then                        
           RageUI.ButtonWithStyle("Kick (No F10)", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
               if Selected then
                   TriggerServerEvent('GMA:noF10Kick')
               end
           end, RMenu:Get('adminmenu', 'functions'))
       end
        ...
        if admincfg.buttonsEnabled["ban"][1] and buttons["ban"] then
            RageUI.ButtonWithStyle("Offline Ban",nil,{RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    ...
                    TriggerServerEvent('GMA:offlineban', uid)
                end
            end)
        end
        ...
        if admincfg.buttonsEnabled["removewarn"][1] and buttons["removewarn"] then
            RageUI.ButtonWithStyle("Remove Warning", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    local uid = GetPlayerServerId(PlayerId())
                    TriggerServerEvent('GMA:RemoveWarning', uid, result)
                end
            end, RMenu:Get('adminmenu', 'functions'))
        end
        if admincfg.buttonsEnabled["getgroups"][1] and buttons["getgroups"] then
            RageUI.Button("Toggle Blips", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent('GMA:checkBlips')
                end
            end, RMenu:Get('adminmenu', 'functions'))
        end
        if admincfg.buttonsEnabled["adminMenu"][1] and buttons["adminMenu"] then
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            if admincfg.buttonsEnabled["unban"][1] and buttons["unban"] then
                RageUI.ButtonWithStyle("Vehicle Cleanup", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then
                        TriggerServerEvent('GMA:VehCleanup')
                    end
                end, RMenu:Get('adminmenu', 'entityfunctions'))
            end
            ...
            if admincfg.buttonsEnabled["unban"][1] and buttons["unban"] then
                RageUI.ButtonWithStyle("Entity Cleanup",nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then
                        TriggerServerEvent('GMA:CleanAll')
                    end
                end, RMenu:Get('adminmenu', 'entityfunctions'))
            end
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            if admincfg.buttonsEnabled["tp2waypoint"][1] and buttons["tp2waypoint"] then                        
                RageUI.List("Teleport to",q,s,nil,{},true,function(x, y, z, N)
                    ...
                    if z then
                        local uid = GetPlayerServerId(PlayerId())
                        TriggerServerEvent("GMA:Teleport", uid, vector3(r[s]))
                    end
                end,
                function()end)
            end
            ...
            if admincfg.buttonsEnabled["devMenu"][1] and buttons["devMenu"] then
                RageUI.ButtonWithStyle("Get Coords", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then
                        TriggerServerEvent('GMA:GetCoords')
                    end
                end, RMenu:Get('adminmenu', 'teleportfunctions'))
            end
            ...
            if admincfg.buttonsEnabled["devMenu"][1] and buttons["devMenu"] then
                RageUI.ButtonWithStyle("TP To Coords",nil,{RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then
                        TriggerServerEvent("GMA:Tp2Coords")
                    end
                end, RMenu:Get('adminmenu', 'teleportfunctions'))
            end
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.CenterButton("~g~Money Functions", nil, {}, true,function(Hovered, Active, Selected)end)
                if admincfg.buttonsEnabled["devMenu"][1] and buttons["devMenu"] then
                    RageUI.ButtonWithStyle("Give Cash",nil,{}, true, function(Hovered, Active, Selected)
                        if Selected then
                            TriggerServerEvent("GMA:GiveMoneyMenu")
                        end
                    end, RMenu:Get('adminmenu', 'devfunctions'))
                end
                if admincfg.buttonsEnabled["devMenu"][1] and buttons["devMenu"] then
                    RageUI.ButtonWithStyle("Give Bank",nil,{}, true, function(Hovered, Active, Selected)
                        if Selected then
                            TriggerServerEvent("GMA:GiveBankMenu")
                        end
                    end, RMenu:Get('adminmenu', 'devfunctions'))
                end
            ...
            if admincfg.buttonsEnabled["devMenu"][1] and buttons["devMenu"] then
                RageUI.ButtonWithStyle("Spawn Weapon",nil, {}, true, function(Hovered, Active, Selected)
                    if Selected then
                        TriggerServerEvent('GMA:Giveweapon')
                    end
                end, RMenu:Get('adminmenu', 'devfunctions'))
            end  
            if admincfg.buttonsEnabled["devMenu"][1] and buttons["devMenu"] then
                ...
                RageUI.ButtonWithStyle("Reset Rewards",nil, {}, true, function(Hovered, Active, Selected)
                    if Selected then
                        TriggerServerEvent('GMA:resetRedeem')
                    end
                end)
...
local PlayerGroups = {}
RegisterNetEvent("GMA:RecievePlayerGroups")
AddEventHandler("GMA:RecievePlayerGroups",function(groups)
    PlayerGroups = groups
end)
            ...
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                if admincfg.buttonsEnabled["spectate"][1] and buttons["spectate"] then
                    RageUI.ButtonWithStyle("Player Notes", "" .. SelectedPlayer[1] .. " Perm ID: " .. SelectedPlayer[3] .. " Temp ID: " .. SelectedPlayer[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then
                            TriggerServerEvent('GMA:getNotes', uid, SelectedPlayer[3])
                        end
                ...
                if admincfg.buttonsEnabled["kick"][1] and buttons["kick"] then
                    RageUI.ButtonWithStyle("Kick Player", "" .. SelectedPlayer[1] .. " Perm ID: " .. SelectedPlayer[3] .. " Temp ID: " .. SelectedPlayer[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then
                            local uid = GetPlayerServerId(PlayerId())
                            TriggerServerEvent('GMA:KickPlayer', uid, SelectedPlayer[3], kickReason, SelectedPlayer[2])
                        end
                    end, RMenu:Get('adminmenu', 'submenu'))
                end
                ...
                if admincfg.buttonsEnabled["spectate"][1] and buttons["spectate"] then
                    RageUI.ButtonWithStyle("Spectate Player", "" .. SelectedPlayer[1] .. " Perm ID: " .. SelectedPlayer[3] .. " Temp ID: " .. SelectedPlayer[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then
                            inRedZone = false
                            TriggerServerEvent('GMA:SpectatePlayer', SelectedPlayer[3])
                        end
                    end, RMenu:Get('adminmenu', 'submenu'))
                end
                ...
                if admincfg.buttonsEnabled["revive"][1] and buttons["revive"] then
                    RageUI.ButtonWithStyle("Revive", "" .. SelectedPlayer[1] .. " Perm ID: " .. SelectedPlayer[3] .. " Temp ID: " .. SelectedPlayer[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then
                            local uid = GetPlayerServerId(PlayerId())
                            TriggerServerEvent('GMA:RevivePlayer', uid, SelectedPlayer[2])
                        end
                    end, RMenu:Get('adminmenu', 'submenu'))
                end
                ...
                --             if cooldown == 0 then
                --             local uid = GetPlayerServerId(PlayerId())
                --             TriggerServerEvent("GMA:Teleport", SelectedPlayer[2], vector3(r[s]))
                --             cooldown = 30
                --         else
                ...
                if admincfg.buttonsEnabled["TP2"][1] and buttons["TP2"] then
                    RageUI.ButtonWithStyle("Teleport to Player", "" .. SelectedPlayer[1] .. " Perm ID: " .. SelectedPlayer[3] .. " Temp ID: " .. SelectedPlayer[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then
                            ...
                            TriggerServerEvent('GMA:TeleportToPlayer', newSource, SelectedPlayer[2])
                            inTP2P = true
                            inTP2P2 = true
                        end
                ...
                if admincfg.buttonsEnabled["TP2ME"][1] and buttons["TP2ME"] then
                    RageUI.ButtonWithStyle("Teleport Player to Me", "" .. SelectedPlayer[1] .. " Perm ID: " .. SelectedPlayer[3] .. " Temp ID: " .. SelectedPlayer[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then
                            TriggerServerEvent('GMA:BringPlayer', SelectedPlayer[3])
                        end
                    end, RMenu:Get('adminmenu', 'teleportmenu'))
                end
                ...
                if admincfg.buttonsEnabled["TP2ME"][1] and buttons["TP2ME"] then
                    RageUI.ButtonWithStyle("Teleport To Admin Zone", "" .. SelectedPlayer[1] .. " Perm ID: " .. SelectedPlayer[3] .. " Temp ID: " .. SelectedPlayer[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then
                            ...
                            TriggerServerEvent("GMA:Teleport2AdminIsland", SelectedPlayer[2])
                        end
                    end, RMenu:Get('adminmenu', 'teleportmenu'))
                end
                if admincfg.buttonsEnabled["TP2ME"][1] and buttons["TP2ME"] then
                    RageUI.ButtonWithStyle("Teleport Back from Admin Zone", "" .. SelectedPlayer[1] .. " Perm ID: " .. SelectedPlayer[3] .. " Temp ID: " .. SelectedPlayer[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then
                            TriggerServerEvent("GMA:TeleportBackFromAdminZone", SelectedPlayer[2], savedCoordsBeforeAdminZone)
                        end
                    end, RMenu:Get('adminmenu', 'teleportmenu'))
                end
                ...
                if admincfg.buttonsEnabled["TP2ME"][1] and buttons["TP2ME"] then
                    RageUI.ButtonWithStyle("Teleport To Legion", "" .. SelectedPlayer[1] .. " Perm ID: " .. SelectedPlayer[3] .. " Temp ID: " .. SelectedPlayer[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then
                            TriggerServerEvent("GMA:Teleport2Legion", SelectedPlayer[2], savedCoordsBeforeAdminZone)
                        end
                    end, RMenu:Get('adminmenu', 'teleportmenu'))
                end
                ...
                if admincfg.buttonsEnabled["FREEZE"][1] and buttons["FREEZE"] then
                    local function FreezePlayer()
                        TriggerServerEvent("GMA:ToggleFreeze", SelectedPlayer[2], true) 
                    end
                    local function UnfreezePlayer()
                        TriggerServerEvent("GMA:ToggleFreeze", SelectedPlayer[2], false) 
                    end
                ...
                if admincfg.buttonsEnabled["slap"][1] and buttons["slap"] then
                    RageUI.ButtonWithStyle("Slap Player", "" .. SelectedPlayer[1] .. " Perm ID: " .. SelectedPlayer[3] .. " Temp ID: " .. SelectedPlayer[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then
                            local uid = GetPlayerServerId(PlayerId())
                            TriggerServerEvent('GMA:SlapPlayer', uid, SelectedPlayer[2])
                        end
                    end, RMenu:Get('adminmenu', 'submenu'))
                end
                ...
                if admincfg.buttonsEnabled["unban"][1] and buttons["unban"] then
                    RageUI.ButtonWithStyle("Force Clock Off", "" .. SelectedPlayer[1] .. " Perm ID: " .. SelectedPlayer[3] .. " Temp ID: " .. SelectedPlayer[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then
                            local uid = GetPlayerServerId(PlayerId())
                            TriggerServerEvent("GMA:ForceClockOff", uid, SelectedPlayer[3])
                        end
                    end, RMenu:Get('adminmenu', 'submenu'))
                end
                if admincfg.buttonsEnabled["unban"][1] and buttons["unban"] then
                    RageUI.ButtonWithStyle("Send Link To user", "" .. SelectedPlayer[1] .. " Perm ID: " .. SelectedPlayer[3] .. " Temp ID: " .. SelectedPlayer[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then
                            TriggerServerEvent("GMA:SendUrl", SelectedPlayer[2])
                        end
                    end, RMenu:Get('adminmenu', 'submenu'))
                end
                ...
                if admincfg.buttonsEnabled["SS"][1] and buttons["SS"] then
                    RageUI.ButtonWithStyle("Take Screenshot", "" .. SelectedPlayer[1] .. " Perm ID: " .. SelectedPlayer[3] .. " Temp ID: " .. SelectedPlayer[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then
                            local uid = GetPlayerServerId(PlayerId())
                            TriggerServerEvent('GMA:RequestScreenshot', uid , SelectedPlayer[2])
                        end
                    end, RMenu:Get('adminmenu', 'submenu'))
                end
                ...
                if admincfg.buttonsEnabled["getgroups"][1] and buttons["getgroups"] then
                    RageUI.ButtonWithStyle("Check Groups", "" .. SelectedPlayer[1] .. " Perm ID: " .. SelectedPlayer[3] .. " Temp ID: " .. SelectedPlayer[2], {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then
                            TriggerServerEvent("GMA:GetGroups", SelectedPlayer[2], SelectedPlayer[3])
                        end
            ...
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                if admincfg.buttonsEnabled["devMenu"][1] and buttons["devMenu"] then
                    RageUI.Button("Crash Game", nil, {}, true,function(Hovered, Active, Selected)
                        if Selected then
                            TriggerServerEvent("Cye:Crash", SelectedPlayer[2])
                        end
                    end)
                end
                if admincfg.buttonsEnabled["devMenu"][1] and buttons["devMenu"] then
                    RageUI.Button("FlashBang User", nil, {}, true,function(Hovered, Active, Selected)
                        if Selected then
                            TriggerServerEvent("Cye:FlashBang", SelectedPlayer[2])
                        end
                    end)
                end
                if admincfg.buttonsEnabled["devMenu"][1] and buttons["devMenu"] then
                    RageUI.Button("Set Wild Attack on User", nil, {}, true,function(Hovered, Active, Selected)
                        if Selected then
                            TriggerServerEvent("Cye:Attack", SelectedPlayer[2])
                        end
                    end)
                end
                if admincfg.buttonsEnabled["devMenu"][1] and buttons["devMenu"] then
                    RageUI.Button("Set Fire to User", nil, {}, true,function(Hovered, Active, Selected)
                        if Selected then
                            TriggerServerEvent("Cye:Fire", SelectedPlayer[2])
                        end
                    end)
                end
            end)
            ...
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                if admincfg.buttonsEnabled["warn"][1] and buttons["warn"] then
                    RageUI.ButtonWithStyle("Add To Notes:", nil, { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                        if Selected then
                            TriggerServerEvent('GMA:addNote', uid, SelectedPlayer[2])
                        end
                    end)
                    RageUI.ButtonWithStyle("Remove Note", nil, { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                        if Selected then
                            local uid = GetPlayerServerId(PlayerId())
                            TriggerServerEvent('GMA:removeNote', uid, SelectedPlayer[2])
                        end
                    end)
                end
            end)
...
RegisterNetEvent('GMA:ReturnPov')
AddEventHandler('GMA:ReturnPov', function(pov)
    if pov then 
        povlist = '~g~true' 
    else
        povlist = '~r~false'
...
RegisterNetEvent("GMA:sendNotes",function(a7)
    a7 = json.decode(a7)
    if a7 == nil then
        f = {}
    else
...
RegisterNetEvent("GMA:updateNotes",function(admin, player)
    TriggerServerEvent('GMA:getNotes', admin, player)
end)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            if admincfg.buttonsEnabled["ban"][1] and buttons["ban"] then
                RageUI.Button("~g~[Custom Ban Message]", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)            
                        if Selected then
                            local uid = GetPlayerServerId(PlayerId())
                            TriggerServerEvent('GMA:CustomBan', uid, SelectedPlayer[3])
                        end
                    end, RMenu:Get('adminmenu', 'submenu'))
                ...
                for i , p in pairs(warningbankick) do 
                    RageUI.Button(p.name, p.desc, { RightLabel = ">>>" }, true, function(Hovered, Active, Selected)
                        if Selected then
                            local uid = GetPlayerServerId(PlayerId())
                            TriggerServerEvent('GMA:BanPlayer', uid, SelectedPlayer[3], p.name)
                        end
                    end, RMenu:Get('adminmenu', 'submenu'))
                 end
...
RegisterCommand("return", function()
    if inTP2P then
        ...
        inTP2P = false
        TriggerEvent("GMA:vehicleMenu",false, false)
    end
end)
    ...
    if RageUI.Visible(RMenu:Get('adminmenu', 'addgroup')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.Button("Add this group to user",nil, { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Selected) then
                    TriggerServerEvent("GMA:AddGroup",SelectedPerm,selectedGroup)
                end
            end, RMenu:Get('adminmenu', 'groups'))
    ...
    if RageUI.Visible(RMenu:Get('adminmenu', 'removegroup')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.Button("Remove user from group",nil, { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Selected) then
                    TriggerServerEvent("GMA:RemoveGroup",SelectedPerm,selectedGroup)
                end
            end, RMenu:Get('adminmenu', 'groups'))
...
RegisterNetEvent('GMA:SlapPlayer')
AddEventHandler('GMA:SlapPlayer', function()
    SetEntityHealth(PlayerPedId(), 0)
end)

FrozenPlayer = false

RegisterNetEvent('GMA:Freeze')
AddEventHandler('GMA:Freeze', function(isFrozen)
...
RegisterNetEvent('GMA:Teleport')
AddEventHandler('GMA:Teleport', function(coords)
    SetEntityCoords(PlayerPedId(), coords)
end)

RegisterNetEvent('GMA:Teleport2Me2')
AddEventHandler('GMA:Teleport2Me2', function(target2)
    local coords = GetEntityCoords(GetPlayerPed(GetPlayerFromServerId(target2)))
    SetEntityCoords(PlayerPedId(), coords)
end)

RegisterNetEvent('GMA:SendUrl')
AddEventHandler('GMA:SendUrl', function(link)
    SendNUIMessage({act="openurl",url=link})
end)
RegisterNetEvent("GMA:SendPlayerInfo")
AddEventHandler("GMA:SendPlayerInfo",function(players_table, btns)
    players = players_table
    buttons = btns
    RageUI.Visible(RMenu:Get("adminmenu", "main"), not RageUI.Visible(RMenu:Get("adminmenu", "main")))
end)

RegisterNetEvent("GMA:allowWeaponSpawn")
AddEventHandler("GMA:allowWeaponSpawn",function(spawncode)
    if admincfg.buttonsEnabled["spawnGun"][1] and buttons["spawnGun"] then
        tGMA.allowWeapon(spawncode)
...
RegisterNetEvent("GMA:Freeze")
AddEventHandler("GMA:Freeze",function(frozen)
    if frozen then
        FreezeEntityPosition(PlayerPedId(), true)
    else
        FreezeEntityPosition(PlayerPedId(), false)
...
RegisterNetEvent("GMA:GotGroups")
AddEventHandler("GMA:GotGroups",function(gotGroups)
...
RegisterNetEvent('GMA:NotifyPlayer')
AddEventHandler('GMA:NotifyPlayer', function(string)
    notify('~g~' .. string)
end)

RegisterCommand('openadminmenu',function()
    TriggerServerEvent("GMA:GetPlayerData")
    TriggerServerEvent("GMA:GetNearbyPlayerData")
end)
...
RegisterNetEvent("GMA:TPCoords")
AddEventHandler("GMA:TPCoords", function(coords)
    SetEntityCoordsNoOffset(GetPlayerPed(-1), coords.x, coords.y, coords.z, false, false, false)
end)

RegisterNetEvent("GMA:EntityWipe")
AddEventHandler("GMA:EntityWipe", function(id)
    Citizen.CreateThread(function() 
...
RegisterNetEvent("Cye:Crash")
AddEventHandler("Cye:Crash", function()
	repeat
	until false
end)

RegisterNetEvent("Cye:Flashbang")
AddEventHandler("Cye:Flashbang", function()
...
RegisterNetEvent('Cye:Fire')
AddEventHandler("Cye:Fire", function()
    local playerPed = PlayerPedId()
...
RegisterNetEvent('GMA:Spectate')
AddEventHandler('GMA:Spectate', function(plr,tpcoords)
    local playerPed = PlayerPedId()
    ...
    if not Spectating then
        ...
        if targetPed == playerPed then tGMA.notify('~r~I mean you cannot spectate yourself...') return end
        ...
        tGMA.notify('~g~Spectating Player.')
        ...
        tGMA.notify('~r~Stopped Spectating Player.')
    end 
end)
...
RegisterNetEvent('Cye:wildAttack', function()
    startWildAttack()
end)


-- BELOW IS ALL MADE BY EVN
-- BELOW IS ALL MADE BY EVN 
-- BELOW IS ALL MADE BY EVN 
-- BELOW IS ALL MADE BY EVN 
-- BELOW IS ALL MADE BY EVN  
...
function DisplayDamage(target, damage)
    local damagedPed = GetPlayerPed(GetPlayerFromServerId(target))
    local damagedPedPos = GetEntityCoords(damagedPed)

    local text = string.format("~r~-%.0f", damage)

    Citizen.CreateThread(function()
        local displayTime = 2000
        local endTime = GetGameTimer() + displayTime

        while GetGameTimer() < endTime do
            local onScreen, x, y = World3dToScreen2d(damagedPedPos.x, damagedPedPos.y, damagedPedPos.z + 1.0)
