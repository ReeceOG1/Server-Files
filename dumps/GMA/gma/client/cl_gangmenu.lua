...
AddEventHandler("playerSpawned",function()
    local h = true
    if h then 
        TriggerServerEvent("GMA:GetGangData")
    end 
end)
RegisterNetEvent("GMA:GotGangData")
AddEventHandler("GMA:GotGangData",function(i,j,k,PO)
    if i==nil then 
        PlayerIsInGang=false 
    else 
        PlayerIsInGang=true
...
RegisterNetEvent("GMA:disbandedGang")
AddEventHandler("GMA:disbandedGang",function()
    PlayerIsInGang=false
    showGangUI=false
    showNoGangUI=true
    ...
    TriggerEvent("GMA:ForceRefreshData")
end)
RegisterNetEvent("GMA:ForceRefreshData")
AddEventHandler("GMA:ForceRefreshData",function()
    TriggerServerEvent("GMA:GetGangData")
end)
RegisterNetEvent("GMA:InviteRecieved")
AddEventHandler("GMA:InviteRecieved",function(o,Q)
    GMAGangInvites[GMAGangInviteIndex]=Q
    GMAGangInviteIndex=GMAGangInviteIndex+1
    tGMA.notify(o)
end)
RegisterNetEvent("GMA:gangNameNotTaken")
AddEventHandler("GMA:gangNameNotTaken",function()
    showNoGangUI=false
    showGangUI=true
    PlayerIsInGang=true 
end)
function func_drawGangUI()
    if showNoGangUI then 
        ...
        DrawAdvancedText(0.558,0.303,0.005,0.0028,0.539,"GMA Gangs",255,255,255,255,7,0)
        ...
        for p,q in pairs(GMAGangInvites)do 
        ...
        if CursorInArea(0.35,0.415,0.415,0.46)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if createGangName~=nil and createGangName~="null"and createGangName~=""then 
                    TriggerServerEvent("GMA:CreateGang",createGangName)
                else 
                    tGMA.notify("~r~No gang name entered!")
                end 
            end 
        else 
            CreateGangSelectionRed=0
        ...
        if CursorInArea(0.435,0.51,0.415,0.46)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if selectedGangInvite~=nil then 
                    selectedGangInvite=GMAGangInvites[selectedGangInvite]
                    local perm1s = tGMA.getUserId()
                    TriggerServerEvent("GMA:addUserToGang",selectedGangInvite,perm1s)
                    GMAGangInvites={}
                    showNoGangUI=false
                    GMAGangInviteIndex=0
                    showGangUI=true
                    ...
                    tGMA.notify("~r~No gang invite selected")
                end 
            end 
        else 
            JoinGangSelectionRed=0
    ...
    if showFundsGangUI then 
        ...
        DrawAdvancedText(0.591,0.378,0.005,0.0028,0.48,"GMA Gang - Funds",255,255,255,255,7,0)
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount ~= nil then
                    if closeToBarclays() then
                        TriggerServerEvent("GMA:depositGangBalance", amount)
                    end
                else
                    tGMA.notify("~r~No amount entered!")
                end
            end
        else
        ...
        if CursorInArea(0.4083, 0.4718, 0.5490, 0.5999) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if closeToBarclays() then
                    TriggerServerEvent("GMA:depositAllGangBalance")
                end
            end
        else
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount ~= nil then
                    if gangPermission >= 3 then
                        if closeToBarclays() then
                            TriggerServerEvent("GMA:withdrawGangBalance", amount)
                        end
                    else
                        tGMA.notify("~r~You don't have a high enough rank to withdraw")
                    end
                else
                    tGMA.notify("~r~No amount entered!")
                end
        ...
        if CursorInArea(0.6088, 0.6739, 0.5481, 0.6018) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if gangPermission >= 3 then
                    if closeToBarclays() then
                        TriggerServerEvent("GMA:withdrawAllGangBalance")
                    end
                else
                    tGMA.notify("~r~You don't have a high enough rank to withdraw")
                end
    ...
    if showMembersGangUI then 
        ...
        DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"GMA Gang - Members",255,255,255,255,7,0)
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if selectedMember~=nil and PlayerIsInGang and gangID~=nil then 
                    if gangPermission>=4 then 
                        TriggerServerEvent("GMA:PromoteUser",gangID,tonumber(selectedMember))
                    else 
                        tGMA.notify("~r~You don't have permission to promote!")
                    end 
                else 
                    tGMA.notify("~r~No gang member selected")
                end 
            end 
        else 
        ...
        if CursorInArea(0.6182,0.6822,0.4138,0.4694)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if selectedMember~=nil and PlayerIsInGang and gangID~=nil then 
                    if gangPermission>=4 then 
                        TriggerServerEvent("GMA:DemoteUser",gangID,selectedMember)
                    else 
                        tGMA.notify("~r~You don't have permission to demote!")
                    end 
                else 
                    tGMA.notify("~r~No gang member selected")
                end 
                ...
                if selectedMember~=nil then 
                    if gangPermission>=3 then 
                        if YesNoConfirm()then 
                            TriggerServerEvent("GMA:kickMemberFromGang",gangID,selectedMember)
                        end 
                    else 
                        tGMA.notify("~r~You don't have permission to kick!")
                    end 
                else 
                    tGMA.notify("~r~No gang member selected")
                end 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if s~=nil then 
                    if gangPermission>=2 then 
                        TriggerServerEvent("GMA:InviteUserToGang",gangID,s)
                    else 
                        tGMA.notify("~r~You don't have permission to invite players")
                    end 
                else 
                    tGMA.notify("No player name entered")
                end 
            end 
        else 
        ...
        if CursorInArea(0.3281,0.3723,0.6768,0.7074)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                PlaySound(-1,"SELECT","HUD_FRONTEND_DEFAULT_SOUNDSET",0,0,1)
                if e<=1 then 
                    tGMA.notify("~r~Lowest page reached")
                else 
                    e=e-1 
                end 
            end 
        ...
        if CursorInArea(0.4130,0.4572,0.6712,0.7064)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                PlaySound(-1,"SELECT","HUD_FRONTEND_DEFAULT_SOUNDSET",0,0,1)
                if e>=f then 
                    tGMA.notify("~r~Max page reached")
                else 
                    e=e+1 
                end 
            end 
    ...
    if showLogsGangUI then 
        DrawRect(0.501,0.525,0.421,0.387,0,0,0,150)
        DrawRect(0.501,0.308,0.421,0.047,255,255,0,248)
        DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"GMA Gang - Logs",255,255,255,255,7,0)
    ...
    if showSettingsGangUI then 
        DrawRect(0.501,0.525,0.421,0.387,0,0,0,150)
        DrawRect(0.501,0.308,0.421,0.047,255,255,0,248)
        DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"GMA Gang - Settings",255,255,255,255,7,0)
        ...
        if CursorInArea(0.3187,0.3937,0.4712,0.5462)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                PlaySound(-1,"SELECT","HUD_FRONTEND_DEFAULT_SOUNDSET",0,0,1)
                if YesNoConfirm()then 
                    TriggerServerEvent("GMA:memberLeaveGang",gangID)
                    setCursor(0)
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if gangPermission>=4 then 
                    if YesNoConfirm()==true and gangID~=nil then 
                        TriggerServerEvent("GMA:DeleteGang",gangID)
                    end 
                else 
                    tGMA.notify("~r~You don't have permission to disband!")
                end 
            end 
        else 
    ...
    --     DrawRect(0.501,0.308,0.421,0.047,255,255,0,248)
    --     DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"GMA Gang - Turfs",255,255,255,255,7,0)
    ...
    if showGangUI then 
        ...
        DrawAdvancedText(0.591,0.399,0.005,0.0028,0.51,"GMA Gangs",255,255,255,255,7,0)
...
RegisterNetEvent("GMA:Notify",function(u)
    tGMA.notify(u)
end)
...
function closeToBarclays()
    ...
    if w<10.0 or x<10.0 or z<10.0 then 
        ...
        tGMA.notify("~r~You must be near Barclays to access gang funds.")
        return false 
    end 
end
