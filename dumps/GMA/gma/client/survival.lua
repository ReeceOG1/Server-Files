        ...
        if in_coma then
			if not calledNHS then
				if IsControlJustPressed(1, 51) then
					tGMA.notify('~g~NHS called to your approximate location')
                    TriggerEvent("GMA:DEATH_SCREEN_NHS_CALLED")
				end
			end
...
AddEventHandler("GMA:countdownEnded",function()
    g = true
end)
    ...
    while true do 
      if IsDisabledControlJustPressed(0,38) then
        if g and in_coma then
            TriggerEvent("GMA:CLOSE_DEATH_SCREEN")
            ...
            tGMA.respawnPlayer()
            DoScreenFadeIn(900)
            TriggerEvent("GMA:StartRespawnCam")
            TriggerEvent("GMA:OpenRespawnMenu")
        end
        Wait(1000)
      end
      Wait(0)
...
Citizen.CreateThread(function() -- coma thread
    ...
    while true do
        ...
        if IsEntityDead(GetPlayerPed(-1)) and not in_coma then --Wait for death check
            ...
            GMAserver.StoreWeaponsDead()
            GMAserver.Coma()
            TriggerEvent('GMA:IsInMoneyComa', true)
            TriggerServerEvent('GMA:InComa')
            GMAserver.MoneyDrop()
            TriggerEvent('GMA:5Seconds')
            TriggerServerEvent("Server:SoundToCoords", plyCoords.x, plyCoords.y, plyCoords.z, 60.0, "Untitled", 0.4);
            tGMA.ejectVehicle()
            in_coma = true
            TriggerServerEvent("GMA:getNumOfNHSOnline")
            TriggerEvent("gma:PlaySound", "dead")
...
RegisterNetEvent("GMA:5Seconds")
AddEventHandler("GMA:5Seconds", function()
    DeathAnim = 5
    while DeathAnim <= 5 and DeathAnim >= 0 do
        Wait(1000)
        DeathAnim = DeathAnim - 1
...
function tGMA.disableComa()
    in_coma = false
    killfeed = false
end

function tGMA.isInComa()
    return in_coma
end

RegisterNetEvent("GMA:FixClient")
AddEventHandler("GMA:FixClient", function()
    ...
    Citizen.CreateThread(function()
        while true do 
            ...
            if resurrectspamm == true then
                ...
                tGMA.stopScreenEffect(cfg.coma_effect)
                TriggerEvent("GMA:CLOSE_DEATH_SCREEN")
                DoScreenFadeIn(500)
                Citizen.Wait(500)
            end 
...
local L = {}
function tGMA.getKillerInfo()
    return L
end

RegisterNetEvent("GMA:getNumberOfDocsOnline",function(I)
    c = I
    e = 60000
    f = e / 1000
    i = 10
    ...
    CreateThread(function()
        ...
        while tGMA.getKillerInfo().ready == nil do
            Wait(0)
        end
        local L = tGMA.getKillerInfo()
        local M = false
        if L.name == nil then
            M = true
        end
        g = false
        TriggerEvent("GMA:SHOW_DEATH_SCREEN", f, L.name or "N/A", L.user_id or "N/A", L.weapon or "N/A", M)
    end)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsEntityDead(PlayerPedId()) then
            ...
            local distance = 0
            local a6 = false
            local az = tGMA.getPedServerId(PedKiller)
            ...
            L.source = az
            L.user_id = tGMA.getPedServerId(PedKiller)
            L.weapon = tostring(R)
            L.ready = true
            ...
            if not g and in_coma and killfeed then
                ...
                TriggerServerEvent("GMA:onPlayerKilled", "finished off", tGMA.getPedServerId(PedKiller), s)
            else
                print("Killed normally")
                TriggerServerEvent("GMA:onPlayerKilled", "killed", tGMA.getPedServerId(PedKiller), R, a6, distance)
                killfeed = true
            end
            Killer = nil
            PedKiller = nil
...
RegisterCommand("kill", function(source, args, rawCommand)
    TriggerServerEvent("GMA:onPlayerKilled", "killed", 1, "WEAPON_SPAR17", false, 100)
end)

function tGMA.reviveComa()
    local ped = GetPlayerPed(-1)
    SetEntityInvincible(ped,false)
    tGMA.setRagdoll(false)
    tGMA.stopScreenEffect(cfg.coma_effect)
    SetEntityHealth(GetPlayerPed(-1), 200)
end

-- kill the player if in coma
function tGMA.killComa()
    if in_coma then
        coma_left = 0
    end
end
...
function tGMA.varyHealth(variation)
    local ped = GetPlayerPed(-1)
...
function tGMA.getHealth()
    return GetEntityHealth(GetPlayerPed(-1))
end

function tGMA.getArmour()
    return GetPedArmour(GetPlayerPed(-1))
end

function tGMA.setHealth(health)
    local n = math.floor(health)
    SetEntityHealth(GetPlayerPed(-1),n)
end

function tGMA.setArmour(armour)
    SetPedArmour(PlayerPedId(), armour)
end

function tGMA.setFriendlyFire(flag)
    local flag = true
    NetworkSetFriendlyFireOption(flag)
    SetCanAttackFriendly(GetPlayerPed(-1), flag, flag)
end

function tGMA.setPolice(flag)
    local flag = false
