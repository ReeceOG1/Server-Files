...
-- pause
AddEventHandler("GMA:pauseChange", function(paused)
  SendNUIMessage({act="pause_change", paused=paused})
end)

-- MENU

function tGMA.openMenuData(menudata)
  SendNUIMessage({act="open_menu", menudata = menudata})
end

function tGMA.closeMenu()
  SendNUIMessage({act="close_menu"})
end

-- PROMPT

function tGMA.prompt(title,default_text)
...
function tGMA.request(id,text,time)
...
RegisterNUICallback("menu",function(data,cb)
  if data.act == "close" then
    GMAserver.closeMenu({data.id})
  elseif data.act == "valid" then
    GMAserver.validMenuChoice({data.id,data.choice,data.mod})
  end
end)
...
RegisterNUICallback("prompt",function(data,cb)
  if data.act == "close" then
    SetNuiFocus(false)
    SetNuiFocus(false)
    GMAserver.promptResult({data.result})
  end
end)
...
RegisterNUICallback("request",function(data,cb)
  if data.act == "response" then
    GMAserver.requestResult({data.id,data.ok})
  end
end)
...
function tGMA.announce(background,content)
...
-- try to fix missing cfg issue (cf: https://github.com/ImagicTheCat/GMA/issues/89)
for i=1,5 do
...
function tGMA.setProgressBar(name,anchor,text,r,g,b,value)
...
function tGMA.setProgressBarValue(name,value)
  SendNUIMessage({act="set_pbar_val", name = name, value = value})
end

-- set progress bar text
function tGMA.setProgressBarText(name,text)
  SendNUIMessage({act="set_pbar_text", name = name, text = text})
end

-- remove a progress bar
function tGMA.removeProgressBar(name)
...
Citizen.CreateThread(function()
  while true do
    ...
    -- open general menu
    if IsControlJustPressed(table.unpack(cfg.controls.phone.open)) and (not tGMA.isInComa() or not cfg.coma_disable_menu) and (not tGMA.isHandcuffed() or not cfg.handcuff_disable_menu) and GetLastInputMethod(0) then 
      GMAserver.openMainMenu({})
    end
    ...
    if pause_menu and not paused then
      paused = true
      TriggerEvent("GMA:pauseChange", paused)
