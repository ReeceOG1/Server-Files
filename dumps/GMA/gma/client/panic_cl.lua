...
tGMA.hasPoliceJob = function(bool)
    police = bool
end

tGMA.PanicCL = function(player, s1, s2, name)
    if police then
    ...
    while true do
        if showPanicBlip and police and not panic then
            ...
            if tostring(GetStreetNameFromHashKey(streetA)) and tostring(GetStreetNameFromHashKey(streetB)) then
                ...
                GMAserver.PanicSV({player,  GetStreetNameFromHashKey(streetA), GetStreetNameFromHashKey(streetB)})
                GMAserver.BlipSV({plyPos.x, plyPos.y, plyPos.z})
            else
                GMAserver.PanicSV({player,  "No street found", ""})
                GMAserver.BlipSV({plyPos.x, plyPos.y, plyPos.z})
            end
        end
