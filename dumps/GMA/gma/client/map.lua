function tGMA.addBlip(x,y,z,idtype,idcolor,text)
...
-- return native id
function tGMA.setNamedBlip(name,x,y,z,idtype,idcolor,text)
  tGMA.removeNamedBlip(name) -- remove old one

  named_blips[name] = tGMA.addBlip(x,y,z,idtype,idcolor,text)
  return named_blips[name]
end

-- remove a named blip
function tGMA.removeNamedBlip(name)
  if named_blips[name] ~= nil then
    tGMA.removeBlip(named_blips[name])
...
-- return marker id
function tGMA.addMarker(x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
...
-- return id
function tGMA.setNamedMarker(name,x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
  tGMA.removeNamedMarker(name) -- remove old marker

  named_markers[name] = tGMA.addMarker(x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
  return named_markers[name]
end

function tGMA.removeNamedMarker(name)
  if named_markers[name] ~= nil then
    tGMA.removeMarker(named_markers[name])
...
function tGMA.setArea(name,x,y,z,radius,height)
...
-- locked: boolean
-- doorswing: -1 to 1
function tGMA.setStateOfClosestDoor(doordef, locked, doorswing)
...
function tGMA.openClosestDoor(doordef)
  tGMA.setStateOfClosestDoor(doordef, false, 0)
end

function tGMA.closeClosestDoor(doordef)
  tGMA.setStateOfClosestDoor(doordef, true, 0)
end
