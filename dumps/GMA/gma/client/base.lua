...
cfgweapons = module("GMAWeap", "cfg/cfg_weaponNames")
local user_id = 0
tGMA = {}
allowedWeapons = {}
...
Tunnel.bindInterface("GMA",tGMA)

-- get server interface
GMAserver = Tunnel.getInterface("GMA","GMA")

-- add client proxy interface (same as tunnel interface)
Proxy.addInterface("GMA",tGMA)

function tGMA.allowWeapon(name)
...
function tGMA.removeWeapon(name)
  if allowedWeapons[name] then
    allowedWeapons[name] = nil
  end
end

function tGMA.ClearWeapons()
  allowedWeapons = {}
end


function tGMA.checkWeapon(name)
...
function tGMA.getPedServerId(a5)
...
-- functions
function tGMA.teleport(x,y,z)
  tGMA.unjail() -- force unjail before a teleportation
  SetEntityCoords(GetPlayerPed(-1), x+0.0001, y+0.0001, z+0.0001, 1,0,0,1)
  GMAserver.updatePos({x,y,z})
end

-- return x,y,z
function tGMA.getPosition()
  local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
  return x,y,z
end

--returns the distance between 2 coordinates (x,y,z) and (x2,y2,z2)
function tGMA.getDistanceBetweenCoords(x,y,z,x2,y2,z2)
...
function tGMA.addPlayer(player)
  players[player] = true
end

function tGMA.removePlayer(player)
  players[player] = nil
end

function tGMA.getNearestPlayers(radius)
  local r = {}

  local ped = GetPlayerPed(i)
  local pid = PlayerId()
  local px,py,pz = tGMA.getPosition()
...
function tGMA.getNearestPlayer(radius)
  local p = nil

  local players = tGMA.getNearestPlayers(radius)
...
function tGMA.notify(msg)
  SetNotificationTextEntry("STRING")
  AddTextComponentString(msg)
  DrawNotification(true, false)
end

function tGMA.notifyPicture(icon, type, sender, title, text)
    SetNotificationTextEntry("STRING")
...
function tGMA.playScreenEffect(name, duration)
...
-- name, see https://wiki.fivem.net/wiki/Screen_Effects
function tGMA.stopScreenEffect(name)
  StopScreenEffect(name)
end
...
function tGMA.setBasePlayers(players)
  baseplayers = players
end

function tGMA.addBasePlayer(player, id)
  baseplayers[player] = id
end

function tGMA.removeBasePlayer(player)
  --baseplayers[player] = nil
end

function tGMA.setUserID(a)
  user_id = a
end
function tGMA.getUserId(Z)
  if Z then
...
-- looping: if true, will GMAly loop the first element of the sequence until stopAnim is called
function tGMA.playAnim(upper, seq, looping)
  if seq.task ~= nil then -- is a task (cf https://github.com/ImagicTheCat/GMA/pull/118)
    tGMA.stopAnim(true)
...
Citizen.CreateThread(
    function()
        while true do
            local ay = {}
            ay.playerPed = tGMA.getPlayerPed()
            ay.playerCoords = tGMA.getPlayerCoords()
            ay.playerId = tGMA.getPlayerId()
            ay.vehicle = tGMA.getPlayerVehicle()
...
function tGMA.createThreadOnTick(aA)
  ax[#ax + 1] = aA
end
local ai = 0
local R = 0
...
function tGMA.getPlayerPed()
  return ai
end
function tGMA.getPlayerVehicle()
  return R, U
end
function tGMA.getPlayerId()
  return S
end
function tGMA.getPlayerCoords()
  return T
end
...
function tGMA.generateUUID(ar, am, type)
    if aj[ar] == nil then
        aj[ar] = {}
    end
    if type == nil then
...
function tGMA.playSpatializedSound(dict,name,x,y,z,range)
  PlaySoundFromCoord(-1,name,x+0.0001,y+0.0001,z+0.0001,dict,0,range+0.0001,0)
end

-- play sound
function tGMA.playSound(dict,name)
  PlaySound(-1,name,dict,0,0,1)
end

--[[
-- not working
function tGMA.setMovement(dict)
  if dict then
...
AddEventHandler("playerSpawned",function()
  TriggerServerEvent("GMAcli:playerSpawned")
  TriggerServerEvent("GMA:gettingUserID")
  TriggerServerEvent("GMA:PoliceCheck")
  TriggerServerEvent("GMA:RebelCheck")
  TriggerServerEvent("GMA:VIPCheck")
  TriggerServerEvent("GMA:getHouses")
end)

AddEventHandler("playerSpawned",function()
  TriggerEvent("GMA:UnFreezeRespawn")
end)

AddEventHandler("onPlayerDied",function(player,reason)
  TriggerServerEvent("GMAcli:playerDied")
end)
...
TriggerServerEvent('GMA:CheckID')

RegisterNetEvent('GMA:CheckIdRegister')
AddEventHandler('GMA:CheckIdRegister', function()
    TriggerEvent('playerSpawned', GetEntityCoords(PlayerPedId()))
end)

RegisterNetEvent("GMA:setUserID")
AddEventHandler("GMA:setUserID", function(serverid)
  user_id = serverid
end)
...
function tGMA.isGrinder(user_id) 
  return grind[user_id] or false
end

function tGMA.isDeveloper(user_id) 
  return developers[user_id] or false
end

function tGMA.getUserId(val)
  if val==nil then
    return user_id
  end
end
...
local BucketValue = 0
function tGMA.getPlayerBucket()
    return BucketValue
end
RegisterNetEvent("GMA:SetPlayerBucket",function(BucketNumber)
...
Citizen.CreateThread(
    function()
        while true do
            if tGMA.isDeveloper(tGMA.getUserId()) then
