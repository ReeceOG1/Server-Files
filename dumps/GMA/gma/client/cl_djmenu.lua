    ...
    RageUI.IsVisible1(RMenu:Get('MUSICMENU','main'),true,false,true,function()
        if not a.active then 
            ...
            RageUI.Button("Help","Assistance message",{},true,function(k,l,m)
                if m then 
                    TriggerEvent("GMA:showNotification",{text="The Video ID is the ID at the end of the YouTube URL after =",height="200px",width="auto",colour="#FFF",background="#32CD32",pos="bottom-right",icon="success"},5000)
                end 
            end)
        end 
    end,function()
            ...
            for o,p in pairs(i)do 
                if i[o]~=nil then 
                    ...
                    RageUI.ButtonWithStyle("ID: "..p[3].." - Started: "..p[5].."","Name: "..p[4].." Distance Away: "..distance.." metres",{RightLabel="→→→"},true,function(k,l,m)
                        if m then 
                            TriggerServerEvent("GMA:adminStopSong",p[1])
                            table.remove(i,o)
                        end 
                    end)
                end 
...
RegisterNetEvent("GMA:playDjSong")
AddEventHandler("GMA:playDjSong",function(s,coords,t,u)
...
RegisterNetEvent("GMA:requestCurrentProgress")
AddEventHandler("GMA:requestCurrentProgress",function(s,coords)
    if not g then 
...
RegisterNetEvent("GMA:toggleDjMenu")
AddEventHandler("GMA:toggleDjMenu",function()
    RageUI.Visible(RMenu:Get('MUSICMENU','main'),true)
    h=true 
end)
RegisterNetEvent("GMA:toggleDjAdminMenu")
AddEventHandler("GMA:toggleDjAdminMenu",function(x)
    RageUI.Visible(RMenu:Get('MUSICMENU','admin'),true)
    i=x 
end)
RegisterNetEvent("GMA:finaliseSong")
AddEventHandler("GMA:finaliseSong",function(y)
    if a.active then 
        TriggerServerEvent("GMA:playDjSongServer",y,a.coords)
    end 
end)
RegisterNetEvent("GMA:updateDjVolume")
AddEventHandler("GMA:updateDjVolume",function(coords,z)
    if specificSongPlaying(coords)then 
        SendNUIMessage({type='djVolume',volume=z})
    end 
end)
RegisterNetEvent("GMA:stopSong")
AddEventHandler("GMA:stopSong",function(coords)
    local A=false
    for o,p in pairs(c)do 
        if p[1]==coords then 
            A=true
...
RegisterNetEvent("GMA:skipDj")
AddEventHandler("GMA:skipDj",function(coords,B)
    if specificSongPlaying(coords)then 
        if B then 
...
function skipDj(B)
    if songPlaying()then 
        TriggerServerEvent("GMA:skipServer",a.coords,B)
    end 
end
function setupDj()
    a.active=true
...
function stopDj()
    if songPlaying()then 
        TriggerServerEvent("GMA:stopSongServer",a.coords)
    end
    a.active=false
...
function updateVolume()
    if songPlaying()then 
        TriggerServerEvent("GMA:updateVolumeServer",a.coords,b.volume)
    end 
end
...
function func_checkDjSongs(J)
    for o,p in pairs(c)do 
        if#(J.playerCoords-p[1])>45.0 then 
            ...
            if not c[o][3]then 
                g=true
                TriggerServerEvent('GMA:requestCurrentProgressServer',o,p[1])
                c[o][3]=true 
            end 
        end 
    end 
...
RegisterNUICallback('returnProgress',function(K,L)
    if K.progress~=nil then 
        if K.progress~=0 then 
            TriggerServerEvent("GMA:returnProgressServer",d,e,K.progress)
        end 
    end
    L('return')
end)
RegisterNetEvent("GMA:returnProgress")
AddEventHandler("GMA:returnProgress",function(s,coords,M,N)
    if g then 
        if specificSongPlaying(coords)then 
            c[s]=nil
            c[s]={coords,s,true}
