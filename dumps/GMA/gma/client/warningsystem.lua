...
RegisterNetEvent("gma:showWarningsOfUser")
AddEventHandler("gma:showWarningsOfUser",function(gmawarningstables)
	showWarningSystem = true
	gmaWarnings = gmawarningstables
end)

RegisterNetEvent("gma:recievedRefreshedWarningData")
AddEventHandler("gma:recievedRefreshedWarningData",function(gmawarningstables)
	gmaWarnings = gmawarningstables
end)
...
RegisterCommand('warnings', function()
    ...
    if showWarningSystem then
        TriggerServerEvent("gma:refreshWarningSystem")
    end
end)
...
Citizen.CreateThread(function()
	while true do
		if showWarningSystem then
			...
			DrawAdvancedText(0.59, 0.194, 0.005, 0.0028, 0.619, 'GMA Punishments', 255, 255, 255, 255, 6, 0)
			...
			for warningID,warningTable in pairs(gmaWarnings) do
