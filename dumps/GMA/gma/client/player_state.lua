...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(GMAConfig.PlayerSavingTime)
    if IsPlayerPlaying(PlayerId()) and state_ready then
        local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
        GMAserver.updatePos({x,y,z})
        GMAserver.updateHealth({tGMA.getHealth()})
        GMAserver.updateArmour({GetPedArmour(PlayerPedId())})
        GMAserver.updateWeapons({tGMA.getWeapons()})
        GMAserver.updateCustomization({tGMA.getCustomization()})
        TriggerServerEvent('GMA:changeHairStyle')
        TriggerServerEvent('GMA:ChangeTattoos')
...
function tGMA.getWeaponTypes()
  return weapon_types
end

function tGMA.getWeapons()
...
function tGMA.giveWeapons(weapons,clear_before)
  local player = GetPlayerPed(-1)
...
function tGMA.giveWeaponAmmo(hash, amount)
...
function tGMA.getDrawableTextures(c,f)
  local d,e=a(c)
  if d then 
...
function tGMA.getCustomization()
  local g=PlayerPedId()
  local h={}
...
function tGMA.setCustomization(h,j,k)
  if h then 
      local g=PlayerPedId()
      local l=nil;
