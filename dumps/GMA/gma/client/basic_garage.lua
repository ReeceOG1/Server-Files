...
function tGMA.spawnGarageVehicle(vtype, name, pos, vehiclename, vehicleplate) -- vtype is the vehicle type (one vehicle per type allowed at the same time)
    ...
    if vehicle == nil then
        ...
        if HasModelLoaded(mhash) then
            ...
            TriggerServerEvent("GMA:PayVehicleTax")
        ...
        tGMA.notify("~r~Vehicle " ..vehiclename.. " is already out.")
    end
end

function tGMA.despawnGarageVehicle(vtype, max_range)
    local Vehicle = tGMA.getNearestVehicle(3)
...
function tGMA.getNearestVehicl2()
    local ped = GetPlayerPed(-1)
    if IsPedSittingInAnyVehicle(ped) then
...
function tGMA.fixeNearestVehicle(radius)
    local veh = tGMA.getNearestVehicle(radius)
    if IsEntityAVehicle(veh) then
        SetVehicleFixed(veh)
    end
end

function tGMA.replaceNearestVehicle(radius)
    local veh = tGMA.getNearestVehicle(radius)
    if IsEntityAVehicle(veh) then
...
function tGMA.getVehicleAtPosition(x, y, z)
    x = x + 0.0001
    y = y + 0.0001
    z = z + 0.0001
...
function tGMA.getNearestOwnedVehicle(radius)
...
-- return ok,x,y,z
function tGMA.getAnyOwnedVehiclePosition()
    for k, v in pairs(vehicles) do
...
-- return x,y,z
function tGMA.getOwnedVehiclePosition(vtype)
...
function tGMA.getOwnedVehicleId(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
...
-- vehicle commands
function tGMA.vc_openDoor(hash, door_index)
...
function tGMA.vc_closeDoor(hash, door_index)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tGMA.vc_detachTrailer(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tGMA.vc_detachTowTruck(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tGMA.vc_detachCargobob(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tGMA.vc_toggleEngine(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tGMA.vc_toggleLock(hash)
    local vehicle = vehicles[hash]
    if vehicle then
        local veh = vehicle[3]
        ...
        if locked then -- unlock
            ...
            tGMA.notify("Vehicle unlocked.")
        else -- lock
            SetVehicleDoorsLocked(veh, 2)
            SetVehicleDoorsLockedForAllPlayers(veh, true)
            tGMA.notify("Vehicle locked.")
        end
    end
end