RMenu.Add('GMAPoliceMenu', 'main', RageUI.CreateMenu("","GMA Police",1250,100, "banners", "police"))
        ...
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                ...
                RageUI.Button("Delete Speedzone" , nil, {}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        TriggerServerEvent('GMA:RemoveZone')
...
RegisterCommand('pd', function()
  if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
    TriggerServerEvent('GMA:OpenPoliceMenu')
  end
end)
...
RegisterNetEvent("GMA:PoliceMenuOpened")
AddEventHandler("GMA:PoliceMenuOpened",function()
...
function createZone() 
  ...
  if speed == 0 then 
    ...
    return
      ...
      TriggerServerEvent('GMA:ActivateZone', message, speed, radius, x, y, z)
end

RegisterNetEvent('GMA:ZoneCreated')
AddEventHandler('GMA:ZoneCreated', function(speed, radius, x, y, z)
...
RegisterNetEvent('GMA:RemovedBlip')
AddEventHandler('GMA:RemovedBlip', function()
...
RegisterNetEvent("GMA:DragPlayer")
AddEventHandler('GMA:DragPlayer', function(pl)
    other = pl
    drag = not drag
end)
	...
	while true do 
		if isInJail == true then 
			if isInArea(v5, 20.4) then 
				...
				GMA.notify({"~r~You have been teleported back to prison!"})
