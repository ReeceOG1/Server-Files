...
function tGMA.toggleHandcuff()
  ...
  if handcuffed then
    ...
    tGMA.playAnim(true,{{"mp_arresting","idle",1}},true)
  else
    DeleteEntity(cuffs)
    tGMA.stopAnim(true)
...
function tGMA.setHandcuffed(flag)
  if handcuffed ~= flag then
    tGMA.toggleHandcuff()
  end
end

function tGMA.isHandcuffed()
  return handcuffed
end

-- (experimental, based on experimental getNearestVehicle)
function tGMA.putInNearestVehicleAsPassenger(radius)
...
function tGMA.putInNetVehicleAsPassenger(net_veh)
...
function tGMA.putInVehiclePositionAsPassenger(x,y,z)
  local veh = tGMA.getVehicleAtPosition(x,y,z)
  if IsEntityAVehicle(veh) then
    for i=1,GetVehicleMaxNumberOfPassengers(veh) do
      if IsVehicleSeatFree(veh,i) then
        SetPedIntoVehicle(GetPlayerPed(-1),veh,i)
...
function tGMA.jail(x,y,z,radius)
  tGMA.teleport(x,y,z) -- teleport to center
  jail = {x+0.0001,y+0.0001,z+0.0001,radius+0.0001}
end

-- unjail the player
function tGMA.unjail()
  jail = nil
end

function tGMA.isJailed()
  return jail ~= nil
end
