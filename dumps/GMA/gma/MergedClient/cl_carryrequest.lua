...
RegisterCommand("carry",function(source, args)
    if not carry.InProgress then
        ...
        if closestPlayer then
            ...
            if targetSrc ~= -1 then
                TriggerServerEvent("GMA:CarryRequest", targetSrc, OMioDioMode)
            else
...
RegisterNetEvent("GMAEXTRAS:StartCarry")
AddEventHandler("GMAEXTRAS:StartCarry", function(targetSrc)
...
RegisterNetEvent("GMAEXTRAS:CarryTargetAsk")
AddEventHandler("GMAEXTRAS:CarryTargetAsk", function(senderSrc)
    carryrequest = true
    Citizen.CreateThread(function()
        while carryrequest do
            if IsControlJustPressed(1, 83) then
                TriggerServerEvent("GMAEXTRAS:CarryAccepted",senderSrc)
                carryrequest = false
            elseif IsControlJustPressed(1, 84) then
                TriggerServerEvent("GMAEXTRAS:CarryDeclined",senderSrc)
                carryrequest = false
            end
            Wait(0)
        end
