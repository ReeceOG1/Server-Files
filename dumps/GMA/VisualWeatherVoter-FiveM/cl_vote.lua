...
function voteWeather(weatherType) 
	...
	if voteInProgress then
		if not voted then
			TriggerServerEvent("GMA:vote", weatherType)
			voted = true
...
function changeWeather()
	...
	TriggerServerEvent("GMA:setCurrentWeather",highestWeather)
end

RegisterNetEvent("GMA:voteFinished")
AddEventHandler("GMA:voteFinished", function(newWeather)
	currentWeather = newWeather
end)

RegisterNetEvent("GMA:startWeatherVote")
AddEventHandler("GMA:startWeatherVote", function()
	voteInProgress = true
...
RegisterNetEvent("GMA:voteStateChange")
AddEventHandler("GMA:voteStateChange",function(type)
	votesTable[type] = votesTable[type] + 1
end)

RegisterCommand("voteweather", function()
	TriggerServerEvent("GMA:tryStartWeatherVote")
end, false)
...
AddEventHandler('playerSpawned', function(spawn)
    TriggerServerEvent("GMA:getCurrentWeather")
