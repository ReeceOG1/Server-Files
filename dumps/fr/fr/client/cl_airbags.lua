...
Citizen.CreateThread(function()
    while true do 
        local e=tFR.getPlayerVehicle()
        if e~=0 and GetPedInVehicleSeat(e,-1)==tFR.getPlayerPed()then 
            local f=GetVehicleEngineHealth(e)
            if f<=b and a[e]==nil then 
                local g=GetVehicleClass(e)
                if g~=8 and g~=16 and g~=15 and g~=13 then 
                    TriggerEvent("fr:PlaySound", "airbag")
                    createAirbags(e)
                end 
            else 
                if a[e]==nil then 
                    d=GetPedArmour(tFR.getPlayerPed())
                end 
            end 
        else 
            d=0 
...
function createAirbags(e)
    ...
    if d>GetPedArmour(tFR.getPlayerPed())then 
        tFR.setArmour(d)
    end
    d=0
