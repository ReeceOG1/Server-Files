...
RegisterNetEvent("FR:prisonBeanbagShotgunClient",function(o)
...
AddEventHandler("FR:onClientSpawn",function(p, q)
    if q then
        ...
        local s = function()
            if b then
                TriggerServerEvent("FR:prisonerSentBackToCell")
                tFR.teleport(a.prisonCells[d].coords.x, a.prisonCells[d].coords.y, a.prisonCells[d].coords.z)
                tFR.notify("Naughty, go back.")
            end
            globalPlayerInPrisonZone = false
        end
        local t = function()
        end
        tFR.createArea("forceStayInPrison", a.prisonMainCoords, 225.0, 100.0, r, s, t, {})
        local u = function(v)
                ...
                if IsControlJustPressed(0, 38) then
                    if not h then
                        if v.type == "startingLocations" then
                            h = true
                            i = v.job
                            TriggerServerEvent("FR:prisonStartJob", v.job)
                        elseif i == v.job then
                            h = true
                        end
                        if h then
                                ...
                                while y do
                                    if v.data.isScenario == nil then
                                        if not IsEntityPlayingAnim(PlayerPedId(),v.data.animDict,v.data.animName,3) then
                                            tFR.loadAnimDict(v.data.animDict)
                            ...
                            Citizen.CreateThread(function()
                                ...
                                tFR.notify(v.data.nextAction)
                                if v.type == "endingLocations" then
                                    i = ""
                                    TriggerServerEvent("FR:prisonEndJob", v.job)
                                end
                                h = false
                            end)
                        end
            ...
            for B, C in pairs(A) do
                if type(C.coords) == "table" then
                    for D = 1, #C.coords, 1 do
                        tFR.createArea("prisonJob_" .. z .. B .. D,C.coords[D],2.0,5.0,u,w,x,{type = z, job = B, data = C, coords = C.coords[D]})
                        tFR.addMarker(C.coords[D].x,C.coords[D].y,C.coords[D].z + 0.05,0.6,0.6,0.6,30,50,255,255,30,27,false,false,false)
                        tFR.add3DTextForCoord(C.action, C.coords[D].x, C.coords[D].y, C.coords[D].z + 1.0, 2.2)
                    end
                else
                    tFR.createArea("prisonJob_" .. z .. B,C.coords,2.0,5.0,u,w,x,{type = z, job = B, data = C, coords = C.coords})
                    tFR.addMarker(C.coords.x,C.coords.y,C.coords.z + 0.05,0.6,0.6,0.6,30,50,255,255,30,27,false,false,false)
                    tFR.add3DTextForCoord(C.action, C.coords.x, C.coords.y, C.coords.z + 1.0, 2.2)
                end
            end
        end
    end
...
RegisterNetEvent("FR:setPrisonGuardOnDuty",function(E)
    globalOnPrisonDuty=E 
end)
RegisterNetEvent("FR:forcePlayerInPrison",function(E)
    b = E
    globalInPrison = E
end)
RegisterNetEvent("FR:prisonUpdateClientTimer",function(F)
    j = false
...
RegisterNetEvent("FR:prisonStopClientTimer",function()
    k = 0
    l = 0
    m = 0
    j = false
...
tFR.createThreadOnTick(func_drawPrisonTimerBar)
RegisterNetEvent("FR:prisonCreateBreakOutAreas",function()
    local H = false
    local I = function()
    ...
    local K = function(v)
        if IsControlJustPressed(0, 38) and not g then
            if tFR.TriggerServerCallback("FR:prisonStartWireCutting") then
                g = true
                tFR.loadAnimDict("anim@gangops@facility@servers@")
                FreezeEntityPosition(PlayerPedId(), true)
                ...
                tFR.notify("~g~Started wire cutting (Press X to cancel)")
                Citizen.CreateThread(function()
                    ...
                    if H then
                        FreezeEntityPosition(PlayerPedId(), false)
                        ClearPedTasks(PlayerPedId())
                        TriggerServerEvent("FR:prisonBreakOutSuccess")
                        local L = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 2.5, 0.0)
                        tFR.teleport(L.x, L.y, L.z)
                    end
                end)
            else
                tFR.notify("You do not have the required equipment!")
            end
        end
        if IsControlJustPressed(0, 73) and g then
            ...
            ClearPedTasks(PlayerPedId())
            FreezeEntityPosition(PlayerPedId(), false)
            tFR.notify("Wire cutting cancelled!")
        end
    end
    for D = 1, #a.escapePoints, 1 do
        tFR.addMarker(a.escapePoints[D].coords.x,a.escapePoints[D].coords.y,a.escapePoints[D].coords.z,0.6,0.6,0.6,200,0,0,255,70,0,false,true,false)
        tFR.createArea("prisonEscapeArea_" .. D,a.escapePoints[D].coords,2.0,5.0,I,J,K,{escapePoint = a.escapePoints[D]})
    end
end)
RegisterNetEvent("FR:prisonCreateItemAreas",function(M)
    local N = function()
    end
    local O = function()
    end
    local P = function(v)
        if globalInPrison then
            tFR.DrawText3D(v.coords,"[E] Pickup "..v.item,0.5)
            if IsControlJustPressed(0, 38) then
                TriggerServerEvent("FR:prisonPickupItem", v.item)
            end
        end
    end
    for z, A in pairs(M) do
        tFR.removeArea("prisonPickupItem_" .. z)
        tFR.createArea("prisonPickupItem_" .. z, A.coords, 2.0, 5.0, N, O, P, {item = z, coords = A.coords})
    end
end)
RegisterNetEvent("FR:prisonRemoveItemAreas",function(Q)
    tFR.removeArea("prisonPickupItem_" .. Q)
end)
RegisterNetEvent("FR:prisonReleased",function()
    b = false
    globalInPrison = false
    tFR.notify("~g~You have been released!")
    tFR.teleport(a.prisonLeaveCoords.x, a.prisonLeaveCoords.y, a.prisonLeaveCoords.z)
    Citizen.Wait(100)
...
local function R()
    ...
    if S == GetHashKey("mp_m_freemode_01") then
        tFR.loadCustomisationPreset("PrisonerMale")
    else
        tFR.loadCustomisationPreset("PrisonerFemale")
    end
end
RegisterNetEvent("FR:putInPrisonOnSpawn",function(T)
    Citizen.Wait(5000)
    tFR.teleport(a.prisonCells[T].coords.x, a.prisonCells[T].coords.y, a.prisonCells[T].coords.z)
    b = true
    globalInPrison = true
    d = T
    Citizen.Wait(100)
    ...
    if S ~= GetHashKey("mp_m_freemode_01") and S ~= GetHashKey("mp_f_freemode_01") then
        tFR.setCustomization({modelhash = "mp_m_freemode_01"})
    end
    Citizen.Wait(2000)
    R()
end)

RegisterNetEvent("FR:prisonTransportWithBus",function(T)
    DetachEntity(PlayerPedId(), false, true)
    R()
    c = true
    local V = getClosestPoliceStation()
    tFR.teleport(V.coords.x, V.coords.y, V.coords.z)
    Citizen.CreateThread(function()
        while c do
    ...
    local W = tFR.spawnVehicle("pbus", V.coords.x, V.coords.y, V.coords.z, V.heading, false, false, false)
    ...
    enter_prisonArrive = function()
        ...
        while #(tFR.getPlayerCoords() - a.prisonArriveMainDoor) > 1 do
            Citizen.Wait(0)
        end
        tFR.teleport(a.prisonCells[T].coords.x, a.prisonCells[T].coords.y, a.prisonCells[T].coords.z)
        Citizen.Wait(100)
        ...
        DeleteEntity(W)
        DeleteEntity(X)
        TriggerServerEvent("FR:prisonArrivedForJail")
        c = false
        b = true
        globalInPrison = true
        d = T
        createPrisonBlips()
        createPrisonCraftMenu()
        tFR.removeArea("prisonArrive")
    end
    exit_prisonArrive = function()
    end
    ontick_prisonArrive = function()
    end
    tFR.createArea("prisonArrive",a.prisonArriveCoords,5.0,10.0,enter_prisonArrive,exit_prisonArrive,ontick_prisonArrive,{})
end)
RegisterNetEvent("FR:prisonSpawnInMedicalBay",function()
    Citizen.Wait(2000)
    tFR.teleport(a.medicalBayBed.x, a.medicalBayBed.y, a.medicalBayBed.z)
    b = true
    Citizen.Wait(100)
    SetEntityHeading(PlayerPedId(), a.medicalBayHeading)
    tFR.loadAnimDict("mp_bedmid")
    TaskPlayAnim(PlayerPedId(), "mp_bedmid", "f_sleep_l_loop_bighouse", 8.0, 1.0, -1, 1, 0, 0, 0, 0)
    RemoveAnimDict("mp_bedmid")
end)
RMenu.Add("frPrisonItemMenu","main",RageUI.CreateMenu("","~w~Craft Items",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_prisonui", "fr_prisonui"))
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('frPrisonItemMenu', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            for D = 1, #a.craftableItems, 1 do
                RageUI.ButtonWithStyle(a.craftableItems[D].name,"Required: " .. a.craftableItems[D].requiredItemString,{RightLabel = "🔨"},true,function(Y, Z, _)
                    ...
                    if _ then
                        if tFR.TriggerServerCallback("FR:prisonCraftItem", currentButton) then
                            local a0 = true
                            FreezeEntityPosition(PlayerPedId(), true)
                            tFR.loadAnimDict("amb@medic@standing@kneel@base")
                            ...
                            tFR.notify("You do not have the required items to craft this!")
                        end
                    end
                end,nil)
            end
...
RegisterCommand("viewprisoners",function()
    if globalOnPrisonDuty then
        if n then
            ...
            local a1 = tFR.TriggerServerCallback("FR:requestPrisonerData")
            if a1 ~= nil then
                n = true
...
function createPrisonCraftMenu()
    tFR.removeMarker(f)
    tFR.removeArea("prisonCraftItemArea")
    local a3 = function()
    ...
    tFR.createArea("prisonCraftItemArea", a6, 1.0, 3.0, a3, a4, a5, {})
    f = tFR.addMarker(a6.x, a6.y, a6.z - 0.975, 0.4, 0.4, 0.5, 30, 50, 255, 255, 30, 27, false, false, false)
end
function createPrisonBlips()
    for D = 1, #a.prisonBlips, 1 do
        e[D] = tFR.addBlip(a.prisonBlips[D].coords.x,a.prisonBlips[D].coords.y,a.prisonBlips[D].coords.z,a.prisonBlips[D].icon,a.prisonBlips[D].colour,a.prisonBlips[D].name)
    end
    if d then
        e[#e + 1] = tFR.addBlip(a.prisonCells[d].coords.x,a.prisonCells[d].coords.y,a.prisonCells[d].coords.z,253,22,"Your prison cell")
    end
end
...
function getClosestPoliceStation()
    for D = 1, #a.allPoliceStations, 1 do
        if #(tFR.getPlayerCoords() - a.allPoliceStations[D].coords) <= 800 then
...
function nearPrisonPayPhone()
    for D = 1, #a.prisonPayPhones, 1 do
        if #(tFR.getPlayerCoords() - a.prisonPayPhones[D].coords) <= 3.0 then
            return true
        end
    end
    return false
