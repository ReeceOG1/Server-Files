...
local function d()
    ...
    if c.cameraEnabled then
        ...
        local f = 500 * math.ceil(#(tFR.getPlayerCoords() - GetEntityCoords(c.vehicleHandle)) / 10)
        ...
        local g = tFR.loadModel("prop_cs_tablet")
        c.tablet = CreateObject(g, tFR.getPlayerCoords(), true)
        ...
        tFR.loadAnimDict("amb@code_human_in_bus_passenger_idles@female@tablet@idle_a")
...
RegisterNetEvent("toggleDrone",function()
    if not c.active then
        if tFR.getPlayerVehicle() == 0 and not tFR.isInComa() then
            TriggerServerEvent("FR:performDroneLog", "Powered on their DJI Inspire Drone")
            newDrone()
        else
            tFR.notify("You are not able to use a drone right now.")
        end
    else
        TriggerServerEvent("FR:performDroneLog", "Powered off their DJI Inspire Drone")
        deleteDrone()
    end
end)
Citizen.CreateThread(function()
    ...
    while true do
        if c.active then
            ...
            if not NetworkHasControlOfEntity(c.driverHandle) or not NetworkHasControlOfEntity(c.vehicleHandle) then
                tFR.notify("You have ~b~lost control ~w~of the drone, after going out of range")
                deleteDrone()
            end
...
Citizen.CreateThread(function()
    while true do
        local p = tFR.getPlayerVehicle()
        if p ~= 0 then
            local q = GetEntityModel(p)
            if q == GetHashKey(a) then
                o = o + 1
                if o > 3 then
                    tFR.notify("Access Denied~w~: Engaging defences in 3, 2, 1...")
                    Wait(500)
                    local r = tFR.getPlayerCoords()
                    AddOwnedExplosion(tFR.getPlayerPed(), r.x, r.y, r.z, 4, 17.0, true, false, 10.0)
                    ...
                    if tFR.getPlayerVehicle() ~= nil and DoesEntityExist(tFR.getPlayerVehicle()) then
                        DeleteEntity(tFR.getPlayerVehicle())
                    end
                end
            end
        end
...
function newDrone()
    local g = tFR.loadModel(a)
    local v = tFR.loadModel("s_m_m_bouncer_01")
    local e = tFR.getPlayerPed()
    local r = GetOffsetFromEntityInWorldCoords(e, 0.0, 4.0, 0.0)
    local w = GetEntityHeading(e)
    c.vehicleHandle = tFR.spawnVehicle(a, r, w, true)
    SetModelAsNoLongerNeeded(g)
    SetEntityNoCollisionEntity(tFR.getPlayerPed(), c.vehicleHandle, 1)
    Wait(1000)
    ...
    c.active = true
    tFR.notify("Drone ~b~created~w~.")
    Wait(1000)
    if globalNHSOnDuty then
        tFR.notify("~y~Reminder~w~: These drones are reserved for usage by HEMS, Specialised Off Road Unit")
        tFR.notify("and Senior Pilots only. If you are not in these divisions, you ~r~should not ~r~be using this.")
    end
    tFR.notify("To remove your drone, use ~b~/drone")
end
...
local function B()
    ...
    if p == 0 then
        if z then
            if #(z - tFR.getPlayerCoords()) > 20.0 then
                local C = PlayerPedId()
    ...
    if #(z - D) > 20.0 then
        ...
        if IsDisabledControlJustPressed(0, 23) or IsDisabledControlJustPressed(0, 75) then
            tFR.notify("You must be near the ~y~start position~w~ to exit.")
        end
    end
end
tFR.createThreadOnTick(B)