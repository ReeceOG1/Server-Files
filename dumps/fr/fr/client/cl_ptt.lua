local a = false
function func_ptt()
    if globalOnPoliceDuty and tFR.canAnim() and not noclipActive then
        local b = tFR.getPlayerPed()
        local c = tFR.getPlayerId()
        if a then
...
tFR.createThreadOnTick(func_ptt)
...
local function f(g)
    ...
    elseif not i and e then
        exports["pma-voice"]:cleFRxProximityMode()
        e = false
    end
end
tFR.createThreadOnTick(f)
