...
RMenu.Add("FRGunstore","mainmenu",RageUI.CreateMenu("","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_gunstoreui","fr_gunstoreui"))
RMenu:Get("FRGunstore","mainmenu"):SetSubtitle("GUNSTORE")
RMenu.Add("FRGunstore","type",RageUI.CreateSubMenu(RMenu:Get("FRGunstore","mainmenu"),"","Purchase Weapon or Ammo",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_gunstoreui","fr_gunstoreui"))
RMenu.Add("FRGunstore","confirm",RageUI.CreateSubMenu(RMenu:Get("FRGunstore","type"),"","Purchase confirm your purchase",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_gunstoreui","fr_gunstoreui"))
RMenu.Add("FRGunstore","vip",RageUI.CreateSubMenu(RMenu:Get("FRGunstore","mainmenu"),"","Purchase Weapon or Ammo",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_gunstoreui","fr_gunstoreui"))
...
RageUI.CreateWhile(1.0, true, function()
                    ...
                    for o,p in pairs(sortedKeys(n))do 
                        ...
                        if p~="_config"then 
                            ...
                            if p=="item|fillUpArmour"then 
                                local z=GetPedArmour(tFR.getPlayerPed())
                                local A=100-z
                                y=A*1000
                                x=true 
                            end
    ...
    if RageUI.Visible(RMenu:Get("FRGunstore", "type")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            end,RMenu:Get("FRGunstore","confirm"))
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            if l then 
                if string.sub(d.model,1,4)=="item"then 
                    TriggerServerEvent("FR:buyWeapon",d.model,d.price,d.name,d.weaponShop,"armour")
                else 
                    if g=="ammo"then 
                        if HasPedGotWeapon(tFR.getPlayerPed(),GetHashKey(d.model),false)then 
                            TriggerServerEvent("FR:buyWeapon",d.model,d.price,d.name,d.weaponShop,"ammo")
                        else 
                            tFR.notify("You do not have the body of this weapon to purchase ammo.")
                        end 
                    else 
                        TriggerServerEvent("FR:buyWeapon",d.model,d.price,d.name,d.weaponShop,"weapon",h)
                    end 
                end 
            end 
            end,RMenu:Get("FRGunstore","confirm"))
            RageUI.ButtonWithStyle("No","",{RightLabel="→→→"},true,function(j,k,l)
            end,RMenu:Get("FRGunstore","mainmenu"))
        end) 
    end
...
RegisterNetEvent("FR:refreshGunStorePermissions",function()
    TriggerServerEvent("FR:requestNewGunshopData")
end)
local G = false
RegisterNetEvent("FR:recieveFilteredGunStoreData")
AddEventHandler("FR:recieveFilteredGunStoreData",function(F)
    i=F 
...
AddEventHandler("FR:recalculateLargeArms",function(G)
    for m,n in pairs(i)do 
...
Citizen.CreateThread(function()
    while true do 
        ...
        if not G and GetSelectedPedWeapon(R) == GetHashKey("WEAPON_MP5TAZER") then
            tFR.setWeapon(R, "WEAPON_UNARMED", true)
        end
        Wait(0)
    end 
end)
    ...
    if E then
        ...
        for m,n in pairs(i)do 
            ...
            for K,U in pairs(P)do 
                ...
                tFR.createArea("gunstore_"..m.."_"..K,U,1.5,6,V,W,X,{})
            end 
        end 
    end
end)
...
function tFR.createGunStore(Z,_,a0)
    ...
    tFR.createArea(a1,a0,1.5,6,V,W,function()
    end)
