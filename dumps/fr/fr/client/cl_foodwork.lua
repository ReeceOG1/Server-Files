...
local c
local d
AddEventHandler("FR:onClientSpawn",function(e, f)
    if f then
        local g = function()
            if not c then
        ...
        local i = function(j)
            if not c and IsControlJustPressed(1, 51) then
                TriggerServerEvent("FR:requestStartCooking", j.recipe)
        ...
        for k, l in pairs(a) do
            for m, n in pairs(l) do
                tFR.createArea("cooking_" .. k .. "_" .. m,n.cookingPosition,1.5,6,g,h,i,{recipe = k, recipeTable = a[k], index = m})
                tFR.addMarker(n.cookingPosition.x,n.cookingPosition.y,n.cookingPosition.z - 0.2,0.5,0.5,0.5,0,50,255,170,50,20,false,false,true)
            end
        end
    end
end)
...
AddEventHandler("FR:onClientSpawn",function(e, f)
    if f then
        ...
        for w, x in pairs(b) do
            tFR.createArea("cooking_staticobjects_" .. w, x[1].position, 100, 5, g, h, i, {})
        end
    end
end)
...
local function B()
    CreateThread(function()
        tFR.startCircularProgressBar("",3000,nil,function()end)
    end)
    local C = PlayerPedId()
    tFR.loadAnimDict("amb@prop_human_bum_bin@enter")
    TaskPlayAnim(C, "amb@prop_human_bum_bin@enter", "enter", 8.0, 8.0, -1, 1, 0.2)
    RemoveAnimDict("amb@prop_human_bum_bin@enter")
    Wait(1500)
    tFR.loadAnimDict("amb@prop_human_bum_bin@exit")
...
local function D(E)
    local F = true
    local G
    tFR.minigameCircularProgressBar({Difficulty = E, Timeout = 100000, onComplete = function(H)
        if H then
            G = true
        else
            G = false
...
function cookBBQ()
    ...
    tFR.startCircularProgressBar("",10000,nil,function()end)
    local L = 0
    ...
    while L < 3 do
        ...
        if H then
            ...
            tFR.announceMpSmallMsg("Disaster", "You dropped the burger patty, start again!", 6, 3000)
        end
        Wait(2500)
        tFR.startCircularProgressBar("",10000,nil,function()end)
        Wait(2500)
    ...
    tFR.announceMpSmallMsg("Success", "You made a burger!", 18, 3000)
    ClearPedTasks(C)
    StopParticleFxLooped(K, true)
end
local function M()
    local C = tFR.getPlayerPed()
    local N = tFR.getPlayerCoords()
    local u = tFR.loadModel("prop_food_bs_tray_02")
    ...
    SetModelAsNoLongerNeeded(u)
    tFR.loadAnimDict("anim@heists@box_carry@")
    TaskPlayAnim(C, "anim@heists@box_carry@", "idle", 2.0, 2.0, -1, 51, 0, false, false, false)
    Citizen.CreateThread(function()
        while tFR.doesAreaExist("temp_foodorder_complete") do
...
local function T(U)
    ...
    local W = tFR.addMarker(V.x, V.y, V.z + 0.3, 0.15, 0.15, 0.15, 255, 215, 0, 170, 50, 0, false, true, true)
    local X = tFR.addBlip(V.x, V.y, V.z, 1, 2, "Delivery Point", 0.6)
    SetBlipFlashes(X, true)
    tFR.createArea("temp_foodorder_complete",V,1.5,6.0,function()
        tFR.removeArea("temp_foodorder_complete")
        tFR.removeMarker(W)
        tFR.removeBlip(X)
...
RegisterNetEvent("FR:cookingInstruction",function(Z)
    for r, _ in pairs(R) do
        tFR.removeArea("temp_cooking_instruction")
        tFR.removeMarker(_)
    end
    for r, Y in pairs(S) do
        tFR.removeBlip(Y)
    ...
    local W = tFR.addMarker(V.x, V.y, V.z + 0.3, 0.15, 0.15, 0.15, 255, 215, 0, 170, 50, 0, false, true, true)
    table.insert(R, W)
    local X = tFR.addBlip(V.x, V.y, V.z, 1, 2, "Ingredient", 0.6)
    SetBlipFlashes(X, true)
    table.insert(S, X)
    local g = function()
    end
    ...
    local i = function()
        if IsControlJustPressed(1, 51) then
            ...
            if Z == "bbq" then
                ...
                tFR.removeMarker(W)
                T(function()
                    TriggerEvent("FR:playItemBoughtSound")
                    TriggerServerEvent("FR:pickupCookingIngredient", c, Z)
                    tFR.setCanAnim(true)
                    tFR.announceMpSmallMsg("Success", "You earned £4,000, now get back to work!", 18, 3000)
                end)
            else
                tFR.removeMarker(W)
                tFR.removeBlip(X)
    ...
    tFR.createArea("temp_cooking_instruction", V, 1.0, 6, g, h, i)
end)
RegisterNetEvent("FR:beginCooking",function(a0)
    TriggerEvent("FR:smallAnnouncement", "Cooking", "Make a " .. a0 .. ".", 15, 5000)
end)
RegisterNetEvent("FR:finishedCooking",function()
    c = nil
end)
