...
local i=0
local U=false
RMenu.Add("identity","main",RageUI.CreateMenu("","City Hall",0,100,'fr_cityhallui', 'fr_cityhallui'))
    ...
    if true then 
        ...
        local i=function()
            if IsControlJustPressed(1,51)then 
                TriggerServerEvent('FR:getIdentity')
                RageUI.ActuallyCloseAll()
                RageUI.Visible(RMenu:Get("identity","main"),not RageUI.Visible(RMenu:Get("identity","main")))
            end 
        end
        for j,k in pairs(b)do 
            tFR.createArea("identity_"..j,k,1.5,6,g,h,i)
            tFR.addMarker(k.x,k.y,k.z-0.2,0.5,0.5,0.5,0,50,255,170,50,20,false,false,true)
        end 
    end 
end)

RegisterNetEvent("FR:gotCurrentIdentity")
AddEventHandler("FR:gotCurrentIdentity", function(firstname,lastname,age)
    j=firstname
    k=lastname
    i=age
end)

RegisterNetEvent("FR:gotNewIdentity")
AddEventHandler("FR:gotNewIdentity", function(firstname,lastname,age)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if j ~= "N/A" then
                ...
                RageUI.ButtonWithStyle("Change your Identity","£5000",{RightLabel="→→→"},true,function(l,m,n)
                    if n then 
                        RageUI.ActuallyCloseAll()
                        TriggerServerEvent('FR:getNewIdentity')
                    end 
                end)
            end
        end)
    ...
    if RageUI.Visible(RMenu:Get('identity', 'confirm')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("Yes","",{RightLabel="→→→"},true,function(j,k,l)
                if l then 
                    TriggerServerEvent("FR:ChangeIdentity", newfirstname, newlastname, tonumber(newage))
                    RageUI.ActuallyCloseAll()
                end 
...
RegisterNetEvent("FR:showIdentity",function(v, w, x, y, z, A, B, C, D)
    o = true
    ...
    local H = tFR.getFontId("Akrobat-ExtraLight")
    x = string.upper(x)
    y = string.upper(y)
    while o do
...
RegisterNetEvent("FR:hideIdentity",function()
    o = false
end)