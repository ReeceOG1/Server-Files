RMenu.Add('vipclubmenu','mainmenu',RageUI.CreateMenu("","~w~FR Club",tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight(),"fr_club", "fr_club"))
RMenu.Add('vipclubmenu','managesubscription',RageUI.CreateSubMenu(RMenu:Get('vipclubmenu','mainmenu'),"","~w~FR Club",tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight(),"fr_club", "fr_club"))
RMenu.Add('vipclubmenu','manageusersubscription',RageUI.CreateSubMenu(RMenu:Get('vipclubmenu','mainmenu'),"","~w~FR Club Manage",tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight(),"fr_club", "fr_club"))
RMenu.Add('vipclubmenu','manageperks',RageUI.CreateSubMenu(RMenu:Get('vipclubmenu','mainmenu'),"","~w~FR Club Perks",tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight(),"fr_club", "fr_club"))
RMenu.Add('vipclubmenu','deathsounds',RageUI.CreateSubMenu(RMenu:Get('vipclubmenu','manageperks'),"","~w~Manage Death Sounds",tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight(),"fr_club", "fr_club"))
RMenu.Add('vipclubmenu','vehicleextras',RageUI.CreateSubMenu(RMenu:Get('vipclubmenu','manageperks'),"","~w~Vehicle Extras",tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight(),"fr_club", "fr_club"))
local a={hoursOfPlus=0,hoursOfPlatinum=0}
local z={}

function tFR.isPlusClub()
...
RegisterCommand("frclub",function()
    TriggerServerEvent('FR:getPlayerSubscription')
    RageUI.ActuallyCloseAll()
    RageUI.Visible(RMenu:Get('vipclubmenu','mainmenu'),not RageUI.Visible(RMenu:Get('vipclubmenu','mainmenu')))
end)

local c = {
    ["FR"] = {checked = true, soundId = "playDead"},
...
local i = tonumber(GetResourceKvpString("fr_damageindicatorcolour")) or 1
Citizen.CreateThread(function()
    local l = GetResourceKvpString("fr_codhitmarkersounds") or "false"
    if l == "false" then
        d = false
        TriggerEvent("FR:codHMSoundsOff")
    else
        d = true
        TriggerEvent("FR:codHMSoundsOn")
    end
    local m = GetResourceKvpString("fr_killlistsetting") or "false"
    if m == "false" then
        e = false
    else
        e = true
    end
    local n = GetResourceKvpString("fr_oldkillfeed") or "false"
    if n == "false" then
        f = false
    else
        f = true
    end
    local o = GetResourceKvpString("fr_damageindicator") or "false"
    if o == "false" then
        g = false
    else
        g = true
...
AddEventHandler("FR:onClientSpawn",function(f, g)
    if g then
        TriggerServerEvent('FR:getPlayerSubscription')
        Wait(5000)
        local u=tFR.getDeathSound()
        local j="playDead"
...
function tFR.setDeathSound(u)
    if tFR.isPlusClub() or tFR.isPlatClub() then 
        SetResourceKvp("fr_deathsound",u)
    else 
        tFR.notify("~r~Cannot change deathsound, not a valid FR Plus or Platinum subscriber.")
    end 
end
function tFR.getDeathSound()
    if tFR.isPlusClub() or tFR.isPlatClub() then 
        local k=GetResourceKvpString("fr_deathsound")
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if tFR.isPlusClub() or tFR.isPlatClub() then
                ...
                RageUI.ButtonWithStyle("Purchase Subscription","",{RightLabel="→→→"},true,function(o,p,q)
                    if q then
                        tFR.OpenUrl("https://frstudios.tebex.io")
                        SendNUIMessage({act="openurl",url="https://frstudios.tebex.io"})
                    end
                end)
            end
    ...
    if RageUI.Visible(RMenu:Get('vipclubmenu', 'managesubscription')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("Sell Plus Subscription days.","~r~If you have already claimed your weekly kit, you may not sell days until the week is over.",{RightLabel = "→→→"},true,function(o, p, q)
                if q then
                    if isInGreenzone then
                        TriggerServerEvent("FR:beginSellSubscriptionToPlayer", "Plus")
                    else
            ...
            RageUI.ButtonWithStyle("Sell Platinum Subscription days.","~r~If you have already claimed your weekly kit, you may not sell days until the week is over.",{RightLabel = "→→→"},true,function(o, p, q)
                if q then
                    if isInGreenzone then
                        TriggerServerEvent("FR:beginSellSubscriptionToPlayer", "Platinum")
                    else
                        notify("~r~You must be in a greenzone to sell.")
                    end
                end
            ...
            if tFR.isDev() then
                if next(z) then
                    ...
                    RageUI.ButtonWithStyle("Set Plus Days","",{RightLabel="→→→"},true,function(o,p,q)
                        if q then
                            TriggerServerEvent("FR:setPlayerSubscription", z.userid, "Plus")
                        end
                    end)
                    RageUI.ButtonWithStyle("Set Platinum Days","",{RightLabel="→→→"},true,function(o,p,q)
                        if q then
                            TriggerServerEvent("FR:setPlayerSubscription", z.userid, "Platinum")
                        end
                    end)    
                else
                ...
                RageUI.ButtonWithStyle("Select Perm ID", nil, { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                    if Selected then
                        permID = tFR.KeyboardInput("Enter Perm ID", "", 10)
                        if permID == nil then 
                            tFR.notify('Invalid Perm ID')
                            return
                        end
                        TriggerServerEvent('FR:getPlayerSubscription', permID)
                    end
                end, RMenu:Get("vipclubmenu", 'manageusersubscription'))
            end
        end)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle(
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(o, p, q)
                            if q then
                                if not globalInPrison and not tFR.isHandcuffed() then
                                    TriggerServerEvent("FR:claimWeeklyKit")
                                else
                    ...
                    local function q()
                        TriggerEvent("FR:codHMSoundsOn")
                        d = true
                        tFR.setCODHitMarkerSetting(d)
                        tFR.notify("~y~COD Hitmarkers now set to " .. tostring(d))
                    end
                    local function r()
                        TriggerEvent("FR:codHMSoundsOff")
                        d = false
                    ...
                    RageUI.Checkbox(
                        ...
                        function()
                            e = true
                            tFR.setKillListSetting(e)
                            tFR.notify("~y~Kill List now set to " .. tostring(e))
                        end,
                        function()
                            e = false
                            tFR.setKillListSetting(e)
                    ...
                    RageUI.Checkbox(
                        ...
                        function()
                            f = true
                            tFR.setOldKillfeed(f)
                            tFR.notify("~y~Old killfeed now set to " .. tostring(f))
                        end,
                        function()
                            f = false
                            tFR.setOldKillfeed(f)
                    ...
                    RageUI.Checkbox(
                        ...
                        function()
                            g = true
                            tFR.setDamageIndicator(g)
                            tFR.notify("~y~Damage Indicator now set to " .. tostring(g))
                        end,
                        function()
                            g = false
                            tFR.setDamageIndicator(g)
                    ...
                    if g then
                        RageUI.List(
                            ...
                            function(A, B, C, D)
                                i = D
                                tFR.setDamageIndicatorColour(i)
                            end,
                            function()
                            end,
                            nil
...
RegisterNetEvent("FR:setVIPClubData",function(y,z)
    a.hoursOfPlus=y
    a.hoursOfPlatinum=z 
end)

RegisterNetEvent("FR:getUsersSubscription",function(userid, plussub, platsub)
    z.userid = userid
...
RegisterNetEvent("FR:userSubscriptionUpdated",function()
    TriggerServerEvent('FR:getPlayerSubscription', permID)
end)
...
Citizen.CreateThread(function()
    while true do 
        ...
        if tFR.isPlusClub() or tFR.isPlatClub()then 
            SetVehicleDirtLevel(tFR.getPlayerVehicle(),0.0)
        end
        Wait(500)
    end 
end)
...
RegisterNetEvent(
    ...
    function(F)
        if e and (tFR.isPlatClub() or tFR.isPlusClub()) and IsPedAPlayer(F) then
            local G = NetworkGetPlayerIndexFromPed(F)
            if G >= 0 then
                local H = GetPlayerServerId(G)
                if H >= 0 then
                    local I = tFR.getPlayerName(G)
