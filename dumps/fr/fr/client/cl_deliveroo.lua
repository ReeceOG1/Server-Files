...
RegisterNetEvent("FR:beginDeliverooJob",function()
    local e = d[math.random(1, #d)]
    b.tempVehicle = tFR.spawnVehicle(e,b.spawnVehicleVector.x,b.spawnVehicleVector.y,b.spawnVehicleVector.z,343,true,true,true)
    b.onJob = true
    tFR.disableCustomizationSave(b.onJob)
    b.previousCustomisation = tFR.getCustomization()
    local f = PlayerPedId()
    SetPedComponentVariation(f, 5, 126, 0, 0)
    SetPedComponentVariation(f, 6, 0, 0, 0)
    SetPedComponentVariation(f, 11, 449, 0, 0)
    tFR.notify("~g~Deliveroo Job started, exit the restaurant and head to the first drop off.")
    while b.onJob do
...
RegisterNetEvent("FR:deliverooJobEnd",function(g)
    if g then
        ...
        tFR.removeMarker(b.tempMarker)
    end
    tFR.setCustomization(b.previousCustomisation)
    b.onJob = false
    tFR.disableCustomizationSave(b.onJob)
    b.tempMarker = 0
    b.tempBlip = 0
...
RegisterNetEvent("FR:deliverooJobReachedNextStop",function(i)
    ...
    if n then
        ...
        local r = function(p)
            if IsControlJustReleased(1, 38) and not b.onJob then
                TriggerServerEvent("FR:attemptBeginDeliverooJob")
            end
        end
        tFR.addBlip(b.startVector.x, b.startVector.y, b.startVector.z, 106, 1, "Deliveroo Job")
        tFR.addMarker(b.startVector.x,b.startVector.y,b.startVector.z - 1,1.0,1.0,1.0,255,0,0,70,50,38,false,false,true)
        tFR.createArea("deliveroo", b.startVector, 1.5, 6, o, q, r, {})
    end
end)
