...
local function i(j, k, l)
    if k < a then
        ...
        if HasEntityClearLosToEntity(tFR.getPlayerPed(), l, 17) then
            return true
        end
    elseif g and tFR.isPlayerInSelectedGang(j) then
        if k < h then
            return true
        end
    end
                            ...
                            if b[o] then
                                ...
                                if i(j, b[o], l) then
                                    ...
                                    if NetworkIsPlayerTalking(o) then
                                        ...
                                        local q, r = tFR.isPlayerInSelectedGang(j)
                                        if g and q then
                ...
                if
                    t ~= m and
                        (IsEntityVisible(t) or
                            not tFR.clientGetPlayerIsStaff(tFR.clientGetUserIdFromSource(u)) and not tFR.inEvent())
                 then
...
Citizen.CreateThread(
    function()
        while true do
            g = tFR.hasGangNamesEnabled() and not tFR.isEmergencyService()
            for n, o in ipairs(GetActivePlayers()) do
                local k = b[o]
                local j = GetPlayerServerId(o)
                if k and i(j, k, GetPlayerPed(o)) and d then
                    local w = nil
                    if g and tFR.isPlayerInSelectedGang(j) then
                        w = tFR.getPlayerName(o)
                    else
...
function tFR.setPlayerNameDistance(k)
    if k == -1 then
...
function tFR.getPlayerNameDistance()
    return a
end
RegisterCommand(
    "farids",
    function(x, y, z)
        if tFR.getStaffLevel() > 2 and tFR.isStaffedOn() then
            local A = y[1]
            if A ~= nil and tonumber(A) then
                a = tonumber(A) + 000.1
                tFR.setPlayerNameDistance(a)
                tFR.notify("~g~Far ids distance set to " .. A .. "m")
            else
                tFR.notify("~r~Please enter a valid range! (/farids [range])")
            end
        end
    end,
    false
