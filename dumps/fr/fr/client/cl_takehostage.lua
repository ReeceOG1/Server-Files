...
local b = module("fr-weapons", "cfg/weapons")
    ...
    if not a.InProgress and u then
        ...
        if i then
            ...
            if w ~= -1 then
                ...
                if x then
                    tFR.setWeapon(PlayerPedId(), foundWeapon, true)
                    a.InProgress = true
                    a.startTime = GetGameTimer()
                    a.targetSrc = w
                    TriggerServerEvent("FR:takeHostageSync", w)
                    a.type = "agressor"
                else
...
RegisterNetEvent("FR:takeHostageSyncTarget",function(y)
...
RegisterNetEvent("FR:takeHostageReleaseHostage",function()
    a.InProgress = false
    a.type = ""
    DetachEntity(PlayerPedId(), true, false)
    q("reaction@shove")
...
RegisterNetEvent("FR:takeHostageKillHostage",function()
    a.InProgress = false
    a.type = ""
...
RegisterNetEvent("FR:takeHostageCl_stop",function()
    a.InProgress = false
    a.type = ""
    ClearPedSecondaryTask(PlayerPedId())
    DetachEntity(PlayerPedId(), true, false)
    ...
    while true do
        if a.type == "agressor" then
            ...
            if IsEntityDead(PlayerPedId()) then
                ...
                RemoveAnimDict("reaction@shove")
                TriggerServerEvent("FR:takeHostageReleaseHostage", a.targetSrc)
            end
            ...
            elseif IsDisabledControlJustPressed(0, 74) and GetGameTimer() - a.startTime > 4000 then
                ...
                RemoveAnimDict("anim@gangops@hostage@")
                TriggerServerEvent("FR:takeHostageKillHostage", a.targetSrc)
                TriggerServerEvent("FR:takeHostageStop", a.targetSrc)
                Wait(100)
...
function tFR.takeHostageInProgress()
    return a.InProgress
end
