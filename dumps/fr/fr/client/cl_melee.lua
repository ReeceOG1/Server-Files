...
RegisterCommand("togglemelee",function()
    if tFR.getUserId()==1 or tFR.getUserId()==2 then 
        c = not c
        tFR.notify('Melee Combat: '..(c and "~g~Enabled" or "Disabled"))
    end 
end)
function tFR.enablePunching(d)
    c = d
end
...
AddEventHandler("FR:onClientSpawn",function(h, i)
    if i then
        ...
        for n, o in pairs(a) do
            tFR.createArea("boxing_" .. n, o.pos, o.radius, 6, j, l, m, {})
        end
    end
end)
...
AddEventHandler("FR:onClientSpawn",function(h, i)
    if i then
        ...
        for n, u in pairs(t) do
            tFR.addMarker(u.x, u.y, u.z, 1.0, 1.0, 1.0, 0, 0, 255, 100, 50, 27, false, false, true)
            tFR.createArea(n .. "_boxingGloves", u, 1.5, 6, j, l, m, {})
        end
    end
end)
local v = {}
function putGloves()
    ...
    local x = tFR.loadModel("prop_boxing_glove_01")
