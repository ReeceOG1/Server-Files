local aJ = false
RegisterNetEvent("FR:adminTicketFeedback",function(aK)
    local aL, aM = tFR.getPlayerVehicle()
    print("Triggered Feedback")
    ...
    if aN then
        local aO = false
        tFR.clientPrompt("Attached Message","",function(aP)
            TriggerServerEvent("FR:adminTicketFeedback", source, aN, aP)
            aO = true
        end)
        while not aO do
            for a2 = 0, 6 do
        ...
        TriggerServerEvent("FR:adminTicketNoFeedback", aK)
    end
