RMenu.Add("cardev","mainmenu",RageUI.CreateMenu("", "", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight(), "fr_adminui", "fr_adminui"))
RMenu:Get("cardev", "mainmenu"):SetSubtitle("Car Dev Menu")
RMenu.Add("cardev","vehiclemods",RageUI.CreateSubMenu(RMenu:Get("cardev", "mainmenu"),"","Vehicle Mods",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight()))
RMenu.Add("cardev","vehiclemodindexes",RageUI.CreateSubMenu(RMenu:Get("cardev", "vehiclemods"),"","Vehicle Mod Indexes",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight()))
RMenu.Add("cardev","extras",RageUI.CreateSubMenu(RMenu:Get("cardev", "mainmenu"),"","Extras",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight()))
RMenu.Add("cardev","colours",RageUI.CreateSubMenu(RMenu:Get("cardev", "mainmenu"),"","Colours",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight()))
local a = false
local b = false
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if b and tFR.getPlayerBucket() == 333 then
                RageUI.ButtonWithStyle("Spawn Vehicle (No Mods)","",{RightLabel = "→→→"},true,function(h, i, j)
                    if j then
                        tFR.clientPrompt("Spawncode:","",function(k)
                            if k ~= "" and tFR.getPlayerBucket() == 333 then
                                ...
                                local n = tFR.spawnVehicle(l,m.x,m.y,m.z,GetEntityHeading(tFR.getPlayerPed()),true,true,true)
                                DecorSetInt(n, "FRACVeh", 955)
                ...
                RageUI.ButtonWithStyle("Delete Vehicle","",{RightLabel = "→→→"},true,function(h, i, j)
                    if j then
                        ...
                        if NetworkGetEntityIsNetworked(o) then
                            ...
                            if p ~= 0 then
                                DeleteEntity(aT)
                                --TriggerServerEvent("FR:cardevDeleteVehicle", p)
                            end
                        else
                            DeleteEntity(o)
                        end
...
RegisterNetEvent("FR:setCarDev",function()
    a = true
end)
RegisterNetEvent("FR:setBucket",function(D)
    if b and D ~= 333 then
