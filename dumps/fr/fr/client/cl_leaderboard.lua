...
RegisterCommand("leaderboardtest",function()
    if tFR.isDev() then
        local b = tFR.createLeaderboard(a)
        Wait(1000)
        tFR.closeLeaderboard(b)
    end
end,false)
local b = 1
local c = {}
...
function tFR.closeLeaderboard(b)
    c[b] = false
end
function tFR.createLeaderboard(m)
    local n = Scaleform("SC_LEADERBOARD")
    n.RunFunction("SET_DISPLAY_TYPE", {1})
    n.RunFunction("SET_MULTIPLAYER_TITLE", {"FR Event"})
...
function tFR.updateScoreboard(u, v, w)
    local x
    ...
    if u == -1 then
        ...
        if x then
            ...
            print("error: FR:updateScoreboard couldn't find row with values", u, v, w)
        end
    end
end
RegisterNetEvent(
    "FR:updateScoreboard",
    function(u, v, w)
        tFR.updateScoreboard(u, v, w)
    end
)
function tFR.setPlayerCanOpenLeaderboard(F)
    e = F
end
function tFR.clearLeaderboardData()
    d = {columns = {}, rows = {}}
end
function tFR.isLeaderboardSetup()
    return table.count(d.rows) > 0
end
function tFR.removePlayerFromLeaderboard(u)
