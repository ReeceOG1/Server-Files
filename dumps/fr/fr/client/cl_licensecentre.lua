...
RMenu.Add('LicenseCentre', 'main', RageUI.CreateMenu("", "" .. "Job Centre", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight(), "fr_licenseui", "fr_licenseui"))
RMenu.Add("LicenseCentre", "confirm", RageUI.CreateSubMenu(RMenu:Get('LicenseCentre', 'main',  tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight())))
...
AddEventHandler("FR:onClientSpawn",function(p, q)
    if q then
        ...
        local u = function(s)
            ...
            tFR.addMarker(licensecentre.location.x, licensecentre.location.y,licensecentre.location.z-0.98,0.7,0.7,0.5,255, 255,255,250,50,27,false,false)
        end
        tFR.createArea("licensecentre", licensecentre.location, 1.5, 6, r, t, u, {})
        tFR.addBlip(licensecentre.location.x, licensecentre.location.y, licensecentre.location.z, 457, 2, "Job Centre", 0.6, true)
    end
end)
...
RegisterNetEvent("FR:RecievedLicenses",function(i)
    b = i
    a = true
end)
RegisterNetEvent("FR:gotOwnedLicenses",function(i)
    for k,v in pairs(licensecentre.licenses) do
        v.notOwned = true
