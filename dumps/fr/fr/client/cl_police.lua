...
local e = false
local f = false
function tFR.isKnockedOut()
    return a
end
function tFR.putInNearestVehicleAsPassenger(j)
    local k = tFR.getClosestVehicle(j)
    if IsEntityAVehicle(k) then
        for l = 1, math.max(GetVehicleMaxNumberOfPassengers(k), 3) do
            if IsVehicleSeatFree(k, l) then
                SetPedIntoVehicle(tFR.getPlayerPed(), k, l)
                return true
            end
        end
    end
...
function tFR.putInNetVehicleAsPassenger(m)
    local k = tFR.getObjectId(m)
    if IsEntityAVehicle(k) then
...
function tFR.putInVehiclePositionAsPassenger(n, o, p)
    local k = tFR.getVehicleAtPosition(n, o, p)
    if IsEntityAVehicle(k) then
        for l = 1, GetVehicleMaxNumberOfPassengers(k) do
            if IsVehicleSeatFree(k, l) then
                SetPedIntoVehicle(tFR.getPlayerPed(), k, l)
...
RegisterNetEvent("FR:knockOut",function()
    if not a and not tFR.isPurge() then
        tFR.setCanAnim(false)
        a = true
        b = r()
    end
end)
RegisterNetEvent("FR:knockOutDisable",function()
    if a then
        local s = PlayerPedId()
...
RegisterNetEvent("FR:drag")
AddEventHandler("FR:drag",function(t)
    d = t
    e = not e
end)
RegisterNetEvent("FR:undrag")
AddEventHandler("FR:undrag",function(t)
    e = false
end)
RegisterNetEvent("FR:clearPoliceStuff",function()
...
local u = {}
RegisterNetEvent("FR:addS60",function(v, w, x)
...
RegisterNetEvent("FR:removeS60",function(x)
    if u[t] == nil then
        return
    else
        local D = u[x]
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(0, 323) or IsControlPressed(0, 27) and not IsUsingKeyboard(2) then
            if not globalSurrenderring and not tFR.isInComa() and not tFR.isHandcuffed() and (tFR.canAnim() or tFR.isTazedByRevive()) then
                DisablePlayerFiring(tFR.getPlayerPed(), true)
                DisableControlAction(0, 22, true)
                DisableControlAction(0, 25, true)
                DisableControlAction(0, 154, true)
                if not IsEntityDead(tFR.getPlayerPed()) then
                    if not c and not GetIsTaskActive(tFR.getPlayerPed(), 298) then
                        c = true
                        tFR.loadAnimDict("missminuteman_1ig_2")    
                        TaskPlayAnim(tFR.getPlayerPed(),"missminuteman_1ig_2","handsup_enter",7.0,1.0,-1,50,0,false,false,false)
                        RemoveAnimDict("missminuteman_1ig_2")
                    end
                end
            end
        end
        if (IsControlJustReleased(1, 323) or IsControlJustReleased(1, 27)) and not globalSurrenderring and c and not tFR.isInComa() and not tFR.isHandcuffed() and tFR.canAnim() then
            c = false
        ...
        if a then
            if tFR.isStaffedOn() then
                TriggerEvent("FR:knockOutDisable")
            elseif not tFR.isInComa() then
                local s = PlayerPedId()
                if not IsEntityPlayingAnim(s, b[1], b[2], 3) then
                    tFR.loadAnimDict(b[1])
                ...
                tFR.notify("You have been knocked out!")
            end
        end
        Wait(0)
    end
...
RMenu.Add("policehandbook","main",RageUI.CreateMenu("Police Handbook", "Officer Handbook", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight()))
...
RegisterNetEvent("FR:toggleHandbook",function()
...
RegisterNetEvent("FR:breathTestResult",function(M,N)
    local O = N
    ...
    TaskPlayAnim(tFR.getPlayerPed(),"weapons@first_person@aim_rng@generic@projectile@shared@core","idlerng_med",1.0,-1,10000,50,0,false,false,false)
...
RegisterNetEvent("FR:beingBreathalysed",function()
    RageUI.Text({message = "~w~You are currently being ~b~breathalysed ~w~by a police officer."})
end)
RegisterNetEvent("FR:breathalyserCommand",function()
        ...
        for Q, R in pairs(P) do
            if GetPlayerPed(R) ~= E then
                ...
                if T < 3.0 then
                    ...
                    TriggerServerEvent("FR:breathalyserRequest", U)
                    break
                end
            end
        end
...
RegisterNetEvent("FR:flashWarrantCard",function()
    local E = PlayerPedId()
    local V = tFR.loadModel("prop_fib_badge")
...
RegisterNetEvent("FR:startSearchingSuspect",function()
    tFR.setCanAnim(false)
    tFR.loadAnimDict("custom@police")
    ...
    while GetGameTimer() - a3 < 10000 do
        if IsDisabledControlJustPressed(0, 73) then
            TriggerServerEvent("FR:cancelPlayerSearch")
            return
        end
        Citizen.Wait(0)
    end
...
local a4 = false
RegisterNetEvent("FR:startBeingSearching",function(a5)
    ...
    a4 = true
    tFR.setCanAnim(false)
    tFR.loadAnimDict("custom@suspect")
    local s = tFR.getPlayerPed()
...
RegisterNetEvent("FR:cancelPlayerSearch",function()
    a4 = false
    tFR.setCanAnim(true)
    local s = tFR.getPlayerPed()
    DetachEntity(s)
...
local ae = ""
local af = false
RegisterNetEvent("FR:receivePoliceCallsign",function(ag, ah, ai)
    a8 = ah
    a9 = ag
    aa = ai
    ab = true
...
RegisterNetEvent("FR:receiveHmpCallsign",function(ag, ah, ai)
    ac = ah
    ad = ag
    ae = ai
    af = true
...
function tFR.getPoliceCallsign()
    return a9
end
function tFR.getPoliceRank()
    return a8
end
function tFR.getPoliceName()
    return aa
end
function tFR.hasPoliceCallsign()
    return ab
end
function tFR.getHmpCallsign()
    return ad
end
function tFR.getHmpRank()
    return ac
end
function tFR.getHmpName()
    return ae
end
function tFR.hasHmpCallsign()
    return af
end
...
tFR.createThreadOnTick(func_drawCallsign)
local aj = 0
local function ak()
    local s = tFR.getPlayerPed()
...
tFR.createThreadOnTick(ak)
function tFR.hasRecentlyShotGun()
    return aj ~= 0 and GetGameTimer() - aj < 600000
end

AddEventHandler("FR:onClientSpawn",function()
    aj = 0
end)

RMenu.Add("trainingWorlds","mainmenu",RageUI.CreateMenu("Training Worlds", "Main Menu", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight()))
local ap = {}
local aq = false
RegisterNetEvent("FR:trainingWorldOpen",function(ar)
    aq = ar
    ...
    if RageUI.Visible(RMenu:Get('trainingWorlds', 'mainmenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            for x, at in pairs(ap) do
                ...
                local av = at.bucket == tFR.getPlayerBucket()
                ...
                RageUI.ButtonWithStyle(at.name,au,aw,true,function(J, K, L)
                    if K and aq then
                        ...
                        if IsControlJustPressed(0, 214) then
                            TriggerServerEvent("FR:trainingWorldRemove", x)
                        end
                    end
                    if L then
                        TriggerServerEvent("FR:trainingWorldJoin", x)
                    end
                end)
                if av then
                    as = av
            ...
            if as then
                RageUI.ButtonWithStyle("Leave Training World",nil,{},true,function(J, K, L)
                    if L then
                        TriggerServerEvent("FR:trainingWorldLeave")
                    end
                end)
            end
            if aq then
                RageUI.ButtonWithStyle("Create Training World",nil,{},true,function(J, K, L)
                    if L then
                        TriggerServerEvent("FR:trainingWorldCreate")
                    end
                end)
            end
        end)
...
RegisterNetEvent("FR:trainingWorldSend",function(x, ax)
    ap[x] = ax
end)
RegisterNetEvent("FR:trainingWorldSendAll",function(ax)
    ap = ax
end)
RegisterNetEvent("FR:trainingWorldRemove",function(x)
    ap[x] = nil
end)
