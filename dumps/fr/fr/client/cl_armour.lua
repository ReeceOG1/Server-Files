    ...
    if true then 
        ...
        local g=function()
            if IsControlJustPressed(1,51)then 
                if tFR.globalOnPoliceDuty() or tFR.globalOnPrisonDuty() then 
                    TriggerServerEvent('FR:getArmour')
                    local soundId=GetSoundId()
                    PlaySoundFrontend(soundId,"Armour_On","DLC_GR_Steal_Miniguns_Sounds",true)
                    ReleaseSoundId(soundId)
                else 
                    tFR.notify("~r~You shouldn't be here...Engaging defenses in 3..2..1...")
                end 
            end 
        end
        for h,i in pairs(a)do 
            tFR.createArea("armour_"..h,i,1.5,6,e,f,g)
            tFR.addMarker(i.x,i.y,i.z-0.2,0.5,0.5,0.5,0,50,255,170,50,20,false,false,true)
        end 
    end 
end)
local z=0
function tFR.setArmour(A,B)
    if A ~= nil then
...
function tFR.getArmour()
    return GetPedArmour(PlayerPedId())
end
Citizen.CreateThread(function()
    while true do 
        if tFR.getArmour()>z then 
            tFR.setArmour(0)
        end
        Wait(0)
    end 
...
local function k()
    ...
    if l==GetHashKey('mp_m_freemode_01') or l==GetHashKey('mp_f_freemode_01') then 
        ...
        while j do 
            ...
            tFR.setWeapon(PlayerPedId(),"weapon_unarmed",true)
        ...
        tFR.setCustomization(p)
        tFR.setArmour(savedArmour)
    else 
        tFR.notify("~r~Custom peds cannot be used with horses.")
    end 
end
Citizen.CreateThread(function()
    if true then 
        ...
        local w=function()
            if IsControlJustPressed(1,51)and not j then 
                if tFR.globalOnPoliceDuty() and not inOrganHeist then 
                    if tFR.globalHorseTrained() or tFR.getUserId()==1 then 
                        k()
                    else 
                        tFR.notify("~r~You do not have the [Horse Trained] whitelist.")
                    end
                else 
                    tFR.notify("~r~This is only available to the MET Police only.")
                end 
            end 
        end
        for x,y in pairs(b)do 
            tFR.createArea("horse_"..x,y,1.5,6,u,v,w)
            tFR.addMarker(y.x,y.y,y.z,1.0,1.0,1.0,0,50,255,170,50,42,false,false,true)
        end 
    end 
end)
