local a = module("fr-weapons", "cfg/weapons")
...
function tFR.getWeapons()
    ...
    for b,c in pairs(a.weapons)do 
        if HasPedGotWeapon(f,c.hash)then
            ...
            --h.attachments=tFR.getAllWeaponAttachments(b)
            d[b]=h 
        end 
    end
    return d 
end
function tFR.removeAllWeapons()
    RemoveAllPedWeapons(tFR.getPlayerPed())
end
...
RegisterCommand("storecurrentweapon",function()
    if s+3000<GetGameTimer()then 
        ...
        if HasPedGotWeapon(PlayerPedId(),'WEAPON_PISTOL50')or HasPedGotWeapon(PlayerPedId(),'WEAPON_MACHINEPISTOL')then 
            ...
            TriggerServerEvent("FR:forceStoreSingleWeapon",g)
        end 
    else 
        tFR.notify("Store weapons cooldown, please wait.")
    end 
end)
            ...
            if not table.has(X, k) then
                for k,v in pairs(a.weapons) do
                    if i == GetHashKey(k) then
                        TriggerServerEvent('FR:acType17', v.name)
                        Wait(60000)
                    end
                end
            end
