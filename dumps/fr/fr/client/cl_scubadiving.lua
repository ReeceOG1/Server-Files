...
local h
local i = false
RMenu.Add("scubadiving","main",RageUI.CreateMenu("", "", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight(), "fr_scubajob", "fr_scubajob"))
RMenu:Get("scubadiving", "main"):SetSubtitle("FR | ~b~Scuba Diving")
local function j()
...
AddEventHandler("FR:onClientSpawn",function(l, m)
    if m then
        local n = tFR.createDynamicPed(a.jobPedModel,a.jobPedPosition,a.jobPedHeading,true,"mini@strip_club@idles@bouncer@base","base",100,false,function(o)
        ...
        tFR.addBlip(a.jobPosition.x, a.jobPosition.y, a.jobPosition.z, 308, 0, "Scuba Diving")
        tFR.addMarker(a.jobPosition.x,a.jobPosition.y,a.jobPosition.z - 0.2,0.5,0.5,0.5,0,50,255,170,50,20,false,false,true)
        local q = function(r)
            j()
        end
        local s = function(r)
        ...
        tFR.createArea("scubajob", a.jobPosition, 1.5, 6, q, s, t, {})
    end
end)
        ...
        RageUI.DrawContent({header=true,glare=false,instructionalButton=true},function()
            if not e then
                RageUI.Button("Request Job",nil,{RightLabel = "→→→"},true,function(u, v, w)
                    if w then
                        TriggerServerEvent("FR:requestScubaJob")
                    end
                end)
            end
            if d ~= 0 then
                RageUI.Button("Claim Reward",nil,{RightLabel = "→→→"},true,function(u, v, w)
                    if w then
                        TriggerServerEvent("FR:claimScubaReward", c)
                        d = 0
                        c = nil
                        e = false
                        i = false
                        ...
                        tFR.setCustomization(h)
                        local x = PlayerPedId()
                        SetEnableScuba(x, false)
                        SetPedMaxTimeUnderwater(x, 10.0)
                        tFR.setMaxUnderWaterUITimenewTime(10.0)
                    end
                end)
            end
        end)
...
local function y()
    for z, A in pairs(b) do
        tFR.removeMarker(A.objectMarker)
        tFR.removeArea(A.objectArea)
...
function startScubaDiving()
    tFR.loadAnimDict("missheistchem2")
    local x = PlayerPedId()
    ...
    tFR.drawNativeNotification("Press ~INPUT_PICKUP~ to start Scuba Diving.")
    while not F do
    ...
    TriggerServerEvent("FR:finishScubaJob", c, d)
end
RegisterNetEvent("FR:createScubaJob",function(J, K)
    c = K
    e = true
    tFR.disableCustomizationSave(e)
    DoScreenFadeOut(500)
    Wait(600)
    ...
    SetBlipAlpha(L, 180)
    local q = function(r)
        tFR.drawNativeNotification("Press ~INPUT_DETONATE~ to begin diving!")
    end
    local s = function(r)
    end
    local t = function(r)
        if IsControlJustReleased(1, 47) then
            ...
            local M = tFR.getPlayerVehicle()
            if M ~= 0 then
                if GetEntityModel(M) ~= GetHashKey("dinghy") then
                    tFR.notify("You must be in a dinghy to start scuba diving!")
                else
                    tFR.removeArea("scubajob_" .. J.name)
                    f = "~y~Search for treasures and return to your boat when finished"
                    createRewardObjects(J)
                    CreateThread(startScubaDiving)
                end
            else
                tFR.notify("You must be in a boat to start scuba diving!")
            end
        end
    end
    SetWaypointOff()
    local N = tFR.addBlip(J.position.x, J.position.y, J.position.z, J.blipId, J.blipColour, "Scuba Diving")
    SetNewWaypoint(J.position.x, J.position.y)
    tFR.createArea("scubajob_" .. J.name, J.position, 250, 15, q, s, t, {})
    h = tFR.getCustomization()
    local p = math.random(1, 15)
    local O = PlayerPedId()
    ...
    g = tFR.spawnVehicle("dinghy",J.dinghySpawnPositions[P].x,J.dinghySpawnPositions[P].y,J.dinghySpawnPositions[P].z,J.dinghySpawnHeading,true,true,true)
    ...
    TriggerServerEvent("FR:scubaSetVehicle", c, R)
    DoScreenFadeIn(2000)
    SetPedScubaGearVariation(PlayerPedId())
    Citizen.CreateThread(function()
        while e do
            if i then
                ...
                SetWaypointOff()
                TriggerServerEvent("FR:finishScubaJob", c, 0)
                TriggerServerEvent("FR:claimScubaReward", c)
                d = 0
                c = nil
                e = false
...
RegisterNetEvent("FR:ScubaBoatDestroyed",function(S)
    if e and S == c then
        i = true
    end
end)
function createRewardObjects(J)
    for z, A in pairs(J.rewardObjects) do
        local T = tFR.loadModel(A.objectModel)
        ...
        local V =
            tFR.addMarker(A.objectPosition.x,A.objectPosition.y,A.objectPosition.z + 0.6,0.5,0.5,0.5,255,223,0,150,50.0,0,false,true,true)
        local q = function(r)
            tFR.drawNativeNotification("Press ~INPUT_PICKUP~ to collect!")
        end
        local s = function(r)
        end
        local t = function(r)
            if IsControlJustReleased(1, 38) and e then
                tFR.notify("~g~Item collected.")
                d = d + 1
                tFR.removeMarker(b[r.objectId].objectMarker)
                tFR.removeArea(b[r.objectId].objectArea)
        ...
        tFR.createArea("scubajob_" .. J.name .. "_obj_" .. z, A.objectPosition, 3.0, 3.0, q, s, t, {objectId = z})
        b[z] = {}
        b[z].objectId = U
        b[z].objectMarker = V
