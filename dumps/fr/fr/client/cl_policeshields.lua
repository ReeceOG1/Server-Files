    ...
    while true do
        if a.active and a.id ~= 0 then
            ...
            if h ~= GetHashKey("WEAPON_UNARMED") and (not b[a.id].allowPistol or not table.has(c, h)) then
                tFR.setWeapon(g, "WEAPON_UNARMED", true)
            end
...
RMenu.Add("policeshields","main",RageUI.CreateMenu("Police Shields", "Shield Management", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight()))
...
RegisterNetEvent("FR:toggleShieldMenu",function(l)
...
function removeShield(m)
    if DoesEntityExist(a.prop) then
        ...
        if m then
            tFR.notify("~g~Success~w~: Shield removed")
        end
    end
end
function createShield(n)
    ...
    local o = false
    local g = tFR.getPlayerPed()
    local p = tFR.getPlayerCoords()
    if a.active then
        removeShield(false)
    end
    a.id = n
    local q = tFR.loadModel(b[n].model)
