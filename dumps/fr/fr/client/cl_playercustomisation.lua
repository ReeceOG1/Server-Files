...
function tFR.getDrawableTextures(d, g)
    local e, f = b(d)
    if e then
...
local k = false
local l = 0
function tFR.setCustomization(i, m, n)
    if i then
        local o = tFR.getArmour()
        local h = PlayerPedId()
        local p = nil
        ...
        local q = tFR.loadModel(p)
        local r = GetEntityModel(h)
        if q then
            if r ~= q or m then
                k = true
                local s = tFR.getWeapons()
                local t = GetEntityHealth(h)
                SetPlayerModel(PlayerId(), p)
                Wait(0)
                tFR.giveWeapons(s, true)
                if n == nil or n == false then
                    print("[FR] Customisation, setting health to ", t)
                    tFR.setHealth(t)
                end
                ...
                print("[FR] Same model detected, not changing model.")
            end
            ...
            print("[FR] Failed to load model", p)
        end
        tFR.setArmour(o)
    end
end
function tFR.isPedScriptGuidChanging()
    return k or GetGameTimer() - l < 3000
end
function tFR.loadCustomisationPreset(w)
    local x = a.presets[w]
    assert(x, string.format("Preset %s does not exist.", w))
    if x.model then
        tFR.setCustomization({modelhash = x.model})
        Citizen.Wait(100)
    end
    local y = PlayerPedId()
