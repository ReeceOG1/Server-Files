...
Citizen.CreateThread(function()
    tFR.createDynamicPed(a.ped.modelHash,a.ped.position,215.0,true,a.ped.animDict,a.ped.animName,30,false,false)
    tFR.addBlip(a.ped.position.x, a.ped.position.y, a.ped.position.z, 357, 81, "Vehicle Impound", 0.8, false)
    Wait(2000)
    local d = function()
    ...
    tFR.createArea("vehicle_impound", a.ped.position, 3.0, 6, d, e, f, {})
end)
RMenu.Add("frimpound","reasons",RageUI.CreateMenu("","Impounding Vehicle...",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_impound", "fr_impound"))
RMenu.Add("frimpound","main",RageUI.CreateMenu("","Your Impounded Vehicles",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_impound", "fr_impound"))
...
RageUI.CreateWhile(1.0, true, function()
    ...
    if RageUI.Visible(RMenu:Get('frimpound', 'reasons')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("~g~Confirm Selection","",{RightLabel = "→→→"},true,function(i, k, j)
                if j then
                    ...
                    TriggerServerEvent("FR:impoundVehicle",b.owner_id,b.owner_name,b.vehicle,b.vehicle_name,m,b.vehicle_net_id)
                    RageUI.ActuallyCloseAll()
                end
            end,function()end)
    ...
    if RageUI.Visible(RMenu:Get('frimpound', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
    ...
    if RageUI.Visible(RMenu:Get('frimpound', 'vehicle_information')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if g ~= 0 then
                ...
                end,RMenu:Get("frimpound", "pay_fine_confirmation"))
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('frimpound', 'pay_fine_confirmation')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if g ~= 0 then
                ...
                RageUI.ButtonWithStyle("~g~Pay","Pay the fine",{RightLabel = "→→→"},true,function(i, k, j)
                    if j then
                        TriggerServerEvent("FR:releaseImpoundedVehicle", g.vehicle)
                    end
                end,RMenu:Get("frimpound", "main"))
...
RegisterNetEvent("FR:receiveInfoForVehicleToImpound",function(r, s, t, u, v)
    b = {owner_id = tonumber(r), owner_name = s, vehicle = t, vehicle_name = u, vehicle_net_id = v}
end)

RegisterNetEvent("FR:receiveImpoundedVehicles",function(w)
    c = w
end)

RegisterNetEvent("FR:impoundSuccess",function(x, t, y, z, A, B, C, D)
    ...
    if O ~= vector3(0, 0, 0) and N ~= vector3(0, 0, 0) then
        tFR.loadModel("flatbed")
        tFR.loadModel("a_m_m_prolhost_01")
        P = tFR.spawnVehicle("flatbed", O.x, O.y, O.z, M, false, true, true)
        ...
        TriggerServerEvent("FR:deleteImpoundEntities",x,NetworkGetNetworkIdFromEntity(G),NetworkGetNetworkIdFromEntity(F))
        u = tFR.spawnVehicle(z, C.x, C.y, C.z, D, false, true, false)
        ...
        TriggerServerEvent("FR:awaitTowTruckArrival", x, R, T)
...
RegisterNetEvent("FR:attachVehToTowCl",function(x, W)
    local u = NetworkGetEntityFromNetworkId(x)
...
function tFR.impoundVehicleOptions(Z, _, x, a0)
    local a1 = g_vehicleHashToVehicleName[_]
    if X(a0) then
        tFR.notifyPicture("polnotification","notification","The vehicle cannot be impounded with a person inside.","Metropolitan Police","Impound",nil,nil)
        return
    end
    local a2 = GetVehicleClass(a0)
    if a2 == 18 then
        tFR.notifyPicture("polnotification","notification","Emergency vehicles cannot be impounded.","Metropolitan Police","Impound",nil,nil)
    elseif a.disallowedVehicleClasses[a2] then
        tFR.notifyPicture("polnotification","notification","That vehicle is too large to be impounded.","Metropolitan Police","Impound",nil,nil)
    else
        TriggerServerEvent("FR:fetchInfoForVehicleToImpound", Z, a1, x)
        RageUI.Visible(RMenu:Get("frimpound", "reasons"), true)
    end
end

function tFR.isVehicleImpounded(u)
    return c[u] ~= nil
end
...
exports("impound", tFR.impoundVehicleOptions)
exports("isImpounded", tFR.isVehicleImpounded)
