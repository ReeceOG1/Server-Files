            ...
            ontick_fishing = function(y)
                if globalFishingOnDuty then
                    if
                     ...
                     then
                        TriggerServerEvent("FR:fishingJobChecks", y.type)
                    end
                    if IsControlJustPressed(0, 47) then
                        local z = GetVehiclePedIsIn(PlayerPedId(), true)
                        tFR.notify("~y~Boat anchor set to " .. tostring(not x))
...
function func_fishingStatus(y)
    ...
    elseif g and c then
        if IsControlJustPressed(0, 102) then
            ...
            if d >= e and d <= f then
                TriggerServerEvent("FR:fishingCaughtFish", d)
            else
                TriggerServerEvent("FR:fishingCaughtFish", d)
...
tFR.createThreadOnTick(func_fishingStatus)
RegisterNetEvent(
    "FR:fishingJobSetCasted",
    function()
        g = true
...
tFR.createThreadOnTick(func_createBar)
