...
function previewBackpack(c)
    ...
    SetPedComponentVariation(tFR.getPlayerPed(), prop0, prop1, prop2, 2)
end

RegisterNetEvent("FR:boughtBackpack",function(prop0, prop1, prop2, size, backpackname)
...
RegisterNetEvent("FR:removeBackpack",function()
    if currentBackpack ~= nil then
        ped = tFR.getPlayerPed()
        SetPedComponentVariation(tFR.getPlayerPed(), 5, 0, 0, 2)
        currentBackpack = nil
        b = nil
        c = nil
    end
...
AddEventHandler("FR:onClientSpawn",function(D, E)
    if E then
        ...
        local i = function(j)
            ...
            DrawAdvancedText(0.595, 0.213, 0.005, 0.0028, 1.0, "FR Backpacks", 255, 255, 255, 255, 1, 0)
                ...
                if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                    ...
                    if currentBackpack ~= backpackname then
                        if c ~= nil then
                            ...
                            TriggerServerEvent("FR:BuyBackpack",prop0,prop1,prop2,backpackname,price,size,backpackstorename)
                        else
                            tFR.notify("~r~No backpack selected!")
                        end
                    else
                        tFR.notify("~r~You already have this backpack!")
                    end
                    Wait(250)
                    EnableAllControlActions(1)
                end
        ...
        for q, r in pairs(a.stores) do
            for s, t in pairs(r) do
                if s == "_config" then
                    ...
                    if B then
                        tFR.addBlip(u, v, w, x, y, z)
                    end
                    tFR.addMarker(u, v, w, 1.0, 1.0, 1.0, 255, 0, 0, 170, 50, 27)
                    tFR.createArea("backpack_" .. q, vector3(u, v, w), 1.5, 6, g, h, i, {storeName = q})
                end
            end
        end
    end
