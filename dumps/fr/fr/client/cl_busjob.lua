...
local b = 10
RegisterNetEvent("FR:beginBusJob",function()
    a.tempVehicle = tFR.spawnVehicle("bus",a.spawnVehicleVector.x,a.spawnVehicleVector.y,a.spawnVehicleVector.z,343,true,true,true)
    a.onJob = true
...
RegisterNetEvent("FR:endBusJob",function()
...
RegisterNetEvent("FR:nextStopReachedBusJob",function(d)
    ...
    local B=tFR.loadModel(passengerPeds[a.stopNumber].pedModel)
    local coords = tFR.getPosition()
    local radius = 10.0
...
RegisterNetEvent("FR:setNextBusJobBlip",function(f)
    a.tempBlip = AddBlipForCoord(f.x, f.y, f.z)
    SetBlipSprite(a.tempBlip, 1)
    SetBlipRoute(a.tempBlip, true)
    a.tempMarker = tFR.addMarker(f.x, f.y, f.z, 1.2, 1.2, 1.2, 255, 0, 0, 125, 50, 0, true, true)
end)
RegisterNetEvent('FR:onClientSpawn')
AddEventHandler("FR:onClientSpawn",function(g,h)
    if h then
        local i = function(j)
        ...
        tFR.addMarker(a.startVector.x,a.startVector.y,a.startVector.z - 1,1.0,1.0,1.0,255,0,0,70,50,39,false,false,true)
        tFR.createArea("bus", a.startVector, 1.5, 6, i, k, l, {})
    end
end)
function alert(msg) 
