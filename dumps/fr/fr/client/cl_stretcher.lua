...
local function q()
    if IsControlJustReleased(0, 299) then
        if NetworkHasControlOfEntity(c) then
            TriggerServerEvent("FR:toggleAmbulanceDoors", tFR.getNetId(c, "Stretchers"))
        else
...
local function C(D, E)
    ...
    if not y(D) or b then
        ...
        tFR.loadAnimDict(a.positions[e].animDict)
        DetachEntity(x, false, false)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if d ~= nil and d ~= 0 then
            ...
            if not b and not IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and tFR.getPlayerVehicle() == 0 then
            ...
            if IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and c == 0 then
                ...
                if H ~= -1 and globalNHSOnDuty then
                    ...
                    if IsControlJustPressed(0, 183) then
                        ...
                        if L > 0 and M ~= 0 then
                            TriggerServerEvent("FR:forcePlayerOnToStretcher", L, M)
                        end
                    end
                end
                r(K)
            ...
            if IsControlJustReleased(0, 51) and not IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and not b then
                if DecorGetInt(d, "inside_ambulance_netid") ~= 0 then
                    local O = tFR.getObjectId(DecorGetInt(d, "inside_ambulance_netid"), "Stretcher Thread")
...
function pushStretcher(x, d)
    Citizen.CreateThread(function()
        ...
        TriggerServerEvent("FR:stretcherAttachPlayer", P)
        ...
        while IsEntityAttachedToEntity(d, x) do
            ...
            if DecorGetInt(d, "inside_ambulance_netid") ~= 0 then
                local O = tFR.getObjectId(DecorGetInt(d, "inside_ambulance_netid"), "pushStretcher(ped, nearbyStretcher)")
                if DecorGetBool(O, "has_stretcher_inside") ~= false then
                    if not NetworkHasControlOfEntity(O) then
                        if GetGameTimer() - Q > 200 then
                            TriggerServerEvent("FR:updateHasStretcherInsideDecor",tFR.getNetId(O, "Stretchers"),false)
                            Q = GetGameTimer()
                        end
                    else
            ...
            if c ~= nil and c ~= 0 then
                ...
                if IsControlJustReleased(0, 300) and (GetVehicleDoorAngleRatio(c, 4) ~= 0 or IsThisModelAHeli(GetEntityModel(c))) and not R then
                    ...
                    if not NetworkHasControlOfEntity(c) then
                        TriggerServerEvent("FR:updateHasStretcherInsideDecor", tFR.getNetId(c, "Stretcher"), true)
                    else
                        DecorSetBool(c, "has_stretcher_inside", true)
                    end
                    if not NetworkHasControlOfEntity(d) then
                        TriggerServerEvent("FR:updateStretcherLocation",tFR.getNetId(d, "Stretchers"),tFR.getNetId(c, "Stretchers"))
                    else
                        DecorSetInt(d, "inside_ambulance_netid", tFR.getNetId(c, "Stretchers"))
                    end
...
RegisterNetEvent("FR:stretcherAttachPlayer",function(P, S)
...
RegisterCommand("stretcher",function()
    if globalNHSOnDuty then
        ...
        local g = tFR.loadModel("trump-stretcher")
        local D = tFR.spawnVehicle(g, v.x, v.y, v.z, GetEntityHeading(x), false, true, true)
...
RegisterCommand("removestretcher",function()
    if globalNHSOnDuty then
        ...
        if V ~= nil then
            TriggerServerEvent("FR:removeStretcher", tFR.getNetId(V, "Stretchers"))
        end
    end
end,false)
RegisterNetEvent("FR:setStretcherInside",function(P, W)
    local D = tFR.getObjectId(P, "FR:setStretcherInside")
    DecorSetInt(D, "inside_ambulance_netid", W)
end)
RegisterNetEvent("FR:setHasStretcherInsideDecor",function(X, Y)
    local c = tFR.getObjectId(X, "FR:setHasStretcherInsideDecor")
    DecorSetBool(c, "has_stretcher_inside", Y)
end)
RegisterNetEvent("FR:toggleAmbulanceDoorStatus",function(X)
    local c = tFR.getObjectId(X, "FR:toggleAmbulanceDoorStatus")
...
function tFR.isUsingStretcher()
    return b
end
RegisterNetEvent("FR:forcePlayerOnToStretcher",function(P)
    if not NetworkDoesNetworkIdExist(P) then
        return
    end
    local D = NetworkGetEntityFromNetworkId(P)
