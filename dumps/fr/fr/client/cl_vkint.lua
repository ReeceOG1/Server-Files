...
RMenu.Add("vkint","mainmenu",RageUI.CreateMenu("Teleporter", "Main Menu", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight()))
local b = nil
local function c(d)
    if tFR.getPlayerCombatTimer() > 0 then
        tFR.notify("You can not teleport whilst you have a combat timer.")
        return
    end
...
Citizen.CreateThread(function()
    for f, d in pairs(a) do
        tFR.addMarker(d.position.x, d.position.y, d.position.z - 1, 1.0, 1.0, 1.0, 255, 255, 255, 170, 50, 27)
        tFR.createArea("vkint_" .. d.id,d.position,2.0,10.0,l,m,function()end,d)
    end
end)
