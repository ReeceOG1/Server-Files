...
RMenu.Add('FRStores', 'main', RageUI.CreateMenu("", "Shop", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight(), 'fr_marketui', 'fr_marketui'))
RMenu.Add("FRStores", "confirm", RageUI.CreateSubMenu(RMenu:Get('FRStores', 'main',  tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight())))

RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("FRStores", "main")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for k, v in pairs(cfg.shopItems) do
                ...
                end, RMenu:Get("FRStores", "confirm"))
            end
        end)
    end
end)
...
local Index = 1
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("FRStores", "confirm")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Button("Confirm Purchase" , nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent("FR:BuyStoreItem", cHash, tonumber(Index))
                end
            end, RMenu:Get("FRStores", "main")) 
        end) 
    end
end)
        ...
        for k, v in pairs(cfg.shops) do
            ...
            if isInArea(v, 1.0) and inMenu == false then
                ...
                if IsControlJustPressed(0, 51) then 
                    ...
                    RageUI.Visible(RMenu:Get("FRStores", "main"), true)
                    cLoaction = v
                end
            end
