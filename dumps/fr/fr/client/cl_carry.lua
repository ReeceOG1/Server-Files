...
RegisterCommand("carry",function(f, g)
    if IsPedInAnyVehicle(PlayerPedId(), true) then
        ...
        if not globalPlayerInPrisonZone or tFR.isStaffedOn() then
            if GetEntityHealth(tFR.getPlayerPed()) > 102 then
                local h = GetEntityCoords(tFR.getPlayerPed())
                distanceToCasino = #(h - d)
                if not carryingBackInProgress and (distanceToCasino > 200 or tFR.isStaffedOn()) then
                    local i = tFR.getPlayerPed()
                    local j = GetClosestPlayer(3)
                    if j ~= -1 then
                        target = GetPlayerServerId(j)
                        if GetEntityHealth(GetPlayerPed(j)) ~= 0 then
                            if not tFR.isStaffedOn() and not globalLFBOnDuty then
                                TriggerServerEvent("CarryPeople:requestCarry", target)
                            else
                                TriggerServerEvent("CarryPeople:sync", GetPlayerServerId(tFR.getPlayerId()), target)
                            end
                        else
            ...
            tFR.notify("You cannot carry in the prison.")
        end
    end
end,false)
RegisterNetEvent("CarryPeople:carryRequest",function()
    tFR.notify("Someone is trying to carry you, press (~y~Y~w~) to accept (~r~L~w~) to refuse")
    e = true
    Citizen.CreateThread(function()
        while e do
            if IsControlJustPressed(0, 246) then
                tFR.notify("~g~Request Accepted")
                ExecuteCommand("carryaccept")
                e = false
            elseif IsControlJustPressed(0, 182) then
                tFR.notify("Request Refused")
...
Citizen.CreateThread(function()
    while true do
        ...
        if carryingBackInProgress then
            ...
            if not tFR.isStaffedOn() and IsPedFalling(l) then
                TriggerServerEvent("CarryPeople:stop", target)
                carryingBackInProgress = false
                ClearPedTasks(l)
                StopAnimTask(l, "missfinale_c2mcs_1", "fin_c2_mcs_1_camman", 0)
...
RegisterNetEvent("CarryPeople:syncTarget",function(target, o, p, q, r, s, t, u, v)
    ...
    AttachEntityToEntity(tFR.getPlayerPed(), w, 0, r, q, s, 0.5, 0.5, u, false, false, false, false, 2, false)
    if v == nil then
        v = 0
    end
...
function func_carryAnimation()
    if carryingBackInProgress then
        DisablePlayerFiring(tFR.getPlayerId(), true)
        if not IsEntityPlayingAnim(tFR.getPlayerPed(), b, a, 3) then
            TaskPlayAnim(tFR.getPlayerPed(), b, a, 8.0, -8.0, 100000, c, 0, false, false, false)
        end
    end
end
tFR.createThreadOnTick(func_carryAnimation)
