        ...
        if t then
            ...
            ontick_prisonControlPanel = function()
                ...
                elseif IsControlJustPressed(0, 168) then
                    if tFR.isDev() and globalOnPrisonDuty then
                        d = true
                        setCursor(1)
                        inGUIFR = true
                    end
    ...
    if d then
            ...
            if j[r].name == "All" then
                ...
                if not CursorInArea(GetArea(0.497, 0.819, 0.185, 0.058)) then
                    ...
                    if f then
                        if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                            if h[j[r].doorHash].currentState == 5 or h[j[r].doorHash].currentState == 0 then
                                TriggerServerEvent("FR:prisonSetDoorState", j[r].doorHash, 1)
                            else
                                TriggerServerEvent("FR:prisonSetDoorState", j[r].doorHash, 5)
                            end
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            f = false
                            SetTimeout(
        ...
        if not CursorInArea(GetArea(0.791, 0.819, 0.081, 0.06)) then
            ...
            if e then
                if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                    c = not c
                    TriggerServerEvent("FR:prisonToggleLockdown", c)
                end
...
tFR.createThreadOnTick(func_drawControlPanel)
RegisterNetEvent(
    "FR:prisonUpdateGuardNumber",
    function(v)
        g = v
    end
)
    ...
    function(s, t)
        if t then
            ...
            ontick_openPrisonDoor = function(w)
                ...
                if g > 4 then
                    if
                        tFR.isStaffedOn() or globalOnPrisonDuty and w.overrideLock or globalOnPoliceDuty and w.pdOpen or
                    ...
                    if
                        tFR.isStaffedOn() or globalOnPrisonDuty or globalOnPoliceDuty and w.pdOpen or
                            (globalNHSOnDuty or globalLFBOnDuty) and w.nhsOpen
                     then
                        x = " (E to toggle lock) "
                        y = true
                ...
                if h[w.doorHash].currentState == 5 or h[w.doorHash].currentState == 0 then
                    tFR.DrawText3D(w.coords, w.doorName .. x .. " 🔓", 0.45, 4)
                else
                    tFR.DrawText3D(w.coords, w.doorName .. x .. " 🔒", 0.45, 4)
                end
                if
                    IsControlJustPressed(0, 38) and
                        (tFR.isStaffedOn() or globalOnPrisonDuty and (y or w.overrideLock) or
                            globalOnPoliceDuty and w.pdOpen or
                            (globalNHSOnDuty or globalLFBOnDuty) and w.nhsOpen)
                 then
                    tFR.loadAnimDict("anim@heists@keycard@")
                    Citizen.CreateThread(
                        function()
                    ...
                    if h[w.doorHash].currentState == 1 then
                        TriggerServerEvent("FR:prisonSetDoorState", w.doorHash, 5)
                    else
                        TriggerServerEvent("FR:prisonSetDoorState", w.doorHash, 1)
                    end
                end
            end
            for k, l in pairs(a.doors) do
