local function a()
    local b = GetResourceKvpString("fr_scope_horizontal")
    if b == nil or b == "" then
        return 50
    else
        return tonumber(b)
...
local function f()
    local b = GetResourceKvpString("fr_scope_vertical")
    if b == nil or b == "" then
        return 30
    else
        return tonumber(b)
...
local i = GetResourceKvpString("fr_scope_enabled") == "true"
local j = false

RMenu.Add("scope","main",RageUI.CreateMenu("", "~w~Scope Settings", tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_settingsui","fr_settingsui"))
...
function tFR.doesCurrentWeaponHaveScope()
    local p = PlayerPedId()
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if i and (j or tFR.doesCurrentWeaponHaveScope()) then
            if j then
