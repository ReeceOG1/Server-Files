RMenu.Add('FRclothing','mainMenu',RageUI.CreateMenu("","Clothing Store",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_clothesui","fr_clothesui"))
RMenu.Add('FRclothing','changePed',RageUI.CreateSubMenu(RMenu:Get('FRclothing','mainMenu'),"","Change Gender"))
RMenu.Add('FRclothing','changeClothing',RageUI.CreateSubMenu(RMenu:Get('FRclothing','mainMenu'),"","Change your Clothes"))
RMenu.Add('FRclothing','clearProps',RageUI.CreateSubMenu(RMenu:Get('FRclothing','mainMenu'),"","Clear Props"))
...
local r = {
    ...
    vector3(-442.67565917969,6012.9057617188,31.716400146484),
...
Citizen.CreateThread(function()
    while true do 
        local G=GetEntityCoords(tFR.getPlayerPed())
        for H=1,#r do 
            if#(G-r[H])<=2.5 and not t then 
                ...
                h=true
                RageUI.Visible(RMenu:Get('FRclothing','mainMenu'),true)
            elseif s==H then 
                ...
                if not RageUI.Visible(RMenu:Get('FRclothing','changeClothing'))then 
                    F()
                end 
            end 
        end
...
RageUI.CreateWhile(1.0, true, function()
    if not n then
        if RageUI.Visible(RMenu:Get('FRclothing', 'mainMenu')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                RageUI.Button('Apply Gang Outfit', 'Applys the outfit set by the Gang Leader.', {RightLabel="→→→"}, true, function(L,M,N)
                    if N then
                            TriggerServerEvent("FR:applyGangFit", gangID)
                        end
                    end)
                end
                RageUI.Button('Apply Outfit Code','Applys an outfit code given by a player with ~y~FR Plus~w~ or ~y~FR Platinum~w~.',{RightLabel="→→→"},true,function(L,M,N)
                    if N then 
                        tFR.clientPrompt("Outfit Code:","",function(Q)
                            local R = string.gsub(Q, "%s+", "")
                            if #R == 5 then
                                TriggerServerEvent("FR:applyOutfitCode", string.upper(R))
                            else
                ...
                end,RMenu:Get('FRclothing','clearProps'))
            end)
        end
        if RageUI.Visible(RMenu:Get('FRclothing', 'changePed')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                RageUI.Button('MP Male','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            tFR.loadCustomisationPreset("DefaultMale")
                        else
                            tFR.notify("Cannot change gender, you're dead.")
                        end
                    end 
                end,function()
                end)
                RageUI.Button('MP Female','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            tFR.loadCustomisationPreset("DefaultFemale")
                        else
                            tFR.notify("Cannot change gender, you're dead.")
                        end
                    end 
        ...
        if RageUI.Visible(RMenu:Get('FRclothing', 'changeClothing')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                m=true
                if h then 
                    for H=1,#a,1 do 
                        ...
                        RageUI.List(P.name,get_drawables(P.type,P.index),a[H].currentListIndex,"Current Texture Index: "..k.textureN.."/"..j-1,{},true,function(L,M,N,Q)
                            ...
                            if N then 
                                ...
                                if k.index==2 then 
                                    ...
                                    TriggerServerEvent("FR:saveClothingHairData",GetPedDrawableVariation(tFR.getPlayerPed(),2),GetPedHairColor(tFR.getPlayerPed()))
...
function current_clothing(R,i,S)
    if R=="prop"then 
        if S then 
            if GetPedPropTextureIndex(tFR.getPlayerPed(),i)~=nil then 
                return GetPedPropTextureIndex(tFR.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedPropIndex(tFR.getPlayerPed(),i)~=nil then 
                if GetPedPropIndex(tFR.getPlayerPed(),i)==-1 then 
                    return GetPedPropIndex(tFR.getPlayerPed(),i)+2 
                else 
                    return GetPedPropIndex(tFR.getPlayerPed(),i)+1 
                end 
            else 
                return 0 
            end 
    ...
    elseif R=="drawable"then 
        ...
        elseif S then 
            if GetPedTextureVariation(tFR.getPlayerPed(),i)~=nil then 
                return GetPedTextureVariation(tFR.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedDrawableVariation(tFR.getPlayerPed(),i)~=nil then 
                return GetPedDrawableVariation(tFR.getPlayerPed(),i)+1 
            else 
                return 0 
            end 
        end 
...
function get_textures(T,R,i)
    if T=="drawable"then 
        ...
        j=GetNumberOfPedTextureVariations(tFR.getPlayerPed(),R,i-1)
        return j 
    else 
        j=GetNumberOfPedPropTextureVariations(tFR.getPlayerPed(),R,i-1)
        return j 
    end 
end
local function Y()
...
Citizen.CreateThread(function()
    while true do 
        if k.index~=nil and RageUI.Visible(RMenu:Get('FRclothing','changeClothing'))then 
...
AddEventHandler("FR:onClientSpawn",function(D, E)
    if E then
		for a7,a8 in pairs(r)do 
            tFR.addBlip(a8.x,a8.y,a8.z,73,3,"Clothing Store",0.7)
            tFR.addMarker(a8.x,a8.y,a8.z,0.6,0.6,0.6,10,255,81,170,50,9,false,false,true,"dp_clothing","top",90.0,90.0,0.0)
        end 
	end
end)
