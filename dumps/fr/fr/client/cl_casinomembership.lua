...
RMenu.Add('frhighrollers','casino',RageUI.CreateMenu("","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('frhighrollers','casino'):SetSubtitle("MEMBERSHIP")
RMenu.Add('frhighrollers','confirmadd',RageUI.CreateSubMenu(RMenu:Get('frhighrollers','casino'),"","Are you sure?",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight()))
RMenu.Add('frhighrollers','confirmremove',RageUI.CreateSubMenu(RMenu:Get('frhighrollers','casino'),"","Are you sure?",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight()))

RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('frhighrollers', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            end,RMenu:Get('frhighrollers','confirmadd'))
            ...
            end,RMenu:Get('frhighrollers','confirmremove'))
        end)
    end
    if RageUI.Visible(RMenu:Get('frhighrollers', 'confirmadd')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("No","",{RightLabel="→→→"},true,function(b,c,d)
                if d then 
                    tFR.notify("~y~Cancelled!")
                end 
            end,RMenu:Get('frhighrollers','casino'))
            RageUI.ButtonWithStyle("Yes","",{RightLabel="→→→"},true,function(b,c,d)
                if d then 
                    TriggerServerEvent("FR:purchaseHighRollersMembership")
                end 
            end,RMenu:Get('frhighrollers','casino'))
        end)
    end
    if RageUI.Visible(RMenu:Get('frhighrollers', 'confirmremove')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("No","",{RightLabel="→→→"},true,function(b,c,d)
                if d then 
                    tFR.notify("~y~Cancelled!")
            ...
            RageUI.ButtonWithStyle("Yes","",{RightLabel="→→→"},true,function(b,c,d)
                if d then 
                    TriggerServerEvent("FR:removeHighRollersMembership")
                end 
            end,RMenu:Get('frhighrollers','casino'))
        end)
    end
...
function showCasinoMembership(e)
    RageUI.Visible(RMenu:Get('frhighrollers','casino'),e)
end
Citizen.CreateThread(function()
    ...
    for g,h in pairs(a)do 
        local i=tFR.loadModel('u_f_m_casinocash_01')
...
AddEventHandler("FR:onClientSpawn",function(D, E)
    if E then
        ...
        for q,h in pairs(a)do 
            tFR.addBlip(h.entryPosition.x,h.entryPosition.y,h.entryPosition.z,682,0,"Casino Memberships",0.7,true)
            tFR.addMarker(h.entryPosition.x,h.entryPosition.y,h.entryPosition.z,1.0,1.0,1.0,138,43,226,70,50,27)
            tFR.createArea("casinomembership_"..q,h.entryPosition,1.5,6,m,o,p,{})
        end 
	end
end)