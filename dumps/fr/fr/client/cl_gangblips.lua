...
RegisterCommand("gangblips", function()
    if not blipon then
        blipon = true
        TriggerServerEvent("FR:EnableGangBlips")
        notify("Gang blips Enabled")
        Citizen.CreateThread(function()
            while blipon do
                Citizen.Wait(5000)
                TriggerServerEvent("FR:GetGangBlipsData")
            end
        end)
    else
        DeleteAllBlips()
...
RegisterNetEvent("FR:ReceiveGangBlipsData")
AddEventHandler("FR:ReceiveGangBlipsData", function(gangBlipsData)
    DeleteAllBlips()
