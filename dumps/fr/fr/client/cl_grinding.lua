...
RegisterNetEvent("FR:playGrindingScenario",function(c, d)
    if not a then
        a = true
        local e = GetGameTimer()
        TaskStartScenarioInPlace(tFR.getPlayerPed(), c, 0, true)
        local f
        if d then
        ...
        CreateThread(function()
            tFR.startCircularProgressBar("",h,nil,function()end)
        end)
...
RegisterNetEvent("FR:playGrindingPickaxe",function()
    if not a then
        a = true
        local e = GetGameTimer()
        ...
        local j = tFR.loadModel("prop_tool_pickaxe")
        ...
        while e + h > GetGameTimer() do
            while IsEntityPlayingAnim(tFR.getPlayerPed(),"melee@large_wpn@streamed_core","ground_attack_on_spot",3) == 1 do
                Wait(0)
            end
        ...
        tFR.notify("Mining currently in progress.")
    end
end)
local l = false
local function m()
    ...
    if q then
        ...
        local u = function(s)
            if s.nearby then
                if IsControlJustReleased(0, 38) and GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                    if not a then
                        m()
                        TriggerServerEvent("FR:requestGrinding", s.drug, s.type)
                        Wait(500)
                    else
                        tFR.notify("Action in progress, please wait.")
                    end
                end
            end
        end
        for v, w in pairs(b) do
            for x, y in pairs(w) do
                tFR.createArea(v .. "_" .. x,y.position,y.radius,6,r,t,u,{drug = v, type = x, nearby = false})
            end
        end
    end
end)
