...
AddEventHandler(
    ...
    function(p, q)
        if q then
            tFR.addMarker(b.x, b.y, b.z, 0.7, 0.7, 0.5, 0, 125, 255, 125, 150, 20, false, false, true)
            tFR.addBlip(b.x, b.y, b, 378, 3, "Organ Heist Police")
            tFR.addMarker(c.x, c.y, c.z, 0.7, 0.7, 0.5, 255, 0, 0, 125, 150, 20, false, false, true)
            tFR.addBlip(c.x, c.y, c.z, 378, 1, "Organ Heist Criminals")
        end
    end
)
    ...
    function()
        while true do
            if f then
                ...
                if IsControlJustPressed(0, 38) then
                    if not globalOnPoliceDuty then
                        if not g then
                            TriggerServerEvent("FR:joinOrganHeist")
                            Wait(100)
                        else
                            tFR.notify("~r~Already joined the Organ Heist!")
                        end
                    else
                        tFR.notify("~r~You are police, please use the other entrance!")
                    end
                end
            elseif e then
                ...
                if IsControlJustPressed(0, 38) then
                    if globalOnPoliceDuty then
                        ...
                        tFR.notify("~r~You are a civillian, please use the other entrance!")
                    end
                end
            end
            ...
            if inGamePhase or inWaitingStage then
                ...
                RemoveWeaponFromPed(tFR.getPlayerPed(), "WEAPON_MOLOTOV")
                RemoveWeaponFromPed(tFR.getPlayerPed(), "WEAPON_FLASHBANG")
                RemoveWeaponFromPed(tFR.getPlayerPed(), "WEAPON_SMOKEGRENADEFR")
                RemoveWeaponFromPed(tFR.getPlayerPed(), "WEAPON_SMOKEGRENADEFRPD")
                RemoveWeaponFromPed(tFR.getPlayerPed(), "WEAPON_BZGAS")
                RemoveWeaponFromPed(tFR.getPlayerPed(), "WEAPON_STUNGUN")
                local r = PlayerPedId()
...
RegisterNetEvent(
    ...
    function(v, w, x, y)
        ...
        if x == "civ" then
            ...
            tFR.createAtm("Organ Heist", k.atmLocation)
        elseif x == "police" then
            SetPedRelationshipGroupHash(PlayerPedId(), m)
        end
        for l, z in pairs(k.gunStores[h]) do
            tFR.createGunStore(z[1], z[2], z[3])
        end
        tFR.setFriendlyFire(false)
        DoScreenFadeOut(1000)
...
RegisterNetEvent(
    "FR:organHeistKillConfirmed",
    function(B)
        PlaySoundFrontend(-1, "Weapon_Upgrade", "DLC_GR_Weapon_Upgrade_Soundset", true)
        tFR.playScreenEffect("MP_Celeb_Win", 0.25)
        tFR.notify("~g~Killed " .. B .. " received £50,000 and 25% Armour")
    end
)
Citizen.CreateThread(
    function()
        while true do
            if inWaitingStage then
                ...
                if #(s - k.pastGate) < 3.0 then
                    ...
                    NetworkFadeInEntity(PlayerPedId(), 0)
                    DoScreenFadeIn(1000)
                    tFR.notify("~r~You got too far from the organ heist and have been teleported back.")
                end
            end
            if inGamePhase then
                local s = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent(
    ...
    function()
        ...
        inGamePhase = false
        d = 600
        tFR.deleteAtm("Organ Heist")
        for l, z in pairs(k.gunStores[h]) do
            tFR.deleteGunStore(z[1])
        end
        h = ""
        k = nil
        tFR.setCanAnim(true)
...
function tFR.setDeathInOrganHeist()
    inOrganHeist = false
    inWaitingStage = false
    inGamePhase = false
end
