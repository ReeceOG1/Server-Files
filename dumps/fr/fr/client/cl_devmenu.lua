RMenu.Add(
    "devmenu",
    "main",
    RageUI.CreateMenu("FR Dev Tools", "Developer Tools", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight())
)
RMenu.Add(
    "devmenu",
    "animations",
...
local z = module("fr-weapons", "cfg/weapons")
RageUI.CreateWhile(
    1.0,
    true,
    function()
            ...
            RageUI.DrawContent(
                ...
                function()
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                tFR.clientPrompt(
                                    "Customization [JSON]",
                                    json.encode(tFR.getCustomization()),
                                    function()
                                    end
                                )
                            end
                        ...
                        function(A, B, C)
                            if C then
                                tFR.clientPrompt(
                                    ...
                                    "",
                                    function(D)
                                        tFR.setCustomization(json.decode(D))
                                    end
                                )
                            end
                        end
                        ...
                        function(A, B, C)
                            if C then
                                ...
                                tFR.clientPrompt(
                                    ...
                                    "",
                                    function(G)
                                        local H = tFR.loadModel(tonumber(G))
                                        if H then
                                            local I =
                                                GetEntityCoords(PlayerPedId()) +
                                                GetEntityForwardVector(PlayerPedId()) * 2.0
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                TriggerEvent("FR:toggleTrafficMenu")
                            end
                        end
                    )
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                local J = tFR.loadModel("a_f_m_beach_01")
                                local K = tFR.getPlayerCoords()
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                local J = tFR.loadModel("s_m_m_pilot_02")
                                local K = tFR.getPlayerCoords()
                                CreatePed(1, J, K.x, K.y, K.z, 0.0, true, true)
                                SetModelAsNoLongerNeeded(J)
                            end
                                ...
                                if string.match(t, S[c]) then
                                    RageUI.ButtonWithStyle(
                                        ...
                                        function(A, B, C)
                                            if C then
                                                u(R)
                                                TaskPlayAnim(tFR.getPlayerPed(), R, S[c], 1.0, -1, -1, 1, 0, 0, 0, 0)
                                                RemoveAnimDict(R)
                                            end
                                        end
                                    )
                    ...
                    for c = 1, #U.anims do
                        RageUI.ButtonWithStyle(
                            ...
                            function(A, B, C)
                                if C then
                                    u(U.dict)
                                    TaskPlayAnim(tFR.getPlayerPed(), U.dict, U.anims[c], 1.0, -1, -1, 1, 0, 0, 0, 0)
        ...
        --                             end
        --                             RequestIpl(S[2])
        --                             SetEntityCoords(tFR.getPlayerPed(), tFR.getPlayerCoords())
        --                             local K = tFR.getPlayerCoords()
        --                             local W = GetInteriorAtCoords(K.x, K.y, K.z)
        --                             tFR.notify("[FR] Ipl ~g~" .. S[1] .. "(" .. S[2] .. ")" .. " ~s~loaded")
        --                         end
        --                     end
        --                 )
        --             end
...
function tFR.gridPositionSaver(aa, L, M)
    tFR.notify("pick 1st coord, press Y when done")
    ...
    Wait(250)
    tFR.notify("pick 2nd coord, press Y when done")
    while not IsControlJustPressed(0, 246) do
        Wait(0)
    end
    local ac = GetEntityCoords(PlayerPedId())
    Wait(250)
    tFR.notify("pick 3rd coord, press Y when done")
    while not IsControlJustPressed(0, 246) do
        Wait(0)
    end
    ...
    for c = 0, M - 1, 1 do
        for af = 0, L - 1, 1 do
            ...
            tFR.loadModel("a_m_m_fatlatin_01")
            local aj = tFR.spawnVehicle(aa, ai.x, ai.y, ai.z, heading, false, false, false)
    ...
    function()
        while true do
            ...
            if a then
                ...
                tFR.DrawText(0.32, 0.00, "X:~s~ " .. roundx, 0.4)
                tFR.DrawText(0.38, 0.00, "Y:~s~ " .. roundy, 0.4)
                tFR.DrawText(0.445, 0.00, "Z:~s~ " .. roundz, 0.4)
                heading = GetEntityHeading(ak)
                roundh = tonumber(string.format("%.2f", heading))
                tFR.DrawText(0.50, 0.00, "H:~s~ " .. roundh, 0.4)
                local al, am, an = table.unpack(GetEntityRotation(PlayerPedId(), 1))
                tFR.DrawText(0.38, 0.03, "RX:~s~ " .. tonumber(string.format("%.2f", al)), 0.4)
                tFR.DrawText(0.44, 0.03, "RY:~s~ " .. tonumber(string.format("%.2f", am)), 0.4)
                tFR.DrawText(0.495, 0.03, "RZ:~s~ " .. tonumber(string.format("%.2f", an)), 0.4)
                speed = GetEntitySpeed(PlayerPedId())
                rounds = tonumber(string.format("%.2f", speed))
                tFR.DrawText(0.40, 0.92, "Player Speed: ~s~" .. rounds, 0.4)
                health = GetEntityHealth(PlayerPedId())
                tFR.DrawText(0.40, 0.95, "Player Health: ~s~" .. health, 0.4)
                camRotX = GetGameplayCamRot(2).x
                tFR.DrawText(0.36, 0.88, "CR X: ~s~" .. tonumber(string.format("%.2f", camRotX)), 0.4)
                camRotY = GetGameplayCamRot(2).y
                tFR.DrawText(0.44, 0.88, "CR Y: ~s~" .. tonumber(string.format("%.2f", camRotY)), 0.4)
                camRotZ = GetGameplayCamRot(2).z
                tFR.DrawText(0.51, 0.88, "CR Z: ~s~" .. tonumber(string.format("%.2f", camRotZ)), 0.4)
                ...
                if IsPedInAnyVehicle(PlayerPedId(), 1) then
                    ...
                    tFR.DrawText(0.015, 0.76, "Engine Health: ~s~" .. vehenground, 0.4)
                    tFR.DrawText(0.015, 0.73, "Body Health: ~s~" .. vehbodround, 0.4)
                    tFR.DrawText(
                        0.015,
                        0.70,
                        "Vehicle Fuel: ~s~" ..
...
function tFR.createDebugObject(Q)
    local aX = tFR.loadModel(Q)
