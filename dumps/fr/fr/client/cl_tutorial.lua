...
RegisterNetEvent("FR:initMoney",function(n, o)
    m = o
end)
local function z8(r,...)
...
local function a3()
    ...
    while table.count(globalVehicleModelHashMapping) == 0 or tFR.getPlayerVehicle() == 0 do
        ...
        elseif
            RageUI.Visible(RMenu:Get("FRGarages", "rented_vehicles")) or RageUI.Visible(RMenu:Get("FRGarages", "customfolders"))
         then
...
local function ad()
    ...
    D()
    V()
    if table.count(VehiclesFetchedTable) == 0 or tFR.isDevMode() then
        if m > 20000 then
            a1()
        end
    else
...
RegisterCommand("tutorial",function()
    if tFR.isDevMode() then
        ad()
    end
end,false)
RegisterNetEvent("FR:playTutorial", ad)
AddEventHandler("FR:onClientSpawn",function(ae, af)
...
RegisterNetEvent("FR:setIsNewPlayer",function()
    f = true
end)
