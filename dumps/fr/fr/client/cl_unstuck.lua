...
local function b(c, d, e)
    ...
    Citizen.CreateThread(function()
        ...
        if not g then
            ...
            Wait(500)
            DoScreenFadeIn(500)
            tFR.notifyPicture("CHAR_BLOCKED","CHAR_BLOCKED","Attempting to teleport to surface... \n\nIf you are still not on the surface please use /calladmin.","FR","Utilities",nil,nil)
            TriggerServerEvent("FR:unstuckSuccessful", d, e)
        else
            tFR.notifyPicture("CHAR_BLOCKED","CHAR_BLOCKED","You moved during the countdown so the teleportation was cancelled.","FR","Utilities",nil,nil)
        end
        f = false
        g = false
    end)
...
RegisterCommand("unstuck",function(q, r, s)
    local h = PlayerPedId()
    local t, p = o()
    if not t or tFR.getPlayerCombatTimer() > 0 then
        tFR.notifyPicture("CHAR_BLOCKED","CHAR_BLOCKED","You are unable to use this right now. Use /calladmin if you still need assistance.","FR","Utilities",nil,nil)
    else
        tFR.loadAnimDict("timetable@amanda@ig_4")
        TaskPlayAnim(h, "timetable@amanda@ig_4", "ig_4_base", 8.0, -8.0, -1, 0, 0.0, 0, 0, 0)
        RemoveAnimDict("timetable@amanda@ig_4")
        Wait(1000)
        b(45, tFR.getPlayerCoords(), p)
    end
end)
...
RegisterCommand("reset",function(q, r, s)
    local h = PlayerPedId()
    local t =
        tFR.getPlayerCombatTimer() > 0 or tFR.isPlayerInRedZone() or GetRoomKeyFromEntity(PlayerPedId()) ~= 0 or
        IsEntityInWater(h) or
        IsPedInAnyVehicle(h, false)
    if u < GetGameTimer() then
        if t then
            tFR.notifyPicture("CHAR_BLOCKED","CHAR_BLOCKED","You are unable to use this right now.","FR","Utilities",nil,nil)
        else
            local v = tFR.getPlayerCoords()
            tFR.teleport(3094.446, -4811.093, 15.26162)
            Wait(500)
            tFR.teleport(v.x, v.y, v.z)
            u = GetGameTimer() + 60000
        end
    else
        tFR.notifyPicture("CHAR_BLOCKED","CHAR_BLOCKED","You must wait 60 seconds before using this again.","FR","Utilities",nil,nil)
    end
end)
