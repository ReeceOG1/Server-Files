...
RegisterCommand("flagf",function(o, p)
    if c ~= "" then
        ...
        if table.count(p) ~= 0 then
            q = table.concat(p, " ")
            TriggerServerEvent("FR:flagVehicleAnpr", string.upper(c), q)
        else
            tFR.notify("Error~w~: You must provide a valid flag reason.")
        end
    end
end)
RegisterCommand("flagr",function(o, p)
    if d ~= "" then
        ...
        if table.count(p) ~= 0 then
            q = table.concat(p, " ")
            TriggerServerEvent("FR:flagVehicleAnpr", string.upper(d), q)
        else
            tFR.notify("Error~w~: You must provide a valid flag reason.")
        end
    end
...
tFR.createThreadOnTick(func_drawFullAnpr)
function anprGetVehicle(u)
    ...
    local w = GetVehiclePedIsIn(tFR.getPlayerPed(), false)
    ...
    if B > 0 and IsEntityAVehicle(B) then
        ...
        if C ~= nil then
            if m[C] ~= nil then
                ...
                tFR.notify(string.format("ANPR Alert~w~: Plate ~b~%s ~w~is flagged for:~b~~n~%s", C, D))
                n = true
...
function isPedInPoliceVehicle()
    if IsPedInAnyVehicle(tFR.getPlayerPed(), false) then
        if GetVehicleClass(tFR.getPlayerVehicle()) == 18 and globalOnPoliceDuty then
            return true
        end
    end
    return false
...
RegisterNetEvent("FR:setFlagVehicles",function(H)
    m = H
end)