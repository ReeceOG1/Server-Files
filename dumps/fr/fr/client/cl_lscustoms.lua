...
--Setup main menu
local LSCMenu = Menu.new("Los Santos Customs","~w~CATEGORIES", 0.16,0.13,0.24,0.36,0,{255,255,255,255})
...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		LSCMenu.config.size.width = f(LSC_Config.menu.width) or 3.85;
		LSCMenu.config.size.height = f(LSC_Config.menu.height) or 0.35;
		...
		elseif	type(LSC_Config.menu.theme) == "string" then
			if LSC_Config.menu.theme == "light" then
				...
				LSCMenu:setColors({ r = 255,g = 255, b = 255, a = 255},{ r = 0,g = 0, b = 0, a = 255},{ r = 0,g = 0, b = 0, a = 255},{ r = 255,g = 255, b = 255, a = 255})
			elseif LSC_Config.menu.theme == "darkred" then
				LSCMenu:setColors({ r = 255,g = 255, b = 255, a = 255},{ r = 0,g = 0, b = 0, a = 255},{ r = 0,g = 0, b = 0, a = 255},{ r = 255,g = 255, b = 255, a = 255})
			elseif LSC_Config.menu.theme == "bluish" then
				LSCMenu:setColors({ r = 255,g = 255, b = 255, a = 255},{ r = 255,g = 255, b = 255, a = 255},{ r = 0,g = 0, b = 0, a = 255},{ r = 0,g = 100, b = 255, a = 255})
			elseif LSC_Config.menu.theme == "greenish" then
				LSCMenu:setColors({ r = 255,g = 255, b = 255, a = 255},{ r = 0,g = 0, b = 0, a = 255},{ r = 0,g = 0, b = 0, a = 255},{ r = 0,g = 200, b = 0, a = 255})
			end
		end
...
local function DriveOutOfGarage(pos)
	Citizen.CreateThread(function()
		...
		local ok, veh_type, model = tFR.getNearestOwnedVehicle(1)
		myveh.model = model
						...
						if LSC_Config.oldenter then			
							if #(vec3(outside.position) - GetEntityCoords(ped)) <= f(5) then
								if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									if pos.locked then
										if not LSC_Config.lock then
											if IsControlJustPressed(1,201) then
												...
												tFR.DrawText(0.5,0.8,"Press ~b~ENTER~w~ to enter ~b~Los Santos Customs~w~",1.0,4,0)
											end
										else
											tFR.DrawText(0.5,0.8,"~r~Locked, please wait~w~",1.0,4,0)
										end
									else
										if IsControlJustPressed(1,201) then
											inside = true
									...
									tFR.DrawText(0.5,0.8,"~r~This vehicle can't be upgraded~w~",1.0,4,0)
								end
							end
						else
...
function LSCMenu:onSelectedIndexChanged(name, button)
	...
	elseif m == "neon color" then
		if myveh.neon then
			...
			LSCMenu:showNotification("You don't have neon lights installed.")
		end
	elseif m == "xenon color" then
		if myveh.mods[22].mod == 1 then
			SetVehicleHeadlightsColour(veh, button.xenon)
		else
			LSCMenu:showNotification("You don't have xenon headlights installed.")
		end
...
AddEventHandler("FR:onClientSpawn",function(D, E)
    if E then
        ...
        for q,h in pairs(garages)do 
            tFR.addBlip(h.inside.position.x,h.inside.position.y,h.inside.position.z,72, nil, "LS Customs")
        end 
	end
end)
...
function tFR.isInsideLsCustoms()
    return inside
end
