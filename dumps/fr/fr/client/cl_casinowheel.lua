...
RegisterNetEvent("FR:frameworkCleanup")
AddEventHandler(
    "FR:frameworkCleanup",
    function()
        DeleteEntity(b)
        ...
        RageUI.IsVisible(
            ...
            function()
                RageUI.ButtonWithStyle(
                    "Spin the wheel of FR (50,000 Chips)",
                    "",
                    {RightLabel = "→→→"},
                    true,
                    function(e, f, g)
                        if g then
                            tFR.notify("Tom Needs To Fix")
                            --TriggerServerEvent("FR:requestSpinLuckyWheel")
                        end
                    end
                )
            end,
...
Citizen.CreateThread(
    function()
        ...
        local i = tFR.loadModel("vw_prop_vw_luckywheel_02a")
        local j = tFR.loadModel(c)
...
function goToWheel()
    if not d then
        ...
        local D = false
        while not D do
            local E = GetEntityCoords(tFR.getPlayerPed())
        ...
        SetEntityHeading(tFR.getPlayerPed(), 10.9754)
        Wait(2000)
    ...
    function(D, E)
        if E then
            ...
            for K, L in pairs(a) do
                tFR.addBlip(L.entryPosition.x, L.entryPosition.y, L.entryPosition.z, 681, 0, "FR Wheel", 0.7, true)
                tFR.createArea("frwheel_" .. K, L.entryPosition, 1.5, 6, F, H, I, {})
            end
        end
    end
)
RegisterNetEvent("FR:syncLuckyWheel")
AddEventHandler(
    "FR:syncLuckyWheel",
    function(M)
        Wait(900)
...
RegisterNetEvent("FR:spinLuckyWheel")
AddEventHandler(
    "FR:spinLuckyWheel",
    function()
        goToWheel()
...
RegisterNetEvent("FR:spinLuckyWheelReaction")
AddEventHandler(
    "FR:spinLuckyWheelReaction",
    function(N)
        local y = tFR.getPlayerPed()
