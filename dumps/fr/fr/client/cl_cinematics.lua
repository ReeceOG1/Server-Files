RMenu.Add("mainmenu","cinematic",RageUI.CreateMenu("", "", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight(), "new_editor", "r_editor_header"))
RMenu:Get("mainmenu", "cinematic"):SetSubtitle("FR Cinematic Editor")
RMenu.Add("load_scene","cinematic",RageUI.CreateSubMenu(RMenu:Get("mainmenu", "cinematic"),"","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("load_scene", "cinematic"):SetSubtitle("Load Scene")
RMenu.Add("scene_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("load_scene", "cinematic"),"","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("scene_manager", "cinematic"):SetSubtitle("Scene Manager")
RMenu.Add("camera_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("scene_manager", "cinematic"),"","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("camera_manager", "cinematic"):SetSubtitle("Camera Manager")
RMenu.Add("screeneffect_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("camera_manager", "cinematic"),"","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("screeneffect_manager", "cinematic"):SetSubtitle("Screen Effect Manager")
RMenu.Add("timecycle_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("camera_manager", "cinematic"),"","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("timecycle_manager", "cinematic"):SetSubtitle("Timecycle Effect Manager")
RMenu.Add("shake_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("camera_manager", "cinematic"),"","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("shake_manager", "cinematic"):SetSubtitle("Shake Effect Manager")
RMenu.Add("add_camera","cinematic",RageUI.CreateSubMenu(RMenu:Get("camera_manager", "cinematic"),"","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("add_camera", "cinematic"):SetSubtitle("Add Camera")
RMenu.Add("camera_focus_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("camera_manager", "cinematic"),"","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("camera_focus_manager", "cinematic"):SetSubtitle("Camera Focus")
RMenu.Add("weather_time_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("mainmenu", "cinematic"),"","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("weather_time_manager", "cinematic"):SetSubtitle("Time/Weather Manager")
RMenu.Add("dof_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("camera_manager", "cinematic"),"","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"new_editor","r_editor_header"))
...
Citizen.CreateThread(function()
    m = json.decode(GetResourceKvpString("fr_scenedata") or "{}")
end)
RegisterNetEvent("FR:openCinematicMenu",function()
    a(true)
end)
function tFR.createCinematicScene(P)
    m[P] = {}
    SetResourceKvp("fr_scenedata", json.encode(m))
    tFR.notify("~g~Scene created & saved.")
end
function tFR.deleteCinematicScene(P)
    m[P] = nil
    SetResourceKvp("fr_scenedata", json.encode(m))
    tFR.notify("~g~Scene deleted & saved.")
end
function tFR.createCamera(P, Q)
    if #m[P] == 0 then
        Q.transition = 100
    end
    table.insert(m[P], Q)
    ...
    SetResourceKvp("fr_scenedata", json.encode(m))
    tFR.notify("~g~Camera created & saved.")
end
function tFR.modifyCamera(P, Q, R)
    m[P][R] = Q
    SetResourceKvp("fr_scenedata", json.encode(m))
    tFR.notify("~g~Camera modified & saved.")
end
function tFR.deleteCamera(P, S)
    table.remove(m[P], S)
    SetResourceKvp("fr_scenedata", json.encode(m))
    tFR.notify("~g~Camera deleted & saved.")
end
function tFR.modifyTransition(n, q, T)
    m[n][q].transition = tonumber(T)
    SetResourceKvp("fr_scenedata", json.encode(m))
    tFR.notify("~g~Transition change saved.")
end
function tFR.modifyCameraBlending(n, q, U)
    m[n][q].blending = l[U]
    SetResourceKvp("fr_scenedata", json.encode(m))
    tFR.notify("~g~Camera Blending change saved.")
end
function tFR.modifyScreeneffect(n, q, V)
    m[n][q].screeneffect = V
    SetResourceKvp("fr_scenedata", json.encode(m))
    tFR.notify("~g~Screeneffect change saved.")
end
function tFR.modifyTimecycleEffect(n, q, W)
    m[n][q].timecycleEffect = W
    SetResourceKvp("fr_scenedata", json.encode(m))
    tFR.notify("~g~Timecycle Effect change saved.")
end
function tFR.setTimecycleIntensity(n, q, f)
    m[n][q].timecycleIntensity = f
    SetResourceKvp("fr_scenedata", json.encode(m))
    tFR.notify("~g~Timecycle intensity change saved.")
end
function tFR.modifyShakeEffect(n, q, X)
    m[n][q].shake = X
    SetResourceKvp("fr_scenedata", json.encode(m))
    tFR.notify("~g~Shake change saved.")
end
function tFR.modifyCameraType(n, q, type)
    m[n][q].type = type
    SetResourceKvp("fr_scenedata", json.encode(m))
    tFR.notify("~g~Camera type change saved.")
end
function tFR.addCameraFocus(n, q, Y, Z)
    m[n][q].focusType = Y
    m[n][q].focusData = Z
    SetResourceKvp("fr_scenedata", json.encode(m))
    tFR.notify("~g~Follow player change saved.")
end
function tFR.setCameraFov(n, q)
    m[n][q].fov = i[A]
    SetResourceKvp("fr_scenedata", json.encode(m))
    tFR.notify("~g~FOV change saved.")
end
    ...
    if RageUI.Visible(RMenu:Get('camera_manager', 'cinematic')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            if q and m[n][q] then
                ...
                RageUI.ButtonWithStyle("Transition: " .. m[n][q].transition .. "ms","",{RightLabel = "→→→"},true,function(a1, a2, a3)
                    if a3 then
                        local T = getSearchField("Transition(in ms):")
                        if tonumber(T) then
                            tFR.modifyTransition(n, q, T)
                        else
                            tFR.notify("Failed to change transition time.")
                        end
                    end
                end,RMenu:Get("camera_manager", "cinematic"))
                ...
                RageUI.List("Timecycle Intensity",f,e,"",{},true,function(a1, a2, a3, a8)
                    ...
                    if a8 ~= e then
                        ...
                        if m[n][q].timecycleEffect ~= "" then
                            ...
                            tFR.setTimecycleIntensity(n, q, f[e])
                        end
                    end
                end,function()end,nil)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            for g = 1, #c do
                RageUI.ButtonWithStyle(c[g],"",{RightLabel = "→→→"},true,function(a1, a2, a3)
                    ...
                    if a3 then
                        tFR.modifyScreeneffect(n, q, c[g])
                    end
                end,RMenu:Get("camera_manager", "cinematic"))
            end
        end)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            for g = 1, #d do
                RageUI.ButtonWithStyle(d[g],"",{RightLabel = "→→→"},true,function(a1, a2, a3)
                    ...
                    if a3 then
                        tFR.modifyTimecycleEffect(n, q, d[g])
                    end
                end,RMenu:Get("camera_manager", "cinematic"))
            end
        end)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("Player","",{RightLabel = "→→→"},true,function(a1, a2, a3)
                if a3 then
                    ...
                    if a9 ~= "" then
                        tFR.addCameraFocus(n, q, "player", a9)
                    end
                end
            end,RMenu:Get("camera_manager", "cinematic"))
            RageUI.ButtonWithStyle("Coordinates","",{RightLabel = "→→→"},true,function(a1, a2, a3)
                if a3 then
                    tFR.clientPrompt("Enter coordinates:","",function(aa)
                        local ab = {}
                        ...
                        tFR.addCameraFocus(n, q, "coord", vector3(ad, ae, af))
                    end)
                end
            end,RMenu:Get("camera_manager", "cinematic"))
        end)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if (o or p) and noclipActive and tFR.getPlayerVehicle() ~= 0 then
            tFR.notify("You may not noclip when in a vehicle.")
            tFR.toggleNoclip()
        end
            ...
            if a0.focusType ~= "" then
                if a0.focusType == "player" then
                    ...
                    if aj then
                        ...
                        print("[FR Cinematic] Failed to point cam at player, could not get entity.")
                    end
...
Citizen.CreateThread(function()
    while true do
        local al, am = tFR.getPlayerVehicle()
        ...
        if an ~= 0 and IsEntityAPed(an) and IsPedAPlayer(an) and not IsEntityVisible(an) then
            ...
            if ao ~= -1 then
                ...
                if ap > 0 then
                    if not tFR.clientGetPlayerIsStaff(tFR.clientGetUserIdFromSource(ap)) then
