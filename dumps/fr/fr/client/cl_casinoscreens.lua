...
function jimmy()
    ...
    while p do 
        if#(tFR.getPlayerCoords()-t)<4.0 then 
...
AddEventHandler("FR:enteredDiamondCasino",function()
    insideDiamondCasino=true;
    startCasinoThreads()
end)
AddEventHandler("FR:exitedDiamondCasino",function()
    insideDiamondCasino=false 
end)
AddEventHandler("FR:bigWinDiamondCasino",function()
    if not insideDiamondCasino then 
        return 
    end;
    w=true 
