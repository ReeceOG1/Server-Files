...
RegisterCommand("panic",function()
    if globalOnPoliceDuty or globalNHSOnDuty or globalOnPrisonDuty or globalLFBOnDuty then
        if b.Cooling == 0 then
            local f = {}
            f.Player = tFR.getPlayerId()
            f.Ped = tFR.getPlayerPed()
            ...
            if globalOnPoliceDuty then
                TriggerServerEvent("FR:newPanic", f, 3)
            elseif globalNHSOnDuty then
                TriggerServerEvent("FR:newPanic", f, 2)
            elseif globalOnPrisonDuty then
                TriggerServerEvent("FR:newPanic", f, 4)
            elseif globalLFBOnDuty then
                TriggerServerEvent("FR:newPanic", f, 5)
            end
...
RegisterNetEvent("FR:returnPanic",function(g, f, h)
    if globalOnPoliceDuty or globalNHSOnDuty or globalOnPrisonDuty or globalLFBOnDuty and h == 5 then
        ...
        if h == 2 then
            tFR.notifyPicture2(f.Headshot,2,string.format("NHS - %s", f.Name),false,"~r~NHS Paramedic in distress! - " .. f.Location)
        elseif h == 3 then
            tFR.notifyPicture2(f.Headshot,2,string.format("MET Police - %s", f.Name),false,"~r~Officer in distress! - " .. f.Location)
        elseif h == 4 then
            tFR.notifyPicture2(f.Headshot,2,string.format("HM Prison - %s", f.Name),false,"~r~Prison Guard in distress! - " .. f.Location)
        elseif h == 5 then
            tFR.notifyPicture2(f.Headshot,2,string.format("LFB - %s", f.Name),false,"~r~LFB Panic Alarm Activated! - " .. f.Location)
        end
        if handle ~= nil then
            UnregisterPedheadshot(handle)
        end
