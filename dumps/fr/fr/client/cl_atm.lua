...
local b = false
RMenu.Add("fratm","mainmenu",RageUI.CreateMenu("", "", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight(), "fr_atmui", "fr_atmui"))
RMenu:Get("fratm", "mainmenu"):SetSubtitle("ATM")
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('fratm', 'mainmenu')) then
                ...
                if Selected then
                    ...
                    if tonumber(e) then
                        if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                            if a then
                                tFR.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                                TriggerServerEvent('FR:Deposit', tonumber(e))
                            else
                                tFR.notify("Not near ATM.")
                            end
                        else
                            tFR.notify("Get out your vehicle to use the ATM")
                        end
                    else
                ...
                if Selected then
                    ...
                    if tonumber(e) then
                        if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                            if a then
                                tFR.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                                TriggerServerEvent('FR:Withdraw', tonumber(e))
                            else
                                tFR.notify("Not near ATM.")
                            end
                        else
            ...
            RageUI.Button("Deposit All", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then
                    if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                        if a then
                            tFR.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                            TriggerServerEvent('FR:DepositAll')
                        else
                            tFR.notify("Not near ATM.")
                        end
                    else
            ...
            RageUI.Button("Withdraw All", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then
                    if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                        if a then
                            tFR.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                            TriggerServerEvent('FR:WithdrawAll')
                        else
                            tFR.notify("Not near ATM.")
                        end
                    else
...
local function g()
    RageUI.CloseAll()
    RageUI.Visible(RMenu:Get("fratm", "mainmenu"), true)
end
local function h()
    RageUI.CloseAll()
    RageUI.Visible(RMenu:Get("fratm", "mainmenu"), false)
end
Citizen.CreateThread(function()
    ...
    for m, n in pairs(a.atms) do
        tFR.createArea("atm_" .. m, n, 1.5, 6, i, k, l, {atmId = m})
        local o = tFR.addBlip(n.x, n.y, n.z, 108, 4, "ATM", 0.8, true)
        tFR.addMarker(n.x, n.y, n.z, 0.7, 0.7, 0.5, 0, 255, 125, 125, 50, 29, false, false, true)
...
function tFR.createAtm(t, u)
    ...
    tFR.createArea(v,u,1.5,6,i,k,function()end)
    local w = tFR.addMarker(u.x, u.y, u.z, 0.7, 0.7, 0.5, 0, 255, 125, 125, 50, 29, false, false, true)
    s[t] = {area = v, marker = w}
end
function tFR.deleteAtm(t)
    local x = s[t]
    if x then
        tFR.removeMarker(x.marker)
        tFR.removeArea(x.area)
        s[t] = nil
    end
end
local y = false
local function z(A)
    ...
    for p = 1, math.random(3, 4) do
        ...
        local E = true
        tFR.minigameCircularProgressBar({Difficulty = D, Timeout = 25000, onComplete = function(F)
            B = F
            E = false
        ...
        if not B then
            ...
            tFR.startCircularProgressBar("",2000,nil,function()end)
            TriggerServerEvent("FR:atmWireCutSparks", A, false)
            Citizen.Wait(2000)
        end
    end
    C = true
...
RegisterNetEvent("FR:atmWireCuttingSuccessSync",function(A)
...
RegisterNetEvent("FR:atmWireCuttingSuccess",function()
...
RegisterNetEvent("FR:atmWireCutSparks",function(A)
    local L = a.robberyAtms[A]
    local M = G(L)
    tFR.loadPtfx("core")
...
RegisterNetEvent("FR:startAtmWireCutting",function(A)
    y = true
    local V = PlayerPedId()
    local L = a.robberyAtms[A]
    tFR.setCanAnim(false)
    ...
    tFR.loadClipSet("move_ped_crouched")
...
RegisterNetEvent("FR:atmInkArea",function(A)
    local L = a.robberyAtms[A]
    tFR.loadPtfx("veh_xs_vehicle_mods")
    for p = 1, 10 do
...
RegisterNetEvent("FR:atmGenericAlarm",function(A)
    local L = a.robberyAtms[A]
...
local function a1(j)
    Y = math.random(6, 12)
    a0 = 0
    TriggerServerEvent("FR:getAtmHasBeenRobbed", j.robberyId)
end
local function a2(j)
    if y then
        TriggerServerEvent("FR:atmStopWireCutting", j.robberyId)
    end
    Z = 0
end
local function a3()
...
local function a7()
    ...
    local a4 = tFR.getPlayerCoords() + GetEntityForwardVector(PlayerPedId()) * 1.0
    ...
    if select(2, GetCurrentPedWeapon(V)) == "WEAPON_CROWBAR" then
        ...
        if IsDisabledControlJustPressed(0, 24) and not _ then
            Citizen.CreateThreadNow(function()
                ...
                tFR.loadAnimDict("melee@small_wpn@streamed_core")
                V = PlayerPedId()
                ...
                Z = Z + 1
                if Z >= Y then
                    TriggerServerEvent("FR:startAtmWireCutting", j.robberyId)
                end
...
Citizen.CreateThread(function()
    for A, L in pairs(a.robberyAtms) do
        tFR.createArea("atmrobbery_" .. A, L.xyz, 1.5, 6, a1, a2, a9, {robberyId = A})
    end
end)
RegisterNetEvent("FR:setAtmHasBeenRobbed",function(ac)
    a0 = ac
end)
