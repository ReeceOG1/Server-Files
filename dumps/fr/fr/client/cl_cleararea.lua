...
RegisterNetEvent("FR:clearVehicles",function()
    for b, c in pairs(GetGamePool("CVehicle")) do
        if NetworkHasControlOfEntity(c) then
            if GetPedInVehicleSeat(c, -1) == 0 and c ~= GetVehiclePedIsIn(tFR.getPlayerPed(), true) then
...
RegisterNetEvent("FR:clearBrokenVehicles",function()
    for b, c in pairs(GetGamePool("CVehicle")) do
        if NetworkHasControlOfEntity(c) then
            if GetPedInVehicleSeat(c, -1) == 0 and c ~= GetVehiclePedIsIn(tFR.getPlayerPed(), true) then
