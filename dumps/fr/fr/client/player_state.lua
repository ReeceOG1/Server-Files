...
Citizen.CreateThread(function()
  while true do
    ...
    if IsPlayerPlaying(PlayerId()) and state_ready then
      FRserver.updateWeapons({tFR.getWeapons()})
    end
  end
end)
...
local userdata = {}
Citizen.CreateThread(function()
    print("[FR] Loading cached user data.")
    userdata = json.decode(GetResourceKvpString("fr_userdata") or "{}")
    if type(userdata) ~= "table" then
      userdata = {}
        --print("[FR] Loading cached user data - failed to load setting to default.")
    else
        --print("[FR] Loading cached user data - loaded.")
    end
end)
function tFR.updateCustomization(b)
    local c = tFR.getCustomization()
    if c.modelhash ~= 0 and IsModelValid(c.modelhash) then
      userdata.customisation = c
      if b then
        SetResourceKvp("fr_userdata", json.encode(userdata))
      end
    end
end
function tFR.updateHealth(b)
  userdata.health = GetEntityHealth(PlayerPedId())
  if b then
      SetResourceKvp("fr_userdata", json.encode(userdata))
  end
end
function tFR.updateArmour(b)
...
Citizen.CreateThread(function()
    ...
    while true do
        Wait(5000)
        if not tFR.isInHouse() and not inOrganHeist and not tFR.isPlayerInRedZone() and not tFR.isInSpectate() then
          tFR.updatePos()
        end
        if not tFR.isStaffedOn() and not customizationSaveDisabled and not spawning and not tFR.isPlayerInAnimalForm() then
            tFR.updateCustomization()
        end
        tFR.updateHealth()
        tFR.updateArmour()
        SetResourceKvp("fr_userdata", json.encode(userdata))
...
function tFR.checkCustomization()
    local c = userdata.customisation
    if c == nil or c.modelhash == 0 or not IsModelValid(c.modelhash) then
        tFR.setCustomization(getDefaultCustomization(), true, true)
    else
        tFR.setCustomization(c, true, true)
    end
end
...
function tFR.spawnAnim(a)
  if a ~= nil then
    ...
    while (not HaveAllStreamingRequestsCompleted(PlayerPedId()) or GetNumberOfStreamingRequests() > 0) and
        ...
        Wait(0)
        print("[FR] Waiting for streaming requests to complete!")
    end
    NewLoadSceneStop()
    tFR.checkCustomization()
    TriggerServerEvent('FR:getPlayerHairstyle')
    TriggerServerEvent('FR:getPlayerTattoos')
    TriggerEvent("FR:playGTAIntro")
    TriggerEvent('FR:ForceRefreshData', -1)
    DoScreenFadeIn(1000)
    ShutdownLoadingScreen()
    ShutdownLoadingScreenNui()
    if not tFR.isDev() then
    ...
    print("[FR] cachedUserData.health", userdata.health)
    print("[FR] cachedUserData.armour", userdata.armour)
    SetEntityHealth(PlayerPedId(), userdata.health or 200)
    tFR.setArmour(userdata.armour)
...
AddEventHandler("FR:playGTAIntro",function()
  if not tFR.isDev() then
    TriggerEvent("fr:PlaySound", "gtaloadin")
  end
end)

RegisterNetEvent("FR:setHairstyle")
AddEventHandler("FR:setHairstyle",function(q)
  if q then
    dad = q["dad"] or 0
    mum = q["mum"] or 0
    skin = q["skin"] or 0
