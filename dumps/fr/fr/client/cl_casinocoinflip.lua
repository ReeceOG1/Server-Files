...
RegisterNetEvent("FR:sendCoinflipTableData")
AddEventHandler("FR:sendCoinflipTableData",function(C)
    f = C
end)
...
Citizen.CreateThread(function()
    ...
    for q = 0, #c.coinflipTables do
        ...
        local DD = tFR.loadModel(D.tableModel)
...
Citizen.CreateThread(function()
    while true do
        if d then
            ...
            if not l and IsControlJustPressed(0, 202) and not g and not RageUI.Visible(RMenu:Get("frcoinflip", "confirm")) then
                local K = tFR.loadAnimDict("anim_casino_b@amb@casino@games@shared@player@")
                NetworkStopSynchronisedScene(Local_198f_255)
                TaskPlayAnim(tFR.getPlayerPed(), K, "sit_exit_left", 1.0, 1.0, 2500, 0)
                d = false
                e = true
...
RegisterNetEvent("FR:sitAtCoinflipTable")
AddEventHandler("FR:sitAtCoinflipTable",function(o, L)
    if L then
        h[L.betId] = L
    end
    goToCoinflipSeat(o)
...
function goToCoinflipSeat(o)
    ...
    TaskGoStraightToCoord(tFR.getPlayerPed(), R.x, R.y, R.z, 1.0, 5000, S.z, 0.01)
    Wait(500)
    local T = v(o)
    local U = A(o)
    local V = NetworkCreateSynchronisedScene(T.x, T.y, T.z, U.x, U.y, U.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tFR.getPlayerPed(),V,"anim_casino_b@amb@casino@games@shared@player@",M(x),2.0,-2.0,13,16,2.0,0)
    ...
    NetworkAddPedToSynchronisedScene(tFR.getPlayerPed(),Locali98f_55,"anim_casino_b@amb@casino@games@shared@player@","idle_cardgames",2.0,-2.0,13,16,1148846080,0)
...
function showHowToCoinflip(W)
    ...
    RageUI.Visible(RMenu:Get("frcoinflip", "instructions"), W)
end
function showCoinflipMenu(W)
    RageUI.Visible(RMenu:Get("frcoinflip", "mainmenu"), W)
end
RMenu.Add("frcoinflip", "instructions", RageUI.CreateMenu("", "COINFLIP", 0, 100, "fr_coinflip", "fr_coinflip"))
RMenu.Add("frcoinflip", "mainmenu", RageUI.CreateMenu("", "COINFLIP", 0, 100, "fr_coinflip", "fr_coinflip"))
RMenu.Add("frcoinflip", "confirm", RageUI.CreateSubMenu(RMenu:Get("frcoinflip", "mainmenu"), "", "COINFLIP", 0, 100, "fr_coinflip", "fr_coinflip"))


RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('frcoinflip', 'instructions')) then
    ...
    if RageUI.Visible(RMenu:Get('frcoinflip', 'mainmenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.Button("Propose Bet","",{RightLabel = "→→→"},true,function(X, Y, Z)
                if Z then
                    ...
                    if tonumber(_) then
                        _ = tonumber(_)
                        if _ > 0 then
                            TriggerServerEvent("FR:proposeCoinflip", _)
                        end
                    end
                end
            end)
                ...
                for a0, L in pairs(h) do
                    RageUI.Button("~y~£" .. getMoneyStringFormatted(L.betAmount),"",{RightBadge=RageUI.BadgeStyle.GoldMedal},true,function(X, Y, Z)
                        if Z then
                            ...
                            if L.user_id == tFR.getUserId() then
                                k = true
                            else
                                k = false
                            end
    ...
    if RageUI.Visible(RMenu:Get('frcoinflip', 'confirm')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if not h[i] then
                RageUI.Visible(RMenu:Get("frcoinflip", "confirm"), false)
                RageUI.Visible(RMenu:Get("frcoinflip", "mainmenu"), true)
            end
            ...
            if k then
                ...
                end,RMenu:Get("frcoinflip", "mainmenu"))
...
RegisterNetEvent("FR:addCoinflipProposal",function(a0, L)
    h[a0] = L
end)

RegisterNetEvent("FR:cancelCoinflipBet",function(a0)
    h[a0] = nil
end)

RegisterNetEvent("FR:clearCoinflipBets",function()
    h = {}
end)

RegisterNetEvent("FR:coinflipOutcome",function(a3, a4)
    l = true
    ...
    SetTimeout(10000,function()
        a6 = true
        if a3 then
            tFR.notify("~g~ You won £" .. getMoneyStringFormatted(a4.amount) .. "!")
            SendNUIMessage({transactionType = "playCasinoWin"})
            if getMoneyStringFormatted(a4.amount) > 10000000 then
                TriggerClientEvent('chatMessage', -1, "^7Diamond Casino |", { 128, 128, 128 }, ""..GetPlayerName(source).." has WON "..tostring(getMoneyStringFormatted(a4.amount)).." chips!", "alert")
            end
        else
            tFR.notify("~r~ You lost £" .. getMoneyStringFormatted(a4.amount) .. "!")
