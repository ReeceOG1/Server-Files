RMenu.Add("policedog","main",RageUI.CreateMenu("Dog Support Unit", "Dog Management", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight()))
AddEventHandler("FR:setPoliceOnDuty",function(a)
    if a then
        RegisterKeyMapping("k9attack", "Police Dog Attack", "keyboard", "INSERT")
    end
end)
AddEventHandler("FR:setPrisonGuardOnDuty",function(b)
    if b then
        RegisterKeyMapping("k9attack", "Police Dog Attack", "keyboard", "INSERT")
    end
end)
...
local i = "K9"
local j = "K9TARGET"
RegisterNetEvent("FR:policeDogMenu",function()
    if globalOnPoliceDuty and tFR.hasPoliceCallsign() then
        RMenu:Get("frscenemenu", "main"):SetSubtitle("MPD~w~: " .. tFR.getPoliceCallsign() .. " - " .. tFR.getPoliceRank() .. " - " .. tFR.getPoliceName())
    end
    if globalOnPrisonDuty and tFR.hasHmpCallsign() then
        RMenu:Get("frscenemenu", "main"):SetSubtitle("HMP~w~: " .. tFR.getHmpCallsign() .. " - " .. tFR.getHmpRank() .. " - " .. tFR.getHmpName())
    end
    RageUI.Visible(RMenu:Get("policedog", "main"), true)
end)
RegisterNetEvent("FR:sendClientRagdoll",function()
    SetPedToRagdoll(tFR.getPlayerPed(), 12000, 12000, 0, 0, 0, 0)
    showNotification("~y~~h~Alert~h~~s~: " .. "You have been bitten by a police dog.")
end)
RegisterNetEvent("FR:policeDogAttack",function()
...
local m = {breed = 1}
local function n()
    local o = GetResourceKvpString("fr_policedog_customisations")
...
local function r(s)
    ...
    local q = k[m.breed]
    p[q] = s
    SetResourceKvp("fr_policedog_customisations", json.encode(p))
end
...
function tpDog()
    local z = GetEntityCoords(tFR.getPlayerPed(), true)
                                    ...
                                    if M < 2.0 then
                                        ...
                                        for O, P in pairs(N) do
                                            if I == GetPlayerPed(P) then
                                                ...
                                                TriggerServerEvent("FR:performDogLog","Instructed dog to attack ID: " .. Q)
                                                TriggerServerEvent("FR:serverDogAttack", Q)
                                                H = true
                                                Wait(1000)
    ...
    while f.currentAction == d.SearchNearby and DoesEntityExist(f.handle) do
        ...
        for W, X in pairs(GetActivePlayers()) do
            ...
            if GetVehiclePedIsUsing(Y) == 0 and IsEntityVisible(Y) and #(GetEntityCoords(Y, true) - L) < 5.0 then
                ...
                local _ = tFR.clientGetUserIdFromSource(Z)
                if tFR.getJobType(_) == "" then
        ...
        if #V > 0 then
            ...
            TriggerServerEvent("FR:policeDogSniffPlayer", a0.source)
        end
        Citizen.Wait(5000)
    end
end
function putInVehicle()
    local a2 = tFR.getNearestVehicle(7.0)
    Citizen.Trace(a2)
...
function removeFromVehicle()
    if IsPedInAnyVehicle(tFR.getPlayerPed(), true) then
...
function newDog(q, a4)
    local a5 = tFR.getPlayerPed()
    if not IsPedInAnyPoliceVehicle(a5) then
        local a6 = tFR.loadModel(q)
        ...
        if DoesEntityExist(f.handle) then
            ...
            TriggerServerEvent("FR:performDogLog", "Created a new police dog")
        end
    end
end
function deleteDog()
    ...
    f.active = false
    f.handle = 0
    TriggerServerEvent("FR:performDogLog", "Deleted dog")
end
function dogFollow()
    if not f.active then
        return
    ...
    if f.currentAction ~= d.Follow then
        ClearPedTasks(f.handle)
        TaskFollowToOffsetOfEntity(f.handle, tFR.getPlayerPed(), 0.0, 0.0, 0.0, 7.0, -1, 10.0, true)
        f.currentAction = d.Follow
    else
        ClearPedTasks(f.handle)
        f.currentAction = d.Follow
...
RegisterNetEvent("FR:policeDogIndicate",function(ad)
