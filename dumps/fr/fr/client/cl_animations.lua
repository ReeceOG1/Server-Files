...
local function c()
    if GetEntityHealth(tFR.getPlayerPed()) <= 102 then
        tFR.notify("~r~You try to move but you remember you're dead")
        return true
    elseif IsEntityDead(tFR.getPlayerPed()) then
        tFR.notify("~r~You try to move but you remember you're dead")
        return true
    else
        return false
...
RegisterCommand(
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and tFR.canAnim() and not IsEntityDead(h) and d() then
...
function surrender()
    ...
    if DoesEntityExist(h) and tFR.canAnim() and not IsEntityDead(h) and (d() or isPlayerNearPrison()) then
        tFR.loadAnimDict("random@arrests")
        tFR.loadAnimDict("random@arrests@busted")
...
function tFR.isPlayerSurrendered()
...
function tFR.isPlayerSurrenderedNoProgressBar()
    return j()
end
local u
RegisterNetEvent("FR:startRobbingPlayer",function()
    u = GetGameTimer()
end)
RegisterNetEvent("FR:endRobbingPlayer",function()
    u = nil
end)
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if B then
                ...
                DeleteEntity(G)
                Wait(750)
                ClearPedSecondaryTask(tFR.getPlayerPed())
                B = false
            else
                Wait(500)
                ...
                tFR.notify("Press ~r~[E]~w~ to take a picture!")
                B = true
            end
        end
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if y then
                ...
                Wait(120)
                tFR.notify("Press ~r~[E]~w~ to take a toke!")
                y = true
            end
        end
        while y do
            ...
            if IsControlJustPressed(0, 38) then
                ...
                Wait(100)
                tFR.notify("You take a huge rip!")
                Wait(7250)
            end
        end
    end,
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if v then
                ...
                Wait(120)
                tFR.notify("Press ~r~[E]~w~ to wave the wand.")
                v = true
            end
        end
            ...
            if
             ...
             then
                if not Y and globalNHSOnDuty then
                    ...
                    Y = false
                    DetachEntity(tFR.getPlayerPed(), false, false)
                    ClearPedSecondaryTask(tFR.getPlayerPed())
                    local a5 = GetClosestPlayer(3)
                    local a7 = GetPlayerServerId(a5)
                    if a7 ~= 0 then
                        TriggerServerEvent("FR:animationSyncStop", a7)
                    end
                end
                Wait(250)
            end
...
RegisterNetEvent(
    ...
    function()
        ...
        SetPedMotionBlur(tFR.getPlayerPed(), true)
        SetPedMovementClipset(tFR.getPlayerPed(), "move_m@drunk@verydrunk", true)
        RemoveAnimSet("move_m@drunk@verydrunk")
        SetPedIsDrunk(tFR.getPlayerPed(), true)
        DoScreenFadeIn(1000)
        ClearPedSecondaryTask(tFR.getPlayerPed())
        DeleteObject(prop)
        Wait(60000)
        ...
        ResetPedMovementClipset(tFR.getPlayerPed(), 0)
        SetPedIsDrunk(tFR.getPlayerPed(), false)
        SetPedMotionBlur(tFR.getPlayerPed(), false)
        RemoveAnimDict(H)
    end
)
RegisterNetEvent(
...
RegisterNetEvent(
    ...
    function(ai)
        ...
        tFR.loadAnimDict("misshair_shop@hair_dressers")
        ...
        tFR.loadModel("prop_clippers_01")
...
RegisterNetEvent(
    "FR:playArmourApplyAnim",
    function()
        tFR.loadAnimDict("clothingtie")
        tFR.setCanAnim(false)
        TaskPlayAnim(PlayerPedId(), "clothingtie", "try_tie_negative_a", 3.0, 3.0, 10000, 51, 0, false, false, false)
        RemoveAnimDict("clothingtie")
        tFR.startCircularProgressBar(
            "",
            10000,
            nil,
            function()
