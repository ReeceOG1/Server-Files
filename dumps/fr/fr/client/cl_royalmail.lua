...
local b = 15
RegisterNetEvent("FR:beginRoyalMailJob",function()
    a.tempVehicle = tFR.spawnVehicle("boxville2",a.spawnVehicleVector.x,a.spawnVehicleVector.y,a.spawnVehicleVector.z,343,true,true,true)
    a.onJob = true
    tFR.notify("~g~Royal Mail Job started, exit the car park on your left and head to your first drop off point.")
    while a.onJob do
...
RegisterNetEvent("FR:royalMailJobEnd",function()
...
RegisterNetEvent("FR:royalMailReachedNextStop",function(c, d)
    ...
    local f = tFR.loadModel("prop_drug_package")
...
RegisterNetEvent("FR:royalMailJobSetNextBlip",function(g)
    a.tempBlip = AddBlipForCoord(g.x, g.y, g.z)
    SetBlipSprite(a.tempBlip, 1)
    SetBlipRoute(a.tempBlip, true)
    a.tempMarker = tFR.addMarker(g.x, g.y, g.z - 1, 2.0, 2.0, 1.0, 200, 20, 0, 50, 50)
end)
    ...
    if i then
        ...
        local m = function(k)
            if IsControlJustReleased(1, 38) and not a.onJob then
                TriggerServerEvent("FR:attemptBeginRoyalMailJob")
            end
        end
        tFR.addBlip(a.startVector.x, a.startVector.y, a.startVector.z, 67, 1, "Royal Mail Job")
        tFR.addMarker(a.startVector.x,a.startVector.y,a.startVector.z,1.0,1.0,1.0,255,0,0,70,50,39,false,false,true)
        tFR.createArea("royalmail", a.startVector, 1.5, 6, j, l, m, {})
    end
end)