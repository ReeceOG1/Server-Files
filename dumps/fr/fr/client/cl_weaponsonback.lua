...
local function g()
    ...
    local i = tFR.getWeapons()
    local j = false
    local k = globalOnPoliceDuty and tFR.getPlayerVehicle() ~= 0
    for l in pairs(i) do
        local m = GetHashKey(l)
        local n = a.weapons[m]
        if n then
            local o = (not k or n.type ~= "Heavy") and (tFR.isEmergencyService() or not isInGreenzone) and not tFR.isPlayerInAnimalForm()
