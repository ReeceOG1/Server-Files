    ...
    if tFR.globalOnPoliceDuty() then 
        if m[1]~=nil then 
            if tonumber(m[1])>100 then 
                d=tonumber(m[1])
                tFR.notify(string.format("~g~Maximum speed set to %smph",tonumber(m[1])))
            else 
                d=101
                tFR.notify("Minimum speed you can set is 101mph!")
            end 
        end 
    else 
        tFR.notify("Speed gun is not enabled!")
    end 
end)
...
tFR.createThreadOnTick(func_drawRadar)
Citizen.CreateThread(function()
    while true do 
        if tFR.globalOnPoliceDuty() and not usingDelgun then
            if GetSelectedPedWeapon(tFR.getPlayerPed())==a then 
                b=true 
            else 
                b=false 
            end
            if b and tFR.getPlayerVehicle()==0 then 
                local n=tFR.getPlayerId()
                if IsPlayerFreeAiming(n)then 
                    ...
                    if p~=0 and GetPedInVehicleSeat(p,-1)==e and not IsPedInAnyPlane(e) and not IsPedInAnyHeli(e) then 
                        ...
                        if k[f]~=nil then 
                            ...
                            tFR.notify(string.format("~h~Vehicle Flagged:~s~~n~Plate %s is flagged for:~n~%s",f,r))
                            soundPlayed=true
                        ...
                        if h>d and h>101 then 
                            ...
                            if not j[q]then 
                                ...
                                TriggerServerEvent("FR:speedGunFinePlayer",GetPlayerServerId(NetworkGetPlayerIndexFromPed(e)),h-d)
                                Citizen.Wait(3000)
                            else 
                                tFR.notify("This vehicle has been fined recently!")
                            end 
                            else i="~w~"
                        end 
                    end 
...
RegisterNetEvent("FR:speedGunPlayerFined")
AddEventHandler("FR:speedGunPlayerFined",function()
    PlaySoundFrontend(-1,"ScreenFlash","MissionFailedSounds",1)
    StartScreenEffect("FocusOut",0,false)
    Wait(2000)
    StopScreenEffect("FocusOut")
end)
AddEventHandler("FR:setFlagVehicles",function(s)
    k=s 
end)