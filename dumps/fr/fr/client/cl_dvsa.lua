...
RMenu.Add('dvsa','main',RageUI.CreateMenu("","Driver & Vehicle Standards Agency", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight(), "dvsa_banner", "dvsa_banner"))
RMenu.Add('dvsa','licence',RageUI.CreateSubMenu(RMenu:Get('dvsa','main'),"","DVSA: ~b~Driving Licence", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight()))
RMenu.Add('dvsa','tests',RageUI.CreateSubMenu(RMenu:Get('dvsa','main'),"","DVSA: ~b~Driving Tests", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight()))
RMenu.Add('dvsa','alerts',RageUI.CreateSubMenu(RMenu:Get('dvsa','main'),"","DVSA: ~b~Alerts", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight()))
local a={}
local b={}
local c={}
local d={}
...
tFR.addMarker(i.test.reception.x,i.test.reception.y,i.test.reception.z-0.96,1.2,1.2,1.2,0,255,125,125,50,27,true,false,false,nil,nil,0.0,0.0,0.0)
local j=tFR.addBlip(i.test.reception.x,i.test.reception.y,i.test.reception.z,523,47,"DVSA Test Centre",1.0,false)
local k=function()
...
tFR.createArea("dvsaTestCentre_",i.test.reception,1.5,6,k,l,m,{})
RegisterNetEvent("FR:dvsaData",function(n,o,p,q)
    h=true
    a=n
    b=o
    c=p
    ...
    for r,s in pairs(i.peds)do 
        if not s.eup then 
            ...
            local ped=tFR.createDynamicPed(s.model,t,s.heading,true,"mini@strip_club@idles@bouncer@base","base",30,false,nil)
        end 
    end
    tFR.loadModel(i.models.camera)
...
RegisterNetEvent("FR:updateDvsaData",function(n,o,p,q)
    if n~=nil then 
        a=n 
    end
    if o~=nil then 
...
RegisterNetEvent("FR:dvsaMessage",function(w,x,y)
    tFR.notifyPicture(i.images.dict,i.images.govLarge,y,w,x)
end)
RegisterNetEvent("FR:beginTestClient",function(z,A)
    if z then 
        TriggerServerEvent("FR:dvsaBucket",false)
        currentTest={active=false,ped=0,vehicle=0,parkingSpace=0,route=0,waypoint=0,blip=0,serious=0,minors=0,minorsReason={},seriousReason={},subtitle=""}
        ...
        Wait(1000)
        currentTest.vehicle=tFR.spawnVehicle(i.test.modelName,i.test.parkingSpaces[A].coords.x,i.test.parkingSpaces[A].coords.y,i.test.parkingSpaces[A].coords.z,i.test.parkingSpaces[A].heading,true,true)
        ...
        local B=tFR.loadModel(i.test.examinerModel)
       ...
       dvsaSound("welcome")
        ...
        tFR.notifyPicture(i.images.dict,i.images.govLarge,"Your assigned examiner for the test is Marcus.","DVSA","UK Government")
        tFR.loadAnimDict("amb@medic@standing@tendtodead@base")
        ...
        Wait(4000)
        tFR.notifyPicture(i.images.dict,i.images.govLarge,"You'll now be given information about the test, listen carefully","DVSA","UK Government")
        initialMoveOff()
    else 
        tFR.notifyPicture(i.images.dict,i.images.govLarge,"We currently have no test availability, please try again shortly.","Callum","Driving Examiner")
...
function handleTestRoute()
    local C=false
    currentTest.blip=tFR.addBlip(i.test.routes[currentTest.route][currentTest.waypoint+1].coords.x,i.test.routes[currentTest.route][currentTest.waypoint+1].coords.y,i.test.routes[currentTest.route][currentTest.waypoint+1].coords.z,0,38,"Waypoint",1.0,false)
    ...
    currentTest.marker=tFR.addMarker(i.test.routes[currentTest.route][currentTest.waypoint+1].coords.x,i.test.routes[currentTest.route][currentTest.waypoint+1].coords.y,i.test.routes[currentTest.route][currentTest.waypoint+1].coords.z,1.6,1.6,1.6,0,89,255,200,50,36,true,true)
    ...
    while currentTest.active do 
        if not C then 
            ...
            local E=#(tFR.getPlayerCoords()-i.test.routes[currentTest.route][currentTest.waypoint+1].coords)
            ...
            if E<3.5 then 
                ...
                if currentTest.waypoint>=#(i.test.routes[currentTest.route])then 
                    ...
                    if currentTest.blip~=0 then 
                        tFR.removeBlip(currentTest.blip)
                    end
                    if currentTest.marker~=0 then 
                        tFR.removeMarker(currentTest.marker)
                    end
                    returnToTestCentre()
                else 
...
function policeChase()
   dvsaSound("policePursuitContinue")
    tFR.notifyPicture(i.images.dict,i.images.govLarge,"You are required to ~b~move over ~w~to allow a police pursuit to continue.","DVSA","UK Government")
    currentTest.subtitle="Move ~y~over"
    local G='blista'
    tFR.loadModel(G)
    local H=vector3(113.0901,-1226.426,37.60364)
    local I=tFR.spawnVehicle(G,H,270.93,false,true)
    ...
    tFR.loadModel(M)
    local N=tFR.spawnVehicle(M,H,270.93,false,true)
...
function returnToTestCentre()
    ...
    currentTest.blip=tFR.addBlip(i.test.parkingSpaces[currentTest.parkingSpace].coords.x,i.test.parkingSpaces[currentTest.parkingSpace].coords.y,i.test.parkingSpaces[currentTest.parkingSpace].coords.z,0,81,"Waypoint",1.0,false)
    currentTest.marker=tFR.addMarker(i.test.parkingSpaces[currentTest.parkingSpace].coords.x,i.test.parkingSpaces[currentTest.parkingSpace].coords.y,i.test.parkingSpaces[currentTest.parkingSpace].coords.z,1.2,1.2,1.2,0,255,125,125,50,0,true,true)
    SetBlipRoute(currentTest.blip,true)
    SetBlipRouteColour(currentTest.blip,38)
   dvsaSound("newDestinationSet")
    ...
    local T=false
    while true do 
        local E=#(tFR.getPlayerCoords()-i.test.parkingSpaces[currentTest.parkingSpace].coords)
        if E<15.0 then 
   ...
   dvsaSound("completePaperwork")
    ...
    if S then 
       dvsaSound("testPassed")
        tFR.notifyPicture(i.images.dict,i.images.govLarge,"You ~b~passed your test with ~y~"..currentTest.minors.." ~w~minors","UK Government","DVSA")
       ...
       dvsaSound("testFailed")
        tFR.notifyPicture(i.images.dict,i.images.govLarge,"Unfortunately you have ~r~failed your test","UK Government","DVSA")
        currentTest.subtitle="Unfortunately you have ~y~failed ~w~your driving test"
        Wait(6000)
        tFR.notifyPicture(i.images.dict,i.images.govLarge,"Your ~r~serious ~w~faults were:","UK Government","DVSA")
        Wait(3000)
       dvsaSound("seriousFaults")
        for r,s in pairs(currentTest.seriousReason)do 
            V=V..", "..currentTest.seriousReason[r]
            tFR.notify("~r~Serious Fault~w~: "..currentTest.seriousReason[r])
            Wait(500)
        end
        tFR.notifyPicture(i.images.dict,i.images.govLarge,"Your ~y~minor ~w~faults were:","UK Government","DVSA")
        Wait(3000)
       dvsaSound("minorFaults")
        for r,s in pairs(currentTest.minorsReason)do 
            U=U..", "..currentTest.minorsReason[r]
            tFR.notify("~y~Minor Fault~w~: "..currentTest.minorsReason[r])
            Wait(500)
        end
        Wait(4000)
    ...
    if tFR.teleport~=nil then 
        tFR.teleport(i.test.finishTestTpCoords.x,i.test.finishTestTpCoords.y,i.test.finishTestTpCoords.z)
    end
    Wait(3000)
    SetTimeout(20000,function()
        currentTest.requested=false 
    end)
    TriggerServerEvent("FR:dvsaBucket",true)
    if S then 
        TriggerServerEvent("FR:candidatePassed",currentTest.serious,currentTest.minors,U)
        currentTest.active=false 
    else 
        TriggerServerEvent("FR:candidateFailed",currentTest.serious,currentTest.minors,V,U)
        currentTest.active=false 
    end
    if currentTest.blip~=0 then 
        tFR.removeBlip(currentTest.blip)
...
RegisterNetEvent("FR:phoneToggledDvsa",function()
...
function pullUpOnRight()
    ...
    tFR.notifyPicture(i.images.dict,i.images.govLarge,"Listen carefully for directions","Controlled Stop","DVSA")
    local T=false
    local Y=false
...
function operateHeadlights()
    PlaySoundFrontend(-1,"Out_Of_Bounds_Timer","DLC_HEISTS_GENERAL_FRONTEND_SOUNDS",1)
    tFR.notifyPicture(i.images.dict,i.images.govLarge,"Listen carefully for directions","Show Me Question","DVSA")
   ...
   dvsaSound("operateMainBeamHeadlights")
    ...
    tFR.notifyPicture(i.images.dict,i.images.govLarge,"Turn on your headlights as requested","Show Me Question","DVSA")
    SetTimeout(20000,function()
        T=true 
    end)
    local _=false
   ...
   dvsaSound("continueToFollow")
    ...
    Wait(2000)
    tFR.notifyPicture(i.images.dict,i.images.govLarge,"Thank you, the show me question is complete","Show Me Question","DVSA")
    currentTest.subtitle="Follow the ~y~sat nav"
end
function stopSignDetection()
    PlaySoundFrontend(-1,"Out_Of_Bounds_Timer","DLC_HEISTS_GENERAL_FRONTEND_SOUNDS",1)
   dvsaSound("stopSign")
    tFR.notifyPicture(i.images.dict,i.images.govLarge,"You are legally required to stop at this sign","Stop Sign","DVSA")
    local a2=false
    local a3=false
    Citizen.CreateThread(function()
...
function emergencyStop()
   ...
   dvsaSound("emergencyStopIntroduction")
    tFR.notifyPicture(i.images.dict,i.images.govLarge,"Listen carefully for directions","Test Instruction","DVSA")
   ...
   dvsaSound("stopNowMessage")
    ...
    tFR.notifyPicture(i.images.dict,i.images.govLarge,"Perform an emergency stop immediately","STOP","DVSA")
    Wait(3000)
    if not GetEntitySpeed(currentTest.vehicle)==0.0 then 
...
function useTablet()
    local B=tFR.loadModel('prop_cs_tablet')
...
function cameraTransition(t)
    ...
    local ped=tFR.getPlayerPed()
    FreezeEntityPosition(ped,true)
    local a5=tFR.getPlayerCoords()
                ...
                if not currentTest.active and not a.full and a.active then 
                    if not currentTest.requested then 
                        RageUI.Button("Begin driving test","Begin your driving test",{RightBadge=RageUI.BadgeStyle.Alert,RightLabel="£"..getMoneyStringFormatted(i.test.price)},true,function(a8,a9,aa)
                            if aa then 
                                if a.banned then 
                                    tFR.notifyPicture(i.images.dict,i.images.govLarge,"Your licence has been suspended","UK Government","DVSA")
                                else 
                                    if not currentTest.requested then 
                                        TriggerServerEvent("FR:beginTest")
                        ...
                        RageUI.Button("No tests available","Try again in a few minutes",{RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                            if aa then 
                                tFR.notifyPicture(i.images.dict,i.images.govLarge,"We have no tests available, please try again in a few minutes.","UK Government","Driving Test")
                            end 
                        end)
                    end 
                else 
                    if not currentTest.active and (a.full or a.active) then 
                        RageUI.Button("Surrender your licence","Surrender your driving licence to the DVSA",{RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                            if aa then 
                                a.full=false
                                a.active=false
                                tFR.notifyPicture(i.images.dict,i.images.govLarge,"You've surrendered your licence. Your points and offences will remain held.","UK Government","Licence Surrendered")
                                TriggerServerEvent("FR:surrenderLicence")
                            end 
                        end)
                    end 
                end 
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            if a.full then 
                RageUI.Button("Licence Type:","This indiates if you hold a full licence",{RightLabel="Full",RightBadge=RageUI.BadgeStyle.Car},true,function(a8,a9,aa)
                    if aa then 
                        tFR.notifyPicture(i.images.dict,i.images.govLarge,"Your licence is full. You have passed your driving test.","UK Government","DVSA")
                    end 
                end)
            else 
                if a.banned then 
                    ...
                    if a.active then 
                        RageUI.Button("Licence Type:","This indiates if you hold a full licence",{RightLabel="Provisional",RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                            if aa then 
                                tFR.notifyPicture(i.images.dict,i.images.govLarge,"Your licence is currently provisional, take a test at the DVSA test centre","UK Government","DVSA")
                            end 
                        end)
                    else 
                        RageUI.Button("Licence Type:","This indiates if you hold a full licence",{RightLabel="No licence",RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                            if aa then 
                                tFR.notifyPicture(i.images.dict,i.images.govLarge,"You do not hold a UK Driving licence.","UK Government","DVSA")
                            end 
                        end)
                        if not f then 
                            RageUI.Button("Apply for a provisional licence","Apply for a provisional licence",{RightBadge=RageUI.BadgeStyle.Car},true,function(a8,a9,aa)
                                if aa then 
                                    TriggerServerEvent("FR:activateLicence")
                                    f=true
                                    tFR.notifyPicture(i.images.dict,i.images.govLarge,"We will process your application shortly. Thank you for submitting.","UK Government","DVSA")
                                end 
                            end)
                        end 
                    end 
            ...
            if a.active then 
                RageUI.Button("Penalty Points:","This indicates your amount of licence points",{RightLabel=a.points},true,function(a8,a9,aa)
                    if aa then 
                        tFR.notifyPicture(i.images.dict,i.images.govLarge,"Your licence currently has "..a.points.." penalty points.","UK Government","DVSA")
                    end 
                end)
                RageUI.Button("Licence Number:","This indicates your licence number",{RightLabel=a.id},true,function(a8,a9,aa)
                    if aa then 
                        tFR.notifyPicture(i.images.dict,i.images.govLarge,"Your licence number is "..a.id..", this is issued with your licence.","UK Government","DVSA")
                    end 
                end)
                RageUI.Button("Licence Issued:","This indicates the date and time of issue",{RightLabel=a.date},true,function(a8,a9,aa)
                    if aa then 
                        tFR.notifyPicture(i.images.dict,i.images.govLarge,"Your licence was issued at "..a.date..".","UK Government","DVSA")
                    end 
                end)
                if next(b) then 
                    ...
                    for r,s in pairs(b)do 
                        RageUI.Button(s.offence,"Date: "..s.date.." | Type: "..s.type,{RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                            if aa then 
                                tFR.notifyPicture(i.images.dict,i.images.govLarge,"You were given "..s.points.." penalty points on "..s.date..".","UK Government","Offence: "..s.offence)
                            end 
                        end)
                    end 
                end 
            ...
            if c~=nil and next(c) then 
                for r,s in pairs(c)do 
                    ...
                    RageUI.Button(s.date.." | Result: "..ac,"~r~Serious Faults~w~: "..s.serious.." | ~b~Minor Faults~w~: "..s.minor,{RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                        if aa then 
                            tFR.notifyPicture(i.images.dict,i.images.govLarge,"Your test result was a "..ac.." ~w~with ~b~"..s.serious.." ~w~serious faults and ~b~"..s.minor.." ~w~minor faults.","UK Government","Driving Test Result")
                            if s.minor~=nil and s.pass and s.minor>0 then 
                                tFR.notifyPicture(i.images.dict,i.images.govLarge,"You received ~b~"..s.minor.." ~w~minors","UK Government","Driving Test Result")
                            elseif not s.pass then 
                                if s.seriousReason~=nil then 
                                    tFR.notifyPicture(i.images.dict,i.images.govLarge,"Your serious faults were: ~b~"..s.seriousReason,"~w~UK Government","Driving Test Result")
                                end
                                if s.minor>0 then 
                                    if s.minorsReason~=nil then 
                                        tFR.notifyPicture(i.images.dict,i.images.govLarge,"You received minors for ~b~"..s.minorsReason,"UK Government","Driving Test Result")
                                    end 
                                end 
                            end 
                        end 
            ...
            if next(d) then
                for r,s in pairs(d)do 
                    ...
                    RageUI.Button(s.title.." "..s.date,"Press to read this message",{RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                        if aa then 
                            tFR.notifyPicture(i.images.dict,i.images.govLarge,s.message,"UK Government",s.title)
                        end 
                    end)
                end
            else
...
Citizen.CreateThread(function()
    while true do 
        ...
        if not a.full and not a.banned and not currentTest.active then 
            local vehicle=tFR.getPlayerVehicle()
            if vehicle~=0 and GetPedInVehicleSeat(vehicle,-1)==tFR.getPlayerPed()then 
...
RegisterNetEvent("FR:speedCameraFlash",function(t,aj)
    for ak=1,2 do 
        local T=false
        Citizen.SetTimeout(450,function()
            T=true 
...
function flashCamera(al)
    ...
    TriggerServerEvent("FR:speedCameraFlashServer",math.ceil(GetEntitySpeed(GetVehiclePedIsIn(tFR.getPlayerPed(), false))*2.2369))
...
RegisterNetEvent("FR:deletePlate",function(ao)
    local ap=tFR.getObjectId(ao)
    if DoesEntityExist(ap)then 
        DeleteEntity(ap)
    end 
end)
function dvsaSound(au)
    SendNUIMessage({transactionType=au}) 
end
function createPlate(vehicle)
    e.active=true
    e.vehicle=vehicle
    local B=tFR.loadModel('prop_lplate')
    local t=tFR.getPlayerCoords()
    ...
    tFR.notifyPicture(i.images.dict,i.images.lPlate,i.notifications.lPlatesAdded,"DVSA","Licence Services")
    tFR.notifyPicture(i.images.dict,i.images.lPlate,i.notifications.lPlatesAdded2,"DVSA","Licence Services")
end
function makeVehicleCrash(as,J,H,L,at)
    tFR.loadModel(as)
    local I=tFR.spawnVehicle(as,H,at,true,true)
    while not DoesEntityExist(I)do 
        Wait(0)
    end
