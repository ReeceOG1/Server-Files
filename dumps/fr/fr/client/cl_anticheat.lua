...
Citizen.CreateThread(
    function()
        while true do
            ...
            a(
                function(d)
                    if DecorGetInt(d, "FRACVeh") ~= 955 then
...
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            local h = tFR.getPlayerId()
            local i = tFR.getPlayerVehicle()
            if i == 0 then
                ...
                if not f and GetUsingseethrough() and not tFR.isPlayerInPoliceHeli() and not tFR.isPlayerInDrone() then
                    TriggerServerEvent("FR:acType13")
                    f = true
                end
            end
            SetPedInfiniteAmmoClip(g, false)
            for c, k in pairs(tFR.getWeapons()) do
...
Citizen.CreateThread(
    function()
        while true do
            ...
            if r > 1.0 then
                TriggerServerEvent("FR:acType8", "PlayerWeaponDamageModifier", r)
                return
            end
            if s > 1.0 then
                TriggerServerEvent("FR:acType8", "PlayerWeaponDefenseModifier", s)
                return
            end
            if t > 1.0 then
                TriggerServerEvent("FR:acType8", "PlayerWeaponDefenseModifier_2", t)
                return
            end
            if u > 1.0 then
                TriggerServerEvent("FR:acType8", "PlayerVehicleDamageModifier", u)
                return
            end
            if v > 1.0 then
                TriggerServerEvent("FR:acType8", "PlayerVehicleDefenseModifier", v)
                return
            end
            if y > 1.0 then
                TriggerServerEvent("FR:acType8", "GetWeaponDamageModifier", y)
                return
            end
            if z > 1.0 then
                TriggerServerEvent("FR:acType8", "GetPlayerMeleeWeaponDamageModifier", z)
                return
            end
...
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            N = O
            if
                P > 0.4 and not IsPedFalling(g) and tFR.getStaffLevel() < 2 and not IsPedInParachuteFreeFall(g) and
            ...
            local d, Q = tFR.getPlayerVehicle()
            if DoesEntityExist(d) and Q and P > 0.2 and tFR.getStaffLevel() < 2 then
                if F ~= d then
                    E = 0
                    F = d
                end
                ...
                if J < 5.0 and R < 2.5 then
                    ...
                    if E > 100 and GetGameTimer() - G > 4000 then
                        TriggerServerEvent("FR:acType1", true)
                        E = 0
                        G = GetGameTimer()
                    end
                end
            end
            if NetworkIsInSpectatorMode() and not tFR.inSpectate() then
                TriggerServerEvent("FR:acType21")
                return
            end
        ...
        while true do
            ...
            for S, T in ipairs(WeaponBL) do
                ...
                if HasPedGotWeapon(PlayerPedId(), GetHashKey(T), false) == 1 then
                    ...
                    TriggerServerEvent("FR:acType2", T)
                    return
                end
            end
            local S, r = StatGetInt(GetHashKey("mp0_shooting_ability"), true)
            if r > 100 then
                TriggerServerEvent("FR:acType18")
                return
            end
        end
    end
...
Citizen.CreateThread(
    function()
        ...
        local S = 0
        while true do
            if S >= 100 and not tFR.isInComa() then
            ...
            if
             ...
             then
                ...
                if g ~= 0 then
                    ...
                    tFR.setHealth(GetEntityHealth(g) + 2)
                end
            else
                Citizen.Wait(1000)
            end
...
local function at()
    ...
    local ax = #(aw - Y)
    if
        ax < 50.0 or av or carryingBackInProgress or piggyBackInProgress or tFR.isPlayerHidingInBoot() or
    ...
    if aq(aB, 100.0) then
        if au - a2 > 5000 then
            TriggerServerEvent("FR:sendVelocityLimit", aw, Y)
            a2 = au
        end
    end
end
...
local function a_()
    ...
    if aW(b0, aC) or aW(b1, aD) or aW(b2, aE) or aW(b3, aF) then
        ...
        if
         ...
         then
            local aQ = aP(d)
            local b4 = tFR.getVehicleIdFromHash(GetEntityModel(d)) or "N/A"
            TriggerServerEvent("FR:sendVehicleStats", b0, aC, b1, aD, b2, aE, b3, aF, aQ, b4)
            aI = au
        end
    end
    aC = b0
...
local b7 = {GetHashKey("demonhawkk"), GetHashKey("rculi"), GetHashKey("pgt322"), GetHashKey("rs3keyvany")}
Citizen.CreateThread(
    function()
        while true do
            local d = tFR.getPlayerVehicle()
            ...
            if GetVehicleHasParachute(d) or GetCanVehicleJump(d) or GetHasRocketBoost(d) and b6 ~= GetHashKey("voltic2") then
                if not table.has(b7, b6) then
                    TriggerServerEvent("FR:acType12", globalVehicleModelHashMapping[b6])
                    return
                end
            end
            Wait(1000)
        ...
        while true do
            ...
            if
                not IsEntityVisible(PlayerPedId()) and not tFR.isInsideLsCustoms() and not noclipActive and
                    ...
                    tFR.getStaffLevel() < 3
                then
                    TriggerServerEvent("FR:acType25")
                    Citizen.Wait(60000)
                end
            Citizen.Wait(1000)
        end
...
RegisterNUICallback(
    "NuiDevTool",
    function()
        TriggerServerEvent("FR:acType22")
    end
)
AddEventHandler(
...
AddEventHandler(
    "onResourceStop",
    function(b8)
        TriggerServerEvent("FR:acType23", b8)
    end
)
print("[FR] - Anti-Cheat initialised")
