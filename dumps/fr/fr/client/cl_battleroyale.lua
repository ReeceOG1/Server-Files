                ...
                function(i, j, k)
                    if k then
                        ...
                        print(
                            ...
                            5,
                            50,
                            {locationIndex = c.location, name = d[c.location]}
                        )
                        TriggerServerEvent(
                            "FR:startEvent",
                            "Battle Royal",
...
local function m()
    if a then
        ...
        DrawAdvancedText(0.591, 0.042, 0.005, 0.0028, 0.993, "FR BATTLEGROUNDS", 255, 255, 255, 255, 6, 0)
    end
end
tFR.createThreadOnTick(m)
local function n()
        ...
        if CursorInArea(GetArea(0.493, 0.944, 0.14, 0.074)) then
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                if g.player.canExitPlane then
                    tFR.setPlayerCanOpenLeaderboard(true)
...
tFR.createThreadOnTick(func_handleClicks)
RegisterNetEvent(
    "FR:syncLootboxesTable",
    function(t, u)
        g.data.lootBoxes = table.copy(b.lootBoxes[u])
        g.data.armourPlates = table.copy(b.armourPlates[u])
        tFR.loadModel("prop_box_ammo03a")
        for v, w in pairs(g.data.lootBoxes) do
            if #(w.coords - g.gas.coords) < g.gas.radius / 2 then
                g.data.lootBoxes[v].areaId = "FRbr_lootbox_" .. v
                tFR.setNamedBlip("FRbr_lootbox_" .. v, w.coords.x, w.coords.y, w.coords.z, 478, 1, "Lootbox", 0.5)
                local x = function(y)
                    if g.data.lootBoxes[y.box] then
                        if g.data.lootBoxes[y.box].entity == nil then
                            g.data.lootBoxes[y.box].entity =
                                CreateObject("prop_box_ammo03a", w.coords, false, false, false, false, false)
                        ...
                        print(string.format("[FR] lootbox with ID: %s is nil", y.box))
                    end
                end
                local z = function(y)
                    if g.data.lootBoxes[y.box] then
                ...
                tFR.createArea("FRbr_lootbox_" .. v, w.coords, 200.0, 6, x, z, A, {box = v})
            end
        end
        SetModelAsNoLongerNeeded("prop_box_ammo03a")
        tFR.loadModel("prop_armour_pickup")
        for v, w in pairs(g.data.armourPlates) do
            if #(w.coords - g.gas.coords) < g.gas.radius / 2 then
                tFR.setNamedBlip("FRbr_armour_" .. v, w.coords.x, w.coords.y, w.coords.z, 175, 1, "Lootbox", 0.5)
                local B = function(C)
                    if g.data.armourPlates[C.plateId] then
                        ...
                        print(
                            string.format(
                                "[FR Events] body armour with ID of %s is nil in table (onEnter)",
                                C.plateId
                            )
                        )
                    end
                ...
                local D = function(C)
                    if g.data.armourPlates[C.plateId] then
                        ...
                        print(
                            string.format(
                                "[FR Events] body armour with ID of %s is nil in table (onLeave)",
                                C.plateId
                            )
                        )
                    end
                ...
                local E = function(C)
                    if g.data.armourPlates[C.plateId] then
                        if C.distance <= 1.5 then
                            ...
                            tFR.DrawText3D(vector3(F.x, F.y, F.z - 0.5), "Press [E] to pickup armour.", 0.2)
                            if IsControlJustPressed(0, 51) then
                                TriggerServerEvent("FR:removeArmourPlate", C.plateId)
                                SetPedComponentVariation(PlayerPedId(), 9, 15, 3, 0)
                            end
                        end
                    end
                end
                tFR.createArea("FRbr_armour_" .. v, w.coords, 200.0, 6, B, D, E, {plateId = v})
                g.data.armourPlates[v].areaId = "FRbr_armour_" .. v
            end
        end
...
RegisterNetEvent(
    "FR:removeLootBox",
    function(G)
        tFR.removeArea("FRbr_lootbox_" .. G)
        tFR.removeNamedBlip("FRbr_lootbox_" .. G)
...
RegisterNetEvent(
    "FR:removeArmourPlateCl",
    function(H)
        tFR.removeArea("FRbr_armour_" .. H)
        tFR.removeNamedBlip("FRbr_armour_" .. H)
...
RegisterNetEvent(
    "FR:startBattlegrounds",
    function(I)
        e = I
        tFR.startGas(b.locations[e].gas.initalRadius, b.locations[e].gas.centre)
        DoScreenFadeOut(1500)
        Wait(1500)
        currentEvent.eventName = "Battle Royale"
        tFR.hideUI()
        tFR.stopEventSequence(true)
        tFR.giveWeapons({["GADGET_PARACHUTE"] = {2}})
        ...
        local J =
            tFR.spawnVehicle("titan", b.locations[e].planeStart, b.locations[e].planeHeading, false, false, true)
...
tFR.registerEventCleanupHandler(
    "FR Battlegrounds",
    function()
        currentEvent.isActive = false
        tFR.setPlayerCanOpenLeaderboard(false)
        for v, w in pairs(g.data.lootBoxes) do
            tFR.removeArea(w.areaId)
            tFR.removeNamedBlip(w.areaId)
...
function tFR.changeGasRadius(M)
    SendNUIMessage({transactionType = "br-gas"})
    g.gas.timeUntilNext = {minutes = 2, seconds = 0}
    tFR.announceMpBigMsg("~r~GAS MOVING", "The gas is closing in!", 5000)
    Wait(4000)
    Citizen.CreateThread(
        function()
            ...
            if g.gas.isActive then
                if g.gas.timeUntilNext.seconds > 0 then
                    ...
                    if g.gas.timeUntilNext.seconds == 0 and g.gas.timeUntilNext.minutes == 0 then
                        tFR.changeGasRadius(g.gas.radius - 400)
                    else
...
RegisterNetEvent(
    ...
    function(p, U)
        ...
        tFR.updateScoreboard(p, 5, g.players[p].kills)
        Q()
    end
)
RegisterNetEvent(
    "FR:removePlayerFromBR",
    function(V)
        if g.players[V] then
        ...
        tFR.removePlayerFromLeaderboard(V)
    end
)
Citizen.CreateThread(
    function()
        while true do
            if
                g.gas.isActive and g.player.hasJumped and not IsPedInParachuteFreeFall(PlayerPedId()) and
                    GetPedParachuteState(PlayerPedId()) <= 2 and
                    currentEvent.isActive and
                    not tFR.isSpectatingEvent()
             then
                if #(tFR.getPlayerCoords().xy - g.gas.coords.xy) > g.gas.radius / 2.0 then
