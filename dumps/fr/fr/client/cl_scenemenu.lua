RMenu.Add("frscenemenu","main",RageUI.CreateMenu("","~b~FR Traffic Scene Menu",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_jobselectorui","metpd"))
RMenu.Add("frscenemenu","objects",RageUI.CreateSubMenu(RMenu:Get("frscenemenu", "main"),"","~b~Spawn Objects",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_jobselectorui","metpd"))
RMenu.Add("frscenemenu","speedZone",RageUI.CreateSubMenu(RMenu:Get("frscenemenu", "main"),"","~b~Set a speed zone",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_jobselectorui","metpd"))
RMenu.Add("frscenemenu","spikes",RageUI.CreateSubMenu(RMenu:Get("frscenemenu", "main"),"","~b~Set a spike trap",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_jobselectorui","metpd"))
local b = {}
local c = {}
    ...
    if RageUI.Visible(RMenu:Get('frscenemenu', 'speedZone')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("Create Speedzone",nil,{},true,function(K, L, M)
                if M then
                    if p == 0 then
                        tFR.notify("Please set a radius")
                    elseif q == 0 then
                        tFR.notify("Please set a speed")
                    else
                        TriggerServerEvent("FR:createSpeedZone", tFR.getPlayerCoords(), q + 0.0, p + 0.0)
                        tFR.notify("~b~Speed zone created")
                    end
                end
            end,nil)
            RageUI.ButtonWithStyle("Delete Speedzone",nil,{},true,function(K, L, M)
                if M then
                    local Q = tFR.getPlayerCoords()
                    local R = nil
                    local S = 250.0
                    ...
                    if R then
                        TriggerServerEvent("FR:deleteSpeedZone", R)
                        tFR.notify("Speed zone removed")
                    end
                end
            end,nil)
        end)
...
RegisterNetEvent("FR:toggleTrafficMenu",function()
    RageUI.Visible(RMenu:Get("frscenemenu", "main"), not RageUI.Visible(RMenu:Get("frscenemenu", "main")))
    if globalOnPoliceDuty and tFR.hasPoliceCallsign() then
        RMenu:Get("frscenemenu", "main"):SetSubtitle("~b~MPD~w~: " .. tFR.getPoliceCallsign() .. " - " .. tFR.getPoliceRank() .. " - " .. tFR.getPoliceName())
    end
end)
...
function deleteObject()
    for n, o in pairs(k) do
        ...
        local Y, Z, _ = table.unpack(GetEntityCoords(tFR.getPlayerPed(), true))
        if DoesObjectOfTypeExistAtCoords(Y, Z, _, 0.9, X, true) then
            ...
            if NetworkGetEntityIsNetworked(a0) and not NetworkHasControlOfEntity(a0) then
                local a1 = tFR.getNetId(a0, "deleteObject()")
                if a1 ~= 0 then
                    TriggerServerEvent("FR:requestSceneObjectDelete", a1)
                end
            else
                DeleteObject(a0)
            end
        ...
        for n, a2 in pairs(b) do
            if DoesEntityExist(a2) then
                if NetworkGetEntityIsNetworked(a2) then
                    ...
                    if a3 ~= 0 then
                        TriggerServerEvent("FR:requestSceneObjectDelete", a3)
                    end
                else
                    DeleteEntity(a2)
                end
...
function deleteSpike()
    local Q = GetEntityCoords(tFR.getPlayerPed())
    local a8 = getClosetSpike(Q)
    if a8 then
        TriggerServerEvent("FR:removeSpike", a8[2])
    end
end
RegisterNetEvent("FR:addSpike",function(Q, a7)
    c[a7] = {Q, a7}
end)
RegisterNetEvent("FR:deleteSpike",function(a7)
    c[a7] = nil
end)
local a9 = 1
Citizen.CreateThread(function()
...
Citizen.CreateThread(function()
    while true do
        local a4, ah = tFR.getPlayerVehicle()
        if a4 ~= 0 and ah then
            local ai = false
            local aj = 1000.0
...
RegisterNetEvent("FR:createSpeedZone",function(T, ak, q, p)
...
RegisterNetEvent("FR:deleteSpeedZone",function(T)
    local U = j[T]
    if U then
        RemoveBlip(U.blip)
