...
local function D()
    ...
    local F = 500 * math.ceil(GetDistanceBetweenCoords(tFR.getPlayerCoords(), GetEntityCoords(b), true) / 10)
    if j then
...
RMenu.Add("g4s","shiftHandler",RageUI.CreateMenu("","Security Company",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_g4sjob", "fr_g4sjob"))
RMenu:Get("g4s", "shiftHandler"):SetSubtitle("Security Company")
RMenu:Get("g4s", "shiftHandler").Closable = true
RMenu.Add("g4s","main",RageUI.CreateMenu("","Security Company",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_g4sjob", "fr_g4sjob"))
...
tFR.addMarker(d.x, d.y, d.z, 1.2, 1.2, 1.2, 0, 255, 125, 125, 50, 0, true, true)
local H = tFR.addBlip(d.x, d.y, d.z, 457, 18, "G4S Depot", 1.0, false)
local I = function()
...
tFR.createArea("g4sDepot_", d, 1.5, 6, I, J, K)
Citizen.CreateThread(function()
    while true do
        local E = tFR.getPlayerPed()
        local L = tFR.getPlayerCoords()
        if a then
            if not u then
                displayJob()
            end
...
Citizen.CreateThread(function()
    while true do
        if not A.hackingEngaged and not a and not A.awaitingResponse then
            if table.count(s) > 0 and tFR.getPlayerVehicle() == 0 then
                for M, N in pairs(s) do
                    ...
                    if table.has(p, Q) or not DoesEntityExist(P) then
                        ...
                        if IsVehicleStopped(P) then
                            local R = #(tFR.getPlayerCoords() - GetEntityCoords(P))
                            ...
                            if R <= 11.0 then
                                ...
                                if IsControlJustPressed(1, 18) then
                                    ...
                                    A.entity = P
                                    A.src = Q
                                    TriggerServerEvent("FR:requestHackingChanceServer", O, Q)
...
local W = "HONGKONG"
local X = 8
RegisterNetEvent("FR:hackingChanceReturn",function(b, Y, Z, _)
    if _ then
        ...
        local o = tFR.loadModel("prop_cs_tablet")
        x = CreateObject(o, 0, 0, 0, true, true, true)
        AttachEntityToEntity(x,tFR.getPlayerPed(),GetPedBoneIndex(tFR.getPlayerPed(), 57005),0.17,0.10,-0.13,24.0,180.0,180.0,true,true,false,true,1,true)
        SetModelAsNoLongerNeeded(o)
    else
        A.awaitingResponse = false
    end
        ...
        if HasScaleformMovieLoaded(T) then
            ...
            if IsScaleformMovieMethodReturnValueReady(U) then
                ...
                elseif ProgramID == 87 then
                    ...
                    TriggerServerEvent("FR:updateHackingOutcome", A.netId, A.src, W, false)
                    DeleteObject(x)
                    break
                ...
                elseif ProgramID == 86 then
                    ...
                    TriggerServerEvent("FR:updateHackingOutcome", A.netId, A.src, W, true)
                    DeleteObject(x)
                    break
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('g4s', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if not j then
                if tFR.getPlayerVehicle() == 0 and y < 4 then
                ...
                RageUI.ButtonWithStyle("End Shift","End your shift",{RightLabel = "→→→"},true,function(a0, a1, a2)
                    if a2 then
                        TriggerServerEvent("FR:toggleShiftG4S", false)
                        a = false
                    end
                end)
                ...
                if not h then
                    RageUI.ButtonWithStyle("Activate Panic","Activate your panic alarm",{RightLabel = "→→→"},true,function(a0, a1, a2)
                        if a2 then
                            TriggerServerEvent("FR:activatePanicServer", c)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if not a then
                RageUI.ButtonWithStyle("Start Shift","Start your shift working for G4S.",{RightLabel = "G4S"},true,function(a0, a1, a2)
                    if a2 then
                        RageUI.Visible(RMenu:Get("g4s", "shiftHandler"), false)
                        TriggerServerEvent("FR:toggleShiftG4S", true)
                    end
                end)
            else
...
function newG4sVehicle()
    ...
    if DoesEntityExist(b) and not IsVehicleDriveable(b, true) then
        ...
        b = tFR.spawnVehicle(n, a3.x, a3.y, a3.z, GetEntityHeading(E), true, true)
        c = VehToNet(b)
        TriggerServerEvent("FR:updateCurrentVehicle", c)
        y = y + 1
    end
end
function securityCheck(E)
    local a4 = tFR.getPlayerVehicle()
    if a4 ~= 0 and a4 ~= nil and b ~= nil and b ~= 0 then
        if a4 ~= b then
            ...
            SetTimeout(15000,function()
                a4 = GetVehiclePedIsIn(tFR.getPlayerPed(), false)
                if a4 ~= 0 then
                    if a4 ~= b then
                        TriggerServerEvent("FR:toggleShiftG4S", false)
                        tFR.removeBlip(k)
...
RegisterNetEvent("FR:resetPanicStatus",function()
    h = false
end)
RegisterNetEvent("FR:updateJobInformation",function(a5)
    if l ~= 0 then
        tFR.removeBlip(l)
    end
    if a5.jobActive then
        if not a5.collected then
            ...
            k = tFR.addBlip(a5.collectionCoords.x,a5.collectionCoords.y,a5.collectionCoords.z,500,18,"G4S Collection",1.0,false)
            SetBlipRoute(k, true)
            SetBlipRouteColour(k, 18)
        elseif not a5.deposited then
            tFR.removeBlip(k)
            l = tFR.addBlip(a5.depositCoords.x,a5.depositCoords.y,a5.depositCoords.z,500,47,"G4S Deposit",1.0,false)
            SetBlipRoute(l, true)
        ...
        displayNotification("New job received:~w~\nCollection: " .. a5.collectionName .. "\nDestination: " .. a5.depositName)
        Wait(3000)
        TriggerServerEvent("FR:acceptJob")
    end
end)
...
RegisterNetEvent("FR:startShiftG4S",function(a6)
    local E = tFR.getPlayerPed()
    local L = tFR.getPlayerCoords()
    a = true
    tFR.disableCustomizationSave(a)
    b = tFR.spawnVehicle(n, B.x, B.y, B.z, B.heading, true, true)
    c = VehToNet(b)
    ...
    z = json.encode(tFR.getCustomization())
    if aa then
        tFR.loadCustomisationPreset("G4SMale")
    else
        tFR.loadCustomisationPreset("G4SFemale")
    end
    Wait(2000)
    TaskVehicleDriveToCoord(tFR.getPlayerPed(),b,-672.356,282.3297,81.43005,convert(250.0),1,GetHashKey(GetEntityModel(b)),60,5,true)
    Wait(5000)
    TaskVehicleTempAction(tFR.getPlayerPed(), b, 27, 10.0)
    Wait(2000)
    imageNotification("CHAR_MP_MEX_DOCKS", "CHAR_MP_MEX_DOCKS", "Use /g4s to access the G4S menu.", a9, "G4S")
end)
RegisterNetEvent("FR:endShiftG4S",function()
    ...
    local E = tFR.getPlayerPed()
    tFR.setCustomization(json.decode(z))
    if C.active then
        toggleCashbox()
    end
    if k ~= 0 then
...
RegisterNetEvent("FR:requestMoneyInVehicle",function()
    ...
    while not f do
        ...
        local R = #(tFR.getPlayerCoords() - ag)
        if R <= 1.0 then
            af = true
    ...
    TriggerServerEvent("FR:updateMoneyInVehicle")
    t = false
end)

RegisterNetEvent("FR:requestMoneyOutVehicle",function()
    local ae = tFR.getPlayerPed()
    while IsPedInAnyVehicle(ae, true) do
        Wait(400)
    end
    ...
    y = 1
    TriggerServerEvent("FR:updateMoneyOutVehicle")
end)
...
RegisterNetEvent("FR:openG4SMenu",function()
    RageUI.Visible(RMenu:Get("g4s", "main"), true)
end)
RegisterNetEvent("FR:imageNotification",function(al, am, an, ao, ap)
...
RegisterNetEvent("FR:bottomMessage",function(ak, av)
    bottomMessage(ak, av)
end)
...
RegisterNetEvent("FR:activatePanicAll",function(az, aA)
...
RegisterNetEvent("FR:activeVansUpdate",function(az, Q, aE)
    if not a then
        local b = tFR.getObjectId(az, "FR:activeVansUpdate 1")
        if b ~= 0 and b ~= nil and DoesEntityExist(b) then
            if aE then
                s[az] = nil
            else
                if not table.has(s, az) then
                    local b = tFR.getObjectId(az, "FR:activeVansUpdate")
                    s[az] = {Q, b, az}
                end
            end
        end
...
RegisterNetEvent("FR:terminateHacking",function()
