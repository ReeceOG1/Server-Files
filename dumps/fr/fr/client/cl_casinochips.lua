...
RMenu.Add('FRChips','cashier',RageUI.CreateMenu("","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('FRChips','cashier'):SetSubtitle("CHIPS")
RMenu.Add('casino_enter','casino',RageUI.CreateMenu("","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('casino_enter','casino'):SetSubtitle("ENTER")
RMenu.Add('casino_exit','casino',RageUI.CreateMenu("","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('casino_exit','casino'):SetSubtitle("EXIT")
RMenu.Add('casino_rooftop_exit','casino',RageUI.CreateMenu("","",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
...
Citizen.CreateThread(function()
    for i, j in pairs(h) do
        tFR.createDynamicPed(j.model,j.pedPosition,100.0,true,"mini@strip_club@idles@bouncer@base","base",75.0,nil,function(k)
            ...
            RageUI.ButtonWithStyle("Buy chips","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    ...
                    if tonumber(q)then 
                        q=tonumber(q)
                        if q>0 then 
                            TriggerServerEvent("FR:buyChips",q)
                        else 
            ...
            RageUI.ButtonWithStyle("Sell chips","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    ...
                    if tonumber(q)then 
                        q=tonumber(q)
                        if q>0 then 
                            TriggerServerEvent("FR:sellChips",q)
                        else 
                            tFR.notify("Negative amount.")
                        end 
                    else 
    ...
    if RageUI.Visible(RMenu:Get('casino_enter', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Enter Diamond Casino","",{RightLabel="→→→"},true,function(n,o,p)
                if p and tFR.getPlayerCombatTimer() == 0 then 
...
function showCasinoChipsCashier(u)
    ...
    RageUI.Visible(RMenu:Get('FRChips','cashier'),u)
end
...
Citizen.CreateThread(function()
    while true do 
        local v=GetEntityCoords(tFR.getPlayerPed())
        a=#(v-e)
        b=#(v-f)
        c=#(v-g)
        Wait(100)
...
AddEventHandler("FR:setDisplayChips",function(x)
    d=getMoneyStringFormatted(tostring(x))
end)

RegisterNetEvent("FR:chipsUpdated",function()
...
AddEventHandler("FR:onClientSpawn",function(D, E)
    if E then
        ...
        for H,k in pairs(h)do 
            ...
            tFR.addMarker(k.entryPosition.x,k.entryPosition.y,k.entryPosition.z,1.0,1.0,1.0,138,43,226,70,50,27)
            tFR.createArea("chipscashier_"..H,k.entryPosition,1.5,6,C,E,F,{})
        end
        for I,J in pairs(z)do 
            tFR.createArea("casinos_"..I,J.position,J.radius,6,function()
            end,function()
            end,G,{})
        end 
    end
