...
local function b()
    ...
    TriggerServerEvent("FR:startThrowSmokeGrenade", c)
    Citizen.Wait(100)
    ...
    TriggerServerEvent("FR:endThrowSmokeGrenade", c, g)
end
local function h(i)
...
RegisterNetEvent("FR:displaySmokeGrenade",function(c, g)
    local l = {coords = g, isDeleted = false}
    tFR.createArea("smoke_" .. tostring(c),g,250.0,1000.0,k,m,function()end,l)
    Citizen.Wait(60000)
    tFR.removeArea("smoke_" .. tostring(c))
    if l.handle then
        StopParticleFxLooped(l.handle, false)
        l.handle = nil
        l.isDeleted = true
