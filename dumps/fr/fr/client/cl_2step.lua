...
Citizen.CreateThread(
    function()
        while true do
            ...
            local g = GetVehiclePedIsIn(tFR.getPlayerPed(), false)
            if g ~= 0 and GetPedInVehicleSeat(g, -1) == f then
                local h, i = tFR.getVehicleInfos(g)
                if h == tFR.getUserId() then
                    if b[i] ~= nil and b[i] ~= 0 then
                        c = i
                        d = true
                        while GetVehiclePedIsIn(tFR.getPlayerPed(), false) ~= 0 and e == false do
                            Wait(100)
                        end
                        e = false
                        d = false
