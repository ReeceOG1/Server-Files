...
local function f()
    ...
    g.position=tFR.getPlayerCoords()
    g.radius=g.radius+0.0
    g.vehicles.speed=g.vehicles.speed+0.0
    TriggerServerEvent("FR:createRPZone",g)
end
...
local function h()
    ...
    if i then 
        TriggerServerEvent("FR:removeRPZone",i)
    end 
end
local inRPZone = false
RMenu.Add("rpzones","mainmenu",RageUI.CreateMenu("","Main Menu",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_adminui","fr_adminui"))
RMenu.Add("rpzones","weapons",RageUI.CreateSubMenu(RMenu:Get("rpzones","mainmenu"),"","Weapons",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_adminui","fr_adminui"))
RMenu.Add("rpzones","vehicles",RageUI.CreateSubMenu(RMenu:Get("rpzones","mainmenu"),"","Vehicles",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight(),"fr_adminui","fr_adminui"))
...
RegisterNetEvent("FR:createRPZone",function(g)
...
RegisterNetEvent("FR:removeRPZone",function(r)
...
local function t(g,u)
    ...
    if not g.weapons.melee and w==GetHashKey('GROUP_MELEE')then 
        tFR.setWeapon(v,'WEAPON_UNARMED')
...
local function G(g)
    local l,u=GetCurrentPedWeapon(tFR.getPlayerPed())
    if u~=0 then 
        t(g,u)
    end
    local y,H=tFR.getPlayerVehicle()
    if y~=0 and H then 
        A(g,y)
    end
...
tFR.createThreadOnTick(I)

function tFR.GetRPZoneInfo()
    local K=tFR.getPlayerCoords()
    for l,g in ipairs(d)do 
        if#(K-g.position)<g.radius then
            return g
