...
RegisterNetEvent("FR:gotCarDealerInstances",function(m)
    a = m
end)
RegisterNetEvent("FR:gotCarDealerCategories",function(n)
    b = n
end)
RegisterNetEvent("FR:carDealerPurchased",function()
    DeleteVehicle(f)
    f = 0
    g = 0
    h = false
    local o = a[d].posVector
    SetEntityCoords(tFR.getPlayerPed(), o.x, o.y, o.z)
end)
local function p(q)
    if q ~= nil then
        d = q
...
local function s(t, u, v, w)
    ...
    if HasModelLoaded(x) then
        local z = CreateVehicle(x, u, v, w, GetEntityHeading(tFR.getPlayerPed()), false, false)
        DecorSetInt(z, "FRACVeh", 955)
        ...
        TaskWarpPedIntoVehicle(tFR.getPlayerPed(), z, -1)
        ...
        tFR.notify("Could not load vehicle")
        return -1
    end
end
CreateThread(function()
    for A, B in pairs(l.simeonsInstances) do
        ...
        tFR.addBlip(C.x, C.y, C.z, D, E)
        tFR.addMarker(C.x, C.y, C.z, 0.7, 0.7, 0.5, G[1], G[2], G[3], 125, 50, F, true, true)
    end
end)
...
function isNearPreviewVector(J)
    if #(GetEntityCoords(tFR.getPlayerPed()) - J) < 5.0 then
        return true
    end
    return false
end
...
Citizen.CreateThread(function()
    while true do
        if a ~= nil then
            for A, B in pairs(a) do
                local C, D, E, F, G, H, I =B.posVector,B.blipId,B.blipColour,B.markerId,B.markerColourTable,B.permissionsTable,B.categorys
                c[A] = #(GetEntityCoords(tFR.getPlayerPed()) - C)
            end
        end
        Wait(250)
    end
end)
RMenu.Add("cardealer","mainmenu",RageUI.CreateMenu("", "", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight(), "fr_simeonsui","fr_simeonsui"))
RMenu:Get("cardealer", "mainmenu"):SetSubtitle("Categories")
RMenu.Add("cardealer","categories",RageUI.CreateSubMenu(RMenu:Get("cardealer", "mainmenu"),"","Vehicles",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight()))
RMenu.Add("cardealer","vehicle",RageUI.CreateSubMenu(RMenu:Get("cardealer", "categories"),"","Options",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight()))
RMenu.Add("cardealer","confirm",RageUI.CreateSubMenu(RMenu:Get("cardealer", "vehicle"),"","Are you sure?",tFR.getRageUIMenuWidth(),tFR.getRageUIMenuHeight()))
                        ...
                        for L, M in pairs(K) do
                            if e == M then
                                if b[M] ~= nil then
                                    ...
                                    RageUI.ButtonWithStyle("No permission","",{RightLabel = ""},true,function(N, O, P)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if d ~= nil then
                ...
                RageUI.ButtonWithStyle("Test Drive " .. i[1],"",{RightLabel = "→→→"},true,function(N, O, P)
                    if P then
                        DeleteVehicle(GetVehiclePedIsIn(tFR.getPlayerPed()))
                        if not k then
                            RageUI.ActuallyCloseAll()
                            hash = GetHashKey(j)
                            ...
                            if HasModelLoaded(hash) then
                                ...
                                testDriveSeconds = 60
                                local S = GetEntityCoords(tFR.getPlayerPed())
                                DecorSetInt(testDriveCar, "FRACVeh", 955)
                                SetModelAsNoLongerNeeded(hash)
                                TaskWarpPedIntoVehicle(tFR.getPlayerPed(), testDriveCar, -1)
                                setVehicleFuel(testDriveCar, 100)
                                tFR.notify("~g~You have 1 minute to test drive this vehicle!")
                                for T = 0, 24 do
                                ...
                                SetTimeout(60000,function()
                                    if k then
                                        ...
                                        SetEntityCoords(tFR.getPlayerPed(), S.x, S.y, S.z)
                                        tFR.notify("Test drive over!")
                                        p()
                                    end
                                end)
                                Citizen.CreateThread(function()
                                ...
                                Citizen.CreateThread(function()
                                    while k do
                                        local U = GetVehiclePedIsIn(tFR.getPlayerPed(), false)
                                        if U ~= nil then
    ...
    if RageUI.Visible(RMenu:Get('cardealer', 'confirm')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Yes","",{RightLabel = "→→→"},true,function(N, O, P)
                if P then
                    TriggerServerEvent("FR:purchaseCarDealerVehicle", e, j)
                    TriggerServerEvent('FR:FetchCars', 'Standard Garage')
                end
            end,RMenu:Get("cardealer", "mainmenu"))
            RageUI.ButtonWithStyle("No","",{RightLabel = "→→→"},true,function(N, O, P)
                if P then
                    tFR.notify("~y~Cancelled!")
                end
            end,
...
tFR.createThreadOnTick(func_previewSimeonsVehicle)
