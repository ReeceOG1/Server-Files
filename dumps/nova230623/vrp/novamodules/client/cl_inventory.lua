            ...
            if EntityType2 and EntityType2 == 3 then
                ...
                if entityModel == model and IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                    ...
                    if distance <= 2.5 and IsControlJustReleased(1, 38) then
                        ...
                        TriggerEvent("NOVA:startCombatTimer", 60)
                        ...
                        TriggerServerEvent('NOVA:LootBag', LootBagIDNew, distance, entityModel)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsControlJustPressed(0, 182) then
            ...
            if IsUsingKeyboard(2) and not tvRP.isInComa() then
                ...
                if not closeToRestart then
					...
					TriggerEvent("NOVA:AllowWeap",drawInventoryUI)
                    ...
                    if drawInventoryUI then
                        ...
                        if BootCar then
                           ...
                           -- VehTypeA = nil;
                            ...
                            TriggerEvent('NOVA:clCloseTrunk')
...
function tvRP.getSpaceInFirstChest()

    return currentInventoryMaxWeight - currentInventoryWeight

end

function tvRP.getSpaceInSecondChest()

    local m = 0

    if next(NOVASecondItemList) == nil then

        return e

    else

        for n, o in pairs(NOVASecondItemList) do
...
RegisterNetEvent("NOVA:FetchPersonalInventory",function(inventory, weight, maxWeight)

    NOVAItemList = inventory
...
RegisterNetEvent("NOVA:SendSecondaryInventoryData",function(p, q, r)

    NOVASecondItemList = p
    ...
    if r then
        ...
        h = GetEntityCoords(tvRP.getPlayerPed())

        if r == "notmytrunk" then

            j = GetEntityCoords(tvRP.getPlayerPed())
...
RegisterNetEvent("NOVA:closeToRestart",function(p)
...
RegisterNetEvent("NOVA:closeSecondInventory",function()
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if f ~= nil and c then
            ...
            local z = GetEntityCoords(tvRP.getPlayerPed())
            ...
            if B > 10.0 then
                ...
                TriggerServerEvent("NOVA:closeChest")
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if drawInventoryUI then
			...
			DrawAdvancedText(0.593, 0.242, 0.005, 0.0028, 0.66, "NOVA Inventory", 255, 255, 255, 255, 7, 0);
			...
			elseif (g ~= nil) then
				...
				local G = sortAlphabetically(NOVASecondItemList);
					...
					if (IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329)) then
						...
						if not lockInventorySoUserNoSpam then
							...
							if a then
								...
								TriggerServerEvent("NOVA:UseItem", a, "Plr");

							elseif (b and (g ~= nil) and c) then

								-- print("pressed use item");

								vRPclient.useInventoryItem({b});
					...
					if (IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329)) then
						...
						if not lockInventorySoUserNoSpam then
							...
							if a then
								...
								TriggerServerEvent("NOVA:UseAllItem", a, "Plr");
						...
						if c then
							...
							if (a and (g ~= nil) and c) then
								...
								if (tvRP.getPlayerCombatTimer() > 0) then
									...
									tvRP.notify("~r~You can not store items whilst in combat.");

								elseif (inventoryType == "CarBoot") then

									-- print("line 546 inventory"..VehTypeA .. a);

                                TriggerServerEvent("NOVA:MoveItem", "Plr", a, VehTypeC, false);

                                -- print("Moved Item: "..a.." to Vehicle: "..VehTypeC);

								elseif (inventoryType == "Housing") then

									TriggerServerEvent("NOVA:MoveItem", "Plr", a, "home", false);

									-- print("line 385 inventory"..a);

								elseif IsLootBagOpening then

									TriggerServerEvent("NOVA:MoveItem", "Plr", a, "LootBag", true);
							...
							elseif (b and (g ~= nil) and c) then
								...
								if (inventoryType == "CarBoot") then
									...
									TriggerServerEvent("NOVA:MoveItem", inventoryType, b, VehTypeC, false);

									-- print("line 395 inventory");

								elseif (inventoryType == "Housing") then

									TriggerServerEvent("NOVA:MoveItem", inventoryType, b, "home", false);

									-- print("line 398 inventory");

								else

									TriggerServerEvent("NOVA:MoveItem", "LootBag", b, LootBagIDNew, true);
						...
						if c then
							...
							if (a and (g ~= nil) and c) then
								...
								elseif (inventoryType == "CarBoot") then
									...
									TriggerServerEvent("NOVA:MoveItemX", "Plr", a, VehTypeA, false, L);

								elseif (inventoryType == "Housing") then

									TriggerServerEvent("NOVA:MoveItemX", "Plr", a, "home", false, L);

								elseif IsLootBagOpening then

									TriggerServerEvent("NOVA:MoveItemX", "Plr", a, "LootBag", true, L);
							...
							elseif (b and (g ~= nil) and c) then
								...
								if (inventoryType == "CarBoot") then
									...
									TriggerServerEvent("NOVA:MoveItemX", inventoryType, b, VehTypeA, false, L);

								elseif (inventoryType == "Housing") then

									TriggerServerEvent("NOVA:MoveItemX", inventoryType, b, "home", false, L);

								else

									TriggerServerEvent("NOVA:MoveItemX", "LootBag", b, LootBagIDNew, true, L);
					...
					if not lockInventorySoUserNoSpam then
						...
						if c then
							...
							if (a and (g ~= nil) and c) then
								...
								local M = tvRP.getSpaceInSecondChest();
								...
								if (L > 0) then
                                    ...
                                    TriggerServerEvent("NOVA:MoveItemAll", "Plr", a, VehTypeC, false);

									elseif (inventoryType == "Housing") then

										TriggerServerEvent("NOVA:MoveItemAll", "Plr", a, "home");

									elseif IsLootBagOpening then

										TriggerServerEvent("NOVA:MoveItemAll", "Plr", a, "LootBag");
							...
							elseif (b and (g ~= nil) and c) then
								...
								local N = tvRP.getSpaceInFirstChest();
								...
								if (L > 0) then
									...
									if (inventoryType == "CarBoot") then
										...
										TriggerServerEvent("NOVA:MoveItemAll", inventoryType, b, VehTypeC, NetworkGetNetworkIdFromEntity(tvRP.getNearestVehicle(3)));

									elseif (inventoryType == "Housing") then

										TriggerServerEvent("NOVA:MoveItemAll", inventoryType, b, "home");

									else

										TriggerServerEvent("NOVA:MoveItemAll", "LootBag", b, LootBagIDNew);
				...
				if (IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329)) then
					...
					if not lockInventorySoUserNoSpam then
						...
						if a then
							...
							TriggerServerEvent("NOVA:GiveItem", a, "Plr");

						elseif b then

							vRPclient.giveToNearestPlayer({b});
				...
				if (IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329)) then
					...
					if not lockInventorySoUserNoSpam then
						...
						if a then
							...
							TriggerServerEvent("NOVA:TrashItem", a, "Plr");

						elseif b then

							tvRP.notify("~r~Please move the item to your inventory in order to trash");
			...
			local G = sortAlphabetically(NOVAItemList);
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if (GetEntityHealth(tvRP.getPlayerPed()) <= 102) then
...
AddEventHandler("NOVA:LockPick2", function()
	TriggerServerEvent("NOVA:ACBan#76");
end);
...
AddEventHandler("NOVA:InventoryOpen", function(toggle, lootbag)
	...
	if toggle then
    	...
    	tvRP.playAnim(true,{{"anim@gangops@facility@servers@bodysearch@","player_search",1}},true)
...
RegisterNetEvent("NOVA:OpenHomeStorage");

AddEventHandler("NOVA:OpenHomeStorage", function(toggle, houseName)
	...
	if (toggle == true) then
		...
		TriggerServerEvent("NOVA:FetchHouseInventory", houseName);
...
RegisterNetEvent("NOVA:SetLootbagFalse");

AddEventHandler("NOVA:SetLootbagFalse", function(a)
...
AddEventHandler("NOVA:clCloseTrunk",function()
...
AddEventHandler("NOVA:clOpenTrunk",function()
    ...
    VehTypeC = NVeh
        ...
        tvRP.vc_openDoor(VehType, 5)

        inventoryType = 'CarBoot'

        TriggerServerEvent('NOVA:FetchPersonalInventory')

        TriggerServerEvent('NOVA:FetchTrunkInventory', VehTypeC)
            ...
            if NearMoneyBag then
                if IsControlJustPressed(0, 38) then
                    if not IsPedSittingInAnyVehicle(Ped) then
                        TriggerServerEvent('NOVA:CollectMoney', NearestMoneyNetID)
                    else
