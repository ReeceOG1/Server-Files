...
Citizen.CreateThread(function()
    while true do
        if not e then
            local h = GetEntityCoords(tvRP.getPlayerPed())
...
function sitDownAtBar(l)
    ...
    TaskGoStraightToCoord(tvRP.getPlayerPed(),followMachineVector.x,followMachineVector.y,followMachineVector.z,1.0,50000,312.2,10.0)
    local h = GetEntityCoords(tvRP.getPlayerPed())
...
function createSitAtBarScene(o, p)
    ...
    NetworkAddPedToSynchronisedScene(tvRP.getPlayerPed(), q, a, animName, 2.0, -1000, 9, 0, 1148846080, 0)
...
Citizen.CreateThread(function()
    while true do
        if insideDiamondCasino then
            if e then
                SetPedCapsule(tvRP.getPlayerPed(), 0.13)
                if IsControlJustPressed(0, 202) then
                    tvRP.loadAnimDict("anim@amb@office@seating@male@var_a@base@")
                    TaskPlayAnim(tvRP.getPlayerPed(),"anim@amb@office@seating@male@var_a@base@","exit",1.0,1.0,-1,0)
