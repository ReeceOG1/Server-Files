...
local c={}
local d={
    [1]={pos=vector3(404.5871887207,-1162.4267578125,29.67),model='vw_prop_vw_colle_alien'},
...
local function e()
    tvRP.loadAnimDict("anim@mp_player_intcelebrationfemale@air_guitar")
...
local function i()
    ...
    if j==0 then 
        a=true
        SetResourceKvpInt("nova_quests_completedall",1)
        TriggerServerEvent("NOVA:claimQuestReward") -- this needs to be done for adding plat days
    end
    return a 
end
local function l()
    hasCompletedAllQuestsInt=GetResourceKvpInt("nova_quests_completedall")
    if not hasCompletedAllQuestsInt then 
        SetResourceKvpInt("nova_quests_completedall",0)
    end
    if hasCompletedAllQuestsInt==0 then 
        for m=1,50 do 
            local n=GetResourceKvpInt("nova_quests_q"..m.."_status")
            if not n then 
                SetResourceKvpInt("nova_quests_q"..m.."_status",0)
                n=0 
            end
            if n==0 then 
                c[m]=true 
...
local function o(p)
    c[p]=nil
    SetResourceKvpInt("nova_quests_q"..p.."_status",1)
    i()
    TriggerServerEvent("NOVA:setQuestCompleted")
end
local function q()
    local j=50
    for k in pairs(c)do 
...
Citizen.CreateThread(function()
    ...
    for w,k in pairs(c)do 
        ...
        tvRP.createArea("quests_prop_id_"..w,undiscoveredQ.pos,100.0,6,x,z,A,{undiscoveredQID=w,undiscoveredQ=undiscoveredQ})
        local B=function()
        end
        local C=function()
        end
        local A=function(y)
            ...
            if IsControlJustReleased(0,38)then 
                ...
                if q()~=50 then 
                    ...
                    DoScreenFadeIn(2000)
                    tvRP.removeArea("quests_prop_id_"..y.undiscoveredQID)
                    tvRP.removeArea("quests_text_id_"..y.undiscoveredQID)
                else 
        ...
        tvRP.createArea("quests_text_id_"..w,undiscoveredQ.pos,2.0,6,B,C,A,{undiscoveredQ=undiscoveredQ,undiscoveredQID=w})
    end 
end)
Citizen.CreateThread(function()
    Wait(5000)
