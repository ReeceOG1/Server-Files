...
AddEventHandler("NOVA:onClientSpawn",function(D, E)
    if E then
		TriggerServerEvent("NOVA:getJobSelectors")
        TriggerServerEvent('NOVA:getFactionWhitelistedGroups')
	end
end)
RegisterNetEvent("NOVA:gotJobSelectors",function(h)
    c = h
    local i = function(j)
        e = j.selectorId
    end
                ...
                if q == e then
                    for s, t in pairs(r.jobs) do
                        RageUI.ButtonWithStyle(t[1],r._config.name,{RightLabel = "→→→"},true,function(u, v, w)
                            if w then
                                TriggerServerEvent("NOVA:jobSelector", q, t[1])
                    ...
                    RageUI.ButtonWithStyle("Unemployed","",{RightLabel = "→→→"},true,function(u, v, w)
                        if w then
                            TriggerServerEvent("NOVA:jobSelector", q, "Unemployed")
                            SetTimeout(1000,function()
                            end)
                        end
                    end)
