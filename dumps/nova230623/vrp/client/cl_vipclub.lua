...
function tvRP.setparachutestting(f)
...
function tvRP.setDeathSound(i)
    if tvRP.isPlusClub() or tvRP.isPlatClub() then 
        ...
        tvRP.notify("~r~Cannot change deathsound, not a valid NOVA Plat subscriber.")
    end 
end
...
RageUI.CreateWhile(1.0, true, function()
    ...
    if RageUI.Visible(RMenu:Get('vipclubmenu', 'manageperks')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            local function R()
                E=true
                tvRP.setparachutestting(E)
                tvRP.notify("~a~Parachute enabled.")
            end
            local function S()
                E=false
                tvRP.setparachutestting(E)
                tvRP.notify("~r~Parachute disabled.")
            end
