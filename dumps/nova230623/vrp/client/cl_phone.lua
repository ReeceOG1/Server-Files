	...
	while (true) do
        if IsControlJustPressed(1, 166) then
            ...
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                TriggerServerEvent('NOVA:TrunkOpened', source)
            else
...
-- RegisterNetEvent("NOVA:handsUpNearest")
-- AddEventHandler("NOVA:handsUpNearest", function(nplayer, nuser_id)
--     nearestHandsUp = IsEntityPlayingAnim(GetPlayerPed(nplayer), "missminuteman_1ig_2", "handsup_enter", 3)
--     if nearestHandsUp then
--         TriggerServerEvent('NOVA:searchNearestPlayer', nplayer, nuser_id)
--     end
-- end)
