...
RMenu.Add('NOVAATM', 'main', RageUI.CreateMenu("", "~b~Cash Point", 1300, 100, "banners", "atm"))

RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('NOVAATM', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Button("Withdraw All", "", {RightLabel = '→→→',}, true, function(Hovered, Active, Selected) 
                if Selected then 
                    tvRP.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
...
Citizen.CreateThread(function()
    while true do 
        ...
        if not MenuOpen and inMarker then 
            MenuOpen = true 
            RageUI.Visible(RMenu:Get('NOVAATM', 'main'), true) 
        end
        if MenuOpen and not inMarker then 
            MenuOpen = false 
            RageUI.Visible(RMenu:Get('NOVAATM', 'main'), false) 
        end
    end
end)
