...
cfg.blackjackTables = {
    ...
    -- Custom Tables [Mlos]
    [4] = {
        dealerPos = vector3(-1833.5, -1184.65, 14.31),
        dealerHeading = 294.81,
        tablePos = vector3(-1832.704,-1184.227,13.29744),
        tableHeading = 120.0,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [5] = {
        dealerPos = vector3(-1839.04, -1193.57, 14.31),
        dealerHeading = 329.65,
        tablePos = vector3(-1838.601,-1192.844,13.29744),
        tableHeading = 149.99996948242,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
}
        ...
        if waitingForBetState then
            ...
            if IsControlJustPressed(0, 201) then --Submit bet [Enter]
                ...
                if tonumber(currentBetAmount) >= 0 then
                    ...
                    notify("~r~Invalid amount. Please note bets cannot be over 5 million!")
                end
            end
    ...
    while true do
        if closestChair ~= -1 and closestChairDist < 2 then 
            if IsControlJustPressed(0, 38) then --Sit down [E]
                if blackjackTableData[closestChair] == false then
                    if closestChair == 8 or closestChair == 12 or closestChair == 16 or closestChair == 20 then
                        notify("~r~This chair is currently disabled till further notice.")
                    else
...
RMenu.Add('NOVAblackjack', 'instructions', RageUI.CreateMenu("", "test",0,100,"casinoui_cards_blackjack", "casinoui_cards_blackjack"))
RMenu:Get('NOVAblackjack', 'instructions'):SetSubtitle("~b~BLACKJACK")
RMenu.Add('NOVAblackjack_high', 'instructions', RageUI.CreateMenu("", "test",0,100,"casinoui_cards_blackjack_high", "casinoui_cards_blackjack_high"))
RMenu:Get('NOVAblackjack_high', 'instructions'):SetSubtitle("~b~BLACKJACK")


RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('NOVAblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            ...
            end, RMenu:Get('NOVAblackjack', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('NOVAblackjack_high', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "The aim of Blackjack is to beat the Dealer's hand without going over 21. This game uses four 52           card decks, which are shuffled at the start of every hand.                                                                                    The dealer will stand on soft 17.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
            ...
            end, RMenu:Get('NOVAblackjack_high', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
function showHowToBlackjack(flag)
    if closestChair < 8 then 
        RageUI.Visible(RMenu:Get('NOVAblackjack', 'instructions'), flag)
    else 
        RageUI.Visible(RMenu:Get('NOVAblackjack_high', 'instructions'), flag)
    end
end
    ...
    while true do
        if blackjackInstructional ~= nil then 
            ...
            if waitingForBetState then 
                ...
                DrawAdvancedNativeText(1.025, 0.853, 0.005, 0.0028, 0.25, "Chips: " .. tostring(math.round(playerAmountOfChips)), 255, 255, 255, 255, 0, 0)
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    --print("[NOVA Casino] start sit at blackjack seat") 
    ...
    --print("[NOVA Casino] blackjack anims loaded") 
    ...
    --print("[NOVA Casino] fVars passed")
    ...
    --print("[NOVA Casino] walking to seat, x: " .. tostring(walkToVector.x) .. " y: " .. tostring(walkToVector.y) .. " z: " .. tostring(walkToVector.z))
    ...
    --print("[NOVA Casino] Blackjack sit at table net scene starting")
    --print("[NOVA Casino] creating Scene at, x: " .. tostring(goToVector.x) .. " y: " .. tostring(goToVector.y) .. " z: " .. tostring(goToVector.z))
    ...
    --print("[NOVA Casino] Blackjack sit at table net scene started")
...
function betChipsForNextHand(chipsAmount,chipsProp,something,chairID,someBool,zOffset)
    ...
    while not HasModelLoaded(chipsProp) do  
        Wait(0)
        --print("[NOVA Casino] Stuck requesting model: " .. tostring(chipsProp))
        RequestModel(chipsProp)
    end
    vVar8 =  vector3(0.0, 0.0, getTableHeading(blackjack_func_368(chairID)))
    local tablePosX,tablePosY,tablePosZ = getTableCoords(blackjack_func_368(chairID))
