...
RMenu.Add('callmanager', 'main', RageUI.CreateMenu("", 'NOVA Call Manager', 1300, 50, "banners", "manager"))
                    ...
                    if k == ticketID then
                        if v[2] == GetPlayerServerId(PlayerId()) then
                            ...
                            CallManagerServer.GetUpdatedCoords({v[2]}, function(targetCoords)
                                ...
                                TriggerServerEvent("NOVA:GiveTicketMoney", v[1], v[2], v[3], true)

                            end)
                            CallManagerServer.RemoveTicket({k, "admin"})
                            name = v[1]
                            isInTicket = true
                            TriggerServerEvent('NOVA:getTempFromPerm',v[2])
                        end
                    end
                end
            end
...
RegisterCommand("return", function()
    if isInTicket then
        ...
        isInTicket = false
		TriggerServerEvent("NOVA:AdminReturned", PermID)
    end
end)


RegisterCommand("tom", function(source, args , rawCommand)
    TriggerServerEvent("NOVA:AdminReturned", permid)
end)
...
AddEventHandler("staffon", function(isInTicket)
    ...
    TriggerEvent("NOVA:vehicleMenu", true, isInTicket)
    if GetEntityHealth(GetPlayerPed(-1)) <= 103 then
    TriggerEvent('NOVA:FixClient')
    end
end)
...
AddEventHandler("staffoff", function()
    isInTicket = false
    TriggerEvent("NOVA:vehicleMenu", false, isInTicket) 
end)

RegisterNetEvent('NOVA:AdminTicketCooldown')
AddEventHandler('NOVA:AdminTicketCooldown', function(source, Reason)
    if AdminTicketCooldown == false then
        TriggerServerEvent('NOVA:sendAdminTicket', source, Reason)
...
RegisterNetEvent('NOVA:PDCallCooldown')
AddEventHandler('NOVA:PDCallCooldown', function(source, Reason)
    if PDCallCooldown == false then
        TriggerServerEvent('NOVA:sendPDCall', source, Reason)
        PDCallCooldown = true
        Wait(60000*2)
...
RegisterNetEvent('NOVA:NHSCallCooldown')
AddEventHandler('NOVA:NHSCallCooldown', function(source, Reason)
    if NHSCallCooldown == false then
        TriggerServerEvent('NOVA:sendNHSCall', source, Reason)
...
RegisterNetEvent('NOVA:PLAYTICKETRECIEVED')
AddEventHandler('NOVA:PLAYTICKETRECIEVED', function(source)
    PlaySoundFrontend(-1, "Apt_Style_Purchase", "DLC_APT_Apartment_SoundSet", 0)
end)

RegisterNetEvent('NOVA:sendPermID')
AddEventHandler('NOVA:sendPermID', function(permid)
    permid = permid
    while isInTicket do
        inRedzone = false
        Wait(0)
