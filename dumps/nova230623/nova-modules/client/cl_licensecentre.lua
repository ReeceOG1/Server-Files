...
-- Credit to jud#8976 for making this for NOVA.

local cfg = module("nova-modules", "cfg/cfg_licenses")
local currentLicense = nil
local currentLicensePrice = nil
local currentType = nil
local inMenu = false

RMenu.Add("NOVA:LicenseStore", "main", RageUI.CreateMenu("", "License Store", 1350, 50,"banners","groups"))
RMenu.Add("NOVA:LicenseStore", "sub", RageUI.CreateSubMenu(RMenu:Get("NOVA:LicenseStore", "main"), "", "~b~License Store", 1350, 50))
RMenu.Add("NOVA:LicenseStore", "confirm", RageUI.CreateSubMenu(RMenu:Get("NOVA:LicenseStore", "sub"), "", "~b~Confirm License Purchase", 1350, 50))

RageUI.CreateWhile(1.0, RMenu:Get("NOVA:LicenseStore", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("NOVA:LicenseStore", "main"), true, false, true, function()

        RageUI.Button("~g~Grinding Licenses", nil, {RightLabel = ">>>"}, true, function(Hovered, Active, Selected)
            if Selected then
                currentType = "grinding"
            end
        end, RMenu:Get("NOVA:LicenseStore", "sub", true))
    ...
    RageUI.IsVisible(RMenu:Get("NOVA:LicenseStore", "sub"), true, false, true, function()
            ...
            if v.type == currentType then
                RageUI.Button(v.name, nil, {RightLabel = "~g~£"..getMoneyStringFormatted(v.price)}, true, function(Hovered, Active, Selected)
                    if Selected then
                        ...
                        RMenu:Get("NOVA:LicenseStore", "confirm"):SetSubtitle("~b~Are you sure you want to buy "..v.name.."?")
                    end
                end, RMenu:Get("NOVA:LicenseStore", "confirm", true))
            end
        end
    ...
    RageUI.IsVisible(RMenu:Get("NOVA:LicenseStore", "confirm"), true, false, true, function()
        ...
        RageUI.Button("~g~Confirm Purchase", nil, {RightLabel = "~g~£"..getMoneyStringFormatted(currentLicensePrice)}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("NOVA:BuyLicense", currentLicense)
            end
        end, RMenu:Get("NOVA:LicenseStore", "main", true))
    ...
    while true do
        ...
        if isInArea(cfg.licenseShopLocation, 1.0) and inMenu == false then
            ...
            if IsControlJustPressed(0, 51) then 
                inMenu = true
                RageUI.Visible(RMenu:Get("NOVA:LicenseStore", "main"), true)
            end
        end
