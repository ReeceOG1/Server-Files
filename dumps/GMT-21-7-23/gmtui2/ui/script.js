$(document).ready(function(){
  ...
  $('input').change(function(){
    $.post('http://gmtui2/updateSkin', JSON.stringify({
      value: false,
      // Face
  ...
  $(document).keypress(function(e) {
    if(e.which == 113){ // A pressed
      $.post('http://gmtui2/rotaterightheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 101){ // E pressed
      $.post('http://gmtui2/rotateleftheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 119 ){ // W pressed
      $.post('http://gmtui2/zoomin', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 115){ // S pressed
      $.post('http://gmtui2/zoomout', JSON.stringify({
        value: 10
      }));
    }
  });
  ...
  $('#tabs label').on('click', function(e){
    //e.preventDefault();
    $.post('http://gmtui2/zoom', JSON.stringify({
  ...
  $(document).keypress(function(e){
    ...
    xTriggered++;
    if(e.which == 13){
      $.post('http://gmtui2/test', JSON.stringify({
        value: xTriggered
      }));
    }
  });
...
function leftWristComponent() {
    $.post('http://gmtui2/updateLeftWristComponent', JSON.stringify({
...
function leftWristTexture() {
    ...
    $.post('http://gmtui2/updateLeftWristTexture', JSON.stringify({
        componentID: $('.montre .active').attr('data'),
...
function rightWristComponent() {
    $.post('http://gmtui2/updateRightWristComponent', JSON.stringify({
...
function rightWristTexture() {
    ...
    $.post('http://gmtui2/updateRightWristTexture', JSON.stringify({
        componentID: $('.montre-right .active').attr('data'),
