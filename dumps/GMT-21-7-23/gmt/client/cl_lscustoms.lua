	...
	if DoesEntityExist(veh) then
        ...
        elseif    type(LSC_Config.menu.theme) == "string" then
            ...
            elseif LSC_Config.menu.theme == "darkred" then
                LSCMenu:setColors({ r = 255,g = 255, b = 255, a = 255},{ r = 0,g = 0, b = 0, a = 255},{ r = 0,g = 0, b = 0, a = 155},{ r = 25,g = 15, b = 15, a = 200})
...
local function DriveOutOfGarage(pos)
	Citizen.CreateThread(function()
		...
		local ok, veh_type, model = tGMT.getNearestOwnedVehicle(1)
		myveh.model = model
						...
						if LSC_Config.oldenter then			
							if #(vec3(outside.position) - GetEntityCoords(ped)) <= f(5) then
								if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									if pos.locked then
										if not LSC_Config.lock then
											if IsControlJustPressed(1,201) then
												...
												tGMT.DrawText(0.5,0.8,"Press ~b~ENTER~w~ to enter ~b~Los Santos Customs~w~",1.0,4,0)
											end
										else
											tGMT.DrawText(0.5,0.8,"~r~Locked, please wait~w~",1.0,4,0)
										end
									else
										if IsControlJustPressed(1,201) then
											inside = true
									...
									tGMT.DrawText(0.5,0.8,"~r~This vehicle can't be upgraded~w~",1.0,4,0)
								end
							end
						else
...
AddEventHandler("GMT:onClientSpawn",function(D, E)
    if E then
        ...
        for q,h in pairs(garages)do 
            tGMT.addBlip(h.inside.position.x,h.inside.position.y,h.inside.position.z,72, nil, "LS Customs")
        end 
	end
end)
...
function tGMT.isInsideLsCustoms()
    return inside
end
