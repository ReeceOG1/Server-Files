...
local h
local i = false
RMenu.Add("scubadiving","main",RageUI.CreateMenu("", "", tGMT.getRageUIMenuWidth(), tGMT.getRageUIMenuHeight(), "gmt_scubajob", "gmt_scubajob"))
RMenu:Get("scubadiving", "main"):SetSubtitle("GMT | ~b~Scuba Diving")
local function j()
...
AddEventHandler("GMT:onClientSpawn",function(l, m)
    if m then
        local n = tGMT.createDynamicPed(a.jobPedModel,a.jobPedPosition,a.jobPedHeading,true,"mini@strip_club@idles@bouncer@base","base",100,false,function(o)
        ...
        tGMT.addBlip(a.jobPosition.x, a.jobPosition.y, a.jobPosition.z, 308, 0, "Scuba Diving")
        tGMT.addMarker(a.jobPosition.x,a.jobPosition.y,a.jobPosition.z - 0.2,0.5,0.5,0.5,0,50,255,170,50,20,false,false,true)
        local q = function(r)
            j()
        end
        local s = function(r)
        ...
        tGMT.createArea("scubajob", a.jobPosition, 1.5, 6, q, s, t, {})
    end
end)
        ...
        RageUI.DrawContent({header=true,glare=false,instructionalButton=true},function()
            if not e then
                RageUI.Button("Request Job",nil,{RightLabel = "→→→"},true,function(u, v, w)
                    if w then
                        TriggerServerEvent("GMT:requestScubaJob")
                    end
                end)
            end
            if d ~= 0 then
                RageUI.Button("Claim Reward",nil,{RightLabel = "→→→"},true,function(u, v, w)
                    if w then
                        TriggerServerEvent("GMT:claimScubaReward", c)
                        d = 0
                        c = nil
                        e = false
                        i = false
                        ...
                        tGMT.setCustomization(h)
                        local x = PlayerPedId()
                        SetEnableScuba(x, false)
                        SetPedMaxTimeUnderwater(x, 10.0)
                        tGMT.setMaxUnderWaterUITimenewTime(10.0)
                    end
                end)
            end
        end)
...
local function y()
    for z, A in pairs(b) do
        tGMT.removeMarker(A.objectMarker)
        tGMT.removeArea(A.objectArea)
...
function startScubaDiving()
    tGMT.loadAnimDict("missheistchem2")
    local x = PlayerPedId()
    ...
    tGMT.drawNativeNotification("Press ~INPUT_PICKUP~ to start Scuba Diving.")
    while not F do
    ...
    TriggerServerEvent("GMT:finishScubaJob", c, d)
end
RegisterNetEvent("GMT:createScubaJob",function(J, K)
    c = K
    e = true
    tGMT.disableCustomizationSave(e)
    DoScreenFadeOut(500)
    Wait(600)
    ...
    SetBlipAlpha(L, 180)
    local q = function(r)
        tGMT.drawNativeNotification("Press ~INPUT_DETONATE~ to begin diving!")
    end
    local s = function(r)
    end
    local t = function(r)
        if IsControlJustReleased(1, 47) then
            local x = tGMT.getPlayerPed()
            local M = tGMT.getPlayerVehicle()
            if M ~= 0 then
                if GetEntityModel(M) ~= GetHashKey("dinghy") then
                    tGMT.notify("~r~You must be in a dinghy to start scuba diving!")
                else
                    tGMT.removeArea("scubajob_" .. J.name)
                    f = "~y~Search for treasures and return to your boat when finished"
                    createRewardObjects(J)
                    CreateThread(startScubaDiving)
                end
            else
                tGMT.notify("~r~You must be in a boat to start scuba diving!")
            end
        end
    end
    SetWaypointOff()
    local N = tGMT.addBlip(J.position.x, J.position.y, J.position.z, J.blipId, J.blipColour, "Scuba Diving")
    SetNewWaypoint(J.position.x, J.position.y)
    tGMT.createArea("scubajob_" .. J.name, J.position, 250, 15, q, s, t, {})
    h = tGMT.getCustomization()
    local p = math.random(1, 15)
    local O = PlayerPedId()
    ...
    g = tGMT.spawnVehicle("dinghy",J.dinghySpawnPositions[P].x,J.dinghySpawnPositions[P].y,J.dinghySpawnPositions[P].z,J.dinghySpawnHeading,true,true,true)
    ...
    TriggerServerEvent("GMT:scubaSetVehicle", c, R)
    DoScreenFadeIn(2000)
    SetPedScubaGearVariation(PlayerPedId())
    Citizen.CreateThread(function()
        while e do
            if i then
                ...
                SetWaypointOff()
                TriggerServerEvent("GMT:finishScubaJob", c, 0)
                TriggerServerEvent("GMT:claimScubaReward", c)
                d = 0
                c = nil
                e = false
...
RegisterNetEvent("GMT:ScubaBoatDestroyed",function(S)
    if e and S == c then
        i = true
    end
end)
function createRewardObjects(J)
    for z, A in pairs(J.rewardObjects) do
        local T = tGMT.loadModel(A.objectModel)
        ...
        local V =
            tGMT.addMarker(A.objectPosition.x,A.objectPosition.y,A.objectPosition.z + 0.6,0.5,0.5,0.5,255,223,0,150,50.0,0,false,true,true)
        local q = function(r)
            tGMT.drawNativeNotification("Press ~INPUT_PICKUP~ to collect!")
        end
        local s = function(r)
        end
        local t = function(r)
            if IsControlJustReleased(1, 38) and e then
                tGMT.notify("~g~Item collected.")
                d = d + 1
                tGMT.removeMarker(b[r.objectId].objectMarker)
                tGMT.removeArea(b[r.objectId].objectArea)
        ...
        tGMT.createArea("scubajob_" .. J.name .. "_obj_" .. z, A.objectPosition, 3.0, 3.0, q, s, t, {objectId = z})
        b[z] = {}
        b[z].objectId = U
        b[z].objectMarker = V
