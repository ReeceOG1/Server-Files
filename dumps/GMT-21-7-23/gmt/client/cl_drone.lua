...
local function d()
    ...
    if c.cameraEnabled then
        ...
        local f = 500 * math.ceil(#(tGMT.getPlayerCoords() - GetEntityCoords(c.vehicleHandle)) / 10)
        ...
        local g = tGMT.loadModel("prop_cs_tablet")
        c.tablet = CreateObject(g, tGMT.getPlayerCoords(), true)
        ...
        tGMT.loadAnimDict("amb@code_human_in_bus_passenger_idles@female@tablet@idle_a")
        TaskPlayAnim(
            e,
...
RegisterNetEvent(
    ...
    function()
        if not c.active then
            if tGMT.getPlayerVehicle() == 0 and not tGMT.isInComa() then
                TriggerServerEvent("GMT:performDroneLog", "Powered on their DJI Inspire Drone")
                newDrone()
            else
                tGMT.notify("You are not able to use a drone right now.")
            end
        else
        ...
        while true do
            if c.active then
                ...
                if not NetworkHasControlOfEntity(c.driverHandle) or not NetworkHasControlOfEntity(c.vehicleHandle) then
                    tGMT.notify("You have ~b~lost control ~w~of the drone, after going out of range")
                    deleteDrone()
                end
...
Citizen.CreateThread(
    function()
        while true do
            local p = tGMT.getPlayerVehicle()
            if p ~= 0 then
                local q = GetEntityModel(p)
                if q == GetHashKey(a) then
                    o = o + 1
                    if o > 3 then
                        tGMT.notify("~r~Access Denied~w~: Engaging defences in 3, 2, 1...")
                        Wait(500)
                        local r = tGMT.getPlayerCoords()
                        AddOwnedExplosion(tGMT.getPlayerPed(), r.x, r.y, r.z, 4, 17.0, true, false, 10.0)
                        ...
                        if tGMT.getPlayerVehicle() ~= nil and DoesEntityExist(tGMT.getPlayerVehicle()) then
                            DeleteEntity(tGMT.getPlayerVehicle())
                        end
                    end
                end
            end
...
function newDrone()
    local g = tGMT.loadModel(a)
    local v = tGMT.loadModel("s_m_m_bouncer_01")
    local e = tGMT.getPlayerPed()
    local r = GetOffsetFromEntityInWorldCoords(e, 0.0, 4.0, 0.0)
    local w = GetEntityHeading(e)
    c.vehicleHandle = tGMT.spawnVehicle(a, r, w, true)
    SetModelAsNoLongerNeeded(g)
    SetEntityNoCollisionEntity(tGMT.getPlayerPed(), c.vehicleHandle, 1)
    Wait(1000)
    ...
    c.active = true
    tGMT.notify("Drone ~b~created~w~.")
    Wait(1000)
    if globalNHSOnDuty then
        tGMT.notify("~y~Reminder~w~: These drones are reserved for usage by HEMS, Specialised Off Road Unit")
        tGMT.notify("and Senior Pilots only. If you are not in these divisions, you ~r~should not ~r~be using this.")
    end
    tGMT.notify("To remove your drone, use ~b~/drone")
end
...
local function B()
    ...
    if p == 0 then
        if z then
            if #(z - tGMT.getPlayerCoords()) > 20.0 then
                local C = PlayerPedId()
    ...
    if #(z - D) > 20.0 then
        ...
        if IsDisabledControlJustPressed(0, 23) or IsDisabledControlJustPressed(0, 75) then
            tGMT.notify("You must be near the ~y~start position~w~ to exit.")
        end
    end
end
tGMT.createThreadOnTick(B)
