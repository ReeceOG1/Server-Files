local a = {twoStepControl = 137, TwoStepCars = {"barnsy", "ker"}}
local b = {}
local c
local d = false
local e = false
Citizen.CreateThread(
    function()
        while true do
            ...
            local g = GetVehiclePedIsIn(tGMT.getPlayerPed(), false)
            if g ~= 0 and GetPedInVehicleSeat(g, -1) == f then
                local h, i = tGMT.getVehicleInfos(g)
                if h == tGMT.getUserId() then
                    if b[i] ~= nil and b[i] ~= 0 then
                        c = i
                        d = true
                        while GetVehiclePedIsIn(tGMT.getPlayerPed(), false) ~= 0 and e == false do
                            Wait(100)
                        end
                        e = false
                        d = false
            ...
            if d then
                if not IsControlPressed(1, 71) and not IsControlPressed(1, 72) then
                    ...
                    if IsPedInAnyVehicle(j) then
                        ...
                        local n = GetVehicleCurrentRpm(GetVehiclePedIsIn(tGMT.getPlayerPed(), false))
