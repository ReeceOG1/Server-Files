RMenu.Add(
    "nhs",
    "main",
    RageUI.CreateMenu("NHS", "Status: ~b~Available", tGMT.getRageUIMenuWidth(), tGMT.getRageUIMenuHeight())
)
RMenu.Add(
    "nhs",
    "equipment",
...
local function C()
    if globalNHSOnDuty or globalLFBOnDuty and not tGMT.getUserId() == 1 then
        local D = PlayerPedId()
        ...
        while true do
            if e.active and e.targetPed then
                ...
                if #(F - G) > 10.0 then
                    e.active = false
                    TriggerServerEvent("GMT:playNhsSound", "threeBeeps")
                    tGMT.notify("~r~Lifepak disconnected.")
                end
            end
            Citizen.Wait(5000)
        end
