...
RMenu.Add("GMTGunstore","mainmenu",RageUI.CreateMenu("","",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"gmt_gunstoreui","gmt_gunstoreui"))
RMenu:Get("GMTGunstore","mainmenu"):SetSubtitle("~b~GUNSTORE")
RMenu.Add("GMTGunstore","type",RageUI.CreateSubMenu(RMenu:Get("GMTGunstore","mainmenu"),"","Purchase Weapon or Ammo",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"gmt_gunstoreui","gmt_gunstoreui"))
RMenu.Add("GMTGunstore","confirm",RageUI.CreateSubMenu(RMenu:Get("GMTGunstore","type"),"","Purchase confirm your purchase",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"gmt_gunstoreui","gmt_gunstoreui"))
RMenu.Add("GMTGunstore","vip",RageUI.CreateSubMenu(RMenu:Get("GMTGunstore","mainmenu"),"","Purchase Weapon or Ammo",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"gmt_gunstoreui","gmt_gunstoreui"))
...
RageUI.CreateWhile(1.0, true, function()
                    ...
                    for o,p in pairs(sortedKeys(n))do 
                        ...
                        if p~="_config"then 
                            ...
                            if p=="item|fillUpArmour"then 
                                local z=GetPedArmour(tGMT.getPlayerPed())
                                local A=100-z
                                y=A*1000
                                x=true 
                            end
    ...
    if RageUI.Visible(RMenu:Get("GMTGunstore", "type")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            end,RMenu:Get("GMTGunstore","confirm"))
    ...
    if RageUI.Visible(RMenu:Get("GMTGunstore", "confirm")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            if l then 
                if string.sub(d.model,1,4)=="item"then 
                    ...
                    if g=="ammo"then 
                        if HasPedGotWeapon(tGMT.getPlayerPed(),GetHashKey(d.model),false)then 
                            TriggerServerEvent("GMT:buyWeapon",d.model,d.price,d.name,d.weaponShop,"ammo")
                        else 
                            tGMT.notify("~r~You do not have the body of this weapon to purchase ammo.")
                        end 
                    else 
            ...
            end,RMenu:Get("GMTGunstore","mainmenu"))
        end) 
    end
...
local function J(m,K)
    ...
    c=K
    if m=="Rebel"then 
        RMenu:Get('GMTGunstore','mainmenu'):SetSpriteBanner("gmt_rebel", "gmt_rebel")
    else 
        RMenu:Get('GMTGunstore','mainmenu'):SetSpriteBanner("gmt_gunstoreui","gmt_gunstoreui")
    end
...
Citizen.CreateThread(function()
    while true do 
        ...
        if not G and GetSelectedPedWeapon(R) == GetHashKey("WEAPON_MP5TAZER") then
            tGMT.setWeapon(R, "WEAPON_UNARMED", true)
        end
        Wait(0)
    end 
end)
            ...
            for K,U in pairs(P)do 
                ...
                local V=function()
                    if GetVehiclePedIsIn(tGMT.getPlayerPed(),false)==0 then 
                        ...
                        tGMT.notify("~r~Exit your vehicle to access the gun store.")
                    end 
                end
                local W=function()
                    L(m)
                ...
                tGMT.createArea("gunstore_"..m.."_"..K,U,1.5,6,V,W,X,{})
            end 
        end 
    end
end)
...
function tGMT.createGunStore(Z,_,a0)
    ...
    tGMT.createArea(a1,a0,1.5,6,V,W,function()
    end)
