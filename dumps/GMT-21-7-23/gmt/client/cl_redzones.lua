...
function tGMT.setRedzoneTimerDisabled(e)
    a = e
end
function tGMT.isPlayerInRedZone()
    return globalInRedzone
end
local f = 0
function tGMT.setPlayerCombatTimer(g, h)
    if tGMT.isPurge() then
        return
    end
    b = g
    if h then
...
Citizen.CreateThread(
    function()
        while true do
            if not a then
                local r = GetEntityCoords(tGMT.getPlayerPed())
                globalInRedzone = false
                for s, k in pairs(d) do
                    if i(r, k, false) then
                        globalInRedzone = true
                        local r = GetEntityCoords(tGMT.getPlayerPed())
                        if not noclipActive or tGMT.getStaffLevel() == 0 then
                            tGMT.setPlayerCombatTimer(30, false)
                        end
                        local t
                        local u = false
                        while not u do
                            r = GetEntityCoords(tGMT.getPlayerPed())
                            while i(r, k, true) do
                                r = GetEntityCoords(tGMT.getPlayerPed())
                                t = r
                                if
                                    IsPedShooting(tGMT.getPlayerPed()) and
                                        GetSelectedPedWeapon(tGMT.getPlayerPed()) ~= GetHashKey("WEAPON_UNARMED")
                                 then
                            ...
                            if b == 0 then
                                ...
                                local v = k.pos - GetEntityCoords(tGMT.getPlayerPed())
                                t = t + v * 0.01
                                if GetVehiclePedIsIn(tGMT.getPlayerPed(), false) == 0 then
                                    ...
                                    SetEntityCoords(tGMT.getPlayerPed(), t.x, t.y, t.z)
                                end
                                SetTimeout(
                                    1000,
                                    function()
...
function tGMT.isEmergencyService()
...
AddEventHandler("GMT:startCombatTimer",function(h)
    if not tGMT.isEmergencyService() then
        tGMT.setPlayerCombatTimer(60, h)
    end
end)
local function y()
    if not tGMT.isEmergencyService() and not tGMT.isInComa() then
        local z = PlayerPedId()
        ...
        if IsPedShooting(z) and not x[A] then
            tGMT.setPlayerCombatTimer(60, true)
        elseif GetPlayerTargetEntity(tGMT.getPlayerId()) and IsControlPressed(0, 24) then
            tGMT.setPlayerCombatTimer(60, true)
        end
    end
    if b > 0 then
...
local function B()
    ...
    if GetSelectedPedWeapon(z) == GetHashKey("WEAPON_MOLOTOV") then
        tGMT.setWeapon(z, "WEAPON_UNARMED", true)
    end
end
local function C()
