        ...
        local g = function(e)
            ...
            if IsControlJustPressed(1, 38)  then
                if GetGameTimer() - cd > 60000 then
                    local h = tGMT.loadModel(a[e.skateparkId].carID)
                    ...
                    DecorSetInt(i, "GMTACVeh", 955)
                    SetVehicleOnGroundProperly(i)
                    SetEntityInvincible(i, false)
                    SetPedIntoVehicle(tGMT.getPlayerPed(), i, -1)
                    SetModelAsNoLongerNeeded(h)
                    cd = GetGameTimer()
                else
                    tGMT.notify('Please wait before spawning another BMX.')
                end
            end
        end
        for j, k in pairs(a) do
            tGMT.createArea("skatepark_" .. j, k.position, 1.5, 6, d, f, g, {skateparkId = j})
