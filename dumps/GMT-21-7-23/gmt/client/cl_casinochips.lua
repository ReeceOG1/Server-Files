...
RMenu.Add('GMTChips','cashier',RageUI.CreateMenu("","",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('GMTChips','cashier'):SetSubtitle("CHIPS")
RMenu.Add('casino_enter','casino',RageUI.CreateMenu("","",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('casino_enter','casino'):SetSubtitle("ENTER")
RMenu.Add('casino_exit','casino',RageUI.CreateMenu("","",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('casino_exit','casino'):SetSubtitle("EXIT")
RMenu.Add('casino_rooftop_exit','casino',RageUI.CreateMenu("","",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
    ...
    if RageUI.Visible(RMenu:Get('casino_enter', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Enter Diamond Casino","",{RightLabel="→→→"},true,function(n,o,p)
                if p and tGMT.getPlayerCombatTimer() == 0 then 
...
Citizen.CreateThread(function()
    while true do 
        local v=GetEntityCoords(tGMT.getPlayerPed())
        a=#(v-e)
        b=#(v-f)
        c=#(v-g)
        Wait(100)
...
AddEventHandler("GMT:onClientSpawn",function(D, E)
    if E then
        ...
        for H,k in pairs(h)do 
            ...
            tGMT.createArea("chipscashier_"..H,k.entryPosition,1.5,6,C,E,F,{})
        end
        for I,J in pairs(z)do 
            tGMT.createArea("casinos_"..I,J.position,J.radius,6,function()
            end,function()
            end,G,{})
        end 
    end
