...
local b = {
    ...
    ["Heroin"] = {
        ...
        ["processing"] = {position = vector3(1593.9248046875, 3585.1362304688, 38.964069366456), radius = 5}
    },
    ["LSD"] = {
        ["mining"] = {position = vector3(5382.7719726562, -5251.4077148438, 34.086650848389), radius = 100},
        ["processing"] = {position = vector3(-2082.3818359375, 2611.9619140625, 3.0839722156525), radius = 20},
        ["refinery"] = {position = vector3(461.9499206543, -3230.3815917969, 6.0695543289185), radius = 25}
    },
    ["Copper"] = {
...
RegisterNetEvent("GMT:playGrindingScenario",function(c, d)
    if not a then
        ...
        TaskStartScenarioInPlace(tGMT.getPlayerPed(), c, 0, true)
        local f
        if d then
...
RegisterNetEvent("GMT:playGrindingPickaxe",function()
    if not a then
        ...
        local j = tGMT.loadModel("prop_tool_pickaxe")
        ...
        while e + h > GetGameTimer() do
            while IsEntityPlayingAnim(tGMT.getPlayerPed(),"melee@large_wpn@streamed_core","ground_attack_on_spot",3) == 1 do
                Wait(0)
            end
        ...
        tGMT.notify("Mining currently in progress.")
    end
end)
local l = false
local function m()
    ...
    if q then
            ...
            if s.nearby then
                if IsControlJustReleased(0, 38) and GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                    if not a then
                        ...
                        tGMT.notify("Action in progress, please wait.")
                    end
                end
            end
        end
        for v, w in pairs(b) do
            for x, y in pairs(w) do
                tGMT.createArea(v .. "_" .. x,y.position,y.radius,6,r,t,u,{drug = v, type = x, nearby = false})
            end
        end
    end
end)
...
Citizen.CreateThread(function()
    ...
    local A = tGMT.loadModel("a_c_hen")
