...
RMenu.Add("loginrewards", "mainmenu", RageUI.CreateMenu("", "Main Menu", tGMT.getRageUIMenuWidth(), tGMT.getRageUIMenuHeight(), "gmt_loginrewards", "gmt_loginrewards"))
    ...
    if RageUI.Visible(RMenu:Get("loginrewards", "mainmenu")) then
        RageUI.DrawContent({header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.Button(string.format("Day %d", t), g(s), {RightLabel = n(t, not s)}, s, function(p, p, u)
                if u then
                    TriggerServerEvent("GMT:claimNextLoginReward")
                    b = b + 1
...
RegisterNetEvent("GMT:setDailyRewardInfo", function(w, x, y)
    b = w
    c = GetGameTimer() + (86400 - (y - x)) * 1000
    if w == 0 then
        Citizen.Wait(20000)
        tGMT.notify("~y~You have not started your daily reward. Do /dailyrewards to start now!")
    end
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        if r() and b > 0 then
            tGMT.notify("~g~You can claim your next daily reward at /dailyrewards!")
            break
        end
        Citizen.Wait(6000)
    end
