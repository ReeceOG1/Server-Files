...
RegisterCommand(
    "getcasinotable",
    function()
        local K = GetEntityCoords(tGMT.getPlayerPed())
...
Citizen.CreateThread(function()
    ...
    for O = 0, #G.blackjackTables, 1 do
        ...
        tGMT.createDynamicPed(dealerModel,P.dealerPos,P.dealerHeading,true,nil,nil,75.0,nil,function(Q)
            ...
            tGMT.loadAnimDict(dealerAnimDict)
...
function resetDealerIdle(U)
    ...
    if DoesEntityExist(U) then
        ...
        TaskPlayAnim(tGMT.getPlayerPed(),"anim_casino_b@amb@casino@games@shared@player@","idle_cardgames",1.0,1.0,-1,0)
        RemoveAnimDict(dealerAnimDict)
    end
end
local V = false
                ...
                if tonumber(W) then
                    ...
                    if W > 0 then
                        if not isChairHighRollers(F) and z > B then
                            tGMT.notify("~r~Maximum bet is " ..tostring(getMoneyStringFormatted(B)) .." please purchase a membership for higher bets.")
                        else
                            z = W
                        end
                    end
            ...
            if IsControlJustPressed(0, 201) then
                if tonumber(z) >= 0 then
                    ...
                    elseif isChairHighRollers(F) and z < B then
                        tGMT.notify("~r~Minimum bet is " .. tostring(getMoneyStringFormatted(B)))
                    elseif z > 1000000000 then
                        tGMT.notify("~r~Maximum bet is £1,000,000,000")
                    else
...
Citizen.CreateThread(function()
    while true do
        if sittingAtBlackjackTable and A then
            SetPedCapsule(tGMT.getPlayerPed(), 0.2)
            if IsControlJustPressed(0, 202) and not i then
                ...
                TaskPlayAnim(tGMT.getPlayerPed(), blackjackAnimDictToLoad, "sit_exit_left", 1.0, 1.0, 2500, 0)
...
function goToBlackjackSeat(an)
    ...
    c = an
    fVar3 = blackjack_func_217(tGMT.getPlayerPed(), blackjack_func_218(c, 0), 1)
    fVar4 = blackjack_func_217(tGMT.getPlayerPed(), blackjack_func_218(c, 1), 1)
    fVar5 = blackjack_func_217(tGMT.getPlayerPed(), blackjack_func_218(c, 2), 1)
    ...
    TaskGoStraightToCoord(tGMT.getPlayerPed(), ao.x, ao.y, ao.z, 1.0, 5000, ap, 0.01)
...
function getNewCardFromMachine(nextCard, Y, ab)
    local aR = tGMT.loadModel(nextCard)
...
function requestCard()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tGMT.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@blackjack@player","request_card",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
...
function putBetOnTable()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tGMT.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@blackjack@player",getAnimNameFromBet(100),4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(3000,function()
        E = true
    end)
...
function angryIBust()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tGMT.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_terrible_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
...
function angryILost()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tGMT.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_bad_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
...
function annoyedIPushed()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tGMT.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_impartial_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
...
function happyIWon()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tGMT.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_good_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
