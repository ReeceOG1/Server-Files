...
RMenu.Add(
    "gmtevents",
    "players",
    RageUI.CreateMenu("", "Players", tGMT.getRageUIMenuWidth(), tGMT.getRageUIMenuHeight(), "banners", "gmt_events")
)
RMenu.Add(
    "gmtevents",
    "players2",
...
local function h()
    return tGMT.isDev() or tGMT.getStaffLevel() >= 6
end
RegisterCommand(
    "events",
    function()
        if tGMT.getUserId() == 1 then
            if h() then
                if currentEvent.isManager == true then
                    ...
                    RageUI.Visible(RMenu:Get("gmtevents", "players"), true)
                else
                    if RageUI.Visible(RMenu:Get("gmtevents", "main")) then
                        RageUI.ActuallyCloseAll()
                    else
                        RageUI.ActuallyCloseAll()
                        RageUI.Visible(RMenu:Get("gmtevents", "main"), true)
                    end
                end
            end
        end
...
RageUI.CreateWhile(
    ...
    function()
            ...
            RageUI.DrawContent(
                ...
                function()
                    if GetGameTimer() - e > 1000 then
                        TriggerServerEvent("GMT:requestIsAnyEventActive")
                        e = GetGameTimer()
                    end
                    if f then
                    ...
                    for n, o in pairs(a) do
                        if table.count(o) > 1 then
                            RageUI.ButtonWithStyle(
                                ...
                                RMenu:Get("gmtevents", "categoryInfo")
                            )
                        else
                            for p, j in pairs(o) do
                                i(j)
                            end
                        end
                    end
        ...
        if RageUI.Visible(RMenu:Get("gmtevents", "categoryInfo")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = true},
                function()
        ...
        if RageUI.Visible(RMenu:Get("gmtevents", "secondary")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = true},
                function()
                    if type(g.menuitems) == "function" then
                        g.menuitems()
                    end
                end
            )
        end
        if RageUI.Visible(RMenu:Get("gmtevents", "players")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = true},
                function()
                    RageUI.Checkbox(
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(k, l, m)
                            if m then
                                TriggerServerEvent("GMT:startEventFully", currentEvent.eventId)
                                RageUI.ActuallyCloseAll()
                            end
                        end
                    )
                    RageUI.ButtonWithStyle(
                        ...
                        function(k, l, m)
                            if m then
                                TriggerServerEvent("GMT:cancelEvent", currentEvent.eventId)
                                RageUI.ActuallyCloseAll()
                            end
                        end
                    )
                    ...
                    for t, u in pairs(currentEvent.players) do
                        RageUI.ButtonWithStyle(
                            ...
                            function(k, l, m)
                                if m then
                                    RMenu:Get("gmtevents", "players2").MetaData = u
                                end
                            end,
                            RMenu:Get("gmtevents", "players2")
                        )
                    end
                end
            )
        end
        if RageUI.Visible(RMenu:Get("gmtevents", "players2")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = true},
                function(g)
                    RageUI.ButtonWithStyle(
                        ...
                        function(k, l, m)
                            if m then
                                TriggerServerEvent("GMT:kickPlayerFromEvent", g.source, currentEvent.eventId)
                            end
                        end
                    )
                end
        ...
        if RageUI.Visible(RMenu:Get("gmtevents", "client")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = true},
                function()
                    RageUI.ButtonWithStyle(
...
function tGMT.registerEventMenuItem(v, w, x)
    if not a[v] then
        a[v] = {}
    end
    a[v][w] = {name = w, menuitems = x}
end
function tGMT.registerEventFullStartHandler(y, z)
    b.start[y] = z
end
function tGMT.registerEventCleanupHandler(y, z)
    b.cleanup[y] = z
end
local function A(B)
    tGMT.removeBlipsForPlayer(B)
    tGMT.removePlayerFromLeaderboard(B.source)
end
RegisterNetEvent(
    "GMT:syncPlayers",
    function(C, D)
        for p, E in pairs(currentEvent.players) do
            local F = true
            for p, B in pairs(C) do
...
function tGMT.stopEventSequence(M)
    RageUI.ActuallyCloseAll()
    isDoingSequence = false
    DestroyCam(I, false)
    DestroyCam(J, false)
...
RegisterNetEvent(
    ...
    function(g)
        ...
        tGMT.setGreenzonesDisabled(true)
        RageUI.Visible(RMenu:Get("gmtevents", "client"), false)
    end
)
local N = "..."
    ...
    function()
        while true do
            if isDoingSequence then
                ...
                if not RageUI.Visible(RMenu:Get("gmtevents", "client")) and not currentEvent.isManager then
                    RageUI.Visible(RMenu:Get("gmtevents", "client"), true)
                elseif not RageUI.Visible(RMenu:Get("gmtevents", "players")) and currentEvent.isManager then
                    RageUI.Visible(RMenu:Get("gmtevents", "players"), true)
                end
            end
            if currentEvent.isActive and currentEvent.drawPlayersTimeBar then
                local O = 0
                for p, B in pairs(currentEvent.players) do
                    if type(B.active) == "nil" or B.active == true then
                        O = O + 1
                    end
                end
                DrawGTATimerBar("~y~PLAYERS:", O, 1)
                tGMT.setWeather("EXTRASUNNY")
            end
            Wait(0)
        end
    end
...
RegisterNetEvent(
    "GMT:eventCleanup",
    function(D, P, Q, R)
        tGMT.stopEventSequence()
        RageUI.Visible(RMenu:Get("gmtevents", "client"), false)
        if b.cleanup[P] then
            b.cleanup[P]()
        end
        globalDisableVehicleFailure = false
        tGMT.endVehicleSelection()
        tGMT.setEventSpectatorMode(false)
        tGMT.clearEventBounds()
        tGMT.enableEventPlayerBlips(false)
        tGMT.enableEventPlayerTags(false, false)
        tGMT.setGreenzonesDisabled(false)
        tGMT.setPlayerCanOpenLeaderboard(false)
        tGMT.clearLeaderboardData()
        if Q and R then
            tGMT.podiumLeaderboard(Q, R)
        elseif tGMT.isPodiumDrawing() then
            tGMT.callCancelPodium()
        end
        currentEvent = {
            players = {},
            isActive = false,
...
function tGMT.setPlayerInvisible(S)
    local T = PlayerPedId()
...
RegisterNetEvent(
    "GMT:announceEventJoinable",
    function(P, U)
        if tGMT.getPlayerCombatTimer() == 0 and not tGMT.getHideEventAnnouncementFlag() and not tGMT.isInTutorial() then
            PlaySound(-1, "CHECKPOINT_PERFECT", "HUD_MINI_GAME_SOUNDSET", false, 0, true)
            tGMT.announceMpBigMsg(
...
function tGMT.announceMpBigMsg(V, W, X, Y, Z)
    if Y then
        local _ = GetSoundId()
...
RegisterNetEvent("GMT:announceMpBigMsg", tGMT.announceMpBigMsg)
function tGMT.setEventMusic(a2)
    currentEvent.playMusic = true
    currentEvent.musicString = a2
    TriggerMusicEvent(a2)
end
function tGMT.getActiveEventPlayers()
    local a3 = {}
    for p, B in pairs(currentEvent.players) do
        if B.active then
            table.insert(a3, B)
...
function tGMT.getEventLocalPlayer()
    local a4 = GetPlayerServerId(PlayerId())
    for p, a5 in pairs(currentEvent.players) do
        if a5.source == a4 then
            return a5
        end
    end
end
function tGMT.getEventPlayerFromSrc(a6)
    for p, a5 in pairs(currentEvent.players) do
...
function tGMT.showCountdownTimer(a9)
...
function tGMT.setEventBounds(ag)
    if ag and table.count(ag) ~= 0 then
        currentEvent.bounds = ag
    end
end
function tGMT.clearEventBounds()
...
Citizen.CreateThread(
    function()
        while true do
            if
                currentEvent.bounds and tGMT.getEventLocalPlayer() and tGMT.getEventLocalPlayer().active and
                    not tGMT.isSpectatingEvent()
             then
...
local function an(ao)
    tGMT.loadPtfx("proj_indep_firework")
    tGMT.loadPtfx("proj_indep_firework_v2")
    ...
    tGMT.loadPtfx("scr_indep_fireworks")
...
local as = false
function tGMT.isPodiumDrawing()
    return as
end
local at = false
function tGMT.callCancelPodium()
    at = true
end
function tGMT.podiumLeaderboard(Q, R)
    as = true
    tGMT.setTime(0, 0, 0)
    RequestIpl("stadium")
    ...
    for p, aD in pairs(au) do
        tGMT.removeArea("3dtext_" .. aD)
    end
    ClearPedTasks(T)
...
RegisterNetEvent(
    ...
    function(aE, aF)
        ...
        if not tGMT.isPodiumDrawing() then
            tGMT.showUI()
        end
    end
)
local aI = false
function tGMT.activateSlowMo()
    aI = true
...
local aM
local aN = false
function tGMT.enableEventPlayerBlips(aO, aP, aQ)
    aL = aP
    aM = aQ or function()
            return true
        end
    aN = aO
    if not aO then
        for p, B in ipairs(currentEvent.players) do
            tGMT.removeBlipsForPlayer(B)
        end
    end
...
local aR = {}
function tGMT.forceCleanupPlayerBlips()
    for p, aS in pairs(aR) do
        for p, aT in pairs(aS) do
...
function tGMT.removeBlipsForPlayer(B)
    if B.blip then
        RemoveBlip(B.blip)
    end
    local aU = aR[B.source]
    if aU then
        for p, aT in pairs(aU) do
            RemoveBlip(aT)
        end
    end
                    ...
                    if aX ~= -1 and aX ~= PlayerId() and table.has(a3, aX) then
                        ...
                        if IsEntityVisible(T) and a5.active and aM(a5.source) then
                            if not a5.blip or aW ~= a5.blip then
                                ...
                                AddTextComponentSubstringPlayerName(tGMT.getPlayerName(aX))
...
local b2 = false
local b3 = false
function tGMT.enableEventPlayerTags(aO, b4, b5, b6)
    b0 = b5
    b1 = b6
    b3 = b4
    b2 = aO
...
function tGMT.isEventPlayerTagEnabled()
    return b2
end
CreateThread(
    function()
            ...
            if b2 then
                ...
                for r, a5 in ipairs(currentEvent.players) do
                    ...
                    if aX ~= -1 and aX ~= PlayerId() then
                        ...
                        local b8 = HasEntityClearLosToEntity(T, b7, 17) or tGMT.isSpectatingEvent()
                        if b8 and IsEntityVisible(b7) and a5.active then
                            if not a5.tag or not IsMpGamerTagActive(a5.tag) then
                                ...
                                if b1 then
                                    ...
                                    b9 = CreateFakeMpGamerTag(b7, tGMT.getPlayerName(aX), false, false, "", 0)
                                end
                                if b0 then
...
function tGMT.setEventIntroMessage(ba, bb, bc, bd)
    EnableAllControlActions(0)
    local _ = GetSoundId()
    PlaySoundFrontend(_, "Frontend_Beast_Freeze_Screen", "FM_Events_Sasquatch_Sounds", false)
    ReleaseSoundId(_)
    if not HasStreamedTextureDictLoaded("gmtui") then
        RequestStreamedTextureDict("gmtui", false)
        while not HasStreamedTextureDictLoaded("gmtui") do
            Wait(0)
        end
    end
    tGMT.hideUI()
    ...
    if bc == 0 then
        ...
        while bg do
            ...
            DrawAdvancedTextNoOutline(
                ...
                35,
                255,
                tGMT.getFontId("Akrobat-ExtraBold"),
                0
            )
            DrawAdvancedTextNoOutline(0.488, 0.883 - bf + be, 0.005, 0.0028, 1.03, bb, 255, 255, 255, 255, 1, 0)
            DrawSprite("gmtui", "slanted_rect", 0.5, be, 1.0, 1.0, 0.0, 255, 255, 255, 180)
        end
    end
    tGMT.showUI()
end
