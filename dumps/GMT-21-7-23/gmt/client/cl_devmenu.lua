RMenu.Add(
    "devmenu",
    "main",
    RageUI.CreateMenu("GMT Dev Tools", "Developer Tools", tGMT.getRageUIMenuWidth(), tGMT.getRageUIMenuHeight())
)
RMenu.Add(
    "devmenu",
    "animations",
            ...
            RageUI.DrawContent(
                ...
                function()
                                ...
                                tGMT.clientPrompt(
                                    ...
                                    function(F)
                                        tGMT.spawnVehicle(
                                            ...
                                            E.y,
                                            E.z,
                                            GetEntityHeading(tGMT.getPlayerPed()),
                                            true,
                                            true,
                                            true
                                        )
                        ...
                        function(A, B, C)
                            if C then
                                ...
                                tGMT.clientPrompt(
                                    ...
                                    "",
                                    function(G)
                                        local H = tGMT.loadModel(tonumber(G))
                                        if H then
                                            local I =
                                                GetEntityCoords(PlayerPedId()) +
                                                GetEntityForwardVector(PlayerPedId()) * 2.0
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                local J = tGMT.loadModel("a_f_m_beach_01")
                                local K = tGMT.getPlayerCoords()
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                local J = tGMT.loadModel("s_m_m_pilot_02")
                                local K = tGMT.getPlayerCoords()
                                CreatePed(1, J, K.x, K.y, K.z, 0.0, true, true)
                                SetModelAsNoLongerNeeded(J)
                            end
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                ...
                                notify("~y~Triggered clear vehicles")
                            end
                        end
                    )
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                ...
                                notify("~y~Triggered clear objects")
                            end
                        end
                    )
                    RageUI.Button(
                                ...
                                if string.match(t, S[c]) then
                                    RageUI.ButtonWithStyle(
                                        ...
                                        function(A, B, C)
                                            if C then
                                                u(R)
                                                TaskPlayAnim(tGMT.getPlayerPed(), R, S[c], 1.0, -1, -1, 1, 0, 0, 0, 0)
                                                RemoveAnimDict(R)
                                            end
                                        end
                                    )
                    ...
                    for c = 1, #U.anims do
                        RageUI.ButtonWithStyle(
                            ...
                            function(A, B, C)
                                if C then
                                    u(U.dict)
                                    TaskPlayAnim(tGMT.getPlayerPed(), U.dict, U.anims[c], 1.0, -1, -1, 1, 0, 0, 0, 0)
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(A, B, C)
                                if C then
                                    ...
                                    RequestIpl(S[2])
                                    SetEntityCoords(tGMT.getPlayerPed(), tGMT.getPlayerCoords())
                                    local K = tGMT.getPlayerCoords()
                                    local W = GetInteriorAtCoords(K.x, K.y, K.z)
                                    tGMT.notify("[GMT] Ipl ~g~" .. S[1] .. "(" .. S[2] .. ")" .. " ~s~loaded")
                                end
                            end
                        )
                    end
...
function tGMT.gridPositionSaver(aa, L, M)
    tGMT.notify("pick 1st coord, press Y when done")
    ...
    Wait(250)
    tGMT.notify("pick 2nd coord, press Y when done")
    while not IsControlJustPressed(0, 246) do
        Wait(0)
    end
    local ac = GetEntityCoords(PlayerPedId())
    Wait(250)
    tGMT.notify("pick 3rd coord, press Y when done")
    while not IsControlJustPressed(0, 246) do
        Wait(0)
    end
    ...
    for c = 0, M - 1, 1 do
        for af = 0, L - 1, 1 do
            ...
            tGMT.loadModel("a_m_m_fatlatin_01")
            local aj = tGMT.spawnVehicle(aa, ai.x, ai.y, ai.z, heading, false, false, false)
    ...
    function()
        while true do
            ...
            if a then
                ...
                tGMT.DrawText(0.32, 0.00, "~r~X: ~s~" .. roundx, 0.4)
                tGMT.DrawText(0.38, 0.00, "~r~Y: ~s~" .. roundy, 0.4)
                tGMT.DrawText(0.445, 0.00, "~r~Z: ~s~" .. roundz, 0.4)
                heading = GetEntityHeading(ak)
                roundh = tonumber(string.format("%.2f", heading))
                tGMT.DrawText(0.50, 0.00, "~r~H:~s~ " .. roundh, 0.4)
                local al, am, an = table.unpack(GetEntityRotation(PlayerPedId(), 1))
                tGMT.DrawText(0.38, 0.03, "~r~RX: ~s~" .. tonumber(string.format("%.2f", al)), 0.4)
                tGMT.DrawText(0.44, 0.03, "~r~RY: ~s~" .. tonumber(string.format("%.2f", am)), 0.4)
                tGMT.DrawText(0.495, 0.03, "~r~RZ: ~s~" .. tonumber(string.format("%.2f", an)), 0.4)
                speed = GetEntitySpeed(PlayerPedId())
                rounds = tonumber(string.format("%.2f", speed))
                tGMT.DrawText(0.40, 0.92, "~r~Player Speed: ~s~" .. rounds, 0.4)
                health = GetEntityHealth(PlayerPedId())
                tGMT.DrawText(0.40, 0.95, "~r~Player Health: ~s~" .. health, 0.4)
                camRotX = GetGameplayCamRot(2).x
                tGMT.DrawText(0.36, 0.88, "~r~CR X: ~s~" .. tonumber(string.format("%.2f", camRotX)), 0.4)
                camRotY = GetGameplayCamRot(2).y
                tGMT.DrawText(0.44, 0.88, "~r~CR Y: ~s~" .. tonumber(string.format("%.2f", camRotY)), 0.4)
                camRotZ = GetGameplayCamRot(2).z
                tGMT.DrawText(0.51, 0.88, "~r~CR Z: ~s~" .. tonumber(string.format("%.2f", camRotZ)), 0.4)
                ...
                if IsPedInAnyVehicle(PlayerPedId(), 1) then
                    ...
                    tGMT.DrawText(0.015, 0.76, "~r~Engine Health: ~s~" .. vehenground, 0.4)
                    tGMT.DrawText(0.015, 0.73, "~r~Body Health: ~s~" .. vehbodround, 0.4)
                    tGMT.DrawText(
                        0.015,
                        0.70,
                        "~r~Vehicle Fuel: ~s~" ..
