...
function tGMT.isPlayerInTurf()
    for E, F in pairs(h) do
        if #(tGMT.getPlayerCoords() - F.position) < F.distance then
            return true
        end
    end
    return false
...
AddEventHandler("GMT:onClientSpawn",function(v, w)
	if w then
		...
		local O = function(M)
			if IsControlJustPressed(1, 51) then
				if not PlayerIsInGang then
					tGMT.notify("~r~You need to be in a gang to capture a turf!")
				elseif not f then
					TriggerServerEvent("GMT:refreshTurfOwnershipData")
		...
		for B, P in pairs(h) do
			tGMT.createArea("turf_" .. B, P.position, 1.5, 6, L, N, O, {turfId = B})
		end
		local Q = function(M)
						...
						if T ~= nil and tonumber(T) then
							...
							if T <= 25 and T >= 0 then
								...
								if GetGameTimer() > K + 30000 then
									...
									tGMT.notify("Rate limit, please wait 30s to change the commission.")
								end
							end
						end
					end
		...
		for B, G in pairs(i) do
			tGMT.createArea("turfmanager_" .. B, G.position, 1.5, 6, Q, R, S, {turfId = B, draw = false})
		end
	end
end)
RegisterNetEvent("GMT:captureOwnershipReturned",function(B, U, V)
	if not U then
		...
		tGMT.notify("Turf already owned!")
	end
end)
function otherMembersStartGangCapture(x, y, W)
    if b[y] then
        ...
        Citizen.CreateThread(function()
			...
			while f do
				local X = GetEntityCoords(tGMT.getPlayerPed(), false)
				local Y = #(X - W)
				if Y >= 37.5 then
					tGMT.notify("Turf capture cancelled, you got too far!")
					f = false
					TriggerServerEvent("GMT:gangDefenseLocationUpdate", y, "Attackers", true)
				elseif GetEntityHealth(tGMT.getPlayerPed()) <= 102 then
					tGMT.notify("Turf capture failed, you died")
					f = false
					TriggerServerEvent("GMT:gangDefenseLocationUpdate", y, "Attackers", true)
				end
				Wait(1000)
...
function startGangCapture(x, W, y)
    if turfData[x].owned then
        tGMT.notify("You already own this turf!")
...
function defendGangCapture(x, a, Z, _, a0)
    ...
    Citizen.CreateThread(function()
		...
		while e do
			...
			if Y < 37.5 and GetEntityHealth(tGMT.getPlayerPed()) > 102 then
			...
			elseif GetEntityHealth(tGMT.getPlayerPed()) <= 102 then
				tGMT.notify("Turf defense failed, you died")
				e = false
				TriggerServerEvent("GMT:gangDefenseLocationUpdate", a, "Defenders", false)
			else
				TriggerServerEvent("GMT:gangDefenseLocationUpdate", a, "Defenders", false)
