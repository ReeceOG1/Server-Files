    ...
    function()
        while true do
                ...
                if IsControlJustPressed(0, 38) then
                    if not globalOnPoliceDuty then
                        if not g then
                            ...
                            tGMT.notify("~r~Already joined the Organ Heist!")
                        end
                    else
                        tGMT.notify("~r~You are police, please use the other entrance!")
                    end
                end
            elseif e then
            ...
            if inGamePhase or inWaitingStage then
                ...
                RemoveWeaponFromPed(tGMT.getPlayerPed(), "WEAPON_SMOKEGRENADEGMT")
                RemoveWeaponFromPed(tGMT.getPlayerPed(), "WEAPON_SMOKEGRENADEGMTPD")
...
RegisterNetEvent(
    ...
    function(B)
        ...
        tGMT.notify("~g~Killed " .. B .. " received £50,000 and 25% Armour")
    end
)
Citizen.CreateThread(
    function()
        while true do
            if inWaitingStage then
                ...
                if #(s - k.pastGate) < 3.0 then
                    ...
                    NetworkFadeInEntity(PlayerPedId(), 0)
                    DoScreenFadeIn(1000)
                    tGMT.notify("~r~You got too far from the organ heist and have been teleported back.")
                end
            end
            if inGamePhase then
                local s = GetEntityCoords(PlayerPedId())
