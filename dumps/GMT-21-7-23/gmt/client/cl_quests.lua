...
local c = nil
local function d()
    tGMT.loadAnimDict("anim@mp_player_intcelebrationfemale@air_guitar")
    TaskPlayAnim(
        PlayerPedId(),
...
local function k(l, m, n, o)
    ...
    tGMT.createArea("quests_prop_" .. p, q, 100.0, 6, r, t, u)
    local v = function()
        tGMT.drawFloatingHelpText("Press [E] to collect.", vector3(q.x, q.y, q.z + 0.2))
        if IsControlJustPressed(0, 38) then
            if l == "CHRISTMAS" then
                SendNUIMessage({transactionType = "christmas_quest"})
            end
            DeleteEntity(b[p])
            TriggerServerEvent("GMT:setQuestCompleted", l, m)
            d()
        end
    end
    tGMT.createArea(
...
RegisterNetEvent(
    "GMT:questSendCompletedPositions",
    function(A, B)
        c = B
...
local function C(l, m)
    local p = l .. "_" .. tostring(m)
    tGMT.removeArea("quests_prop_" .. p)
    tGMT.removeArea("quests_text_" .. p)
end
RegisterNetEvent(
    "GMT:questCollected",
    function(D, l, m, E)
        C(l, m)
        if D then
            e("~g~+14 Days GMT Club Platinum", "You collected them all!", 600)
