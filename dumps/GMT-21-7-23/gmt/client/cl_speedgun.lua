    ...
    if tGMT.globalOnPoliceDuty() then 
        if m[1]~=nil then 
            if tonumber(m[1])>100 then 
                d=tonumber(m[1])
                tGMT.notify(string.format("~g~Maximum speed set to %smph",tonumber(m[1])))
            else 
                d=101
                tGMT.notify("~r~Minimum speed you can set is 101mph!")
            end 
        end 
    else 
        tGMT.notify("~r~Speed gun is not enabled!")
    end 
end)
...
tGMT.createThreadOnTick(func_drawRadar)
Citizen.CreateThread(function()
    while true do 
        if tGMT.globalOnPoliceDuty() and not usingDelgun then
            if GetSelectedPedWeapon(tGMT.getPlayerPed())==a then 
                b=true 
            else 
                b=false 
            end
            if b and tGMT.getPlayerVehicle()==0 then 
                local n=tGMT.getPlayerId()
                if IsPlayerFreeAiming(n)then 
                    ...
                    if p~=0 and GetPedInVehicleSeat(p,-1)==e and not IsPedInAnyPlane(e) and not IsPedInAnyHeli(e) then 
                        ...
                        if k[f]~=nil then 
                            ...
                            tGMT.notify(string.format("~h~Vehicle Flagged:~s~~n~Plate %s is flagged for:~n~%s",f,r))
                            soundPlayed=true
                        ...
                        if h>d and h>101 then 
                            ...
                            if not j[q]then 
                                ...
                                tGMT.notify("~r~This vehicle has been fined recently!")
                            end 
                            else i="~w~"
                        end 
                    end 
