...
local n = m["White"]
local o = GetResourceKvpString("gmt_gang_colour") or "White"
local function p()
...
AddEventHandler(
    ...
    function(q, r, k, s, t, u, v)
        if q == nil then
            ...
            PlayerIsInGang = true
            TriggerServerEvent("GMT:setPersonalGangBlipColour", o)
            e = {}
            gangLogs = {}
            h = 1
            i = 1
...
AddEventHandler(
    "GMT:InviteReceived",
    function(y, z)
        GMTGangInvites[GMTGangInviteIndex] = z
...
function func_drawGangUI()
    if d == "none" then
        ...
        for A, B in pairs(GMTGangInvites) do
        ...
        if CursorInArea(0.35, 0.415, 0.415, 0.46) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if createGangName ~= nil and createGangName ~= "null" and createGangName ~= "" then
                    ...
                    tGMT.notify("~r~No gang name entered!")
                end
            end
        else
            CreateGangSelectionRed = 0
        ...
        if CursorInArea(0.435, 0.51, 0.415, 0.46) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if selectedGangInvite ~= nil then
                    ...
                    tGMT.notify("~r~No gang invite selected")
                end
            end
        else
            JoinGangSelectionRed = 0
       ...
       -- DrawAdvancedText(0.590, 0.660, 0.005, 0.0028, 0.4, "View Contributions", 255, 255, 255, 255, 6, 0)
        ...
        if CursorInArea(0.3083, 0.3718, 0.5490, 0.5999) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount ~= nil then
                    ...
                    tGMT.notify("~r~No amount entered!")
                end
            end
        else
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount ~= nil then
                    if gangPermission >= 3 then
                        TriggerServerEvent("GMT:withdrawGangBalance", gangID, amount)
                    else
                        tGMT.notify("~r~You don't have a high enough rank to withdraw")
                    end
                else
                    tGMT.notify("~r~No amount entered!")
                end
        ...
        if CursorInArea(0.6088, 0.6739, 0.5481, 0.6018) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if gangPermission >= 3 then
                    TriggerServerEvent("GMT:withdrawAllGangBalance", gangID)
                else
                    tGMT.notify("~r~You don't have a high enough rank to withdraw")
                end
            end
    ...
    if d == "contributions" then
        ...
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "GMT Gang - Contributions", 255, 255, 255, 255, 7, 0)
    ...
    if d == "members" then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if selectedMember ~= nil and PlayerIsInGang and gangID ~= nil then
                    if gangPermission >= 4 then
                        ...
                        tGMT.notify("~r~You don't have permission to promote!")
                    end
                else
                    tGMT.notify("~r~No gang member selected")
                end
            end
        else
        ...
        if CursorInArea(0.6182, 0.6822, 0.435, 0.491) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if selectedMember ~= nil and PlayerIsInGang and gangID ~= nil then
                    if gangPermission >= 4 then
                        if selectedMember == tGMT.getUserId() then
                            tGMT.notify("~r~You can't demote yourself.")
                        else
                            TriggerServerEvent("GMT:DemoteUser", gangID, selectedMember)
                        end
                    else
                        tGMT.notify("~r~You don't have permission to demote!")
                    end
                else
                    tGMT.notify("~r~No gang member selected")
                end
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if selectedMember ~= nil then
                    if gangPermission >= 3 then
                        ...
                        tGMT.notify("~r~You don't have permission to kick!")
                    end
                else
                    tGMT.notify("~r~No gang member selected")
                end
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if D ~= nil then
                    if gangPermission >= 2 then
                        TriggerServerEvent("GMT:InviteUserToGang", gangID, D)
                    else
                        tGMT.notify("~r~You don't have permission to invite players")
                    end
                else
                    tGMT.notify("No player name entered")
                end
            end
        else
    ...
    if d == "settings" then
        ...
        if CursorInArea(0.3187, 0.3937, 0.3712, 0.4462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                k.blips = not k.blips
                SetResourceKvp("gmt_gang_blips", tostring(k.blips))
                TriggerEvent('GMT:ForceRefreshData', -1)
            end
        else
        ...
        if CursorInArea(0.4197, 0.4932, 0.3712, 0.4462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                k.pings = not k.pings
                SetResourceKvp("gmt_gang_pings", tostring(k.pings))
            end
        else
        ...
        if CursorInArea(0.3187, 0.3937, 0.4712, 0.5462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                k.names = not k.names
                SetResourceKvp("gmt_gang_names", tostring(k.names))
                --print("Names", k.names)
            end
        else
        ...
        if CursorInArea(0.4197, 0.4932, 0.5712, 0.6462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if gangPermission >= 4 then
                    ...
                    tGMT.notify("~r~You don't have permission to disband!")
                end
            end
        else
        ...
        if CursorInArea(0.31, 0.39, 0.6712, 0.7064) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                SetResourceKvp("gmt_gang_healthui", tostring(k.healthui))
            end
        else
        ...
        if CursorInArea(0.41, 0.49, 0.6712, 0.7064) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if gangPermission >= 4 then
                    ...
                    tGMT.notify("~r~You don't have permission to set gang fit!")
                end
            end
        else
        ...
        if CursorInArea(0.595, 0.645, 0.6155, 0.6405) then
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                SetResourceKvp("gmt_gang_colour", o)
    ...
    if d == "security" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, k.theme.r, k.theme.g, k.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "GMT gang - security", 255, 255, 255, 255, 7, 0)
        ...
        DrawAdvancedText(0.575, 0.575, 0.005, 0.0028, 0.46, z and "Yes" or "No", 255, 255, 255, 255, 6, 1)
    ...
    if d == "theme" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, k.theme.r, k.theme.g, k.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "GMT Gang - Theme", 255, 255, 255, 255, 7, 0)
        DrawAdvancedText(
            0.7,
            0.360,
            0.005,
        ...
        if CursorInArea(0.5187, 0.5828, 0.4138, 0.4694) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                tGMT.CopyToClipboard(k.theme.r .. "," .. k.theme.g .. "," .. k.theme.b)
                tGMT.notify("~g~Theme copied to clipboard.")
            end
        else
        ...
        if CursorInArea(0.5187, 0.5828, 0.5407, 0.5962) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                tGMT.gangMenuTheme(math.random(1, 255), math.random(1, 255), math.random(1, 255))
            end
        else
                    ...
                    function(O)
                        if O ~= "" then
                            ...
                            if P[1] ~= nil and P[2] ~= nil and P[3] ~= nil then
                                tGMT.gangMenuTheme(tonumber(P[1]), tonumber(P[2]), tonumber(P[3]))
                            else
...
function GetPlayerPermID()
    AddTextEntry("FMMC_MPM_NA", "Enter the players Perm ID you want to invite:")
    DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "", "", "", "", "", 30)
    while UpdateOnscreenKeyboard() == 0 do
        DisableAllControlActions(0)
        Wait(0)
...
RegisterNetEvent(
    ...
    function(Q, M)
        for k, n in pairs(GMTGangMembers) do
            if n[2] == Q then
                GMTGangMembers[k].colour = M
            end
        end
    end
)
...
--     T = T:reverse():gsub("(%d%d%d)", "%1,") -- Breaks shit
--     return S .. T:reverse():gsub("^,", "") .. U
-- end
function tGMT.gangMenuTheme(V, W, X)
    if V ~= nil and W ~= nil and X ~= nil then
        k.theme = {r = V, g = W, b = X}
        SetResourceKvp("gmt_gang_theme", json.encode(k.theme))
    else
        tGMT.notify("~r~Invalid value")
    end
end
function tGMT.hasGangNamesEnabled()
    local P = p()
    return P and k.names
end


function tGMT.isPlayerInSelectedGang(Y)
    local P = p()
    if P then
        local Q = tGMT.clientGetUserIdFromSource(Y)
        if Q and tGMT.getJobType(Q) == "" then
            for a, b in pairs(GMTGangMembers) do
                if Q == b[2] then
...
function tGMT.hasGangBlipsEnabled()
...
RegisterCommand(
    "drawmarker",
    function()
       -- if k.pings and not tGMT.isEmergencyService() and not globalHideUi and not tGMT.inEvent() then
            local _ = GetGameplayCamCoord()
            local a0 = tGMT.rotationToDirection(GetGameplayCamRot(2))
            ...
            if c ~= vector3(0.0, 0.0, 0.0) then
                TriggerServerEvent("GMT:sendGangMarker", gangID, c)
            end
       -- end
    end
)
...
local function a2(a3)
    if tGMT.getGangPingMarkerIndex() == 2 and not tGMT.isEmergencyService() and not tGMT.inEvent() then
    ...
    function(a3, a4)
        if k.pings and not tGMT.isEmergencyService() and not globalHideUi and not tGMT.inEvent() then
            ...
            if a5 < 1000.0 then
                ...
                if a4 then
                    Z[a3] = {coords = a4, time = GetGameTimer(), dist = a5, creator = a3, checkpoint = a2(a4)}
                    if tGMT.getGangPingSound() > 1 then
                        SendNUIMessage(
                            {
                                transactionType = "gangping" .. tGMT.getGangPingSound() - 1,
                                volume = tGMT.getGangPingVolume() * 0.1
                            }
                        )
                    end
                else
    ...
    function()
        while true do
            if k ~= nil and k.pings and PlayerIsInGang then
                for a, b in pairs(Z) do
                    if Z[a] ~= nil then
                        if tGMT.getGangPingMarkerIndex() == 3 then
                            tGMT.DrawSprite3d(
                                {
                                    pos = b.coords + vector3(0.0, 0.0, b.dist / 100),
                                    textureDict = "gmt_gang",
                                    textureName = "ping",
                                    width = 0.06,
                                    height = 0.1,
                                    r = k.theme.r,
                        ...
                        tGMT.DrawText3D(b.coords, b.creator .. "\n" .. tostring(math.floor(b.dist)) .. "m", 0.2)
                    end
                end
            end
            Citizen.Wait(0)
...
local function an()
    ...
    if k ~= nil and k.healthui and PlayerIsInGang then
       if P and not tGMT.isEmergencyService() and not globalHideUi and not tGMT.inEvent() then
            local ap = 0
            local aq = tGMT.getShowHealthPercentageFlag()
            if GMTGangMembers ~= nil then
                for ar, as in pairs(GMTGangMembers) do
                ...
                --   if lastseen == "~g~Online" and tGMT.getJobType(id) == "" then
                        local at = true
                        local au = nil
                        local av = nil
                        local ad = ac[id]
                        ...
                        local aw = tGMT.getTempFromPerm(id)
                        if aw then
...
tGMT.createThreadOnTick(an)
function tGMT.setGangUIXPos(ax)
    local ay = tonumber(string.match(ax, "%d+")) / 100
    ae = 0.008 * ay
end
function tGMT.setGangUIYPos(ax)
    local ay = tonumber(string.match(ax, "%d+")) / 100
    af = 0.35 * ay
end
