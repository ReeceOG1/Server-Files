...
RegisterNetEvent("GMT:doAcid",function()
    ...
    local r = tGMT.loadModel("a_c_hen")
    RequestAnimDict(p)
    while not HasAnimDictLoaded(p) do
        Wait(0)
    end
    TaskPlayAnim(tGMT.getPlayerPed(), p, q, 2.0, 2.0, -1, 51, 0, false, false, false)
    RemoveAnimDict(p)
...
DoAcid = function(x)
    ...
    while GetGameTimer() - A < x do
        local D = GetEntityCoords(tGMT.getPlayerPed())
        ...
        if E > 1.0 then
            ...
            if z == 5 then
                z = 0
                local F = C - D
                local G = GetEntityVelocity(tGMT.getPlayerPed())
                SetEntityCoordsNoOffset(tGMT.getPlayerPed(), D.x + F.x, D.y + F.y, D.z)
                ForcePedMotionState(tGMT.getPlayerPed(), -1115154469, 1, 1, 0)
                SetEntityVelocity(tGMT.getPlayerPed(), G.x, G.y, G.z)
            end
            C = GetEntityCoords(tGMT.getPlayerPed())
        end
        DrawToons()
        DrawCubes()
        if e and not f then
...
InitPed = function()
    ...
    while GetScreenCoordFromWorldCoord(L.x, L.y, L.z) and not IsPointOnRoad(L.x, L.y, L.z) do
        J = math.random(0, 359)
        K = math.random(50, 80)
        L = GetEntityCoords(tGMT.getPlayerPed()) + PointOnSphere(0.0, J)
        Citizen.Wait(0)
    end
...
TrackEnt = function()
    while true do
        local E = GetVecDist(GetEntityCoords(tGMT.getPlayerPed()), GetEntityCoords(g))
        if E > 5.0 then
            TaskGoToEntity(g, tGMT.getPlayerPed(), -1, 4.0, 100.0, 1073741824, 0)
            Wait(1000)
        else
            ...
            if not d or GetGameTimer() - d > 1000 then
                d = GetGameTimer()
                TaskTurnPedToFaceEntity(g, tGMT.getPlayerPed(), -1)
            end
...
DrawCubes = function()
    local X = GetEntityCoords(tGMT.getPlayerPed())
    local T = 10
    for U, V in pairs(b) do
        local Y = 0.1
        local Z = 0.1
...
RegisterNetEvent("GMT:cocaineEffect",function()
    if ae then
        ...
        while t < 10 do
            tGMT.varyHealth(-10)
            tGMT.notify("~y~I don't feel too good..")
            Wait(5000)
            t = t + 1
        end
...
RegisterNetEvent("GMT:heroinEffect",function()
    ...
    tGMT.enablePunching(true)
    Wait(30000)
    ClearTimecycleModifier()
    tGMT.enablePunching(false)
