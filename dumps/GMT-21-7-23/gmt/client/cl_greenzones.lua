...
local l = {
    ...
    --     colour = 2,
    --     id = 1,
    --     pos = vector3(235.7520904541,-1374.2039794922,46.07243347168),
    --     dist = 40,
    --     nonRP = false,
    --     setBit = false,
    --     maxHeight = 210.0
...
local m = {
    ...
    --{vector3(235.7520904541,-1374.2039794922,46.07243347168),40.0, 2, 180}
}
...
function tGMT.areGreenzonesDisabled()
    return i
end
function tGMT.setGreenzonesDisabled(p)
    i = p
end
n(
    function()
        for q, r in pairs(m) do
            if not r[5] and not tGMT.isPurge() or r[5] and tGMT.isPurge() then
...
n(
    function()
        local purgeActive = tGMT.isPurge()
        while true do
            local t = tGMT.getPlayerPed()
            local u = tGMT.getPlayerCoords()
            for v, w in pairs(l) do
                if not w.isPurge and not purgeActive or w.isPurge and purgeActive then
                    ...
                    SetPlayerInvincible(tGMT.getPlayerId(), false)
                    ...
                    SetNetworkVehicleAsGhost(tGMT.getPlayerVehicle(), false)
                end
            end
            o(250)
        end
...
AddEventHandler("GMT:onClientSpawn",function(y, z)
    if z then
        ...
        tGMT.createArea("cityzone",vector3(-225.30703735352, -916.74755859375, 31.216938018799),750.0,100,A,C,D,{})
    end
end)
local function E()
    local F = tGMT.getPlayerCoords()
    local G = nil
    for H = 1, 25 do
    ...
    if G then
        local K, L = tGMT.getPlayerVehicle()
        if K ~= 0 then
    ...
    while true do
        ...
        if not tGMT.areGreenzonesDisabled() then
            ...
            if b or c then
                ...
                ClearPedBloodDamage(t)
                if usingDelgun then
                    tGMT.setWeapon(t, "WEAPON_STAFFGUN", true)
                else
                    tGMT.setWeapon(t, "WEAPON_UNARMED", true)
                end
                ResetPedVisibleDamage(t)
                ClearPedLastWeaponDamage(t)
            ...
            if b then
                ...
                local U, V = tGMT.getPlayerCombatTimer()
                if U > 0 and V then
                    E()
                end
            end
...
RegisterCommand('togglecitycap', function()
    if tGMT.getUserId() == 1 then
        cityCap = not cityCap
        tGMT.notify('City Cap: '..(cityCap and "~g~Enabled" or "~r~Disabled"))
    end
end)