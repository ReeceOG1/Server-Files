...
Citizen.CreateThread(function()
    ...
    userdata = json.decode(GetResourceKvpString("gmt_userdata") or "{}")
    if type(userdata) ~= "table" then
      userdata = {}
        print("[GMT] Loading cached user data - failed to load setting to default.")
    else
        print("[GMT] Loading cached user data - loaded.")
    end
end)
function tGMT.updateCustomization(b)
    local c = tGMT.getCustomization()
    if c.modelhash ~= 0 and IsModelValid(c.modelhash) then
      userdata.customisation = c
      if b then
        SetResourceKvp("gmt_userdata", json.encode(userdata))
      end
    end
end
function tGMT.updateHealth(b)
  userdata.health = GetEntityHealth(PlayerPedId())
  if b then
      SetResourceKvp("gmt_userdata", json.encode(userdata))
  end
end
function tGMT.updateArmour(b)
...
Citizen.CreateThread(function()
    ...
    while true do
        Wait(5000)
        if not tGMT.isInHouse() and not inOrganHeist and not tGMT.isPlayerInRedZone() and not tGMT.isInSpectate() then
          tGMT.updatePos()
        end
        if not tGMT.isStaffedOn() and not customizationSaveDisabled and not spawning and not tGMT.isPlayerInAnimalForm() then
            tGMT.updateCustomization()
        end
        tGMT.updateHealth()
        tGMT.updateArmour()
        SetResourceKvp("gmt_userdata", json.encode(userdata))
...
function tGMT.checkCustomization()
    ...
    if c == nil or c.modelhash == 0 or not IsModelValid(c.modelhash) then
        ...
        tGMT.setCustomization(c, true, true)
    end
end
...
function tGMT.spawnAnim(a)
  if a ~= nil then
    ...
    print("[GMT] cachedUserData.health", userdata.health)
    SetEntityHealth(PlayerPedId(), userdata.health or 200)
    SetEntityVisible(PlayerPedId(), true, false)
    FreezeEntityPosition(PlayerPedId(), false)
    if not tGMT.isDevMode() then
        Citizen.Wait(2000)
    end
    tGMT.showUI()
...
AddEventHandler("GMT:playGTAIntro",function()
  if not tGMT.isDev() then
    TriggerEvent("gmt:PlaySound", "gtaloadin")
  end
end)

RegisterNetEvent("GMT:setHairstyle")
AddEventHandler("GMT:setHairstyle",function(q)
  if q then
    dad = q["dad"] or 0
    mum = q["mum"] or 0
    skin = q["skin"] or 0
