...
local function u()
    ...
    if v ~= 1 then
        ...
        local z = tGMT.rotationToDirection(x)
...
local function G(H)
    ...
    if noclipActive and tGMT.getStaffLevel() == 0 then
        return
    end
    inventoryType = nil
    D = true
    ...
    ForcePedAiAndAnimationUpdate(t, false, false)
    if not tGMT.inEvent() and tGMT.globalOnPoliceDuty() then
...
local function ah(ai)
    ...
    if aj ~= 0 then
        return tGMT.notify("~r~Can not repair vehicle with a person in the driver seat.")
    end
    if NetworkGetEntityIsNetworked(ai) then
        local ak = NetworkGetNetworkIdFromEntity(ai)
        if ak ~= 0 then
            TriggerServerEvent("GMT:attemptRepairVehicle", ai, ak)
        end
    end
end
local al = false
...
local function aF(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("GMT:requestTransport", as)
        end
    end
end
local function aG(ab)
...
RegisterNetEvent("GMT:globalOnPoliceDuty")
AddEventHandler(
    "GMT:globalOnPoliceDuty",
    function(aK)
        f = aK
    end
)
RegisterNetEvent("GMT:globalOnNHSDuty")
AddEventHandler(
    "GMT:globalOnNHSDuty",
    function(aK)
        g = aK
...
function tGMT.getSelectedEntity()
    return b, c
end
AddEventHandler(
    "GMT:showRadioWheel",
...
RegisterCommand(
    "lootclosestbag",
    function()
        if not playerIsAlive() or tGMT.getPlayerVehicle() ~= 0 or GetRenderingCam() ~= -1 then
            return
        end
        local t = tGMT.getPlayerPed()
        local aO = tGMT.getPlayerCoords()
        local aP = 0
        local aQ = 3.0
