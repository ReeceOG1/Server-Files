...
RMenu.Add("emotesmenu","mainmenu",RageUI.CreateMenu("GMT Emotes","Main Menu",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
RMenu.Add("emotesmenu","emotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"GMT Emotes","Emotes",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
RMenu.Add("emotesmenu","searchemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"GMT Emotes","Search Emotes",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
RMenu.Add("emotesmenu","danceemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"GMT Emotes","Dance Emotes",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
RMenu.Add("emotesmenu","customemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"GMT Emotes","Custom Emotes",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
RMenu.Add("emotesmenu","gunemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"GMT Emotes","Gun Emotes",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
RMenu.Add("emotesmenu","propemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"GMT Emotes","Prop Emotes",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
RMenu.Add("emotesmenu","sharedemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"GMT Emotes","Shared Emotes",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
RMenu.Add("emotesmenu","walkingstyles",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"GMT Emotes","Walking Styles",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
RMenu.Add("emotesmenu","moods",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"GMT Emotes","Moods",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
local b = false
local c = ""
local d = {}
local e = 0
...
local function p(q)
    ...
    if not q and GetGameTimer() - e < 1500 then
        tGMT.notify("~r~Emotes are being rate limited.")
        return
    end
    b = false
    f = nil
...
local function F(E)
    if tGMT.getPlayerVehicle() ~= 0 then
        tGMT.notify("~r~You cannot use scenarios whilst in a vehicle.")
        return
    end
    local A = PlayerPedId()
    ...
    elseif E[1] == "MaleScenario" then
        if tGMT.getModelGender() == "male" then
            ...
            tGMT.notify("This scenario is male only.")
        end
...
local function a6(E)
    if not tGMT.canAnim() or tGMT.isHandcuffed() then
        return true
    end
    local A = PlayerPedId()
    ...
    local a9 = tGMT.getPlayerVehicle()
...
local function a7(E)
    if a6(E) then
        tGMT.notify("~r~Can not this emote at this time.")
        return
    end
...
local function ag(ah)
    if a6() then
        tGMT.notify("~r~Can not use shared emotes at this time.")
        return
    end
    local ai = a8()
    if ai ~= 0 then
        i = ah
        j = ai
        TriggerServerEvent("GMT:sendSharedEmoteRequest", ai, ah)
    else
        tGMT.notify("~r~No player is near by.")
    end
end
local function aj(E)
    Citizen.CreateThreadNow(function()
        local ak = E[1]
        tGMT.loadAnimDict(ak)
    ...
    if RageUI.Visible(RMenu:Get('emotesmenu', 'searchemotes')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if foundMatch == false then
                emoteString = tGMT.KeyboardInput("Enter Emote Name", "", 10)
...
local function aF(aC, aD, aE)
    if #aD < 1 then
        tGMT.notify("~r~No emote name was specified.")
        return
    end
    ...
    elseif a.guns[ah] then
        ...
        tGMT.notify("~r~Invalid emote name was specified.")
    end
end
...
RegisterCommand("walk",function(aC, aD, aE)
    if #aD < 1 then
        tGMT.notify("No walk name was specified.")
        return
    end
    local ah = aD[1]
    if not ah then
    ...
    if a.walks[ah] then
        ...
        tGMT.notify("~r~Invalid walk name was specified.")
    end
end)
RegisterNetEvent("GMT:sendSharedEmoteRequest",function(aG, ah)
    if a.shared[ah] and not a6() then
        k = ah
        l = aG
        m = GetGameTimer()
        tGMT.notify("~y~Y~w~ to accept, ~r~L~w~ to refuse (~g~" .. a.shared[ah][3] .. "~w~)")
    end
end)
RegisterNetEvent("GMT:receiveSharedEmoteRequest",function(ah)
    p(false)
    Citizen.Wait(300)
    a7(a.shared[ah])
end)
RegisterNetEvent("GMT:receiveSharedEmoteRequestSource",function()
...
local function aT()
    if k then
        ...
        elseif IsControlJustPressed(1, 246) then
            if a6() then
                tGMT.notify("You can not use emotes at this time.")
            else
                TriggerServerEvent("GMT:receiveSharedEmoteRequest", l, a.shared[k][4])
            end
            aK()
        end
    end
    if b and f and g then
        ...
        if not IsEntityPlayingAnim(PlayerPedId(), f, g, 3) or tGMT.getPlayerCombatTimer() > 0 then
            p(true)
        end
    end
end
function tGMT.playEmote(ah)
    if a.emotes[ah] then
        a7(a.emotes[ah])
    elseif a.dances[ah] then
        a7(a.dances[ah])
