...
RMenu.Add(
    "rpzones",
    "mainmenu",
    RageUI.CreateMenu("", "~b~Main Menu", tGMT.getRageUIMenuWidth(), tGMT.getRageUIMenuHeight(), "banners", "admin")
)
RMenu.Add(
    "rpzones",
    "weapons",
...
local function I(g)
    local n, x = GetCurrentPedWeapon(tGMT.getPlayerPed())
    if x ~= 0 and x ~= GetHashKey("WEAPON_STAFFGUN") then
        w(g, x)
    end
    local A, J = tGMT.getPlayerVehicle()
    if A ~= 0 and J then
        C(g, A)
    end
