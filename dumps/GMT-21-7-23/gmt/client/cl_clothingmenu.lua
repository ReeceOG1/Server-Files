RMenu.Add('GMTclothing','mainMenu',RageUI.CreateMenu("","Clothing Store",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"gmt_clothesui","gmt_clothesui"))
RMenu.Add('GMTclothing','changePed',RageUI.CreateSubMenu(RMenu:Get('GMTclothing','mainMenu'),"","Change Gender"))
RMenu.Add('GMTclothing','changeClothing',RageUI.CreateSubMenu(RMenu:Get('GMTclothing','mainMenu'),"","Change your Clothes"))
RMenu.Add('GMTclothing','clearProps',RageUI.CreateSubMenu(RMenu:Get('GMTclothing','mainMenu'),"","Clear Props"))
...
Citizen.CreateThread(function()
    while true do 
        local G=GetEntityCoords(tGMT.getPlayerPed())
        for H=1,#r do 
        ...
        if RageUI.Visible(RMenu:Get('GMTclothing', 'mainMenu')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                if PlayerIsInGang then
                    RageUI.Button("Apply Gang Outfit","Applys the outfit set by the Gang Leader.",{RightLabel = "→→→"},function(L, M, N)
                        if N then
                            TriggerServerEvent("GMT:applyGangFit", gangname)
                        end
                    end)
                end
        ...
        if RageUI.Visible(RMenu:Get('GMTclothing', 'changePed')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                RageUI.Button('MP Male','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            tGMT.loadCustomisationPreset("DefaultMale")
                        else
                            tGMT.notify("~r~Cannot change gender, you're dead.")
                        end
                    end 
                end,function()
                end)
                RageUI.Button('MP Female','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            tGMT.loadCustomisationPreset("DefaultFemale")
                        else
                            tGMT.notify("~r~Cannot change gender, you're dead.")
                        end
                    end 
                        ...
                        RageUI.List(P.name,get_drawables(P.type,P.index),a[H].currentListIndex,"Current Texture Index: "..k.textureN.."/"..j-1,{},true,function(L,M,N,Q)
                            ...
                            if N then 
                                ...
                                if k.index==2 then 
                                    ...
                                    TriggerServerEvent("GMT:saveClothingHairData",GetPedDrawableVariation(tGMT.getPlayerPed(),2),GetPedHairColor(tGMT.getPlayerPed()))
...
function current_clothing(R,i,S)
    if R=="prop"then 
        if S then 
            if GetPedPropTextureIndex(tGMT.getPlayerPed(),i)~=nil then 
                return GetPedPropTextureIndex(tGMT.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedPropIndex(tGMT.getPlayerPed(),i)~=nil then 
                if GetPedPropIndex(tGMT.getPlayerPed(),i)==-1 then 
                    return GetPedPropIndex(tGMT.getPlayerPed(),i)+2 
                else 
                    return GetPedPropIndex(tGMT.getPlayerPed(),i)+1 
                end 
            else 
                return 0 
            end 
    ...
    elseif R=="drawable"then 
        ...
        elseif S then 
            if GetPedTextureVariation(tGMT.getPlayerPed(),i)~=nil then 
                return GetPedTextureVariation(tGMT.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedDrawableVariation(tGMT.getPlayerPed(),i)~=nil then 
                return GetPedDrawableVariation(tGMT.getPlayerPed(),i)+1 
            else 
                return 0 
            end 
        end 
...
function get_textures(T,R,i)
    if T=="drawable"then 
        ...
        j=GetNumberOfPedTextureVariations(tGMT.getPlayerPed(),R,i-1)
        return j 
    else 
        j=GetNumberOfPedPropTextureVariations(tGMT.getPlayerPed(),R,i-1)
        return j 
    end 
end
local function Y()
