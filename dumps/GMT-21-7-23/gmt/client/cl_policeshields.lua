    ...
    while true do
        if a.active and a.id ~= 0 then
            ...
            if h ~= GetHashKey("WEAPON_UNARMED") and (not b[a.id].allowPistol or not table.has(c, h)) then
                tGMT.setWeapon(g, "WEAPON_UNARMED", true)
            end
...
RMenu.Add("policeshields","main",RageUI.CreateMenu("Police Shields", "Shield Management", tGMT.getRageUIMenuWidth(), tGMT.getRageUIMenuHeight()))
...
function removeShield(m)
    if DoesEntityExist(a.prop) then
        ...
        if m then
            tGMT.notify("~g~Success~w~: Shield removed")
        end
    end
end
function createShield(n)
    ...
    local o = false
    local g = tGMT.getPlayerPed()
    local p = tGMT.getPlayerCoords()
    if a.active then
        removeShield(false)
    end
    a.id = n
    local q = tGMT.loadModel(b[n].model)
    ...
    tGMT.loadAnimDict(b[n].animDict)
