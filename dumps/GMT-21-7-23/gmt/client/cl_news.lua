...
function createNewsItemInitally(i)
    ...
    tGMT.loadAnimDict(l.animations.dict)
...
function removeNewsItem(i)
    if a.items[i].holding then
        ...
        DetachEntity(tGMT.getObjectId(a.items[i].netId, "removeNewsItem (1)"), true, true)
        DeleteEntity(tGMT.getObjectId(a.items[i].netId, "removeNewsItem (2)"))
        a.items[i].netId = nil
            ...
            for i, s in pairs(a.items) do
                if a.items[i].holding then
                    if not IsEntityPlayingAnim(j, a.items[i].animations.dict, a.items[i].animations.name, 3) then
                        tGMT.loadAnimDict(a.items[i].animations.dict)
                        TaskPlayAnim(j, 1.0, -1, -1, 50, 0, 0, 0, 0, false, false)
                        TaskPlayAnim(
                            j,
...
Citizen.CreateThread(function()
    while true do
        ...
        if a.items["camera"].holding and IsControlJustReleased(1, 38) then
            ...
            tGMT.notifyPicture("bbcnotification","notification","Press ~r~LEFT ALT ~s~to change overlay or ~r~BACKSPACE ~s~to exit recording mode.","BBC News","Now Recording",nil,nil)
