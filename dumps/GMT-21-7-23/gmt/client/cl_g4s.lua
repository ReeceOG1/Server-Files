...
local function B()
    ...
    local D = 500 * math.ceil(GetDistanceBetweenCoords(tGMT.getPlayerCoords(), GetEntityCoords(b), true) / 10)
    if j then
...
RMenu.Add(
    "g4s",
    "shiftHandler",
    RageUI.CreateMenu("", "Security Company", tGMT.getRageUIMenuWidth(), tGMT.getRageUIMenuHeight(), "banners", "g4s")
)
...
tGMT.createArea("g4sDepot_", d, 1.5, 6, G, H, I)
Citizen.CreateThread(
    function()
        while true do
            local C = tGMT.getPlayerPed()
            local J = tGMT.getPlayerCoords()
            if a then
                if not u then
                    displayJob()
                end
                    ...
                    for K, L in pairs(s) do
                        ...
                        if table.has(p, O) or not DoesEntityExist(N) then
                            ...
                            if IsVehicleStopped(N) then
                                local P = #(tGMT.getPlayerCoords() - GetEntityCoords(N))
    ...
    function(b, W, X, Y)
        if Y then
            ...
            AttachEntityToEntity(
                v,
                tGMT.getPlayerPed(),
                GetPedBoneIndex(tGMT.getPlayerPed(), 57005),
                0.17,
                0.10,
                -0.13,
                24.0,
    ...
    function()
        if RageUI.Visible(RMenu:Get("g4s", "main")) then
            RageUI.DrawContent(
                ...
                function()
                    if not j then
                        if tGMT.getPlayerVehicle() == 0 and w < 4 then
...
function newG4sVehicle()
    ...
    if DoesEntityExist(b) and not IsVehicleDriveable(b, true) then
        ...
        b = tGMT.spawnVehicle(n, a1.x, a1.y, a1.z, GetEntityHeading(C), true, true)
        c = VehToNet(b)
...
function securityCheck(C)
    local a2 = tGMT.getPlayerVehicle()
    if a2 ~= 0 and a2 ~= nil and b ~= nil and b ~= 0 then
        if a2 ~= b then
            ...
            SetTimeout(
                15000,
                function()
                    a2 = GetVehiclePedIsIn(tGMT.getPlayerPed(), false)
                    if a2 ~= 0 then
                        if a2 ~= b then
                            TriggerServerEvent("GMT:toggleShiftG4S", false)
                            tGMT.removeBlip(k)
...
RegisterNetEvent(
    ...
    function(a4)
        ...
        b = tGMT.spawnVehicle(n, z.x, z.y, z.z, z.heading, true, true)
        c = VehToNet(b)
...
RegisterNetEvent(
    ...
    function()
        ...
        while not f do
            ...
            local P = #(tGMT.getPlayerCoords() - ae)
            if P <= 1.0 then
                ad = true
...
RegisterNetEvent(
    ...
    function(ax, O, aC)
        if not a then
            local b = tGMT.getObjectId(ax, "GMT:activeVansUpdate 1")
            if b ~= 0 and b ~= nil and DoesEntityExist(b) then
                if aC then
                    s[ax] = nil
                else
                    if not table.has(s, ax) then
                        local b = tGMT.getObjectId(ax, "GMT:activeVansUpdate")
                        s[ax] = {O, b, ax}
                    end
                end
            end
