...
function tGMT.setTime(f, g, h)
    b = {h = f, m = g, s = h}
end
function tGMT.overrideTime(f, g, h)
    b.h = f
    b.m = g
    b.s = h
    c = true
end
function tGMT.cancelOverrideTimeWeather()
    c = false
end
function tGMT.setWeather(i)
    a = i
end
...
tGMT.createThreadOnTick(func_manageTimeAndWeather)
...
function voteWeather(C)
    if j then
        if not k then
            ...
            k = true
            tGMT.notify("~g~Vote sent!")
        else
            tGMT.notify("~r~You have already voted!")
        end
    else
        tGMT.notify("~r~Vote not in progress, start a vote with /voteweather!")
    end
end
function resetVotes()
    m = {
