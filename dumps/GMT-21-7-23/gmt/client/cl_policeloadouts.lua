...
RMenu.Add("policeloadouts","main",RageUI.CreateMenu("","Please Select Division",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"gmt_gunstoreui","gmt_gunstoreui"))
RMenu.Add("policeloadouts","confirm",RageUI.CreateSubMenu(RMenu:Get("policeloadouts", "main"),"","Confirm Selection",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
...
AddEventHandler("GMT:onClientSpawn",function(p, q)
    if q then
        ...
        for k,v in pairs(loadoutCoords) do
            tGMT.createArea("police_loadouts_"..k, v, 1.5, 6, r, t, u, {locationId = k})
        end
    end
end)