RMenu.Add("cardev","mainmenu",RageUI.CreateMenu("", "", tGMT.getRageUIMenuWidth(), tGMT.getRageUIMenuHeight(), "gmt_cardev", "gmt_cardev"))
RMenu:Get("cardev", "mainmenu"):SetSubtitle("Car Dev Menu")
RMenu.Add("cardev","vehiclemods",RageUI.CreateSubMenu(RMenu:Get("cardev", "mainmenu"),"","Vehicle Mods",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
RMenu.Add("cardev","vehiclemodindexes",RageUI.CreateSubMenu(RMenu:Get("cardev", "vehiclemods"),"","Vehicle Mod Indexes",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
RMenu.Add("cardev","extras",RageUI.CreateSubMenu(RMenu:Get("cardev", "mainmenu"),"","Extras",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
RMenu.Add("cardev","colours",RageUI.CreateSubMenu(RMenu:Get("cardev", "mainmenu"),"","Colours",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
local a = false
local b = false
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('cardev', 'mainmenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if b and tGMT.getPlayerBucket() == 333 then
                RageUI.ButtonWithStyle("Spawn Vehicle (No Mods)","",{RightLabel = "→→→"},true,function(h, i, j)
                    if j then
                        tGMT.clientPrompt("Spawncode:","",function(k)
                            if k ~= "" and tGMT.getPlayerBucket() == 333 then
                                local l = tGMT.loadModel(k)
                                TriggerServerEvent("GMT:logVehicleSpawn", k, "/cardev")
                                local m = tGMT.getPlayerCoords()
                                local n = tGMT.spawnVehicle(l,m.x,m.y,m.z,GetEntityHeading(tGMT.getPlayerPed()),true,true,true)
                                DecorSetInt(n, "GMTACVeh", 955)
                                SetVehicleOnGroundProperly(n)
                                SetEntityInvincible(n, false)
                                SetPedIntoVehicle(tGMT.getPlayerPed(), n, -1)
                ...
                RageUI.ButtonWithStyle("Delete Vehicle","",{RightLabel = "→→→"},true,function(h, i, j)
                    if j then
                        local o = GetVehiclePedIsIn(tGMT.getPlayerPed(), false)
                        if NetworkGetEntityIsNetworked(o) then
                            local p = NetworkGetNetworkIdFromEntity(o)
                            if p ~= 0 then
                                DeleteEntity(aT)
                                --TriggerServerEvent("GMT:cardevDeleteVehicle", p)
                            end
                        else
                            DeleteEntity(o)
                        end
                ...
                RageUI.ButtonWithStyle("Cycle through seats","",{RightLabel = "→→→"},true,function(h, i, j)
                    if j then
                        ...
                        if IsPedInAnyVehicle(q) then
                            ...
                            tGMT.notify("~r~Not in a vehicle.")
                        end
                    end
                end)
    ...
    if RageUI.Visible(RMenu:Get('cardev', 'vehiclemods')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            for w, x in pairs(f) do
                if GetNumVehicleMods(tGMT.getPlayerVehicle(), w) > 0 then
    ...
    if RageUI.Visible(RMenu:Get('cardev', 'vehiclemodindexes')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if GetNumVehicleMods(tGMT.getPlayerVehicle(), selectedModType) == 0 then
                RageUI.Text({message = "No available mod indexes for this mod type for this vehicle."})
            else
                for w = 0, GetNumVehicleMods(tGMT.getPlayerVehicle(), selectedModType) do
                    RageUI.ButtonWithStyle("Mod " .. w,"",{RightLabel = "→→→"},true,function(h, i, j)
                        if j then
                            SetVehicleModKit(tGMT.getPlayerVehicle(), 0)
                            SetVehicleMod(tGMT.getPlayerVehicle(), selectedModType, w)
                        end
                    end,RMenu:Get("cardev", "vehiclemodindexes"))
                end
            end
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            for z, A in pairs(g) do
                RageUI.Button(A.name,nil,{},true,function(h, i, j)
                    if j then
                        SetVehicleColours(tGMT.getPlayerVehicle(), A.colorindex, A.colorindex)
                    end
                end)
            end
        end)
...
RegisterCommand("cardev",function(C)
    if a and not tGMT.isPurge() then
        B()
    end
end)
RegisterNetEvent("GMT:setCarDev",function()
    a = true
end)
RegisterNetEvent("GMT:setBucket",function(D)
    if b and D ~= 333 then
...
local function ae()
    if not b or tGMT.getPlayerBucket() ~= 333 then
        if J then
            ac(false)
        end
        return
    ...
    L()
    local af = tGMT.getFontId("Akrobat-ExtraBold")
