...
function previewBackpack(c)
    ...
    SetPedComponentVariation(tGMT.getPlayerPed(), prop0, prop1, prop2, 2)
end
...
RegisterNetEvent("GMT:removeBackpack",function()
    if currentBackpack ~= nil then
        ...
        SetPedComponentVariation(tGMT.getPlayerPed(), 5, 0, 0, 2)
        currentBackpack = nil
        b = nil
        c = nil
    end
...
AddEventHandler("GMT:onClientSpawn",function(D, E)
    if E then
        ...
        local i = function(j)
            ...
            DrawAdvancedText(0.595, 0.213, 0.005, 0.0028, 1.0, "GMT Backpacks", 255, 255, 255, 255, 1, 0)
                ...
                if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                    ...
                    if currentBackpack ~= backpackname then
                        if c ~= nil then
                            ...
                            tGMT.notify("~r~No backpack selected!")
                        end
                    else
                        tGMT.notify("~r~You already have this backpack!")
                    end
                    Wait(250)
                    EnableAllControlActions(1)
                end
        ...
        for q, r in pairs(a.stores) do
            for s, t in pairs(r) do
                if s == "_config" then
                    ...
                    tGMT.createArea("backpack_" .. q, vector3(u, v, w), 1.5, 6, g, h, i, {storeName = q})
                end
            end
        end
    end
