...
RMenu.Add(
    "radios",
    "mainmenu",
    RageUI.CreateMenu("Radios", "Main Menu", tGMT.getRageUIMenuWidth(), tGMT.getRageUIMenuHeight())
)
RMenu.Add(
    "radios",
    "advancedeffects",
...
RegisterNetEvent("GMT:clientJoinRadioChannel", function(q)
        if b[q] and c ~= q then
            J(q, b[q])
        end
end)
...
RegisterNetEvent("GMT:clientLeaveRadioChannel",function()
    if c ~= 0 then
        t()
    end
end)
AddEventHandler("GMT:openRadioConfig",function()
    RageUI.Visible(RMenu:Get("radios", "mainmenu"), true)
end)
function tGMT.radioUIVisible(Y)
    e(Y)
end
