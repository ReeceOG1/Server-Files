--[[local a = false
local b = module("cfg/events/cfg_battleRoyalee")
...
local function m()
    if a then
        ...
        DrawAdvancedText(0.591, 0.042, 0.005, 0.0028, 0.993, "GMT BATTLEGROUNDS", 255, 255, 255, 255, 6, 0)
    end
end
tGMT.createThreadOnTick(m)
local function n()
        ...
        if CursorInArea(GetArea(0.493, 0.944, 0.14, 0.074)) then
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                if g.player.canExitPlane then
                    tGMT.setPlayerCanOpenLeaderboard(true)
    ...
    function(t, u)
        ...
        for v, w in pairs(g.data.lootBoxes) do
            if #(w.coords - g.gas.coords) < g.gas.radius / 2 then
                ...
                tGMT.createArea("gmtbr_lootbox_" .. v, w.coords, 200.0, 6, x, z, A, {box = v})
            end
        end
        SetModelAsNoLongerNeeded("prop_box_ammo03a")
        tGMT.loadModel("prop_armour_pickup")
            ...
            if #(w.coords - g.gas.coords) < g.gas.radius / 2 then
                ...
                local B = function(C)
                    if g.data.armourPlates[C.plateId] then
                        ...
                        print(
                            string.format(
                                "[GMT Events] body armour with ID of %s is nil in table (onEnter)",
                                C.plateId
                            )
                        )
                    end
                ...
                local D = function(C)
                    if g.data.armourPlates[C.plateId] then
                        ...
                        print(
                            string.format(
                                "[GMT Events] body armour with ID of %s is nil in table (onLeave)",
                                C.plateId
                            )
                        )
                    end
                ...
                local E = function(C)
                    if g.data.armourPlates[C.plateId] then
                        if C.distance <= 1.5 then
                            ...
                            tGMT.DrawText3D(vector3(F.x, F.y, F.z - 0.5), "Press [E] to pickup armour.", 0.2)
                ...
                tGMT.createArea("gmtbr_armour_" .. v, w.coords, 200.0, 6, B, D, E, {plateId = v})
                g.data.armourPlates[v].areaId = "gmtbr_armour_" .. v
            end
        end
...
RegisterNetEvent(
    "GMT:startBattlegrounds",
    function(I)
        e = I
        tGMT.startGas(b.locations[e].gas.initalRadius, b.locations[e].gas.centre)
        DoScreenFadeOut(1500)
        Wait(1500)
        currentEvent.eventName = "Battle Royalee"
        tGMT.hideUI()
        tGMT.stopEventSequence(true)
        tGMT.giveWeapons({["GADGET_PARACHUTE"] = {2}})
        SetEntityCoords(PlayerPedId(), b.locations[e].planeStart)
        tGMT.loadModel("titan")
        for o, p in pairs(GetActivePlayers()) do
            if p ~= PlayerId() then
        ...
        local J =
            tGMT.spawnVehicle("titan", b.locations[e].planeStart, b.locations[e].planeHeading, false, false, true)
...
tGMT.registerEventCleanupHandler(
    "GMT Battlegrounds",
    function()
        currentEvent.isActive = false
        tGMT.setPlayerCanOpenLeaderboard(false)
        for v, w in pairs(g.data.lootBoxes) do
            tGMT.removeArea(w.areaId)
            tGMT.removeNamedBlip(w.areaId)
...
function tGMT.changeGasRadius(M)
    SendNUIMessage({transactionType = "br-gas"})
    g.gas.timeUntilNext = {minutes = 2, seconds = 0}
    tGMT.announceMpBigMsg("~r~GAS MOVING", "The gas is closing in!", 5000)
    Wait(4000)
    Citizen.CreateThread(
        function()
...
tGMT.createThreadOnTick(
    function()
        if currentEvent.isActive and currentEvent.eventName == "Battle Royalee" then
            for h = 1, 3 do
            ...
            if g.gas.isActive then
                if g.gas.timeUntilNext.seconds > 0 then
                    ...
                    if g.gas.timeUntilNext.seconds == 0 and g.gas.timeUntilNext.minutes == 0 then
                        tGMT.changeGasRadius(g.gas.radius - 400)
                    else
...
RegisterNetEvent(
    ...
    function(p, U)
        ...
        tGMT.updateScoreboard(p, 5, g.players[p].kills)
        Q()
    end
)
RegisterNetEvent(
    "GMT:removePlayerFromBR",
    function(V)
        if g.players[V] then
        ...
        tGMT.removePlayerFromLeaderboard(V)
    end
)
Citizen.CreateThread(
    function()
        while true do
            if
             ...
             then
                if #(tGMT.getPlayerCoords().xy - g.gas.coords.xy) > g.gas.radius / 2.0 then
