...
local u = {}
RMenu.Add("GMTjackpot","instructions",RageUI.CreateMenu("", "", tGMT.getRageUIMenuWidth(), tGMT.getRageUIMenuHeight(), "gmt_jackpotui", "gmt_jackpotui"))
RMenu:Get("GMTjackpot", "instructions"):SetSubtitle("JACKPOT")
...
Citizen.CreateThread(function()
    while true do
        if insideDiamondCasino then
            b = 1000
            a = -1
            local y = GetEntityCoords(tGMT.getPlayerPed())
            for z = 0, 15, 1 do
                local A = t[z]
                local B = #(y - A)
                if B < b then
...
function goToJackpotChair(E)
    ...
    PlayFacialAnim(tGMT.getPlayerPed(), "enter_left_readyidle_facial", H)
    RemoveAnimDict(H)
    local O
    local P
            ...
            if j then
                ...
                if u ~= nil then
                    for R, S in pairs(u) do
                        if S.user_id == tGMT.getUserId() then
    ...
    while true do
        if sittingAtInsideTrack and f then
            ...
            if IsControlJustPressed(0, 202) and not g then
                ...
                PlayFacialAnim(tGMT.getPlayerPed(), "exit_left_facial", H)
...
function getTrackGenderAnimDict()
    local ax = GetEntityModel(tGMT.getPlayerPed())
...
RegisterNetEvent("GMT:cleanupJackpot",function()
    ...
    if e ~= -1 then
        ...
        NetworkAddPedToSynchronisedScene(tGMT.getPlayerPed(), az, H, I, 2.0, -2.0, 5, 0, 1148846080, 0)
