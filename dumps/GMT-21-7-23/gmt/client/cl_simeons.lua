...
RegisterNetEvent("GMT:carDealerPurchased",function()
    ...
    SetEntityCoords(tGMT.getPlayerPed(), o.x, o.y, o.z)
end)
local function p(q)
    if q ~= nil then
        d = q
...
local function s(t, u, v, w)
    ...
    if HasModelLoaded(x) then
        local z = CreateVehicle(x, u, v, w, GetEntityHeading(tGMT.getPlayerPed()), false, false)
        DecorSetInt(z, "GMTACVeh", 955)
        ...
        TaskWarpPedIntoVehicle(tGMT.getPlayerPed(), z, -1)
        ...
        tGMT.notify("~r~Could not load vehicle")
        return -1
    end
end
...
function isNearPreviewVector(J)
    if #(GetEntityCoords(tGMT.getPlayerPed()) - J) < 5.0 then
        return true
    end
    return false
end
...
Citizen.CreateThread(function()
    while true do
        if a ~= nil then
            for A, B in pairs(a) do
                local C, D, E, F, G, H, I =B.posVector,B.blipId,B.blipColour,B.markerId,B.markerColourTable,B.permissionsTable,B.categorys
                c[A] = #(GetEntityCoords(tGMT.getPlayerPed()) - C)
            end
        end
        Wait(250)
    end
end)
RMenu.Add("cardealer","mainmenu",RageUI.CreateMenu("", "", tGMT.getRageUIMenuWidth(), tGMT.getRageUIMenuHeight(), "gmt_simeonsui","gmt_simeonsui"))
RMenu:Get("cardealer", "mainmenu"):SetSubtitle("Categories")
RMenu.Add("cardealer","categories",RageUI.CreateSubMenu(RMenu:Get("cardealer", "mainmenu"),"","Vehicles",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
RMenu.Add("cardealer","vehicle",RageUI.CreateSubMenu(RMenu:Get("cardealer", "categories"),"","Options",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
RMenu.Add("cardealer","confirm",RageUI.CreateSubMenu(RMenu:Get("cardealer", "vehicle"),"","Are you sure?",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight()))
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if d ~= nil then
                ...
                RageUI.ButtonWithStyle("Test Drive " .. i[1],"",{RightLabel = "→→→"},true,function(N, O, P)
                    if P then
                        DeleteVehicle(GetVehiclePedIsIn(tGMT.getPlayerPed()))
                        if not k then
                            RageUI.ActuallyCloseAll()
                            hash = GetHashKey(j)
                            ...
                            if HasModelLoaded(hash) then
                                ...
                                testDriveSeconds = 60
                                local S = GetEntityCoords(tGMT.getPlayerPed())
                                DecorSetInt(testDriveCar, "GMTACVeh", 955)
                                SetModelAsNoLongerNeeded(hash)
                                TaskWarpPedIntoVehicle(tGMT.getPlayerPed(), testDriveCar, -1)
                                setVehicleFuel(testDriveCar, 100)
                                tGMT.notify("~g~You have 1 minute to test drive this vehicle!")
                                for T = 0, 24 do
                                ...
                                SetTimeout(60000,function()
                                    if k then
                                        ...
                                        SetEntityCoords(tGMT.getPlayerPed(), S.x, S.y, S.z)
                                        tGMT.notify("~g~Test drive over!")
                                        p()
                                    end
                                end)
                                Citizen.CreateThread(function()
                                ...
                                Citizen.CreateThread(function()
                                    while k do
                                        local U = GetVehiclePedIsIn(tGMT.getPlayerPed(), false)
                                        if U ~= nil then
...
tGMT.createThreadOnTick(func_previewSimeonsVehicle)
