...
Citizen.CreateThread(
    function()
        while true do
            ...
            a(
                function(d)
                    if DecorGetInt(d, "GMTACVeh") ~= 955 then
...
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            local h = tGMT.getPlayerId()
            local i = tGMT.getPlayerVehicle()
            if i == 0 then
                ...
                if not f and GetUsingseethrough() and not tGMT.isPlayerInPoliceHeli() and not tGMT.isPlayerInDrone() then
                    TriggerServerEvent("GMT:acType13")
                    f = true
                end
            end
            SetPedInfiniteAmmoClip(g, false)
            for c, k in pairs(tGMT.getWeapons()) do
...
AddEventHandler(
    ...
    function(n)
        ...
        TriggerServerEvent("GMT:acType4")
        l = true
        n(nil)
    end
)
...
Citizen.CreateThread(
    function()
        while true do
            ...
            if r > 1.0 then
                TriggerServerEvent("GMT:acType8", "PlayerWeaponDamageModifier", r)
                return
            end
            if s > 1.0 then
                TriggerServerEvent("GMT:acType8", "PlayerWeaponDefenseModifier", s)
                return
            end
            if t > 1.0 then
                TriggerServerEvent("GMT:acType8", "PlayerWeaponDefenseModifier_2", t)
                return
            end
            if u > 1.0 then
                TriggerServerEvent("GMT:acType8", "PlayerVehicleDamageModifier", u)
                return
            end
            if v > 1.0 then
                TriggerServerEvent("GMT:acType8", "PlayerVehicleDefenseModifier", v)
                return
            end
            if y > 1.0 then
                TriggerServerEvent("GMT:acType8", "GetWeaponDamageModifier", y)
                return
            end
            if z > 1.0 then
                TriggerServerEvent("GMT:acType8", "GetPlayerMeleeWeaponDamageModifier", z)
                return
            end
...
function tGMT.isPlayerAboveGround()
...
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            N = O
            if
                P > 0.4 and not IsPedFalling(g) and tGMT.getStaffLevel() < 2 and not IsPedInParachuteFreeFall(g) and
                    not carryingBackInProgress and
                    not piggyBackInProgress and
                    not tGMT.takeHostageInProgress() and
                    GetPedParachuteState(g) <= 0 and
                    not IsPedRagdoll(g) and
                    not IsPedRunning(g) and
                    not tGMT.isPlayerRappeling() and
                    not tGMT.isPlayerAboveGround() and
                    not tGMT.isPlayerHidingInBoot() and
                    not tGMT.isSpectatingEvent() and
                    not noclipActive
             then
                if not IsPedInAnyVehicle(g, 1) then
                    D = D + 1
                    if D > 100 then
                        TriggerServerEvent("GMT:acType1", false)
                        D = 0
                    end
                end
            end
            local d, Q = tGMT.getPlayerVehicle()
            if DoesEntityExist(d) and Q and P > 0.2 and tGMT.getStaffLevel() < 2 then
                if F ~= d then
                    E = 0
                    F = d
                end
                ...
                if J < 5.0 and R < 2.5 then
                    ...
                    if E > 100 and GetGameTimer() - G > 4000 then
                        TriggerServerEvent("GMT:acType1", true)
                        E = 0
                        G = GetGameTimer()
                    end
                end
            ...
            if NetworkIsInSpectatorMode() and not tGMT.inSpectate() then
                TriggerServerEvent("GMT:acType21")
                return
            end
            if GetCamFov(GetRenderingCam()) == 50.0 and not tGMT.isInSpectate() and not tGMT.isInTutorial() and tGMT.getStaffLevel() == 0 then
                TriggerServerEvent("GMT:acType24")
                Wait(60000)
            end
            Wait(0)
        end
    ...
    function()
        while true do
            if GetLabelText("notification_buffer") ~= "NULL" then
                TriggerServerEvent("GMT:acType7")
            end
        ...
        while true do
            ...
            for S, T in ipairs(WeaponBL) do
                ...
                if HasPedGotWeapon(PlayerPedId(), GetHashKey(T), false) == 1 then
                    ...
                    TriggerServerEvent("GMT:acType2", T)
                    return
                end
            end
            local S, r = StatGetInt(GetHashKey("mp0_shooting_ability"), true)
            if r > 100 then
                TriggerServerEvent("GMT:acType18")
                return
            end
        end
    end
    ...
    function()
        ...
        while true do
            if S >= 100 and not tGMT.isInComa() then
                TriggerServerEvent("GMT:acType6")
                return
            end
            if
                not tGMT.isStaffedOn() and GetEntityHealth(PlayerPedId()) > 102 and not spawning and
                    not tGMT.isPedScriptGuidChanging()
             then
                local h = PlayerId()
                local g = PlayerPedId()
        ...
        while true do
            ...
            if IsPedShooting(f) and not table.has(V, i) then
                ...
                if R == GetMaxAmmoInClip(f, i) then
                    TriggerServerEvent("GMT:acType8")
                    Wait(60000)
                end
            end
            Wait(0)
...
local function a_()
    ...
    if aW(b0, aC) or aW(b1, aD) or aW(b2, aE) or aW(b3, aF) then
        ...
        if
         ...
         then
            local aQ = aP(d)
            local b4 = tGMT.getVehicleIdFromHash(GetEntityModel(d)) or "N/A"
...
local b7 = {GetHashKey("barnsy")}
Citizen.CreateThread(
    function()
        while true do
            local d = tGMT.getPlayerVehicle()
            local b6 = GetEntityModel(d)
            if GetVehicleHasParachute(d) or GetCanVehicleJump(d) or GetHasRocketBoost(d) and b6 ~= GetHashKey("voltic2") then
...
Citizen.CreateThread(
    function()
        ...
        while true do
            if
                GetPlayerInvincible(PlayerId()) and not isInGreenzone and not tGMT.isInsideLsCustoms() and
                    not tGMT.isStaffedOn() and
                    not noclipActive and
                    not tGMT.isInComa() and
                    not tGMT.isInSpectate() and
                    not tGMT.getInRPZone() and
                    not tGMT.isInTutorial()
             then
                TriggerServerEvent("GMT:acType15")
                Citizen.Wait(60000)
            end
            if
                GetPlayerInvincible_2(PlayerId()) and not isInGreenzone and not tGMT.isInsideLsCustoms() and
                    not tGMT.isStaffedOn() and
                    not noclipActive and
                    not tGMT.isInComa() and
                    not tGMT.isInSpectate() and
            ...
            if
                not IsEntityVisible(PlayerPedId()) and not tGMT.isInsideLsCustoms() and not noclipActive and
                    not tGMT.isInSpectate() and
                    not tGMT.isInTutorial() and
                    not tGMT.inEvent() and
                    not tGMT.isPlayerInDrone() and
                    not tGMT.isSpectatingEvent() and
                    not spawning and
                    tGMT.getStaffLevel() < 3
                then
                    TriggerServerEvent("GMT:acType25")
                    Citizen.Wait(60000)
                end
            Citizen.Wait(1000)
        end
...
RegisterNUICallback(
    "NuiDevTool",
    function()
        TriggerServerEvent("GMT:acType22")
    end
)
AddEventHandler(
    "onClientResourceStop",
    function(b8)
        TriggerServerEvent("GMT:acType23")
    end
)
AddEventHandler(
    "onResourceStop",
    function(b8)
        TriggerServerEvent("GMT:acType23", b8)
    end
)
print("[GMT] - Anti-Cheat initialised") -- (credits - ker)