...
local function k(vehicle, l)
    ...
    if l and #(tGMT.getPlayerCoords() - l) > 1.0 then
        return false
    end
    return true
end
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            local q = tGMT.getPlayerVehicle()
            local r = tGMT.getClosestVehicle(7.0)
            local s = GetVehicleClass(r)
            local t = GetVehicleDoorLockStatus(r)
            if q == 0 and a[s] and GetEntityHealth(tGMT.getPlayerPed()) > 102 and not noclipActive then
                if r and r ~= 0 then
                    o = 0
                            ...
                            if not b(vehicle) then
                                ...
                                if k(vehicle, l) then
                                    if t <= 1 then
                                        ...
                                        tGMT.setWeapon(p, "WEAPON_UNARMED", true)
                                        j = true
                                        ...
                                        Wait(100)
                                        tGMT.loadAnimDict("timetable@floyd@cryingonbed@base")
                                        ...
                                        while z and j do
                                            ...
                                            if IsDisabledControlPressed(0, 177) then
                                                if GetVehicleDoorLockStatus(r) <= 1 then
                                                    ...
                                                    tGMT.notify("~r~Vehicle is locked, cannot get in boot.")
                                                end
                                            end
                                        ...
                                        tGMT.notify("~r~Vehicle is locked, cannot get out of boot.")
                                    end
                                else
                                    tGMT.notify("~r~You and the vehicle must remain stationary to get in.")
                                end
                            else
                                tGMT.notify("~r~Someone is already in this boot.")
                            end
                        end
                    else
                        ClearHelp(true)
