function tGMT.add3DTextForCoord(a, b, c, d, e)
    local function f(g)
        tGMT.DrawText3D(g.coords, g.text, 0.2)
    end
    local h = tGMT.generateUUID("3dtext", 8, "alphanumeric")
    tGMT.createArea("3dtext_" .. h,vector3(b, c, d),e,6.0,function()end,function()end,f,{coords = vector3(b, c, d), text = a})
end
function tGMT.drawFloatingHelpText(a, i)
...
function tGMT.DrawText3D(i, a, j, k, l, m, noShadow, noOutline)
...
function tGMT.DrawText(b, c, a, j, k, p, m, q)
    SetTextScale(j, j)
    SetTextFont(k or 0)
    if p then
