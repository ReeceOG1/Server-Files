DMclient = {}
Tunnel.bindInterface("DM",DMclient)
Proxy.addInterface("DM",DMclient)
DMserver = Tunnel.getInterface("DM","DM")


function DMclient.SendAdminMessage(message)
    AdminMessage(message)
end


function DMclient.Notify(message)
    Draw_Native_Notify(message)
end

function DMclient.SetHeatlh(Health)
    SetEntityHealth(PlayerPedId(), Health)
end


function DMclient.ScreenFade(Duration)
    DoScreenFadeOut(Duration-100)
    Wait(Duration)
    DoScreenFadeIn(Duration)
end

function DMclient.ShowWarning(Title, Body)
    if Title == "Anticheat" then
        SendNUIMessage({
            transactionType = "anticheat",
        })
        AddTextEntry("QM_NO_3", "Ears Working!?!?!?!?")
    else
        AddTextEntry("QM_NO_3", "i didnt know what to put here :)")
    end
	AddTextEntry("FACES_WARNH2", Title)
	AddTextEntry("QM_NO_0", Body)
	while true do
		Citizen.Wait(0)
