RMenu.Add('AAMenu', 'main', RageUI.CreateMenu("","~y~AA ~w~Menu",1300,100, "banners", "aamenu"))
...
RegisterCommand('aamenu', function()
    TriggerServerEvent('LUNA:openAAMenu')
end)

function tLUNA.openAAMenu()
    RageUI.Visible(RMenu:Get('AAMenu', 'main'), not RageUI.Visible(RMenu:Get('AAMenu', 'main')))
end

function JartalFix(veh)
    local JartalPed = GetPlayerPed(-1)
    if IsPedInAnyVehicle(JartalPed) then
        local Jartal = GetVehiclePedIsIn(JartalPed)
        SetVehicleEngineHealth(Jartal, 9999)
        SetVehiclePetrolTankHealth(Jartal, 9999)
        SetVehicleFixed(Jartal)
