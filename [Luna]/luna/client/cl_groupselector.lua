...
RegisterNetEvent("LUNA:onClientSpawn")
AddEventHandler("LUNA:onClientSpawn",function(D, E)
    if E then
		TriggerServerEvent("LUNA:getJobSelectors")
        TriggerServerEvent('LUNA:getFactionWhitelistedGroups')
	end
end)
RegisterNetEvent("LUNA:gotJobSelectors",function(h)
    ...
    local l = function(j)
        ...
        DrawText3DD(b[j.selectorId].position.x,b[j.selectorId].position.y,b[j.selectorId].position.z,"Press [E] to open Job Selector.",n,o,p)
    end
    for q, r in pairs(c) do
        tLUNA.createArea("selector_" .. q, r.position, 1.5, 6, i, k, l, {selectorId = q})
        tLUNA.addMarker(r.position.x, r.position.y, r.position.z - 1, 1.0, 1.0, 1.0, 255, 0, 0, 170, 50, 27)
        tLUNA.addBlip(r.position.x,r.position.y,r.position.z,r._config.blipid,r._config.blipcolor,r._config.name)
    end
end)
                ...
                if q == e then
                    for s, t in pairs(r.jobs) do
                        RageUI.ButtonWithStyle(t[1],r._config.name,{RightLabel = "→→→"},true,function(u, v, w)
                            if w then
                                TriggerServerEvent("LUNA:jobSelector", q, t[1])
                    ...
                    RageUI.ButtonWithStyle("Unemployed","",{RightLabel = "→→→"},true,function(u, v, w)
                        if w then
                            TriggerServerEvent("LUNA:jobSelector", q, "Unemployed")
                        end
                    end)
                end
            end
