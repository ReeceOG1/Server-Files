	...
	while (true) do
        if IsControlJustPressed(1, 166) then
            ...
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                TriggerServerEvent('LUNA:TrunkOpened', source)
            else
...
RegisterNetEvent("LUNA:handsUpNearest")
AddEventHandler("LUNA:handsUpNearest", function(nplayer, nuser_id)
    nearestHandsUp = IsEntityPlayingAnim(GetPlayerPed(nplayer), "missminuteman_1ig_2", "handsup_enter", 3)
    if nearestHandsUp then
        TriggerServerEvent('LUNA:searchNearestPlayer', nplayer, nuser_id)
    end
end)
