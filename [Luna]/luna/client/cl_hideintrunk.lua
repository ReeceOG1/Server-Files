...
local j = false
function tLUNA.isPlayerHidingInBoot()
    return j
end
local function k(vehicle, l)
    if tLUNA.isHandcuffed() then
        return false
    end
    ...
    if l and #(tLUNA.getPlayerCoords() - l) > 1.0 then
        return false
    end
    return true
end
Citizen.CreateThread(
    function()
        ...
        while true do
            Citizen.Wait(o)
            local p = tLUNA.getPlayerPed()
            local q = tLUNA.getPlayerVehicle()
            local r = tLUNA.getClosestVehicle(7.0)
            local s = GetVehicleClass(r)
            local t = GetVehicleDoorLockStatus(r)
            if q == 0 and a[s] and GetEntityHealth(tLUNA.getPlayerPed()) > 102 and not noclipActive then
                if r and r ~= 0 then
                    o = 0
                    local u = GetEntityBoneIndexByName(r, "boot")
                    local v = GetWorldPositionOfEntityBone(r, u)
                    local w = #(v - tLUNA.getPlayerCoords())
                    if w < 2 then
                        DrawMarker(
                            0,
                            v.x,
                        ...
                        if IsDisabledControlJustReleased(1, 137) and k(vehicle) then exports.rprogress:Start("", 2000)
                            if not b(vehicle) then
                                local l = tLUNA.getPlayerCoords()
                                if k(vehicle, l) then
                                    if t <= 1 then
                                        tLUNA.setCanAnim(false)
                                        tLUNA.setWeapon(p, "WEAPON_UNARMED", true)
                                        j = true
                                        ...
                                        Wait(100)
                                        tLUNA.loadAnimDict("timetable@floyd@cryingonbed@base")
                                        TaskPlayAnim(
                                            p,
                                        ...
                                        while z and j do
                                            ...
                                            if IsDisabledControlPressed(0, 177) then
                                                if GetVehicleDoorLockStatus(r) <= 1 then
                                                    ...
                                                    tLUNA.notify("~r~Vehicle is locked, cannot get in boot.")
                                                end
                                            end
                                        ...
                                        SetVehicleDoorOpen(r, 5, false, false)
                                        tLUNA.setCanAnim(true)
                                        Wait(1000)
                                        SetVehicleDoorShut(r, 5, false)
                                    else
                                        tLUNA.notify("~r~Vehicle is locked, cannot get out of boot.")
                                    end
                                else
                                    tLUNA.notify("~r~You and the vehicle must remain stationary to get in.")
                                end
                            else
                                tLUNA.notify("~r~Someone is already in this boot.")
                            end
                        end
                    else
                        ClearHelp(true)
