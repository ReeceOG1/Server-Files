...
RMenu.Add(
    "loginrewards",
    "mainmenu",
    RageUI.CreateMenu("", "Main Menu", tLUNA.getRageUIMenuWidth(), tLUNA.getRageUIMenuHeight(), "banners", "rewards")
)
local b = 0
local c = 0
local function d(e)
        ...
        RageUI.IsVisible(
            ...
            function()
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(p, p, u)
                        if u then
                            TriggerServerEvent("LUNA:claimNextLoginReward")
                            b = b + 1
...
RegisterNetEvent(
    "LUNA:setDailyRewardInfo",
    function(w, x, y)
        b = w
        c = GetGameTimer() + (86400 - (y - x)) * 1000
        if w == 0 then
            Citizen.Wait(20000)
            LUNA.notify("~y~You have not started your daily reward. Do /dailyrewards to start now!")
        end
    end
)
RegisterCommand(
...
Citizen.CreateThread(
    function()
        ...
        while true do
            if r() and b > 0 then
                LUNA.notify("~g~You can claim your next daily reward at /dailyrewards!")
                break
            end
            Citizen.Wait(6000)
        end
