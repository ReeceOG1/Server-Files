...
tLUNA.hasPoliceJob = function(bool)
    police = bool
end

tLUNA.PanicCL = function(player, s1, s2, name)
    if police then
...
tLUNA.BlipCL = function(tx, ty, tz)
    if police then
    ...
    while true do
        if showPanicBlip and police and not panic then
            ...
            if tostring(GetStreetNameFromHashKey(streetA)) and tostring(GetStreetNameFromHashKey(streetB)) then
                ...
                LUNAserver.PanicSV({player,  GetStreetNameFromHashKey(streetA), GetStreetNameFromHashKey(streetB)})
                LUNAserver.BlipSV({plyPos.x, plyPos.y, plyPos.z})
            else
                LUNAserver.PanicSV({player,  "No street found", ""})
                LUNAserver.BlipSV({plyPos.x, plyPos.y, plyPos.z})
            end
        end
