...
RMenu.Add('ergblackjack', 'instructions', RageUI.CreateMenu("", "test",1300, 50,"casinoui_cards_blackjack", "casinoui_cards_blackjack"))
RMenu:Get('ergblackjack', 'instructions'):SetSubtitle("Blackjack Table")
RMenu.Add('ergblackjack_high', 'instructions', RageUI.CreateMenu("", "test",1300, 50,"casinoui_cards_blackjack_high", "casinoui_cards_blackjack_high"))
RMenu:Get('ergblackjack_high', 'instructions'):SetSubtitle("High Rollers Table")


RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('ergblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare=  false, instructionalButton = true }, function()           
            ...
            end, RMenu:Get('ergblackjack', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('ergblackjack_high', 'instructions')) then
        RageUI.DrawContent({ header = true, glare=  false, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "The aim of Blackjack is to beat the Dealer's hand without going over 21. This game uses four 52           card decks, which are shuffled at the start of every hand.                                                                                    The dealer will stand on soft 17.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
            ...
            end, RMenu:Get('ergblackjack_high', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
function showHowToBlackjack(flag)
    if closestChair < 8 then 
        RageUI.Visible(RMenu:Get('ergblackjack', 'instructions'), flag)
    else 
        RageUI.Visible(RMenu:Get('ergblackjack_high', 'instructions'), flag)
    end
end
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    --print("[Nebu Casino] start sit at blackjack seat") 
    ...
    --print("[Nebu Casino] blackjack anims loaded") 
    ...
    --print("[Nebu Casino] fVars passed")
    ...
    --print("[Nebu Casino] walking to seat, x: " .. tostring(walkToVector.x) .. " y: " .. tostring(walkToVector.y) .. " z: " .. tostring(walkToVector.z))
    ...
    --print("[Nebu Casino] Blackjack sit at table net scene starting")
    --print("[Nebu Casino] creating Scene at, x: " .. tostring(goToVector.x) .. " y: " .. tostring(goToVector.y) .. " z: " .. tostring(goToVector.z))
    ...
    --print("[Nebu Casino] Blackjack sit at table net scene started")
...
AddEventHandler("Blackjack:blackjackLose",function(tableId)
    if closeToCasino then
        ...
        TriggerEvent("luna:PlaySound", "casino_lose")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
AddEventHandler("Blackjack:blackjackWin",function(tableId)
    if closeToCasino then
        ...
        TriggerEvent("luna:PlaySound", "casino_win")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
function betChipsForNextHand(chipsAmount,chipsProp,something,chairID,someBool,zOffset)
    ...
    while not HasModelLoaded(chipsProp) do  
        Wait(0)
        --print("[Nebu Casino] Stuck requesting model: " .. tostring(chipsProp))
        RequestModel(chipsProp)
    end
    vVar8 =  vector3(0.0, 0.0, getTableHeading(blackjack_func_368(chairID)))
    local tablePosX,tablePosY,tablePosZ = getTableCoords(blackjack_func_368(chairID))
