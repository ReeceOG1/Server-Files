...
RegisterCommand("gangblips", function()
    if not blipon then
        blipon = true
        TriggerServerEvent("LUNA:EnableGangBlips")
        notify("Gang blips Enabled")
        Citizen.CreateThread(function()
            while blipon do
                Citizen.Wait(5000)
                TriggerServerEvent("LUNA:GetGangBlipsData")
            end
        end)
    else
        DeleteAllBlips()
...
RegisterNetEvent("LUNA:ReceiveGangBlipsData")
AddEventHandler("LUNA:ReceiveGangBlipsData", function(gangBlipsData)
    DeleteAllBlips()
