...
AddEventHandler("playerSpawned",function()
    local h = true
    if h then 
        TriggerServerEvent("LUNA:GetGangData")
    end 
end)
RegisterNetEvent("LUNA:GotGangData")
AddEventHandler("LUNA:GotGangData",function(i,j,k,PO)
    if i==nil then 
        PlayerIsInGang=false 
    else 
        PlayerIsInGang=true
...
RegisterNetEvent("LUNA:disbandedGang")
AddEventHandler("LUNA:disbandedGang",function()
    PlayerIsInGang=false
    showGangUI=false
    showNoGangUI=true
    ...
    TriggerEvent("LUNA:ForceRefreshData")
end)
RegisterNetEvent("LUNA:ForceRefreshData")
AddEventHandler("LUNA:ForceRefreshData",function()
    TriggerServerEvent("LUNA:GetGangData")
end)
RegisterNetEvent("LUNA:InviteRecieved")
AddEventHandler("LUNA:InviteRecieved",function(o,Q)
    LUNAGangInvites[LUNAGangInviteIndex]=Q
    LUNAGangInviteIndex=LUNAGangInviteIndex+1
    tLUNA.notify(o)
end)
RegisterNetEvent("LUNA:gangNameNotTaken")
AddEventHandler("LUNA:gangNameNotTaken",function()
    showNoGangUI=false
    showGangUI=true
    PlayerIsInGang=true 
end)
function func_drawGangUI()
    if showNoGangUI then 
        ...
        DrawAdvancedText(0.558,0.303,0.005,0.0028,0.539,"LUNA Gangs",255,255,255,255,7,0)
        ...
        for p,q in pairs(LUNAGangInvites)do 
        ...
        if CursorInArea(0.35,0.415,0.415,0.46)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if createGangName~=nil and createGangName~="null"and createGangName~=""then 
                    TriggerServerEvent("LUNA:CreateGang",createGangName)
                else 
                    tLUNA.notify("~r~No gang name entered!")
                end 
            end 
        else 
            CreateGangSelectionRed=0
        ...
        if CursorInArea(0.435,0.51,0.415,0.46)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if selectedGangInvite~=nil then 
                    selectedGangInvite=LUNAGangInvites[selectedGangInvite]
                    local perm1s = tLUNA.getUserId()
                    TriggerServerEvent("LUNA:addUserToGang",selectedGangInvite,perm1s)
                    LUNAGangInvites={}
                    showNoGangUI=false
                    LUNAGangInviteIndex=0
                    showGangUI=true
                    ...
                    tLUNA.notify("~r~No gang invite selected")
                end 
            end 
        else 
            JoinGangSelectionRed=0
    ...
    if showFundsGangUI then 
        ...
        DrawRect(0.501, 0.374, 0.421, 0.047, 0,255,255,255)
        DrawAdvancedText(0.591,0.378,0.005,0.0028,0.48,"LUNA Gang - Funds",255,255,255,255,7,0)
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount ~= nil then
                    if closeToBarclays() then
                        TriggerServerEvent("LUNA:depositGangBalance", amount)
                    end
                else
                    tLUNA.notify("~r~No amount entered!")
                end
            end
        else
        ...
        if CursorInArea(0.4083, 0.4718, 0.5490, 0.5999) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if closeToBarclays() then
                    TriggerServerEvent("LUNA:depositAllGangBalance")
                end
            end
        else
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount ~= nil then
                    if gangPermission >= 3 then
                        if closeToBarclays() then
                            TriggerServerEvent("LUNA:withdrawGangBalance", amount)
                        end
                    else
                        tLUNA.notify("~r~You don't have a high enough rank to withdraw")
                    end
                else
                    tLUNA.notify("~r~No amount entered!")
                end
        ...
        if CursorInArea(0.6088, 0.6739, 0.5481, 0.6018) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if gangPermission >= 3 then
                    if closeToBarclays() then
                        TriggerServerEvent("LUNA:withdrawAllGangBalance")
                    end
                else
                    tLUNA.notify("~r~You don't have a high enough rank to withdraw")
                end
    ...
    if showMembersGangUI then 
        ...
        DrawRect(0.501,0.308,0.421,0.047,0,255,255,255)
        DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"LUNA Gang - Members",255,255,255,255,7,0)
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if selectedMember~=nil and PlayerIsInGang and gangID~=nil then 
                    if gangPermission>=4 then 
                        TriggerServerEvent("LUNA:PromoteUser",gangID,tonumber(selectedMember))
                    else 
                        tLUNA.notify("~r~You don't have permission to promote!")
                    end 
                else 
                    tLUNA.notify("~r~No gang member selected")
                end 
            end 
        else 
        ...
        if CursorInArea(0.6182,0.6822,0.4138,0.4694)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if selectedMember~=nil and PlayerIsInGang and gangID~=nil then 
                    if gangPermission>=4 then 
                        TriggerServerEvent("LUNA:DemoteUser",gangID,selectedMember)
                    else 
                        tLUNA.notify("~r~You don't have permission to demote!")
                    end 
                else 
                    tLUNA.notify("~r~No gang member selected")
                end 
                ...
                if selectedMember~=nil then 
                    if gangPermission>=3 then 
                        if YesNoConfirm()then 
                            TriggerServerEvent("LUNA:kickMemberFromGang",gangID,selectedMember)
                        end 
                    else 
                        tLUNA.notify("~r~You don't have permission to kick!")
                    end 
                else 
                    tLUNA.notify("~r~No gang member selected")
                end 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if s~=nil then 
                    if gangPermission>=2 then 
                        TriggerServerEvent("LUNA:InviteUserToGang",gangID,s)
                    else 
                        tLUNA.notify("~r~You don't have permission to invite players")
                    end 
                else 
                    tLUNA.notify("No player name entered")
                end 
            end 
        else 
        ...
        if CursorInArea(0.3281,0.3723,0.6768,0.7074)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                PlaySound(-1,"SELECT","HUD_FRONTEND_DEFAULT_SOUNDSET",0,0,1)
                if e<=1 then 
                    tLUNA.notify("~r~Lowest page reached")
                else 
                    e=e-1 
                end 
            end 
        ...
        if CursorInArea(0.4130,0.4572,0.6712,0.7064)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                PlaySound(-1,"SELECT","HUD_FRONTEND_DEFAULT_SOUNDSET",0,0,1)
                if e>=f then 
                    tLUNA.notify("~r~Max page reached")
                else 
                    e=e+1 
                end 
            end 
    ...
    if showLogsGangUI then 
        DrawRect(0.501,0.525,0.421,0.387,0,0,0,150)
        DrawRect(0.501,0.308,0.421,0.047,0,255,255,255)
        DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"LUNA Gang - Logs",255,255,255,255,7,0)
        ...
        DrawAdvancedText(0.596,0.476,0.005,0.0028,1.88,"NEVER COMING",205,14,0,255,7,0)
    end
    if showSettingsGangUI then 
        DrawRect(0.501,0.525,0.421,0.387,0,0,0,150)
        DrawRect(0.501,0.308,0.421,0.047,0,255,255,255)
        DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"LUNA Gang - Settings",255,255,255,255,7,0)
        ...
        if CursorInArea(0.3187,0.3937,0.4712,0.5462)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                PlaySound(-1,"SELECT","HUD_FRONTEND_DEFAULT_SOUNDSET",0,0,1)
                if YesNoConfirm()then 
                    TriggerServerEvent("LUNA:memberLeaveGang",gangID)
                    setCursor(0)
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if gangPermission>=4 then 
                    if YesNoConfirm()==true and gangID~=nil then 
                        TriggerServerEvent("LUNA:DeleteGang",gangID)
                    end 
                else 
                    tLUNA.notify("~r~You don't have permission to disband!")
                end 
            end 
        else 
    ...
    --     DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"LUNA Gang - Turfs",255,255,255,255,7,0)
    ...
    if showGangUI then 
        ...
        DrawRect(0.501,0.396,0.375,0.046,0,255,255,255)
        DrawAdvancedText(0.591,0.399,0.005,0.0028,0.51,"LUNA Gangs",255,255,255,255,7,0)
        ...
        DrawAdvancedText(0.642,0.534,0.005,0.0028,0.4,"Gang Blips",255,255,255,255,7,0)
...
RegisterNetEvent("LUNA:Notify",function(u)
    tLUNA.notify(u)
end)
...
function closeToBarclays()
    ...
    if w<10.0 or x<10.0 or z<10.0 then 
        ...
        tLUNA.notify("~b~You must be near ~r~Santander ~b~to access gang funds.")
        return false 
    end 
end
