local a = {}
local b = 1
function tLUNA.createDynamicPed(c, d, e, f, g, h, i, j, k)
    local c = tLUNA.loadModel(c)
    local l = CreatePed(26, c, d.x, d.y, d.z, e, false, false)
    if not j then
        a[b] = {entity = l,modelHash = c,position = d,heading = e,static = f,animDict = g,animName = h,minDistance = i,distanceToPlayer = 0.0,cb = k,created = true}
        b = b + 1
    else
        a[j].entity = l
    end
        ...
        for n, o in pairs(a) do
            ...
            if a[n].distanceToPlayer <= o.minDistance then
                if not o.created then
                    tLUNA.createDynamicPed(o.modelHash,o.position,o.heading,o.static,o.animDict,o.animName,o.minDistance,n,o.cb)
                    a[n].created = true
                end
            else
                if o.created then
