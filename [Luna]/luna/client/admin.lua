...
RegisterCommand('noclip', function(source, args, RawCommand)
    ...
    TriggerServerEvent('LUNA:noClip')
    end
end)
...
function tLUNA.isNoclip()
    return noclip
end



index = 1 -- DONT TOUCH OR CNR WILL TOUCH YOU
...
Citizen.CreateThread(function()
    while true do
        ...
        if noclip then
            ...
            local dx, dy, dz = tLUNA.getCamDirection()
        ...
        if EntityCleanupGun then 
            ...
            if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_STAFFGUN') then
                if IsPlayerFreeAiming(plr) then 
                    ...
                    if yes then 
                        tLUNA.notify('~g~Deleted Entity: ' .. GetEntityModel(entity))
...
local function teleportToWaypoint()
    Citizen.CreateThread(function()
        ...
        if success then
            ...
            DoScreenFadeIn(250)
            tLUNA.notify('~g~Teleport success!')
        end
    end)
end
...
RegisterNetEvent("LUNA:vehicleMenu")
AddEventHandler("LUNA:vehicleMenu",function(DioMode, isInTicket)
    if getStaffLevel() > 0 then
	OMioDioMode = DioMode
    ticketStatus = isInTicket
    TriggerEvent('greenzoneStaffMode', OMioDioMode)
	if not OMioDioMode then
		tLUNA.setCustomization(adminTicketSavedCustomization)
        tLUNA.setRedzoneTimerDisabled(false)
        ...
        adminTicketSavedCustomization = tLUNA.getCustomization()
        gender = getModelGender()
        if gender == "male" then
            z="mp_m_freemode_01"
            local A=loadModel(z)
            tLUNA.setRedzoneTimerDisabled(true)
            tLUNA.setCustomization({modelhash=A})
            Wait(100)
...
RegisterCommand("dv", function()
    if OMioDioMode then
        TriggerEvent( "Ker:deleteVehicle" )
    else
...
RegisterCommand("fix", function()
    if OMioDioMode or tLUNA.getUserId() == 2 or tLUNA.getUserId() == 1 then
        TriggerServerEvent("Ker:fixVehicle")
    end
end)

RegisterNetEvent("Ker:fixVehicle")
...
RegisterNetEvent("LUNA:showBlips")
AddEventHandler("LUNA:showBlips",function()
    blips = not blips
    if blips then
        tLUNA.notify("~g~Blips enabled")
    else
        tLUNA.notify("~r~Blips disabled")
...
RegisterNetEvent("Ker:deleteVehicle")
AddEventHandler("Ker:deleteVehicle", function()
...
function GiveStaffClothing()
    ...
    elseif getStaffLevel() == 11 then
        SetPedComponentVariation(B,11,398,1,00) -- [Jacket]
