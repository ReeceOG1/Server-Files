...
function tLUNA.spawnGarageVehicle(vtype, name, pos, vehiclename, vehicleplate) -- vtype is the vehicle type (one vehicle per type allowed at the same time)
    ...
    if vehicle == nil then
        ...
        if HasModelLoaded(mhash) then
            ...
            TriggerServerEvent("LUNA:PayVehicleTax")
        ...
        tLUNA.notify("~r~Vehicle is already out.")
    end
end

function tLUNA.despawnGarageVehicle(vtype, max_range)
    if not IsPedInAnyVehicle(PlayerPedId(), false) then
        tLUNA.notify("~r~You need to be in a vehicle to store.")
        return
    end
    local Vehicle = tLUNA.getNearestVehicle(3)
...
function tLUNA.getNearestVehicle(radius)
    local x, y, z = tLUNA.getPosition()
...
function tLUNA.getNearestVehicl2()
    local ped = GetPlayerPed(-1)
    if IsPedSittingInAnyVehicle(ped) then
...
function tLUNA.fixeNearestVehicle(radius)
    local veh = tLUNA.getNearestVehicle(radius)
    if IsEntityAVehicle(veh) then
        SetVehicleFixed(veh)
    end
end

function tLUNA.replaceNearestVehicle(radius)
    local veh = tLUNA.getNearestVehicle(radius)
    if IsEntityAVehicle(veh) then
...
function tLUNA.getVehicleAtPosition(x, y, z)
    x = x + 0.0001
    y = y + 0.0001
    z = z + 0.0001
...
function tLUNA.getNearestOwnedVehicle(radius)
...
-- return ok,x,y,z
function tLUNA.getAnyOwnedVehiclePosition()
    for k, v in pairs(vehicles) do
...
-- return x,y,z
function tLUNA.getOwnedVehiclePosition(vtype)
...
function tLUNA.getOwnedVehicleId(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
...
-- vehicle commands
function tLUNA.vc_openDoor(hash, door_index)
...
function tLUNA.vc_closeDoor(hash, door_index)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tLUNA.vc_detachTrailer(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tLUNA.vc_detachTowTruck(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tLUNA.vc_detachCargobob(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tLUNA.vc_toggleEngine(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tLUNA.vc_toggleLock(hash)
    local vehicle = vehicles[hash]
    if vehicle then
        local veh = vehicle[3]
        ...
        if locked then -- unlock
            ...
            tLUNA.notify("Vehicle unlocked.")
        else -- lock
            SetVehicleDoorsLocked(veh, 2)
            SetVehicleDoorsLockedForAllPlayers(veh, true)
            tLUNA.notify("Vehicle locked.")
        end
    end
end