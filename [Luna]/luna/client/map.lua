function tLUNA.addBlip(x,y,z,idtype,idcolor,text)
...
-- return native id
function tLUNA.setNamedBlip(name,x,y,z,idtype,idcolor,text)
  tLUNA.removeNamedBlip(name) -- remove old one

  named_blips[name] = tLUNA.addBlip(x,y,z,idtype,idcolor,text)
  return named_blips[name]
end

-- remove a named blip
function tLUNA.removeNamedBlip(name)
  if named_blips[name] ~= nil then
    tLUNA.removeBlip(named_blips[name])
...
-- return marker id
-- function tLUNA.addMarker(x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
...
-- remove marker
-- function tLUNA.removeMarker(id)
...
-- return id
-- function tLUNA.setNamedMarker(name,x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
--   tLUNA.removeNamedMarker(name) -- remove old marker

--   named_markers[name] = tLUNA.addMarker(x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
--   return named_markers[name]
-- end

-- function tLUNA.removeNamedMarker(name)
--   if named_markers[name] ~= nil then
--     tLUNA.removeMarker(named_markers[name])
...
function tLUNA.setArea(name,x,y,z,radius,height)
...
-- remove area
function tLUNA.removeArea(name)
...
-- locked: boolean
-- doorswing: -1 to 1
function tLUNA.setStateOfClosestDoor(doordef, locked, doorswing)
...
function tLUNA.openClosestDoor(doordef)
  tLUNA.setStateOfClosestDoor(doordef, false, 0)
end

function tLUNA.closeClosestDoor(doordef)
  tLUNA.setStateOfClosestDoor(doordef, true, 0)
end
