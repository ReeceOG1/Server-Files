...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(LUNAConfig.PlayerSavingTime)
    if IsPlayerPlaying(PlayerId()) and state_ready then
        local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
        LUNAserver.updatePos({x,y,z})
        LUNAserver.updateHealth({tLUNA.getHealth()})
        LUNAserver.updateArmour({GetPedArmour(PlayerPedId())})
        LUNAserver.updateWeapons({tLUNA.getWeapons()})
        LUNAserver.updateCustomization({tLUNA.getCustomization()})
        TriggerServerEvent('LUNA:changeHairStyle')
        TriggerServerEvent('LUNA:ChangeTattoos')
    end
  end
end)
...
function tLUNA.getWeaponTypes()
  return weapon_types
end

local g = true
local h = 0
function tLUNA.canAnim()
    return g
end
function tLUNA.setCanAnim(i)
    if i then
        if h > 0 then
            h = h - 1
        end
        if h == 0 then
            ...
            print(string.format("Can not setCanAnim(true) as %d other references exist", h))
        end
    else
        h = h + 1
        g = false
...
function tLUNA.giveWeapons(weapons,clear_before)
  local player = GetPlayerPed(-1)
...
function tLUNA.giveWeaponAmmo(hash, amount)
...
function tLUNA.getDrawables(part)
...
function tLUNA.getDrawableTextures(part,drawable)
  local isprop, index = parse_part(part)
  if isprop then
...
function tLUNA.getCustomization()
...
function tLUNA.getCustomization2()
  local ped = PlayerPedId()
...
function tLUNA.setCustomization(custom) -- indexed [drawable,texture,palette] components or props (p0...) plus .modelhash or .model
    ...
    if custom then
      ...
      if mhash ~= nil then
        ...
        if HasModelLoaded(mhash) then
          ...
          local weapons = tLUNA.getWeapons()
          if GetEntityModel(PlayerPedId()) ~= GetHashKey("mp_m_freemode_01") then
            SetPlayerModel(PlayerId(), mhash)
          end
          tLUNA.giveWeapons(weapons,true)
