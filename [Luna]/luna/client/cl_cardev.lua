...
local m = module("luna-vehicles", "cfg/cfg_garages")
m=m.garage_types
                ...
                RageUI.Button("Cycle through seats" , nil, { RightLabel = '→→→'}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        ...
                        if IsPedInAnyVehicle(p) then
                            ...
                            tLUNA.notify("~r~Not in a vehicle.")
                        end
                    end
                end)
                ...
                if b then
                    RageUI.Checkbox("Return to normal Universe", "~r~Leave Car Dev Mode", b, {}, function(Hovered, Active, Selected, Checked)
                        if Selected then
                            ...
                            b=false
                            TriggerServerEvent("LUNA:setCarDev",b)
                        end
                    end)
                else
...
RegisterCommand('cardev', function()
    TriggerServerEvent('LUNA:openCarDev')
end)

function tLUNA.openCarDev()
...
function TakeVehScreenshot()
    ...
    Wait(3000)
    TriggerServerEvent('LUNA:takeCarScreenshot')
    Wait(5000)
...
local function a1()
    ...
    TriggerServerEvent('LUNA:getCarDevDebug',{a2})
end
...
local function a8()
    ...
    if CursorInArea(0.25,0.31,0.12,0.15)and IsDisabledControlJustPressed(0,24)then 
        a1()
        TriggerEvent("luna:PlaySound", "apple") 
        notify("~b~Successfully sent data via discord")
    end
