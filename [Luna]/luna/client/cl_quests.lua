...
local function e()
    tLUNA.loadAnimDict("anim@mp_player_intcelebrationfemale@air_guitar")
    TaskPlayAnim(
        PlayerPedId(),
...
local function i()
    ...
    if j == 0 then
        a = true
        SetResourceKvpInt("luna_quests_completedall", 1)
        TriggerServerEvent("LUNA:claimQuestReward")
    end
    return a
end
local function l()
    hasCompletedAllQuestsInt = GetResourceKvpInt("luna_quests_completedall")
    if not hasCompletedAllQuestsInt then
        SetResourceKvpInt("luna_quests_completedall", 0)
    end
    if hasCompletedAllQuestsInt == 0 then
        for m = 1, 50 do
            local n = GetResourceKvpInt("luna_quests_q" .. m .. "_status")
            if not n then
                SetResourceKvpInt("luna_quests_q" .. m .. "_status", 0)
                n = 0
            end
            if n == 0 then
                c[m] = true
...
local function o(p)
    c[p] = nil
    SetResourceKvpInt("luna_quests_q" .. p .. "_status", 1)
    TriggerServerEvent("LUNA:setQuestCompleted")
    i()
end
local function q()
    local j = 50
            ...
            local A = function(y)
                ...
                if IsControlJustReleased(0, 38) then
                    ...
                    if q() ~= 50 then
                        ...
                        DoScreenFadeIn(2000)
                        tLUNA.removeArea("quests_prop_id_" .. y.undiscoveredQID)
                        tLUNA.removeArea("quests_text_id_" .. y.undiscoveredQID)
                    else
                        r("~g~+7 Days LUNA Club Platinum", "You collected them all!", 600)
                        PlaySoundFrontend(-1, "Hack_Success", "DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS", 1)
                        ShakeGameplayCam("FAMILY5_DRUG_TRIP_SHAKE", 1.0)
