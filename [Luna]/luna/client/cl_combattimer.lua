...
function tLUNA.setRedzoneTimerDisabled(e)
    a = e
end
function tLUNA.isPlayerInRedZone()
    return globalInRedzone
end
local f = 0
function tLUNA.setPlayerCombatTimer(g, h)
    if tLUNA.isPurge() then
        return
    end
    b = g
    if h then
    ...
    if GetGameTimer() - f > 2500 or tLUNA.isStaffedOn() then
        TriggerServerEvent("LUNA:setCombatTimer", g)
        f = GetGameTimer()
    end
end
function tLUNA.getPlayerCombatTimer()
    return b, c
end
...
Citizen.CreateThread(function()
    for k, v in pairs(d) do
        local blip = AddBlipForRadius(v.pos.x, v.pos.y, v.pos.z, v.radius)

        SetBlipHighDetail(blip, true)
        SetBlipColour(blip, 1) -- adjust the color as needed
        SetBlipAlpha (blip, 128) -- adjust the alpha as needed

        -- Optional: Add a label to the blip
        -- BeginTextCommandSetBlipName("STRING")
        -- AddTextComponentString(k) -- This would set the blip's name to the key (e.g. "Rebel", "Heroin", etc.)
...
Citizen.CreateThread(
    function()
        while true do
            if not a then
                local r = GetEntityCoords(tLUNA.getPlayerPed())
                globalInRedzone = false
                for s, k in pairs(d) do
                    if i(r, k, false) then
                        globalInRedzone = true
                        local r = GetEntityCoords(tLUNA.getPlayerPed())
                        if not noclipActive or tLUNA.getStaffLevel() == 0 then
                            tLUNA.setPlayerCombatTimer(30, false)
                        end
                        local t
                        local u = false
                        while not u do
                            r = GetEntityCoords(tLUNA.getPlayerPed())
                            while i(r, k, true) do
                                r = GetEntityCoords(tLUNA.getPlayerPed())
                                t = r
                                if
                                    IsPedShooting(tLUNA.getPlayerPed()) and
                                        GetSelectedPedWeapon(tLUNA.getPlayerPed()) ~= GetHashKey("WEAPON_UNARMED")
                                 then
                                    tLUNA.setPlayerCombatTimer(60, true)
                                end
                                if b == 0 then
                                    DrawAdvancedText(
                                        0.931,
                            ...
                            if b == 0 then
                                ...
                                local v = k.pos - GetEntityCoords(tLUNA.getPlayerPed())
                                t = t + v * 0.01
                                if GetVehiclePedIsIn(tLUNA.getPlayerPed(), false) == 0 then
                                    TaskGoStraightToCoord(
                                        ...
                                        8.0,
                                        1000,
                                        GetEntityHeading(tLUNA.getPlayerPed()),
                                        0.0
                                    )
                                    local w = GetSoundId()
                                    PlaySoundFrontend(w, "End_Zone_Flash", "DLC_BTL_RB_Remix_Sounds", true)
                                    ReleaseSoundId(w)
                                    tLUNA.announceMpBigMsg("~r~WARNING", "Get back in the redzone!", 2000)
                                else
                                    SetEntityCoords(tLUNA.getPlayerPed(), t.x, t.y, t.z)
                                end
                                SetTimeout(
                                    1000,
                                    function()
                                        ClearPedTasks(tLUNA.getPlayerPed())
                                    end
                                )
                            end
                            Wait(0)
...
Citizen.CreateThread(
    function()
        while true do
            if b > 0 then
                if a then
                    tLUNA.setPlayerCombatTimer(0, false)
                else
                    b = b - 1
                    if b == 0 then
                        c = false
...
function tLUNA.isEmergencyService()
    return globalOnPoliceDuty or globalOnPrisonDuty or globalNHSOnDuty or globalLFBOnDuty
end
AddEventHandler(
    "LUNA:startCombatTimer",
    function(h)
        if not tLUNA.isEmergencyService() then
            tLUNA.setPlayerCombatTimer(60, h)
        end
    end
)
local function y()
    if not tLUNA.isEmergencyService() and not tLUNA.isInComa() then
        local z = PlayerPedId()
        ...
        if IsPedShooting(z) and not x[A] then
            tLUNA.setPlayerCombatTimer(60, true)
        elseif GetPlayerTargetEntity(tLUNA.getPlayerId()) and IsControlPressed(0, 24) then
            tLUNA.setPlayerCombatTimer(60, true)
        end
    end
    if b > 0 then
