...
function tLUNA.toggleHandcuff()
  ...
  if handcuffed then
    ...
    tLUNA.playAnim(true,{{"mp_arresting","idle",1}},true)
  else
    DeleteEntity(cuffs)
    tLUNA.stopAnim(true)
...
function tLUNA.setHandcuffed(flag)
  if handcuffed ~= flag then
    tLUNA.toggleHandcuff()
  end
end

function tLUNA.isHandcuffed()
  return handcuffed
end

-- (experimental, based on experimental getNearestVehicle)
function tLUNA.putInNearestVehicleAsPassenger(radius)
...
function tLUNA.putInNetVehicleAsPassenger(net_veh)
...
function tLUNA.putInVehiclePositionAsPassenger(x,y,z)
  local veh = tLUNA.getVehicleAtPosition(x,y,z)
  if IsEntityAVehicle(veh) then
    for i=1,GetVehicleMaxNumberOfPassengers(veh) do
      if IsVehicleSeatFree(veh,i) then
        SetPedIntoVehicle(GetPlayerPed(-1),veh,i)
...
function tLUNA.jail(x,y,z,radius)
  tLUNA.teleport(x,y,z) -- teleport to center
  jail = {x+0.0001,y+0.0001,z+0.0001,radius+0.0001}
end

-- unjail the player
function tLUNA.unjail()
  jail = nil
end

function tLUNA.isJailed()
  return jail ~= nil
end
