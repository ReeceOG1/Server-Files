...
RegisterCommand("carry",function(source, args)
    if not carry.InProgress then
        ...
        if closestPlayer then
            ...
            if targetSrc ~= -1 then
                TriggerServerEvent("LUNA:CarryRequest", targetSrc, OMioDioMode)
            else
...
RegisterNetEvent("LUNAEXTRAS:StartCarry")
AddEventHandler("LUNAEXTRAS:StartCarry", function(targetSrc)
...
RegisterNetEvent("LUNAEXTRAS:CarryTargetAsk")
AddEventHandler("LUNAEXTRAS:CarryTargetAsk", function(senderSrc)
    carryrequest = true
    Citizen.CreateThread(function()
        while carryrequest do
            if IsControlJustPressed(1, 83) then
                TriggerServerEvent("LUNAEXTRAS:CarryAccepted",senderSrc)
                carryrequest = false
            elseif IsControlJustPressed(1, 84) then
                TriggerServerEvent("LUNAEXTRAS:CarryDeclined",senderSrc)
                carryrequest = false
            end
            Wait(0)
        end
