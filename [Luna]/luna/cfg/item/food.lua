-- basic luna food.lua with tacos which give health.
...
local function gen(ftype, health)
    ...
    local name = LUNA.getItemName(idname)

    choices[act] = {function(player,choice)
      local user_id = LUNA.getUserId(player)
      if user_id ~= nil then
        if LUNA.tryGetInventoryItem(user_id,idname,1,false) then
          ...
          if ftype == "drink" then
            LUNAclient.notify(player,{"~b~Drinking "..name.."."})
            play_drink(player)
          elseif ftype == "eat" then
            LUNAclient.notify(player,{"~o~Eating "..name.."."})
            play_eat(player)
          end
