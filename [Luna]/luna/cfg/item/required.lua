...
items["money"] = {"Money","Packed money.",function(args)
  ...
  choices["Unpack"] = {function(player,choice,mod)
    ...
    -- if user_id ~= nil then
    --   local amount = LUNA.getInventoryItemAmount(user_id, idname)
    --   LUNA.prompt(player, "How much to unpack ? (max "..amount..")", "", function(player,ramount)
    --     ramount = parseInt(ramount)
    --     if LUNA.tryGetInventoryItem(user_id, idname, ramount, true) then -- unpack the money
    --       LUNA.giveMoney(user_id, ramount)
...
items["money_binder"] = {"Money binder","Used to bind £1000 of money.",function(args)
  ...
  choices["Bind money"] = {function(player,choice,mod) -- bind the money
    ...
    --   if money >= 1000 then
    --     if LUNA.tryGetInventoryItem(user_id, idname, 1, true) and LUNA.tryPayment(user_id,1000) then
    --       LUNA.giveInventoryItem(user_id, "money", 1000, true)
    --       LUNA.closeMenu(player)
    --     end
    --   else
    --     LUNAclient.notify(player,{LUNA.lang.money.not_enough()})
    --   end
    -- end
  end}
...
elseif args[2] == "WEAPON_AX50" then 
  return "AX-50"
elseif args[2] == "WEAPON_PHANTOM" then 
  return "LUNA Nebula Rifle"
elseif args[2] == "WEAPON_CMPCARBINE" then 
  return "LUNA Trader Carbine"
elseif args[2] == "WEAPON_ODIN" then 
  return "Odin"
elseif args[2] == "WEAPON_SPACEFLIGHTMP5" then 
  return "SpaceFlight SMG"
...
elseif args[2] == "WEAPON_RGXVANDAL" then 
  return "RGX Vandal"
elseif args[2] == "WEAPON_GLOW" then 
  return "GLOW SMG"
...
local wbody_choices = function(args)
  ...
  choices["Equip"] = {function(player,choice)
    local user_id = LUNA.getUserId(player)
    if user_id ~= nil then
      if LUNA.tryGetInventoryItem(user_id, fullidname, 1, true) then -- give weapon body
        local weapons = {}
        weapons[args[2]] = {ammo = 0}
        LUNAclient.giveWeapons(player, {weapons})
...
elseif args[2] == "WEAPON_VESPER" then 
  ...
  elseif args[2] == "WEAPON_SPACEFLIGHTMP5" then 
    return 10
  elseif args[2] == "WEAPON_WESTYARES" then 
    return 10
 -- VIP Gunstore -- 
elseif args[2] == "WEAPON_GOLDAK" then 
  return 10
...
local wammo_choices = function(args)
  ...
  choices["Load"] = {function(player,choice)
    local user_id = LUNA.getUserId(player)
    if user_id ~= nil then
      local amount = LUNA.getInventoryItemAmount(user_id, fullidname)
      TriggerClientEvent('checkAmmo', player)
      LUNA.prompt(player, "Amount to load ? (max "..amount..")", "", function(player,ramount)
        ramount = parseInt(ramount)

        LUNAclient.getWeapons(player, {}, function(uweapons)
            for i,v in pairs(LUNAAmmoTypes[ammotype]) do
               if uweapons[v] ~= nil then -- check if the weapon is equiped
                      if LUNA.tryGetInventoryItem(user_id, fullidname, ramount, true) then -- give weapon ammo
                        local weapons = {}
                        weapons[v] = {ammo = ramount}
                        LUNAclient.giveWeapons(player, {weapons,false})
                        LUNA.closeMenu(player)
                        TriggerEvent('LUNA:RefreshInventory', player)
                        return
                      end
                    --end
                 end 
...
items["heavyarmour"] = {"Heavy Armour Plate","Armour",function(args)
  local choices = {}
  choices['Equip'] = {function(player,choice)
    local PermID = LUNA.getUserId(player)
    local Ped = GetPlayerPed(player)
    local PedArmour = GetPedArmour(Ped)
    if Cooldown[player] then
      return LUNAclient.notify(player,{"~r~Server is still proccesing your request."})
    else
      if PermID ~= nil then
        if PedArmour > 95 then
          LUNAclient.notify(player,{"~r~You already have armour."})
        else
          if LUNA.hasPermission(PermID, "rebel.guns") then
            if LUNA.tryGetInventoryItem(PermID, "heavyarmour", 1, false) then
              Cooldown[player] = true;
              LUNAclient.playAnim(player, {false, armour_seq, false}) -- anim
              Wait(2500)
              LUNAclient.setArmour(player,{100})
              LUNAclient.notify(player,{"~g~Applied Armour Plate"})
              Cooldown[player] = false;
            end
          else
            LUNAclient.notify(player,{"~r~Missing Rebel License."})
          end
        end
      end
    end
...
local function play_morphine(player)
  ...
  LUNAclient.playAnim(player,{true,morphine_seq,false})
end
...
choices['Equip'] = {function(player,choice)
  local PermID = LUNA.getUserId(player)
  if PermID ~= nil then 
  if LUNA.tryGetInventoryItem(PermID, "morphine", 1, false) then 
...
-- items["repair_kit"] = {"Repair Kit", "Used to repair vehicles", function(args)
--   local choices = {}
--   choices["Use"] = {function(player,choice)
--       local user_id = LUNA.getUserId(player)
--       if user_id ~= nil then
--           -- Trigger the /repair command when the player uses the repair kit
--           -- This assumes that the /repair command is defined elsewhere in your scripts
...
for i,v in pairs(LUNAAmmoTypes) do
  -- print(i)
