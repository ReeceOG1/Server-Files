...
function LUNA.getUserGroups(user_id)
...
function LUNA.addUserGroup(user_id, group)
    print("[ID]: " .. user_id.. " Got group: ".. group)
    if not LUNA.hasGroup(user_id, group) then
        local user_groups = LUNA.getUserGroups(user_id)
        ...
        if ngroup then
            ...
            TriggerEvent("LUNA:playerJoinGroup", user_id, group, gtype)
        end
    end
end
...
function LUNA.getUserGroupByType(user_id, gtype)
    local user_groups = LUNA.getUserGroups(user_id)
    for k, v in pairs(user_groups) do
        local kgroup = groups[k]
        if kgroup then
...
function LUNA.getUsersByGroup(group)
    local users = {}

    for k, v in pairs(LUNA.rusers) do
        if LUNA.hasGroup(tonumber(k), group) then
...
function LUNA.getUsersByPermission(perm)
    local users = {}

    for k, v in pairs(LUNA.rusers) do
        if LUNA.hasPermission(tonumber(k), perm) then
            table.insert(users, tonumber(k))
        end
    end
...
function LUNA.removeUserGroup(user_id, group)
    local user_groups = LUNA.getUserGroups(user_id)
    if group ~= nil and group ~= "" then
        ...
        TriggerEvent("LUNA:playerLeaveGroup", user_id, group, gtype)
...
function LUNA.hasGroup(user_id, group)
    local user_groups = LUNA.getUserGroups(user_id)
...
-- check if the user has a specific permission
function LUNA.hasPermission(user_id, perm)
    local user_groups = LUNA.getUserGroups(user_id)
    ...
    if fchar == "@" then -- special aptitude permission
        ...
        if #parts == 3 then -- decompose group.aptitude.operator
            ...
            local alvl = math.floor(LUNA.expToLevel(LUNA.getExp(user_id, group, aptitude)))
    ...
    elseif fchar == "#" then -- special item permission
        ...
        if #parts == 2 then -- decompose item.operator
            ...
            local amount = LUNA.getInventoryItemAmount(user_id, item)
...
function LUNA.hasPermissions(user_id, perms)
...
local function ch_select(player, choice)
    ...
    if user_id ~= nil then
        LUNA.addUserGroup(user_id, choice)
        ...
        for k, v in pairs(selectors) do
            ...
            if gcfg and menu then
                ...
                local function selector_enter()
                    local user_id = LUNA.getUserId(source)
                    if user_id ~= nil and LUNA.hasPermissions(user_id, gcfg.permissions or {}) then
                ...
                LUNAclient.addBlip(source, {x, y, z, gcfg.blipid, gcfg.blipcolor, k})
                LUNAclient.addMarker(source, {x, y, z - 1, 0.7, 0.7, 0.5, 255, 154, 24, 125, 150})

                LUNA.setArea(source, "LUNA:gselector:" .. k, x, y, z, 1, 1.5, selector_enter, selector_leave)
            end
        end
    end
end
    ...
    if first_spawn then
        ...
        if user ~= nil then
            for k, v in pairs(user) do
                LUNA.addUserGroup(user_id, v)
            end
        end

        -- add default group user
        LUNA.addUserGroup(user_id, "user")
    end
