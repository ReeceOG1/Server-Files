    ...
    --   TriggerClientEvent('ECP:ToggleNUIFocus', player, false)
      LUNA.prompt(player,lang.inventory.trash.prompt({LUNA.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
        ...
        --   TriggerClientEvent('ECP:ToggleNUIFocus', player, true)
          TriggerEvent('LUNA:RefreshInventory', LUNA.getUserSource(user_id))
          ...
          LUNA.prompt(player,lang.inventory.give.prompt({LUNA.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
            ...
            if new_weight <= LUNA.getInventoryMaxWeight(nuser_id) then
              if LUNA.tryGetInventoryItem(user_id,idname,amount,true) then
                ...
                TriggerEvent('LUNA:RefreshInventory', nplayer)
...
function LUNA.giveInventoryItem(user_id,idname,amount,notify)
  ...
  if data and amount > 0 then
    ...
    TriggerClientEvent('LUNA:FetchInventoryCL', LUNA.getUserSource(user_id))
    if notify then
...
function LUNA.RunTrashTask(source, itemName)
    local choices = LUNA.getItemChoices(itemName)
...
function LUNA.RunGiveTask(source, itemName)
    local choices = LUNA.getItemChoices(itemName)
...
function LUNA.RunInventoryTask(source, itemName)
    local choices = LUNA.getItemChoices(itemName)
    if choices['Use'] then 
...
function LUNA.getUserItemAmount(user_id,idname)
  local data = LUNA.getUserDataTable(user_id)
...
function LUNA.getUserHasItem(user_id,idname)
  local data = LUNA.getUserDataTable(user_id)
