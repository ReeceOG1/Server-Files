...
function LUNA.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  LUNAclient.openMenuData(source,{menudata})
end

-- force close player menu
function LUNA.closeMenu(source)
  LUNAclient.closeMenu(source,{})
end
...
function LUNA.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  LUNAclient.prompt(source,{title,default_text})
end
...
function LUNA.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  LUNAclient.request(source,{id,text,time}) -- send request to client
...
function LUNA.registerMenuBuilder(name, builder)
...
function LUNA.buildMenu(name, data, cbr)
...
function LUNA.openMainMenu(source)
  LUNA.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    LUNA.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tLUNA.closeMenu(id)
...
function tLUNA.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tLUNA.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tLUNA.requestResult(id,ok)
...
function tLUNA.openMainMenu()
  LUNA.openMainMenu(source)
end
...
function LUNA.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for LUNA.addStaticMenuChoices calls
    ...
    for k,v in pairs(cfg.static_menus) do
      ...
      if menu and smenu then
        local function smenu_enter()
          local user_id = LUNA.getUserId(source)
          if user_id ~= nil and LUNA.hasPermissions(user_id,smenu.permissions or {}) then
        ...
        LUNAclient.addBlip(source,{x,y,z,smenu.blipid,smenu.blipcolor,smenu.title})
        LUNAclient.addMarker(source,{x,y,z-1,0.7,0.7,0.5,255,226,0,125,150})

        LUNA.setArea(source,"LUNA:static_menu:"..k,x,y,z,1,1.5,smenu_enter,smenu_leave)
      end
    end
  end
end
...
AddEventHandler("LUNA:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    LUNAclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
