...
-- name = partid
function LUNA.openSkinshop(source, parts)
    ...
    if user_id ~= nil then
        ...
        if data.cloakroom_idle ~= nil then
            LUNAclient.notify(source, {lang.common.wearing_uniform()})
        end

        -- get old customization to compute the price
        LUNAclient.getCustomization(source, {}, function(old_custom)
            ...
            local ontexture = function(player, choice)
                ...
                LUNAclient.setCustomization(source, {custom})
            end
            ...
            local ondrawable = function(player, choice, mod)
                if mod == 0 then -- tex variation
                    ...
                    LUNAclient.getDrawableTextures(source, {parts[choice], drawable[1]}, function(n)
                        -- print(n)
                        textures[choice][2] = n
            ...
            for k, v in pairs(parts) do -- for each part, get number of drawables and build menu
                ...
                -- get max drawables
                LUNAclient.getDrawables(source, {v}, function(n)
                    drawables[k][2] = n -- set max
                   
                end)

                -- get max textures for this drawable
                LUNAclient.getDrawableTextures(source, {v, drawables[k][1]}, function(n)
            ...
            menudata.onclose = function(player)
                -- compute price
                LUNAclient.getCustomization(source, {}, function(custom)
                    local price = 0
                    ...
                    if LUNA.tryPayment(user_id, price) then
                        if price > 0 then
                            LUNAclient.notify(source, {lang.money.paid({price})})
                        end
                    else
                        LUNAclient.notify(source, {lang.money.not_enough()})
                        -- revert changes
                        LUNAclient.setCustomization(source, {old_custom})
                    end
                end)
            end
            -- open menu
        ...
        for k, v in pairs(skinshops) do
            ...
            local skinshop_enter = function(source)
                ...
                if user_id ~= nil then
                    ...
                    LUNA.openSkinshop(source, parts)
                end
            end
            ...
            LUNAclient.addBlip(source, {x, y, z, 73, 3, lang.skinshop.title()})
            LUNAclient.addMarker(source, {x, y, z - 1, 0.7, 0.7, 0.5, 0, 255, 125, 125, 150})

            LUNA.setArea(source, "LUNA:skinshop" .. k, x, y, z, 1, 1.5, skinshop_enter, skinshop_leave)
        end
    end
end
