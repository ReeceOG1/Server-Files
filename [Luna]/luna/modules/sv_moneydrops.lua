...
function tLUNA.MoneyDrop()
    ...
    if LUNA.tryPayment(user_id,money) then
        MoneydropEntities[moneydropnetid].Money = money
    end
end

    RegisterNetEvent('LUNA:Moneydrop')
    AddEventHandler('LUNA:Moneydrop', function(netid)
        local source = source
        if MoneydropEntities[netid] and not MoneydropEntities[netid][3] and #(GetEntityCoords(MoneydropEntities[netid][1]) - GetEntityCoords(GetPlayerPed(source))) < 10.0 then
            ...
            if user_id ~= nil then
                ...
                TriggerClientEvent("LUNA:MoneyNotInBag",source)
                if MoneydropEntities[netid].Money ~= 0 then
                    --print("YES")
                    LUNA.giveMoney(user_id,MoneydropEntities[netid].Money)
                    LUNAclient.notify(source,{"You have taken £"..tonumber(MoneydropEntities[netid].Money)})
                    MoneydropEntities[netid].Money = 0
                else
                    --print("NO")
                end
            else
                LUNAclient.notify(source,{"~r~The money drop is already being taken"})
