local Tunnel = module("luna", "lib2/Tunnel")
local Proxy = module("luna", "lib2/Proxy")
local LUNA = Proxy.getInterface("LUNA")
local LUNAclient = Tunnel.getInterface("LUNA","Blackjack")
...
AddEventHandler("Blackjack:requestSitAtBlackjackTable", function(chairId)
    ...
    if source ~= nil then
        ...
        if chairId > 7 and LUNA.hasGroup({user_id,"HighRoller"}) then
        ...
        if chairId > 7 and not LUNA.hasGroup({user_id,"HighRoller"}) then
            LUNAclient.notify(source,{'~r~You need to be a HighRoller to sit at this table.'})
            return
        end
        if chairId <= 7 then
            blackjackTables[chairId] = source
                    ...
                    MySQL.query("casinochips/get_chips", {user_id = user_id}, function(rows, affected)
                        ...
                        if chips >= betAmount then
                            if chairId < 8 and betAmount > 100000 then
                                LUNAclient.notify(source,{'~r~Maximum bet at this table is £100,000.'})
                                return
                            end
                            ...
                            TriggerClientEvent("blackjack:notify",source,"~g~Bet placed: " .. getMoneyStringFormatted(betAmount) .. " chips.")
                        else 
                                        ...
                                        while nextCardCount >= 1 do
                                            ...
                                            if blackjackGameData[gameId][source][2][nextCardCount] == true then --if hit 
                                                ...
                                                if currentHand > 21 then
                                                    ...
                                                    TriggerClientEvent("blackjack:notify",source,"~r~-"..getMoneyStringFormatted(lostAmount).." chips")
                                                ...
                                                if dealerHand > 21 then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("blackjack:notify",source,"~g~+"..getMoneyStringFormatted(potentialWinAmount).." chips")
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', source, "^1[Diamond Casino]", { 128, 128, 128 }, ""..GetPlayerName(source).." has WON "..tostring(getMoneyStringFormatted(potentialWinAmount)).." chips!", "goodalert")
                                                            end
                                                        end
                                                    end
                                                ...
                                                elseif currentHand == dealerHand then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("blackjack:notify",source,"~r~-"..getMoneyStringFormatted(potentialPushAmount).." chips")
                                                            if potentialPushAmount > 10000000 then
