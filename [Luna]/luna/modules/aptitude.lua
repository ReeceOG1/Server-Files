...
function LUNA.defAptitudeGroup(group, title)
  gaptitudes[group] = {_title = title}
end

-- max_exp: -1 => LUNA
function LUNA.defAptitude(group, aptitude, title, init_exp, max_exp)
...
function LUNA.getAptitudeDefinition(group, aptitude)
  local vgroup = gaptitudes[group]
...
function LUNA.getAptitudeGroupTitle(group)
...
function LUNA.getUserAptitudes(user_id)
...
function LUNA.varyExp(user_id, group, aptitude, amount)
  local def = LUNA.getAptitudeDefinition(group, aptitude)
  local uaptitudes = LUNA.getUserAptitudes(user_id)
  if def ~= nil and uaptitudes ~= nil then
    -- apply variation
    local exp = uaptitudes[group][aptitude]
    local level = math.floor(LUNA.expToLevel(exp)) -- save level before variation
    ...
    if player ~= nil then
      local group_title = LUNA.getAptitudeGroupTitle(group)
      local aptitude_title = def[1]

      --- exp
      if amount < 0 then
        LUNAclient.notify(player,{lang.aptitude.lose_exp({group_title,aptitude_title,-1*amount})})
      elseif amount > 0 then
        LUNAclient.notify(player,{lang.aptitude.earn_exp({group_title,aptitude_title,amount})})
      end
      --- level up/down
      local new_level = math.floor(LUNA.expToLevel(exp))
      local diff = new_level-level
      if diff < 0 then
        LUNAclient.notify(player,{lang.aptitude.level_down({group_title,aptitude_title,new_level})})
      elseif diff > 0 then
        LUNAclient.notify(player,{lang.aptitude.level_up({group_title,aptitude_title,new_level})})
      end
    end
  end
end

function LUNA.levelUp(user_id, group, aptitude)
  local exp = LUNA.getExp(user_id,group,aptitude)
  local next_level = math.floor(LUNA.expToLevel(exp))+1
  local next_exp = LUNA.levelToExp(next_level)
  local add_exp = next_exp-exp
  LUNA.varyExp(user_id, group, aptitude, add_exp)
end

function LUNA.levelDown(user_id, group, aptitude)
  local exp = LUNA.getExp(user_id,group,aptitude)
  local prev_level = math.floor(LUNA.expToLevel(exp))-1
  local prev_exp = LUNA.levelToExp(prev_level)
  local add_exp = prev_exp-exp
  LUNA.varyExp(user_id, group, aptitude, add_exp)
end

function LUNA.getExp(user_id, group, aptitude)
  local uaptitudes = LUNA.getUserAptitudes(user_id)
...
function LUNA.setExp(user_id, group, aptitude, amount)
  local exp = LUNA.getExp(user_id, group, aptitude)
  LUNA.varyExp(user_id, group, aptitude, amount-exp)
end

-- return float
function LUNA.expToLevel(exp)
  return (math.sqrt(1+8*exp/exp_step)-1)/2
end

-- return integer
function LUNA.levelToExp(lvl)
...
for k,v in pairs(cfg.gaptitudes) do
  LUNA.defAptitudeGroup(k,v._title or "")
  for l,w in pairs(v) do
    if l ~= "_title" then
      LUNA.defAptitude(k,l,w[1],w[2],w[3])
    end
  end
end
...
local function ch_aptitude(player,choice)
  ...
  if user_id ~= nil then
    if player_apts[player] then -- hide
      ...
      LUNAclient.removeDiv(player,{"user_aptitudes"})
    else -- show
      local content = ""
      local uaptitudes = LUNA.getUserAptitudes(user_id)
      for k,v in pairs(uaptitudes) do
        -- display group
        content = content..lang.aptitude.display.group({LUNA.getAptitudeGroupTitle(k)}).."<br />"
        for l,w in pairs(v) do
          local def = LUNA.getAptitudeDefinition(k,l)
          if def then
            -- display aptitude
            local exp = uaptitudes[k][l]
            local flvl = LUNA.expToLevel(exp)
...
.div_user_aptitudes .dprogressbar{
  ...
  height: 20px;
      ...
      LUNAclient.setDiv(player,{"user_aptitudes",css, content})
    end
  end
end
