...
AddEventHandler("LSC:buttonSelected", function(name, button)
	...
	if user_id then
		TriggerClientEvent("LSC:buttonSelected", source, name, button, LUNA.tryFullPayment(LUNA.getUserId(source), button.price or 0)) -- money
	end
end)
...
AddEventHandler("LSC:finished", function(veh)
	...
	local user_id = LUNA.getUserId(source)
	if user_id then
		MySQL.execute("LUNAls/update_vehicle_modifications", {user_id = user_id, vehicle = veh.model, modifications = json.encode({color = veh.color, extraColor = veh.extracolor, neon = veh.neon, neonColor = veh.neoncolor, xenonColor = veh.xenoncolor, smokeColor = veh.smokecolor, wheelType = veh.wheeltype, bulletProofTyres = veh.bulletProofTyres, windowTint = veh.windowtint, plateIndex = veh.plateindex, mods = veh.mods})})
	end
end)
...
AddEventHandler("LSC:applyModifications", function (model, vehicle)
	...
	if model and vehicle and user_id then
		local rows = MySQL.query("LUNAls/get_vehicle_modifications", {user_id = user_id, vehicle = model}, function(rows, affected) 
			if rows ~= nil then 
				if #rows > 0 then
