...
AddEventHandler("playerJoining", function()
    ...
    exports['ghmattimysql']:execute("SELECT * FROM gmt_dvsa WHERE user_id = @user_id", {user_id = user_id}, function(result)
        if next(result) then 
            for k,v in pairs(result) do
                if v.user_id == user_id then
                    ...
                    local updateddata = exports['ghmattimysql']:executeSync("SELECT * FROM gmt_dvsa WHERE user_id = @user_id", {user_id = user_id})[1]
            ...
            exports['ghmattimysql']:execute("INSERT INTO gmt_dvsa (user_id,licence,datelicence) VALUES (@user_id, 'none',"..os.date("%d/%m/%Y")..")", {user_id = user_id})
            local data1 = {}
            local licence = {}
            local date = os.date("%d/%m/%Y")
            local updateddata = exports['ghmattimysql']:executeSync("SELECT * FROM gmt_dvsa WHERE user_id = @user_id", {user_id = user_id})[1]
...
AddEventHandler("GMT:dvsaBucket", function(bool)
    ...
    elseif not bool then
        if currenttests[user_id] ~= nil then
            GMTclient.notify(source,{'You already have a test in progress.'})
            return
        end
        ...
        tGMT.setBucket(source, currenttests[user_id].bucket)
    end
end)
...
AddEventHandler("GMT:candidatePassed", function(seriousissues,minorissues,minorreasons)
    ...
    local licence
    local user_id = GMT.getUserId(source)
    exports['ghmattimysql']:execute('SELECT * FROM gmt_dvsa WHERE user_id = @user_id', {user_id = user_id}, function(GotLicence)
        ...
        if licence == "active" then
            exports['ghmattimysql']:execute("UPDATE gmt_dvsa SET licence = 'full', testsaves = @testsaves WHERE user_id = @user_id", {user_id = user_id,testsaves=json.encode(previoustests)}, function() end)
            Wait(100)
            dvsaUpdate(user_id)
        end
    end)
...
AddEventHandler("GMT:candidateFailed", function(seriousissues,minorissues,seriousreasons,minorreasons)    
    ...
    exports['ghmattimysql']:execute('SELECT * FROM gmt_dvsa WHERE user_id = @user_id', {user_id = user_id}, function(GotLicence)
        ...
        if licence == "active" then
            exports['ghmattimysql']:execute("UPDATE gmt_dvsa SET testsaves = @testsaves WHERE user_id = @user_id", {user_id = user_id,testsaves=json.encode(previoustests)}, function() end)
            Wait(100)
            dvsaUpdate(user_id)
        end
    end)
...
AddEventHandler("GMT:beginTest", function()
    ...
    if data.licence == "active" then
        ...
        TriggerEvent("GMT:acBan", user_id, 11, GetPlayerName(source), source, 'Attempted to Trigger Driving Test with a non-active licence.')
    end
end)
...
AddEventHandler("GMT:surrenderLicence", function()
    ...
    if data.licence == "banned" then
        GMTclient.notify(source,{'You are already banned from driving.'})
        TriggerEvent("GMT:acBan", user_id, 11, GetPlayerName(source), source, 'Attempted to surrender licence when already banned.')
        return
    end
    if data.licence == "active" or data.licence == "full" then
        exports['ghmattimysql']:execute("UPDATE gmt_dvsa SET licence = @licence WHERE user_id = @user_id", {licence = "none", user_id = user_id})
        exports['ghmattimysql']:execute("UPDATE gmt_dvsa SET id = @id WHERE user_id = @user_id", {id = uuid, user_id = user_id})
        Wait(100)
        dvsaUpdate(user_id)
    end
end)
...
AddEventHandler("GMT:activateLicence", function()
    ...
    if data.licence == "none" then
        exports['ghmattimysql']:execute("UPDATE gmt_dvsa SET licence = @licence, datelicence = @datelicense WHERE user_id = @user_id", {licence = "active", datelicense = os.date("%d/%m/%Y"), user_id = user_id})
        exports['ghmattimysql']:execute("UPDATE gmt_dvsa SET id = @id WHERE user_id = @user_id", {id = uuid, user_id = user_id})
        Wait(100)
        dvsaUpdate(user_id)
    end
...
RegisterServerEvent("GMT:speedCameraFlashServer",function(speed)
    ...
    if tonumber(bank) > fine then
        ...
        GMTclient.notify(source,{'You could not afford the fine. Benefits paid.'})
        return
    end
end)
...
AddEventHandler('GMT:speedGunFinePlayer', function(temp, speed)
    ...
    if GMT.hasPermission(user_id, 'police.armoury') then
      ...
      GMTclient.notify(source, { "Fined "..GetPlayerName(temp).." £"..getMoneyStringFormatted(fine).." for going "..speed.."MPH over the speed limit."})
    end
end)
...
RegisterCommand('setup', function(source)
    ...
    if GMT.hasPermission(user_id, 'police.armoury') then
        if speedTraps[user_id] then
            ...
            GMTclient.notify(source,{'Speed Trap Removed.'})
        else
            local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(source)))
            GMTclient.addBlip(-1,{x,y,z,419,0,"Speed Camera",2.5})
            speedTraps[user_id] = {x,y,z}
            GMTclient.notify(source,{'~g~Speed Trap Setup.'})
        end
    end
end)