...
AddEventHandler('GMT:checkTurfCapture', function(turfid)
    local source = source
	local user_id = GMT.getUserId(source)
	if not GMT.hasPermission(user_id, 'police.armoury') or not GMT.hasPermission(user_id, 'nhs.menu') then
		if turfData[turfid].cooldown > 0 then 
			GMTclient.notify(source, {'This turf is on cooldown for another '..turfData[turfid].cooldown..' seconds.'})
			return
		end
		exports['ghmattimysql']:execute('SELECT * FROM gmt_gangs', function(gotGangs)
			for K,V in pairs(gotGangs) do
...
AddEventHandler('GMT:initiateGangCapture', function(x,y)
	...
	if not GMT.hasPermission(user_id, 'police.armoury') or not GMT.hasPermission(user_id, 'nhs.menu') then
		if not turfData[x].beingCaptured then
								...
								exports['ghmattimysql']:execute('SELECT * FROM gmt_gangs', function(gotGangs)
									for K,V in pairs(gotGangs) do
										if V.gangname == turfData[x].gangOwner then
											for I,L in pairs(json.decode(V.gangmembers)) do
												if GMT.getUserSource(I) ~= nil then
			...
			GMTclient.notify(source, {'This turf is currently being captured.'})
		end
	else
		GMTclient.notify(source, {'You cannot capture a turf while on duty.'})
	end
end)
					...
					for I,L in pairs(json.decode(V.gangmembers)) do
						if tostring(user_id) == I then
							...
							for a,b in pairs(json.decode(V.gangmembers)) do
								...
								if GMT.getUserSource(tonumber(a)) ~= nil then
			...
			exports['ghmattimysql']:execute('SELECT * FROM gmt_gangs', function(gotGangs)
				for a,b in pairs(gotGangs) do
					if v.gangOwner == b.gangname then
						...
						exports['ghmattimysql']:execute('UPDATE gmt_gangs SET funds = funds+@funds WHERE gangname = @gangname', {funds = amount, gangname = b.gangname})
					end
				end
			end)
		end
