...
local function Get_Client_Discord_ID(source)
	return exports['ghmattimysql']:executeSync("SELECT discord_id FROM `gmt_verification` WHERE user_id = @user_id", {user_id = GMT.getUserId(source)})[1].discord_id
end
...
function tGMT.getFactionGroups(source)
    local source = source
...
function tGMT.checkForRole(user_id, role_id)
	local discord_id = exports['ghmattimysql']:executeSync("SELECT discord_id FROM `gmt_verification` WHERE user_id = @user_id", {user_id = user_id})[1].discord_id
