...
AddEventHandler('GMT:initiateStoreRobbery', function(store)
    ...
    if GMT.hasPermission(user_id, "police.armoury") then
        ...
        if #GMT.getUsersByPermission('police.armoury') > 2 then
            for k,v in pairs(stores) do
                if k == store then
                    if v.cooldown == 0 then
                        ...
                        TriggerEvent('GMT:PDRobberyCall', source, v.storename, v.position)
                    else
            ...
            GMTclient.notify(source, {'~r~There are not enough police on duty to rob a store.'})
            TriggerClientEvent('GMT:resetStorePed', source, store)
        end
    end
end)
...
AddEventHandler('GMT:completeSafeCracking', function(store)
    local source = source
    local user_id = GMT.getUserId(source)
    GMT.giveMoney(user_id, math.random(85000, 150000))
