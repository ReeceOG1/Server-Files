...
local availableItems = {
    {"wbody|WEAPON_UMP45GMT", 1},
    {"9mm Bullets", 250},
    {"wbody|WEAPON_MOSINGMT", 1},
...
AddEventHandler("GMT:playerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
       if #activeCrates > 0 then
            for k,v in pairs(activeCrates) do
                TriggerClientEvent('GMT:addCrateDropRedzone', source, v, crateLocations[v])
            end
       end
    end
end)
...
RegisterServerEvent('GMT:openCrate', function(crateID)
    ...
    if activeCrates[crateID].timeTillOpen > 0 then
        GMTclient.notify(source, {'~r~Loot crate unlocking in '..activeCrates[crateID].timeTillOpen..' seconds.'})
    else
