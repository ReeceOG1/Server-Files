...
RegisterNetEvent("GMTHousing:Buy")
AddEventHandler("GMTHousing:Buy", function(house)
    local source = source
                ...
                if noowner == nil then
                    getUserAddress(user_id, function(address) -- check if user already has a home
                        if GMT.tryFullPayment(user_id,v.buy_price) then --try payment
                            ...
                            GMTclient.notify(player,{"~g~You bought "..k.."!"}) --notify
                            ...
                            local webhook = 'https://discord.com/api/webhooks/1121808816183451780/KkUWhifcEm7l3uuJmxDyhoCVqdBkHkFeZT0YCgr8dySNmMSOMMKkCxbLnrxBElt-9iMp'
                            local embed = {
                                {
                                    ["color"] = "16777215",
                                    ["title"] = "GMT House Logs",
                            ...
                            GMTclient.notify(player,{"~r~You do not have enough money to buy "..k}) --not enough money
                        end
                    end)
                else
                    GMTclient.notify(player,{"~r~Someone already owns "..k})
                end
                if noowner ~= nil then
...
RegisterNetEvent("GMTHousing:Enter")
AddEventHandler("GMTHousing:Enter", function(house)
    local user_id = GMT.getUserId(source)
    local player = GMT.getUserSource(user_id)
        ...
        if huser_id ~= nil then
            if huser_id == user_id then
                accessHome(user_id, house, 1, function(ok) --enter home
                    if not ok then
                        GMTclient.notify(player,{"Unable to enter home"}) --notify unable to enter home for whatever reason
                    end
                end)
            else
                if hplayer ~= nil then --check if home owner is online
                    GMTclient.notify(player,{"~r~You do not own this home, Knocked on door!"})
                    GMT.request(hplayer,name.." knocked on your door!", 30, function(v,ok) --knock on door
                        if ok then
                            GMTclient.notify(player,{"~g~Doorbell Accepted"}) --doorbell accepted
                            accessHome(user_id, house, 1, function(ok) --enter home
                                if not ok then
                                    GMTclient.notify(player,{"~r~Unable to enter home!"}) --notify unable to enter home for whatever reason
                                end
                            end)
                        end
                        if not ok then
                            GMTclient.notify(player,{"~r~Doorbell Refused "}) -- doorbell refused
                        end
                    end)
                else
                    GMTclient.notify(player,{"~r~Home owner not online!"}) -- home owner not online
                end
            end
        else
            GMTclient.notify(player,{"~r~Nobody owns "..house..""}) --no home owner & user_id already doesn't have a house
        end
    end)
end)

RegisterNetEvent("GMTHousing:Leave")
AddEventHandler("GMTHousing:Leave", function(house)
    local user_id = GMT.getUserId(source)
    local player = GMT.getUserSource(user_id)

    leaveHome(user_id, house, 1, function(ok) --leave home
        if not ok then
            GMTclient.notify(player,{"~r~Unable to leave home!"}) --notify if some error
        end
    end)
end)

RegisterNetEvent("GMTHousing:Sell")
AddEventHandler("GMTHousing:Sell", function(house)
    local user_id = GMT.getUserId(source)
    local player = GMT.getUserSource(user_id)
    ...
    getUserByAddress(house, 1, function(huser_id)
        if huser_id == user_id then
                            ...
                            if target ~= nil then
                                GMT.prompt(player,"Price £: ","",function(player, amount) --ask for price
                                    if tonumber(amount) and tonumber(amount) > 0 then
                                        GMT.request(target,GetPlayerName(player).." wants to sell: " ..house.. " Price: £"..amount, 30, function(target,ok) --request new owner if they want to buy
                                            if ok then --bought
                                                ...
                                                if GMT.tryFullPayment(buyer_id,amount) then
                                                    ...
                                                    GMTclient.notify(player,{"~g~You have successfully sold "..house.." to ".. GetPlayerName(target).." for £"..amount.."!"}) --notify original owner
                                                    GMTclient.notify(target,{"~g~"..GetPlayerName(player).." has successfully sold you "..house.." for £"..amount.."!"}) --notify new owner
                                                    local webhook = 'https://discord.com/api/webhooks/1121808928599191734/f243vJ73W0C5gE5FO-7Y3ktR-qeD2f3BCm_-LQuvjd86AEWft7HZwLnBEhMFqeN0OPRW'
                                                    local embed = {
                                                        {
                                                            ["color"] = "16777215",
                                                            ["title"] = "GMT House Logs",
                                                    ...
                                                    GMTclient.notify(player,{"".. GetPlayerName(target).." doesn't have enough money!"}) --notify original owner
                                                    GMTclient.notify(target,{"~r~You don't have enough money!"}) --notify new owner
                                                end
                                            else
                                                GMTclient.notify(player,{""..GetPlayerName(target).." has refused to buy "..house.."!"}) --notify owner that refused
                                                GMTclient.notify(target,{"~r~You have refused to buy "..house.."!"}) --notify new owner that refused
                                            end
                                        end)
                                    else
                                        GMTclient.notify(player,{"~r~Price of home needs to be a number!"}) -- if price of home is a string not a int
                                    end
                                end)
                            else
            ...
            GMTclient.notify(player,{"~r~You do not own "..house.."!"})
        end
    end)
end)

RegisterNetEvent('GMTHousing:Rent')
AddEventHandler('GMTHousing:Rent', function(house)
    local user_id = GMT.getUserId(source)
    local player = GMT.getUserSource(user_id)
                                            ...
                                            if tonumber(duration) and tonumber(duration) > 0 then
                                                GMT.prompt(player, "Please replace text with YES or NO to confirm", "Rent Details:\nHouse: "..house.."\nRent Cost: "..amount.."\nDuration: "..duration.." hours\nRenting to player: "..GetPlayerName(target).."("..target_id..")",function(player,details)
                                                    if string.upper(details) == 'YES' then
                                                        ...
                                                        GMT.request(target,GetPlayerName(player).." wants to rent: " ..house.. " for "..duration.." hours, for £"..amount, 30, function(target,ok) --request new owner if they want to buy
                                                            if ok then 
                                                                local buyer_id = GMT.getUserId(target) --get perm id of new owner
                                                                amount = tonumber(amount) --convert amount str to int
                                                                if GMT.tryFullPayment(buyer_id,amount) then
                                                                    ...
                                                                    GMTclient.notify(player,{"~g~You have successfully rented "..house.." to ".. GetPlayerName(target).." for £"..amount.."!"}) --notify original owner
                                                                    GMTclient.notify(target,{"~g~"..GetPlayerName(player).." has successfully rented you "..house.." for £"..amount.."!"}) --notify new owner
                                                                    local webhook = 'https://discord.com/api/webhooks/1121809002163077211/Ikkj-OtX5cCIZpDTbttUAMyy3NVqHYnWzJE3jWOnZmxk9C68fvdLiE7H5FTJKuEukCrA'
                                                                    local embed = {
                                                                        {
                                                                            ["color"] = "16777215",
                                                                            ["title"] = "GMT House Logs",
                                                                ...
                                                                GMTclient.notify(player,{""..GetPlayerName(target).." has refused to rent "..house.."!"}) --notify owner that refused
                                                                GMTclient.notify(target,{"~r~You have refused to rent "..house.."!"}) --notify new owner that refused
                                                            end
                                                        end)
                                                    end
                                                end)
