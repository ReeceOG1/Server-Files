...
MySQL.createCommand("casinochips/remove_chips", "UPDATE gmt_casino_chips SET chips = (chips - @amount) WHERE user_id = @user_id")
    ...
    if source ~= nil then
        ...
        if chairId > 7 then
            if GMT.hasGroup(user_id,"Highroller") then
                ...
                GMTclient.notify(source,{'You need to be a Highroller to sit at this table.'})
                return
            end
        end
        if chairId <= 7 then
                    ...
                    MySQL.query("casinochips/get_chips", {user_id = user_id}, function(rows, affected)
                        ...
                        if chips >= betAmount then
                            if chairId < 8 and betAmount > 100000 then
                                GMTclient.notify(source,{'Maximum bet at this table is £100,000.'})
                                return
                            end
                            ...
                            tGMT.sendWebhook('blackjack-bet',"GMT Blackjack Logs", "> Player Name: **"..GetPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Amount: **"..getMoneyStringFormatted(tostring(betAmount)).."**")
                        else 
