function rank(_, arg)
    ...
    if GMT.getUserSource(user_id) ~= nil then
        ...
        exports['ghmattimysql']:execute("SELECT * FROM gmt_user_data WHERE user_id = @user_id", {user_id = user_id}, function(result) 
            if #result > 0 then
                local dvalue = json.decode(result[1].dvalue)
                local groups = dvalue.groups
                groups[rank] = true
                exports['ghmattimysql']:execute("UPDATE gmt_user_data SET dvalue = @dvalue WHERE user_id = @user_id", {dvalue = json.encode(dvalue), user_id = user_id}, function() end)
            end
        end)
    end  
    tGMT.sendWebhook('donation',"GMT Donation Logs", "> Player PermID: **"..user_id.."**\n> Package: **"..rank.."**")
end
...
function moneybag(_, arg)
    ...
    if GMT.getUserSource(user_id) ~= nil then
        ...
        exports['ghmattimysql']:execute("UPDATE gmt_user_moneys SET bank = bank + @amount WHERE user_id = @user_id", {amount = amount, user_id = user_id}, function() end)
    end
...
function plus(_, arg)
    ...
    tGMT.getSubscriptions(user_id, function(cb, plushours, plathours)
        TriggerClientEvent('GMT:smallAnnouncement', user_id, 'GMT Studios', "You have received "..newhours.."hrs of GMT Plus we appreciate your support!\n", 18, 10000)
        if cb then
...
function platinum(_, arg)
    ...
    local newhours = tonumber(arg[2])
    tGMT.getSubscriptions(user_id, function(cb, plushours, plathours)
        TriggerClientEvent('GMT:smallAnnouncement', user_id, 'GMT Studios', "You have received "..newhours.."hrs of GMT Platinum we appreciate your support!\n", 18, 10000)
        if cb then
