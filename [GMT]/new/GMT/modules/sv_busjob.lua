...
RegisterNetEvent("GMT:attemptBeginBusJob")
AddEventHandler("GMT:attemptBeginBusJob",function()
    local source=source
    local user_id=GMT.getUserId(source)
    local BusJobTable={}
    if GMT.hasGroup(user_id,"Bus Driver")then
        if not usersInBusJob[user_id] then
            ...
            TriggerClientEvent("GMT:beginBusJob",source)
            Wait(100)
            TriggerClientEvent("GMT:setNextBusJobBlip",source,busJobStops[BusJobTable.stopNumber].stopPosition)
        else
            GMTclient.notify(source,{"You are already a Bus Driver."})
        end
    else
        GMTclient.notify(source,{"~r~You are not clocked on as a Bus Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(busJobStops)do
                    if GMT.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(GMT.getUserSource(k)))-busJobStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            GMT.giveBankMoney(k,pay)
                            TriggerClientEvent("GMT:nextStopReachedBusJob",GMT.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#busJobStops then
                                TriggerClientEvent("GMT:endBusJob", GMT.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInBusJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("GMT:setNextBusJobBlip",GMT.getUserSource(k),busJobStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
