          ...
          if k == args[2] then
            if v.policeWeapon then
              if GMT.hasPermission(user_id, 'police.onduty.permission') then
                GMTclient.giveWeapons(player, {weapons, false})
              else
                GMTclient.notify(player, {'You can not equip this weapon.'})
              end
            else
              GMTclient.giveWeapons(player, {weapons, false})
            end
...
local wammo_choices = function(args)
  ...
  choices["Load"] = {function(player,choice)
    ...
    if user_id ~= nil then
      ...
      if string.find(fullidname, "Police") and not GMT.hasPermission(user_id, 'police.onduty.permission') then
        GMTclient.notify(player, {'You cannot load this ammo.'})
