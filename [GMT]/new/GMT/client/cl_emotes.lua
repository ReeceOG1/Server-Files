...
local function p(q)
    ...
    if not q and GetGameTimer() - e < 1500 then
        tGMT.notify("Emotes are being rate limited.")
        return
    end
    b = false
    f = nil
...
local function F(E)
    if tGMT.getPlayerVehicle() ~= 0 then
        tGMT.notify("Can not use scenarios whilst in a vehicle.")
        return
    end
    local A = PlayerPedId()
...
local function a7(E)
    if a6(E) then
        tGMT.notify("Can not this emote at this time.")
        return
    end
...
local function ag(ah)
    if a6() then
        tGMT.notify("Can not use shared emotes at this time.")
        return
    end
    local ai = a8()
    if ai ~= 0 then
        ...
        tGMT.notify("No player is near by.")
    end
end
local function aj(E)
...
local function aF(aC, aD, aE)
    if #aD < 1 then
        tGMT.notify("No emote name was specified.")
        return
    end
