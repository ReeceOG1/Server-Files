...
RegisterCommand("carry",function(f, g)
    if IsPedInAnyVehicle(PlayerPedId(), true) then
        ...
        if not globalPlayerInPrisonZone or tGMT.isStaffedOn() then
                    ...
                    if j ~= -1 then
                        ...
                        if GetEntityHealth(GetPlayerPed(j)) ~= 0 then
                            if not tGMT.isStaffedOn() and not globalLFBOnDuty then
                                ...
                                TriggerServerEvent("CarryPeople:sync", GetPlayerServerId(tGMT.getPlayerId()), target)
                            end
                        else
            ...
            tGMT.notify("You cannot carry in the prison.")
        end
    end
end,false)
RegisterNetEvent("CarryPeople:carryRequest",function()
    tGMT.notify("Someone is trying to carry you, press (~y~Y~w~) to accept (L~w~) to refuse")
    e = true
