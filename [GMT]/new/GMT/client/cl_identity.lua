...
local i=0
local U=false
RMenu.Add("identity","main",RageUI.CreateMenu("","City Hall",0,100,'gmt_cityhallui', 'gmt_cityhallui'))
...
Citizen.CreateThread(function()
    if true then 
        ...
        for j,k in pairs(b)do 
            tGMT.createArea("identity_"..j,k,1.5,6,g,h,i)
            tGMT.addMarker(k.x,k.y,k.z-0.2,0.5,0.5,0.5,0,50,255,170,50,20,false,false,true)
        end 
    end 
end)

RegisterNetEvent("GMT:gotCurrentIdentity")
AddEventHandler("GMT:gotCurrentIdentity", function(firstname,lastname,age)
    j=firstname
    k=lastname
    i=age
end)
...
AddEventHandler("GMT:gotNewIdentity", function(firstname,lastname,age)
...
RegisterNetEvent("GMT:showIdentity",function(v, w, x, y, z, A, B, C, D)
    o = true
    ...
    local H = tGMT.getFontId("Akrobat-ExtraLight")
    x = string.upper(x)
    y = string.upper(y)
    while o do
