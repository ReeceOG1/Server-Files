    ...
    if t then
        ...
        ontick_prisonControlPanel = function()
            ...
            elseif IsControlJustPressed(0, 168) then
                local u = tGMT.getUserId()
                if tGMT.isDev() then --and globalOnPrisonDuty then
                    d = true
                    setCursor(1)
                    inGUIGMT = true
                end
        ...
        for r = 1, #a.controlPanels, 1 do
            tGMT.createArea("prisonControlPanel_" .. r,a.controlPanels[r].coords,1.5,5,enter_prisonControlPanel,exit_prisonControlPanel,ontick_prisonControlPanel,{})
        end
    end
end)
...
RegisterNetEvent("GMT:prisonSyncLockdown",function(v)
    c = v
    if c then
        RefreshInterior(b)
...
tGMT.createThreadOnTick(func_drawControlPanel)
RegisterNetEvent("GMT:prisonUpdateGuardNumber",function(w)
    g = w
end)
AddEventHandler("GMT:onClientSpawn",function(s, t)
    if t then
        ...
        ontick_openPrisonDoor = function(x)
            ...
            local z = false
            if g > 4 then
                if tGMT.isStaffedOn() or globalOnPrisonDuty and x.overrideLock or globalOnPoliceDuty and x.pdOpen or (globalNHSOnDuty or globalLFBOnDuty) and x.nhsOpen then
                    y = " (E to toggle lock) "
                end
            else
                if tGMT.isStaffedOn() or globalOnPrisonDuty or globalOnPoliceDuty and x.pdOpen or (globalNHSOnDuty or globalLFBOnDuty) and x.nhsOpen then
                    y = " (E to toggle lock) "
                    z = true
                end
            end
            ...
            if h[x.doorHash].currentState == 5 or h[x.doorHash].currentState == 0 then
                tGMT.DrawText3D(x.coords, x.doorName .. y .. " 🔓", 0.45, 4)
            else
                tGMT.DrawText3D(x.coords, x.doorName .. y .. " 🔒", 0.45, 4)
            end
            if IsControlJustPressed(0, 38) and (tGMT.isStaffedOn() or globalOnPrisonDuty and (z or x.overrideLock) or globalOnPoliceDuty and x.pdOpen or (globalNHSOnDuty or globalLFBOnDuty) and x.nhsOpen) then
                tGMT.loadAnimDict("anim@heists@keycard@")
                Citizen.CreateThread(function()
                ...
                if h[x.doorHash].currentState == 1 then
                    TriggerServerEvent("GMT:prisonSetDoorState", x.doorHash, 5)
                else
                    TriggerServerEvent("GMT:prisonSetDoorState", x.doorHash, 1)
                end
            end
        end
        for k, l in pairs(a.doors) do
            for r = 1, #l, 1 do
                tGMT.createArea("openPrisonDoor_" .. l[r].doorHash,l[r].coords,1.5,5,enter_openPrisonDoor,exit_openPrisonDoor,ontick_openPrisonDoor,{doorName = l[r].name,doorHash = l[r].doorHash,coords = l[r].coords,overrideLock = l[r].overrideLocks or false,nhsOpen = l[r].nhsOpen or false,pdOpen = l[r].pdOpen or false})
            end
        end
        local A = function()
            TriggerServerEvent("GMT:enterPrisonAreaSyncDoors")
        end
        tGMT.createArea("prisonSyncDoorsOnAreaEnter",a.prisonMainCoords,250,250,A,function()end,function()end,{})
    end
end)
RegisterNetEvent('GMT:prisonAreaSyncDoors', function(table)
    h = table
    print(json.encode(h))
...
RegisterNetEvent("GMT:prisonSyncAllDoors",function(B)
    h = B
    for k, l in pairs(a.doors) do
        for r = 1, #l, 1 do
...
RegisterNetEvent("GMT:prisonSetAllDoorStates",function(C)
    if C ~= nil then
        for k, l in pairs(h) do
            l.currentState = C
...
RegisterNetEvent("GMT:prisonSyncDoor",function(D, C)
