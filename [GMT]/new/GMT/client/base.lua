...
function tGMT.allowWeapon(name)
...
function tGMT.removeWeapon(name)
  if allowedWeapons[name] then
...
function tGMT.checkWeapon(name)
...
function tGMT.notifyPicture2(icon, type, sender, title, text)
    SetNotificationTextEntry("STRING")
...
RegisterCommand('hidechat', function()
  tGMT.notify("~g~/showui to re-enable Chat")
  TriggerEvent('GMT:hideChat', true)
end)
