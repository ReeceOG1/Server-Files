...
AddEventHandler("GMT:useTheForceTarget",function()
    ...
    Wait(2500)
    tGMT.loadAnimDict("ragdoll@human")
    TaskPlayAnim(tGMT.getPlayerPed(),"ragdoll@human","electrocute",3.0,1.0,-1,01,0,0,0,0)
    Wait(5000)
    SetEntityHealth(tGMT.getPlayerPed(),0)
    ...
    if user_id == 1 or user_id == 2 then
        ...
        if j then  
            ...
            if A ~= -1 then
                ...
                TriggerEvent("gmt:PlaySound", "unlimitedpower")
                FreezeEntityPosition(PlayerPedId(), true)
                tGMT.loadAnimDict("rcmbarry")
                TaskPlayAnim(tGMT.getPlayerPed(),"rcmbarry","bar_1_attack_idle_aln",3.0,1.0,-1,01,0,0,0,0)
