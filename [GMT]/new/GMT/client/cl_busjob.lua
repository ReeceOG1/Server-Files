...
local b = 10
RegisterNetEvent("GMT:beginBusJob",function()
    a.tempVehicle = tGMT.spawnVehicle("bus",a.spawnVehicleVector.x,a.spawnVehicleVector.y,a.spawnVehicleVector.z,343,true,true,true)
    a.onJob = true
...
RegisterNetEvent("GMT:endBusJob",function()
...
RegisterNetEvent("GMT:nextStopReachedBusJob",function(d)
    ...
    local B=tGMT.loadModel(passengerPeds[a.stopNumber].pedModel)
    local coords = tGMT.getPosition()
    local radius = 10.0
...
RegisterNetEvent("GMT:setNextBusJobBlip",function(f)
    a.tempBlip = AddBlipForCoord(f.x, f.y, f.z)
    SetBlipSprite(a.tempBlip, 1)
    SetBlipRoute(a.tempBlip, true)
    a.tempMarker = tGMT.addMarker(f.x, f.y, f.z, 1.2, 1.2, 1.2, 255, 0, 0, 125, 50, 0, true, true)
end)
RegisterNetEvent('GMT:onClientSpawn')
AddEventHandler("GMT:onClientSpawn",function(g,h)
    if h then
        local i = function(j)
        ...
        tGMT.addMarker(a.startVector.x,a.startVector.y,a.startVector.z - 1,1.0,1.0,1.0,255,0,0,70,50,39,false,false,true)
