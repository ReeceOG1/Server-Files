...
local e = false
AddEventHandler("GMT:hsSoundsOn",function()
    e = true
end)
AddEventHandler("GMT:hsSoundsOff",function()
    e = false
end)
local f = false
AddEventHandler("GMT:codHMSoundsOn",function()
    f = true
end)
AddEventHandler("GMT:codHMSoundsOff",function()
    f = false
end)
...
AddEventHandler("entityDamaged",function(w, x, n, y)
    ...
    if x == z and w ~= z and IsEntityAPed(w) and GetEntityHealth(w) > 105 and not IsEntityDead(w) then
        Citizen.Wait(0)
        TriggerEvent("GMT:onPlayerDamagePed", w)
    end
end)
CreateThread(function()
...
local A = 0
RegisterNetEvent("GMT:onEntityHealthChange",function(B, u, v, C)
