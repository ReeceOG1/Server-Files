...
RegisterNetEvent("GMT:flashbangExplode",function(coords)
    if #(coords - tGMT.getPlayerCoords()) <= b then
...
function explodeFlashbang(coords)
    ...
    TaskPlayAnim(tGMT.getPlayerPed(),"anim@heists@ornate_bank@thermal_charge","cover_eyes_intro",-8.0,-8.0,1000,50,8.0,0,0,0)
...
function func_checkForFlashbang()
    if not a then
        if GetSelectedPedWeapon(tGMT.getPlayerPed()) == GetHashKey("WEAPON_FLASHBANG") then
            a = true
        else
            a = false
        end
    else
        if IsPedShooting(tGMT.getPlayerPed()) and not c then
            c = true
...
tGMT.createThreadOnTick(func_checkForFlashbang)