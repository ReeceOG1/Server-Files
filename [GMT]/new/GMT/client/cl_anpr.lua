...
RegisterCommand("flagf",function(o, p)
    if c ~= "" then
        ...
        if table.count(p) ~= 0 then
            ...
            tGMT.notify("Error~w~: You must provide a valid flag reason.")
        end
    end
end)
...
tGMT.createThreadOnTick(func_drawFullAnpr)
function anprGetVehicle(u)
    ...
    local w = GetVehiclePedIsIn(tGMT.getPlayerPed(), false)
    ...
    if B > 0 and IsEntityAVehicle(B) then
        ...
        if C ~= nil then
            if m[C] ~= nil then
                ...
                tGMT.notify(string.format("ANPR Alert~w~: Plate ~b~%s ~w~is flagged for:~b~~n~%s", C, D))
                n = true
...
function isPedInPoliceVehicle()
    if IsPedInAnyVehicle(tGMT.getPlayerPed(), false) then
        if GetVehicleClass(tGMT.getPlayerVehicle()) == 18 and globalOnPoliceDuty then
            return true
        end
    end
    return false
