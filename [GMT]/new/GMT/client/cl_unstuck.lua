...
local function b(c, d, e)
    ...
    Citizen.CreateThread(function()
        ...
        if not g then
            ...
            Wait(500)
            DoScreenFadeIn(500)
            tGMT.notifyPicture("CHAR_BLOCKED","CHAR_BLOCKED","Attempting to teleport to surface... \n\nIf you are still not on the surface please use /calladmin.","GMT","Utilities",nil,nil)
            TriggerServerEvent("GMT:unstuckSuccessful", d, e)
        else
            tGMT.notifyPicture("CHAR_BLOCKED","CHAR_BLOCKED","You moved during the countdown so the teleportation was cancelled.","GMT","Utilities",nil,nil)
        end
        f = false
        g = false
    end)
...
RegisterCommand("unstuck",function(q, r, s)
    local h = PlayerPedId()
    local t, p = o()
    if not t or tGMT.getPlayerCombatTimer() > 0 then
        tGMT.notifyPicture("CHAR_BLOCKED","CHAR_BLOCKED","You are unable to use this right now. Use /calladmin if you still need assistance.","GMT","Utilities",nil,nil)
    else
        tGMT.loadAnimDict("timetable@amanda@ig_4")
        TaskPlayAnim(h, "timetable@amanda@ig_4", "ig_4_base", 8.0, -8.0, -1, 0, 0.0, 0, 0, 0)
        RemoveAnimDict("timetable@amanda@ig_4")
        Wait(1000)
        b(45, tGMT.getPlayerCoords(), p)
    end
end)
...
RegisterCommand("reset",function(q, r, s)
    local h = PlayerPedId()
    local t =
        tGMT.getPlayerCombatTimer() > 0 or tGMT.isPlayerInRedZone() or GetRoomKeyFromEntity(PlayerPedId()) ~= 0 or
        IsEntityInWater(h) or
        IsPedInAnyVehicle(h, false)
    if u < GetGameTimer() then
        if t then
            tGMT.notifyPicture("CHAR_BLOCKED","CHAR_BLOCKED","You are unable to use this right now.","GMT","Utilities",nil,nil)
        else
        ...
        tGMT.notifyPicture("CHAR_BLOCKED","CHAR_BLOCKED","You must wait 60 seconds before using this again.","GMT","Utilities",nil,nil)
    end
end)
