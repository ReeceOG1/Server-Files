    ...
    function()
        while true do
            ...
            if a then
                ...
                tGMT.DrawText(0.32, 0.00, "X:~s~ " .. roundx, 0.4)
                tGMT.DrawText(0.38, 0.00, "Y:~s~ " .. roundy, 0.4)
                tGMT.DrawText(0.445, 0.00, "Z:~s~ " .. roundz, 0.4)
                heading = GetEntityHeading(ak)
                roundh = tonumber(string.format("%.2f", heading))
                tGMT.DrawText(0.50, 0.00, "H:~s~ " .. roundh, 0.4)
                local al, am, an = table.unpack(GetEntityRotation(PlayerPedId(), 1))
                tGMT.DrawText(0.38, 0.03, "RX:~s~ " .. tonumber(string.format("%.2f", al)), 0.4)
                tGMT.DrawText(0.44, 0.03, "RY:~s~ " .. tonumber(string.format("%.2f", am)), 0.4)
                tGMT.DrawText(0.495, 0.03, "RZ:~s~ " .. tonumber(string.format("%.2f", an)), 0.4)
                speed = GetEntitySpeed(PlayerPedId())
                rounds = tonumber(string.format("%.2f", speed))
                tGMT.DrawText(0.40, 0.92, "Player Speed: ~s~" .. rounds, 0.4)
                health = GetEntityHealth(PlayerPedId())
                tGMT.DrawText(0.40, 0.95, "Player Health: ~s~" .. health, 0.4)
                camRotX = GetGameplayCamRot(2).x
                tGMT.DrawText(0.36, 0.88, "CR X: ~s~" .. tonumber(string.format("%.2f", camRotX)), 0.4)
                camRotY = GetGameplayCamRot(2).y
                tGMT.DrawText(0.44, 0.88, "CR Y: ~s~" .. tonumber(string.format("%.2f", camRotY)), 0.4)
                camRotZ = GetGameplayCamRot(2).z
                tGMT.DrawText(0.51, 0.88, "CR Z: ~s~" .. tonumber(string.format("%.2f", camRotZ)), 0.4)
                ...
                if IsPedInAnyVehicle(PlayerPedId(), 1) then
                    ...
                    tGMT.DrawText(0.015, 0.76, "Engine Health: ~s~" .. vehenground, 0.4)
                    tGMT.DrawText(0.015, 0.73, "Body Health: ~s~" .. vehbodround, 0.4)
                    tGMT.DrawText(
                        0.015,
                        0.70,
                        "Vehicle Fuel: ~s~" ..
