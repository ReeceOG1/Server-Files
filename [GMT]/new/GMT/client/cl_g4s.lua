...
local function D()
    ...
    local F = 500 * math.ceil(GetDistanceBetweenCoords(tGMT.getPlayerCoords(), GetEntityCoords(b), true) / 10)
    if j then
...
RMenu.Add("g4s","shiftHandler",RageUI.CreateMenu("","Security Company",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"gmt_g4sjob", "gmt_g4sjob"))
RMenu:Get("g4s", "shiftHandler"):SetSubtitle("Security Company")
RMenu:Get("g4s", "shiftHandler").Closable = true
RMenu.Add("g4s","main",RageUI.CreateMenu("","Security Company",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"gmt_g4sjob", "gmt_g4sjob"))
...
local H = tGMT.addBlip(d.x, d.y, d.z, 457, 18, "G4S Depot", 1.0, false)
local I = function()
...
tGMT.createArea("g4sDepot_", d, 1.5, 6, I, J, K)
Citizen.CreateThread(function()
    while true do
        local E = tGMT.getPlayerPed()
        local L = tGMT.getPlayerCoords()
        if a then
            if not u then
                displayJob()
            end
...
Citizen.CreateThread(function()
    while true do
        if not A.hackingEngaged and not a and not A.awaitingResponse then
            if table.count(s) > 0 and tGMT.getPlayerVehicle() == 0 then
                for M, N in pairs(s) do
                    ...
                    if table.has(p, Q) or not DoesEntityExist(P) then
                        ...
                        if IsVehicleStopped(P) then
                            local R = #(tGMT.getPlayerCoords() - GetEntityCoords(P))
                            ...
                            if R <= 11.0 then
                                ...
                                if IsControlJustPressed(1, 18) then
                                    ...
                                    A.entity = P
                                    A.src = Q
                                    TriggerServerEvent("GMT:requestHackingChanceServer", O, Q)
...
function toggleCashbox()
    if C.active then
        ...
        local o = tGMT.loadModel("cashbox")
        local E = tGMT.getPlayerPed()
...
local W = "HONGKONG"
local X = 8
RegisterNetEvent("GMT:hackingChanceReturn",function(b, Y, Z, _)
    if _ then
        ...
        tGMT.setWeapon(tGMT.getPlayerPed(), "weapon_unarmed", true)
        local o = tGMT.loadModel("prop_cs_tablet")
        x = CreateObject(o, 0, 0, 0, true, true, true)
        AttachEntityToEntity(x,tGMT.getPlayerPed(),GetPedBoneIndex(tGMT.getPlayerPed(), 57005),0.17,0.10,-0.13,24.0,180.0,180.0,true,true,false,true,1,true)
        SetModelAsNoLongerNeeded(o)
    else
        A.awaitingResponse = false
    end
        ...
        if HasScaleformMovieLoaded(T) then
            ...
            if IsScaleformMovieMethodReturnValueReady(U) then
                ...
                elseif ProgramID == 87 then
                    ...
                    TriggerServerEvent("GMT:updateHackingOutcome", A.netId, A.src, W, false)
                    DeleteObject(x)
                    break
                ...
                elseif ProgramID == 86 then
                    ...
                    TriggerServerEvent("GMT:updateHackingOutcome", A.netId, A.src, W, true)
                    DeleteObject(x)
                    break
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('g4s', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if not j then
                if tGMT.getPlayerVehicle() == 0 and y < 4 then
...
function newG4sVehicle()
    ...
    if DoesEntityExist(b) and not IsVehicleDriveable(b, true) then
        ...
        b = tGMT.spawnVehicle(n, a3.x, a3.y, a3.z, GetEntityHeading(E), true, true)
        c = VehToNet(b)
    ...
    if a4 ~= 0 and a4 ~= nil and b ~= nil and b ~= 0 then
        if a4 ~= b then
            ...
            SetTimeout(15000,function()
                a4 = GetVehiclePedIsIn(tGMT.getPlayerPed(), false)
                if a4 ~= 0 then
                    if a4 ~= b then
                        TriggerServerEvent("GMT:toggleShiftG4S", false)
                        tGMT.removeBlip(k)
...
RegisterNetEvent("GMT:resetPanicStatus",function()
    h = false
end)
RegisterNetEvent("GMT:updateJobInformation",function(a5)
    if l ~= 0 then
        tGMT.removeBlip(l)
    end
    if a5.jobActive then
        if not a5.collected then
            ...
            k = tGMT.addBlip(a5.collectionCoords.x,a5.collectionCoords.y,a5.collectionCoords.z,500,18,"G4S Collection",1.0,false)
            SetBlipRoute(k, true)
            SetBlipRouteColour(k, 18)
        elseif not a5.deposited then
            tGMT.removeBlip(k)
            l = tGMT.addBlip(a5.depositCoords.x,a5.depositCoords.y,a5.depositCoords.z,500,47,"G4S Deposit",1.0,false)
            SetBlipRoute(l, true)
...
RegisterNetEvent("GMT:startShiftG4S",function(a6)
    local E = tGMT.getPlayerPed()
    local L = tGMT.getPlayerCoords()
    a = true
    tGMT.disableCustomizationSave(a)
    b = tGMT.spawnVehicle(n, B.x, B.y, B.z, B.heading, true, true)
    c = VehToNet(b)
    ...
    z = json.encode(tGMT.getCustomization())
    if aa then
        tGMT.loadCustomisationPreset("G4SMale")
    else
        tGMT.loadCustomisationPreset("G4SFemale")
    end
    Wait(2000)
    TaskVehicleDriveToCoord(tGMT.getPlayerPed(),b,-672.356,282.3297,81.43005,convert(250.0),1,GetHashKey(GetEntityModel(b)),60,5,true)
    Wait(5000)
    TaskVehicleTempAction(tGMT.getPlayerPed(), b, 27, 10.0)
    Wait(2000)
    imageNotification("CHAR_MP_MEX_DOCKS", "CHAR_MP_MEX_DOCKS", "Use /g4s to access the G4S menu.", a9, "G4S")
end)
RegisterNetEvent("GMT:endShiftG4S",function()
    ...
    local E = tGMT.getPlayerPed()
    tGMT.setCustomization(json.decode(z))
    if C.active then
        toggleCashbox()
    end
    if k ~= 0 then
...
RegisterNetEvent("GMT:requestMoneyInVehicle",function()
    ...
    while not f do
        ...
        local R = #(tGMT.getPlayerCoords() - ag)
        if R <= 1.0 then
            af = true
...
RegisterNetEvent("GMT:requestMoneyOutVehicle",function()
    local ae = tGMT.getPlayerPed()
    while IsPedInAnyVehicle(ae, true) do
        Wait(400)
    end
...
RegisterNetEvent("GMT:openG4SMenu",function()
    RageUI.Visible(RMenu:Get("g4s", "main"), true)
end)
RegisterNetEvent("GMT:imageNotification",function(al, am, an, ao, ap)
...
RegisterNetEvent("GMT:bottomMessage",function(ak, av)
    bottomMessage(ak, av)
end)
...
RegisterNetEvent("GMT:activatePanicAll",function(az, aA)
...
RegisterNetEvent("GMT:activeVansUpdate",function(az, Q, aE)
    if not a then
        local b = tGMT.getObjectId(az, "GMT:activeVansUpdate 1")
        if b ~= 0 and b ~= nil and DoesEntityExist(b) then
            if aE then
                s[az] = nil
            else
                if not table.has(s, az) then
                    local b = tGMT.getObjectId(az, "GMT:activeVansUpdate")
                    s[az] = {Q, b, az}
                end
            end
        end
...
RegisterNetEvent("GMT:terminateHacking",function()
