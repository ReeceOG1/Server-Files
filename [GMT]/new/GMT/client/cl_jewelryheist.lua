...
RegisterNetEvent(
    "GMT:jewelrySyncSetupReady",
    function(v)
        c = v
    end
)
RegisterNetEvent(
    "GMT:jewelryCreateDevicePickup",
    function(w)
        f =
            tGMT.addMarker(
            w.coords.x,
        ...
        ontick_collectDevice = function()
            if IsControlJustPressed(0, 38) and not d then
                ...
                SetEntityHeading(tGMT.getPlayerPed(), w.h)
                local E =
                ...
                Citizen.CreateThread(
                    function()
                        Wait(20000)
                        TriggerServerEvent("GMT:jewelryCollectDevice")
                        FreezeEntityPosition(tGMT.getPlayerPed(), false)
                        ClearPedTasks(tGMT.getPlayerPed())
                        tGMT.teleport(w.coords.x, w.coords.y, w.coords.z)
                        d = false
                        RemoveAnimDict(x)
                    end
                )
        ...
        SetTimeout(
            600000,
            function()
                tGMT.removeArea("jewelry_collect_device")
                tGMT.removeBlip(e)
            end
        )
    end
...
RegisterNetEvent(
    ...
    function()
        ...
        TriggerEvent(
            ...
            function(K, L)
                if K then
                    tGMT.notify("~g~Succesfully hacked!")
                    TriggerServerEvent("GMT:jewelryDoorHackSuccess")
                else
                    tGMT.notify("~r~Failed. Reason: " .. L)
                end
                m = false
            end
        )
...
RegisterNetEvent(
    "GMT:jewelryStartComputerHackSf",
    function()
        Citizen.CreateThread(
            function()
                Wait(2500)
...
RegisterNetEvent(
    "GMT:jewelrySyncLootAreas",
    function(V, U)
        if U then
            createJewelryArea(V)
        else
            tGMT.removeArea("break_glass_" .. V)
            tGMT.removeMarker(j[V])
        end
    end
)
RegisterNetEvent(
    "GMT:jewelrySyncModelSwaps",
    function(V)
        if #(GetEntityCoords(tGMT.getPlayerPed()) - a.jewelryCases[V].coords) <= 350 then
            local W =
    ...
    if not j[V] then
        ...
        ontick_breakJewelryCase = function(X)
            if IsControlJustPressed(0, 38) and not l and GetEntityHealth(PlayerPedId()) > 102 then
                if not globalOnPoliceDuty then
                    if GetSelectedPedWeapon(tGMT.getPlayerPed()) ~= GetHashKey("WEAPON_UNARMED") then
                        RequestScriptAudioBank("DLC_GMTHEIST\\GLASS_BREAK", false)
                        local Y = math.random(1, 3)
                        l = true
                        local Z = tGMT.getPlayerCoords()
                        ...
                        if W ~= 0 then
                            ...
                            StartParticleFxNonLoopedOnEntity(
                                "scr_jewel_cab_smash",
                                GetCurrentPedWeaponEntityIndex(tGMT.getPlayerPed()),
                                0.0,
                                0.0,
                                0.0,
                                0.0,
                            ...
                            SetEntityHeading(tGMT.getPlayerPed(), a.jewelryCases[V].heading)
                            SetModelAsNoLongerNeeded(newModel)
                        ...
                        if X.modelHash == GetHashKey("des_jewel_cab4_start") then
                            ...
                            a0(tGMT.getPlayerPed(), tGMT.getPlayerCoords(), true)
                        else
                            TaskPlayAnim(
                                tGMT.getPlayerPed(),
                                "missheist_jewel",
                            ...
                            a0(tGMT.getPlayerPed(), tGMT.getPlayerCoords(), false)
                        end
                        FreezeEntityPosition(tGMT.getPlayerPed(), false)
                        TriggerServerEvent("GMT:jewelryGrabLoot", X.caseId)
                        Citizen.Wait(1000)
                        SetModelAsNoLongerNeeded("prop_jewel_04a")
                        RemoveAnimDict("missheist_jewel")
                        l = false
                    else
                        tGMT.notify("~r~You must be holding a weapon to smash the glass!")
                    end
                else
                    TriggerServerEvent("GMT:jewelryPoliceSeizeLoot", X.caseId)
                    tGMT.removeArea("break_glass_" .. X.caseId)
                end
            end
        end
        j[V] =
    ...
    function(a3, a4)
        if a4 then
            ...
            ontick_jewelryTeleporterEnter = function()
                if IsControlJustPressed(0, 38) then
                    SetEntityHeading(tGMT.getPlayerPed(), 217.38)
                    tGMT.teleport(a.exitTeleporterCoords.x, a.exitTeleporterCoords.y, a.exitTeleporterCoords.z)
                end
            end
            tGMT.addBlip(
            ...
            ontick_jewelryTeleporterExit = function()
                if IsControlJustPressed(0, 38) then
                    SetEntityHeading(tGMT.getPlayerPed(), 217.38)
                    tGMT.teleport(a.enterTeleporterCoords.x, a.enterTeleporterCoords.y, a.enterTeleporterCoords.z)
                end
            end
            tGMT.addMarker(
        ...
        if a4 then
            ...
            ontick_aiMissionTeleporterEnter = function()
                if IsControlJustPressed(0, 38) then
                    if c then
                        ...
                        b = true
                        Citizen.Wait(1000)
                        TriggerServerEvent("GMT:jewelrySetupHeistStart")
                    else
                        tGMT.notify("~r~You cannot enter right now.")
                    end
                end
            end
            tGMT.addBlip(
            ...
            ontick_aiMissionTeleporterExit = function()
                if IsControlJustPressed(0, 38) then
                    ...
                    b = false
                    TriggerServerEvent("GMT:jewelrySetupHeistLeave")
                end
            end
            tGMT.addMarker(
                a.aiMissionTeleporterExit.x,
...
AddEventHandler(
    ...
    function(q)
        if q == "killed" then
            if b then
                TriggerServerEvent("GMT:jewelrySetupHeistLeave")
                TriggerEvent("GMT:jewelryRemoveDeviceArea")
                b = false
            end
        end
    end
...
RegisterNetEvent(
    ...
    function(a5)
        ...
        TaskCombatPed(y, tGMT.getPlayerPed(), 0, 0)
        SetPedAccuracy(y, 30)
    end
)
function coolCameraThingOne(W, _, a6)
    ...
    PointCamAtCoord(a7, tGMT.getPlayerCoords().x, tGMT.getPlayerCoords().y, tGMT.getPlayerCoords().z)
...
RegisterNetEvent(
    "GMT:jewelryAlarmTriggered",
    function()
        tGMT.announceMpSmallMsg("ALERT", "An alarm has been triggered at the jewelry store", 9, 10000)
    end
)