...
RegisterCommand("blipsoff",function()
    if tGMT.globalOnPoliceDuty() or tGMT.globalNHSOnDuty() then 
        tGMT.notify('Emergency blips disabled.')
        a=false
        d()
        g()
    end 
...
RegisterNetEvent("GMT:disableFactionBlips",function()
    a=false
        ...
        if a or tGMT.isInComa() then 
            ...
            for e,f in ipairs(GetActivePlayers())do
                ...
                if j~=p then 
                    ...
                    if q~=-1 then 
                        local r=tGMT.clientGetUserIdFromSource(q)
                        local s=tGMT.getJobType(r)
                        if s~="" and r ~= tGMT.getUserId() then 
...
RegisterNetEvent("GMT:sendFarBlips",function(w)
    if not u and a then
        g()
        for e,x in pairs(w)do 
            if x.source~=v and GetPlayerFromServerId(x.source)== -1 and x.bucket == tGMT.getPlayerBucket() then 
...
RegisterNetEvent("GMT:Revive",function()
    if not a then
        Citizen.Wait(1000)
        d()
    end
