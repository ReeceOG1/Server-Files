...
RegisterNetEvent("GMT:initMoney",function(n, o)
    m = o
end)
local function z8(r,...)
...
local function r(s, t)
    ...
    while #(tGMT.getPlayerCoords() - s) > 4.0 do
        Citizen.Wait(0)
    end
    RemoveBlip(v)
    DeleteCheckpoint(u)
...
local function w(x, y, z)
    tGMT.loadModel("metrotrain")
    tGMT.loadModel("s_m_m_lsmetro_01")
    local A = 0
    while true do
...
local function O()
    ...
    local E = PlayerPedId()
    while true do
        local R = #(tGMT.getPlayerCoords() - b)
        if R < 3.0 then
    ...
    tGMT.loadAnimDict("cellphone@")
    tGMT.loadModel("prop_amb_phone")
    p("ring")
    Citizen.Wait(2000)
    local H = tGMT.getPlayerCoords()
...
local function V()
    ...
    while tGMT.getPlayerVehicle() ~= l do
        Citizen.Wait(0)
    end
    h = nil
    DeleteCheckpoint(u)
...
local function X(Y, Z, _)
    ...
    while #(tGMT.getPlayerCoords() - Y) > a0 do
        if _ then
            _()
        end
        Citizen.Wait(0)
...
local function a1()
    ...
    local C = tGMT.getPlayerVehicle()
    while tGMT.getPlayerVehicle() ~= 0 do
        Citizen.Wait(0)
    end
    ...
    while table.count(VehiclesFetchedTable) == 0 do
        ...
        elseif DoesEntityExist(testDriveCar) then
            ...
            local a2 = #(vector3(-34.113563537598, -1101.7242431641, 25.422456741333) - tGMT.getPlayerCoords()) <= 2.5
            if a2 then
...
local function a3()
    ...
    while table.count(globalVehicleModelHashMapping) == 0 or tGMT.getPlayerVehicle() == 0 do
        ...
        elseif
            RageUI.Visible(RMenu:Get("GMTGarages", "rented_vehicles")) or RageUI.Visible(RMenu:Get("GMTGarages", "customfolders"))
         then
            RageUI.Visible(RMenu:Get("GMTGarages", "main"), true)
            notify("~r~This section is not avaliable during the tutorial")
        else
            local a2 = #(vector3(-51.893981933594, -1112.7712402344, 25.438014984131) - tGMT.getPlayerCoords()) < 2.5
            if a2 then
    ...
    Citizen.Wait(1000)
    SetEntityInvincible(tGMT.getPlayerVehicle(), true)
    i = nil
    h = nil
end
local function a4()
...
local function a6()
    ...
    while true do
        if not RageUI.IsAnyMenuOfTypeVisible("GMTclothing") then
            ...
            local a2 = #(vector3(123.60697174072, -219.52917480469, 53.557830810547) - tGMT.getPlayerCoords()) < 2.5
            if a2 then
...
local function ad()
    ...
    D()
    V()
    if table.count(VehiclesFetchedTable) == 0 or tGMT.isDevMode() then
        if m > 20000 then
            a1()
        end
    else
...
RegisterCommand("tutorial",function()
    if tGMT.isDevMode() then
        ad()
    end
end,false)
RegisterNetEvent("GMT:playTutorial", ad)
AddEventHandler("GMT:onClientSpawn",function(ae, af)
    if af and not tGMT.isPurge() then
        Citizen.Wait(10000)
...
local function ag()
    if g then
        ...
        if not tGMT.isPurge() then
            ...
            tGMT.setWeapon(E, "WEAPON_UNARMED", true)
        end
    end
end
tGMT.createThreadOnTick(ag)
RegisterNetEvent("GMT:setIsNewPlayer",function()
    f = true
end)
function tGMT.isNewPlayer()
    return f
end
function tGMT.isInTutorial()
    return g
end
