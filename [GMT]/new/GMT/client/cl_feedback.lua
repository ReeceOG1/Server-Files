local aJ = false
RegisterNetEvent("GMT:adminTicketFeedback",function(aK)
    ...
    if aN then
        local aO = false
        tGMT.clientPrompt("Attached Message","",function(aP)
            TriggerServerEvent("GMT:adminTicketFeedback", aK, aN, aP)
            aO = true
        end)
        while not aO do
            for a2 = 0, 6 do
        ...
        TriggerServerEvent("GMT:adminTicketNoFeedback", aK)
    end
