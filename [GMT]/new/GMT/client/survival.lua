        ...
        while true do
            if IsDisabledControlJustPressed(0, 38) then
                if b and not g then
                    ...
                    if not tGMT.isPlayerInRedZone() and not tGMT.isPlayerInTurf() then
                        PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
                        tGMT.notify("~g~NHS have been notified.")
        ...
        while true do
            if b then
                ...
                if not IsEntityDead(I) then
                    ...
                    if not IsEntityPlayingAnim(I, a.dict, a.anim, 3) and not tGMT.isUsingStretcher() then
                        if a.dict ~= nil then
                            tGMT.loadAnimDict(a.dict)
...
RegisterNetEvent(
    "GMT:getNumberOfDocsOnline",
    function(L)
        if not tGMT.isPurge() then
            if tGMT.isPlayerInRedZone() or tGMT.isPlayerInTurf() then
        ...
        if not tGMT.isPurge() then
            if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then
                ...
                if not tGMT.globalOnPoliceDuty() then
...
function tGMT.isTazed(Z)
    return Y or IsPedBeingStunned(PlayerPedId(), 0) or tGMT.isPedBeingTackled() or
...
Citizen.CreateThread(
    function()
        ...
        while true do
            Citizen.Wait(0)
            if IsEntityDead(PlayerPedId()) and not tGMT.isPedScriptGuidChanging() then
                Citizen.Wait(500)
                ...
                if a4 == PlayerId() or a4 == nil then
                    ...
                    local ab = tGMT.getPlayerName(a4)
                    O.name = ab
                    aa = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(a7))
                end
                O.source = a8
                O.user_id = tGMT.getUserId(a8)
