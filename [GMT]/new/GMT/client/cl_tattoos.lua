RMenu.Add("GMTtattoos","mainMenu",RageUI.CreateMenu("","Tattoo Store",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"shopui_title_tattoos2","shopui_title_tattoos2"))
...
AddEventHandler("GMT:onClientSpawn",function(D, E)
    if E then
        ...
        local G = function()
            ...
            j = nil
            ClearPedDecorations(tGMT.getPlayerPed())
            for H = 1, #i, 1 do
                AddPedDecorationFromHashes(tGMT.getPlayerPed(),GetHashKey(i[H].collection),GetHashKey(e[i[H].collection][i[H].current].nameHash))
            end
            c = false
            h = {}
        end
        ...
        for B, J in pairs(f) do
            ...
            Wait(100)
            tGMT.createArea("tattoParlour_" .. B, J, 1.5, 6, F, G, I)
...
RegisterNetEvent("GMT:setTattoos",function(Q)
    ...
    for H = 1, #R, 1 do
        ...
        AddPedDecorationFromHashes(tGMT.getPlayerPed(),GetHashKey(R[H].collection),GetHashKey(e[R[H].collection][R[H].current].nameHash))
        table.insert(h, R[H])
    end
end)
...
function drawTattoo(_, a0, a1)
    SetEntityHeading(tGMT.getPlayerPed(), 297.7296)
    local C = tGMT.getPlayerPed()
    local a2 = false
    if _ ~= nil and a0 ~= nil and a1 ~= nil then
        if p then
            ...
            if GetEntityModel(tGMT.getPlayerPed()) == -1667301416 then
                tGMT.loadCustomisationPreset("TattooMale")
            else
                tGMT.loadCustomisationPreset("TattooFemale")
            end
            ClearPedDecorations(tGMT.getPlayerPed())
            for H = 1, #i, 1 do
                AddPedDecorationFromHashes(tGMT.getPlayerPed(),GetHashKey(i[H].collection),GetHashKey(e[i[H].collection][i[H].current].nameHash))
            end
            AddPedDecorationFromHashes(tGMT.getPlayerPed(), GetHashKey(a0), GetHashKey(e[a0][_].nameHash))
            if not DoesCamExist(b) then
                b = CreateCam("DEFAULT_SCRIPTED_CAMERA", true)
                SetCamCoord(b, GetEntityCoords(tGMT.getPlayerPed()))
                SetCamRot(b, 0.0, 0.0, 0.0)
                SetCamActive(b, true)
                RenderScriptCams(true, false, 0, true, true)
                SetCamCoord(b, GetEntityCoords(tGMT.getPlayerPed()))
            end
            local S, a3, a4 = table.unpack(GetEntityCoords(tGMT.getPlayerPed()))
