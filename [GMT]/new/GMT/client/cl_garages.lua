...
RMenu.Add('GMTGarages', 'main', RageUI.CreateMenu("", "~b~Garages",tGMT.getRageUIMenuWidth(), tGMT.getRageUIMenuHeight(), "gmt_garageui", "gmt_garageui"))
...
RMenu:Get('GMTGarages', 'owned_vehicles'):SetSubtitle("~b~Garage Mangement Menu")
    ...
    if ay == nil or not DoesEntityExist(ay[2]) then
        ...
        if valetCalled then
            ...
            if tostring(aE) ~= "vector3(0, 0, 0)" and tostring(aB) ~= "vector3(0, 0, 0)" then
                ...
                DecorSetInt(as, "vRP_owner", tGMT.getUserId())
...
function canVehicleBeSold(e)
    return not vehicleCannotBeSold[string.lower(e)]
end

function canVehicleBeRented(e)
    return not vehicleCannotBeRented[string.lower(e)]
end
...
RegisterCommand("car",function(bY, bZ, b_)
    if tGMT.getStaffLevel() >= 6 and not tGMT.isPurge() then
        ...
        if ao == "corsa" and tGMT.getUserId() ~= 1 then
            ...
            if #(ap - c0) < 600.0 or (tGMT.getUserId() == 1) then
