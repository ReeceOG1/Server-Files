...
Citizen.CreateThread(function()
    ...
    for q = 0, #c.coinflipTables do
        ...
        local DD = tGMT.loadModel(D.tableModel)
    ...
    while true do
        if d then
            ...
            if not l and IsControlJustPressed(0, 202) and not g and not RageUI.Visible(RMenu:Get("gmtcoinflip", "confirm")) then
                local K = tGMT.loadAnimDict("anim_casino_b@amb@casino@games@shared@player@")
                NetworkStopSynchronisedScene(Local_198f_255)
                TaskPlayAnim(tGMT.getPlayerPed(), K, "sit_exit_left", 1.0, 1.0, 2500, 0)
                d = false
                e = true
...
function goToCoinflipSeat(o)
    ...
    TaskGoStraightToCoord(tGMT.getPlayerPed(), R.x, R.y, R.z, 1.0, 5000, S.z, 0.01)
    Wait(500)
    local T = v(o)
    local U = A(o)
    local V = NetworkCreateSynchronisedScene(T.x, T.y, T.z, U.x, U.y, U.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tGMT.getPlayerPed(),V,"anim_casino_b@amb@casino@games@shared@player@",M(x),2.0,-2.0,13,16,2.0,0)
    ...
    NetworkAddPedToSynchronisedScene(tGMT.getPlayerPed(),Locali98f_55,"anim_casino_b@amb@casino@games@shared@player@","idle_cardgames",2.0,-2.0,13,16,1148846080,0)
...
RMenu.Add("gmtcoinflip", "instructions", RageUI.CreateMenu("", "COINFLIP", 0, 100, "gmt_coinflip", "gmt_coinflip"))
RMenu.Add("gmtcoinflip", "mainmenu", RageUI.CreateMenu("", "COINFLIP", 0, 100, "gmt_coinflip", "gmt_coinflip"))
RMenu.Add("gmtcoinflip", "confirm", RageUI.CreateSubMenu(RMenu:Get("gmtcoinflip", "mainmenu"), "", "COINFLIP", 0, 100, "gmt_coinflip", "gmt_coinflip"))
                ...
                for a0, L in pairs(h) do
                    RageUI.Button("~y~£" .. getMoneyStringFormatted(L.betAmount),"",{RightBadge=RageUI.BadgeStyle.GoldMedal},true,function(X, Y, Z)
                        if Z then
                            ...
                            if L.user_id == tGMT.getUserId() then
                                k = true
                            else
                                k = false
                            end
...
RegisterNetEvent("GMT:coinflipOutcome",function(a3, a4)
    ...
    SetTimeout(10000,function()
        a6 = true
        if a3 then
            tGMT.notify("~g~ You won £" .. getMoneyStringFormatted(a4.amount) .. "!")
            ...
            tGMT.notify("~r~ You lost £" .. getMoneyStringFormatted(a4.amount) .. "!")
