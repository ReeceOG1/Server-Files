...
Citizen.CreateThread(function()
    while true do
		...
		if IsControlPressed(1, 19) and IsControlJustPressed(1, 58) and IsPedArmed(tGMT.getPlayerPed(), 7) and not tGMT.isPurge() then
			...
			if c ~= GetHashKey("WEAPON_UNARMED") then
				...
				if d ~= GetHashKey("GROUP_UNARMED") and d ~= GetHashKey("GROUP_MELEE") and d ~= GetHashKey("GROUP_THROWN") then
					if not inOrganHeist then
						TriggerServerEvent("GMT:Knockout")
					end
				end
			end
			Wait(1000)
		...
		if IsControlPressed(1, 19) and IsControlJustPressed(1,32) then 
			if not IsPauseMenuActive() and not IsPedInAnyVehicle(tGMT.getPlayerPed(), true) and not IsPedSwimming(tGMT.getPlayerPed()) and not IsPedSwimmingUnderWater(tGMT.getPlayerPed()) and not IsPedShooting(tGMT.getPlayerPed()) and not IsPedDiving(tGMT.getPlayerPed()) and not IsPedFalling(tGMT.getPlayerPed()) and GetEntityHealth(tGMT.getPlayerPed()) > 105 and not tGMT.isHandcuffed() and not tGMT.isInRadioChannel() then
		...
		if IsControlPressed(1, 19) and IsControlJustPressed(1,29) then 
			if not IsPedInAnyVehicle(tGMT.getPlayerPed(),false) then
				local closestPlayer = tGMT.GetClosestPlayer(4)
