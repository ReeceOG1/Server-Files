local a = false
function func_ptt()
    if globalOnPoliceDuty and tGMT.canAnim() and not noclipActive then
        local b = tGMT.getPlayerPed()
        local c = tGMT.getPlayerId()
        if a then
...
tGMT.createThreadOnTick(func_ptt)
...
local function f(g)
    ...
    elseif not i and e then
        exports["pma-voice"]:cleGMTxProximityMode()
        e = false
    end
end
tGMT.createThreadOnTick(f)
