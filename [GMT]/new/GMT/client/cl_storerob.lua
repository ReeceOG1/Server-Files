    ...
    if not IsPedDeadOrDying(x) then
        if IsEntityPlayingAnim(x, "mp_am_hold_up", "holdup_victim_20s", 3) then
            ...
            if not IsPedDeadOrDying(x) then
                ...
                local A = tGMT.loadModel("prop_poly_bag_01")
                        ...
                        function()
                            while true do
                                ...
                                if DoesEntityExist(B) then
                                    if #(GetEntityCoords(tGMT.getPlayerPed()) - GetEntityCoords(B)) <= 1.5 then
                                        PlaySoundFrontend(
                                            -1,
                                            "ROBBERY_MONEY_TOTAL",
...
Citizen.CreateThread(
    function()
        while true do
            local t = GetEntityCoords(tGMT.getPlayerPed(), true)
            if r ~= nil then
                for v, w in pairs(r) do
...
Citizen.CreateThread(
    function()
        for v, w in pairs(r) do
            local G = tGMT.loadModel(w.shopNpcModel)
            local H =
                CreatePed(
                26,
                G,
            ...
            SetPedFleeAttributes(H, 0, 0)
            local I = tGMT.loadModel(w.prop_safe)
            local J = tGMT.loadModel(w.prop_door)
            local K = tGMT.loadModel(w.money_prop)
            local L = tGMT.loadModel(w.money_prop2)
            local M = tGMT.loadModel(w.money_prop3)
            local N = tGMT.loadModel(w.money_prop4)
        ...
        while true do
            local U = GetEntityCoords(tGMT.getPlayerPed())
            for v, w in pairs(r) do
                             ...
                             then
                                if not q then
                                    if GetSelectedPedWeapon(tGMT.getPlayerPed()) == GetHashKey("WEAPON_UNARMED") then
                                        tGMT.notify("~r~You need a weapon in your hands to rob this store!")
                                    else
...
RegisterNetEvent(
    ...
    function(Y, v)
        q = Y
        local Z = GetEntityCoords(tGMT.getPlayerPed())
        while q do
            local U = GetEntityCoords(tGMT.getPlayerPed())
            if #(Z - U) > 20 then
...
RegisterNetEvent(
    ...
    function(v)
        ...
        local G = tGMT.loadModel(r[v].shopNpcModel)
        local H =
            CreatePed(
            26,
            G,
            ...
            for v, w in pairs(r) do
                if w.distanceToPlayer < 2.0 then
                    ...
                    elseif w.robberyInProgress then
                        ...
                        if IsControlJustReleased(1, 51) then
                            tGMT.notify("~g~Started cracking safe..")
                            LoadResources()
...
function RunMiniGame(v, ab, ac)
    ...
    while safeCrackingInProgress do
        ...
        elseif safeCrackingState == "cracking" then
            ...
            local ae = GetEntityHealth(tGMT.getPlayerPed())
...
function GetSafeDoorAnimOffsetPosition(ah, ai, aj)
    ...
    return vector3(ah.x + ao, ah.y + ap, GetEntityCoords(tGMT.getPlayerPed()).z)
end
function PlaySafeCrackIntroAnim(aq, h)
    local ar = "mini@safe_cracking"
    local as = "step_into"
    loadAnimDict(ar)
    TaskPlayAnimAdvanced(tGMT.getPlayerPed(), ar, as, aq.x, aq.y, aq.z, 0.0, 0.0, h, 8.0, 8.0, -1, 2, 0.7, 0, 0)
    Wait(0)
    Wait(1000)
    safeCrackingState = "cracking"
end
function HandleSafeDialMovement()
    local at = tGMT.getPlayerPed()
    local ar = "mini@safe_cracking"
    local au = ""
    ...
    loadAnimDict(ar)
    if not IsEntityPlayingAnim(tGMT.getPlayerPed(), ar, au, 3) then
        TaskPlayAnimAdvanced(tGMT.getPlayerPed(), ar, au, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    end
end
function HandleIncorrectMovement(ad, b)
    ...
    loadAnimDict(ar)
    TaskPlayAnimAdvanced(tGMT.getPlayerPed(), ar, ax, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    d = "rotation.idle"
end
