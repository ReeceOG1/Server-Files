...
--Setup main menu
local LSCMenu = Menu.new("Los Santos Customs","~b~CATEGORIES", 0.16,0.13,0.24,0.36,0,{255,255,255,255})
								...
								if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									if pos.locked then
										if not LSC_Config.lock then
											...
											drawTxt("Locked, please wait",4,1,0.5,0.8,1.0,255,255,255,255)
										end
									else
										if IsControlJustPressed(1,201) then
											inside = true
									...
									drawTxt("This vehicle can't be upgraded",4,1,0.5,0.8,1.0,255,255,255,255)
								end
							end
						else
