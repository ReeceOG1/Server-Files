...
Citizen.CreateThread(function()
    while true do
        if f then
            drawNativeNotification("Press ~INPUT_PICKUP~ to play the Organ Heist!")
            if IsControlJustPressed(0, 38) then
                if not globalOnPoliceDuty then
                    if not g then
                        ...
                        tGMT.notify("Already joined the Organ Heist!")
                    end
                else
                    tGMT.notify("~b~You are police, please use the other entrance!")
                end
            end
        elseif e then
            drawNativeNotification("Press ~INPUT_PICKUP~ to play the Organ Heist!")
            if IsControlJustPressed(0, 38) then
                if globalOnPoliceDuty then
                    ...
                    tGMT.notify("~r~You are a civillian, please use the other entrance!")
                end
            end
        end
        ...
        if inGamePhase or inWaitingStage then
            DrawGTATimerBar("Police:", table.count(i), 3)
            DrawGTATimerBar("Criminals:", table.count(j), 2)
            drawNativeText("Kill the enemy team and survive.")
            RemoveWeaponFromPed(tGMT.getPlayerPed(), "WEAPON_MOLOTOV")
            RemoveWeaponFromPed(tGMT.getPlayerPed(), "WEAPON_FLASHBANG")
            RemoveWeaponFromPed(tGMT.getPlayerPed(), "WEAPON_SMOKEGRENADE")
            RemoveWeaponFromPed(tGMT.getPlayerPed(), "WEAPON_BZGAS")
            RemoveWeaponFromPed(tGMT.getPlayerPed(), "WEAPON_STUNGUN")
            local r = PlayerPedId()
...
RegisterNetEvent("GMT:teleportToOrganHeist",function(v, w, x, y)
    local r = PlayerPedId()
    d = w
    canPlayAnim = false
    h = x
    ...
    if x == "civ" then
        ...
        tGMT.createAtm("Organ Heist", k.atmLocation)
...
RegisterNetEvent("GMT:addOrganHeistPlayer",function(A, x)
    if x == "civ" then
        j[A] = true
    elseif x == "police" then
        i[A] = true
    ...
    while true do
        if inWaitingStage then
            ...
            if #(s - k.pastGate) < 3.0 then
                ...
                NetworkFadeInEntity(PlayerPedId(), 0)
                DoScreenFadeIn(1000)
                tGMT.notify("You got too far from the organ heist and have been teleported back.")
            end
        end
        if inGamePhase then
            local s = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent("GMT:removeFromOrganHeist",function(F)
    if i[F] then
        i[F] = nil
    end
    if j[F] then
...
RegisterNetEvent("GMT:endOrganHeist",function()
...
RegisterNetEvent("GMT:endOrganHeistWinner",function(x)
    Wait(10000)
    winOrganHeist(x)
    local r = PlayerPedId()
    tGMT.setFriendlyFire(true)
...
function tGMT.setDeathInOrganHeist()
    inOrganHeist = false
    inWaitingStage = false
    inGamePhase = false
end
