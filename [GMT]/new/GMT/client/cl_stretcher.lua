...
local function q()
    if IsControlJustReleased(0, 299) then
        if NetworkHasControlOfEntity(c) then
            TriggerServerEvent("GMT:toggleAmbulanceDoors", tGMT.getNetId(c, "Stretchers"))
        else
...
local function C(D, E)
    ...
    if not y(D) or b then
        ...
        tGMT.loadAnimDict(a.positions[e].animDict)
        DetachEntity(x, false, false)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if d ~= nil and d ~= 0 then
            ...
            if not b and not IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and tGMT.getPlayerVehicle() == 0 then
            ...
            if IsControlJustReleased(0, 51) and not IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and not b then
                if DecorGetInt(d, "inside_ambulance_netid") ~= 0 then
                    local O = tGMT.getObjectId(DecorGetInt(d, "inside_ambulance_netid"), "Stretcher Thread")
...
function pushStretcher(x, d)
    Citizen.CreateThread(function()
        ...
        while IsEntityAttachedToEntity(d, x) do
            ...
            if DecorGetInt(d, "inside_ambulance_netid") ~= 0 then
                local O = tGMT.getObjectId(DecorGetInt(d, "inside_ambulance_netid"), "pushStretcher(ped, nearbyStretcher)")
                if DecorGetBool(O, "has_stretcher_inside") ~= false then
                    if not NetworkHasControlOfEntity(O) then
                        if GetGameTimer() - Q > 200 then
                            TriggerServerEvent("GMT:updateHasStretcherInsideDecor",tGMT.getNetId(O, "Stretchers"),false)
                            Q = GetGameTimer()
                        end
                    else
            ...
            if c ~= nil and c ~= 0 then
                ...
                if IsControlJustReleased(0, 300) and (GetVehicleDoorAngleRatio(c, 4) ~= 0 or IsThisModelAHeli(GetEntityModel(c))) and not R then
                    ...
                    if not NetworkHasControlOfEntity(c) then
                        TriggerServerEvent("GMT:updateHasStretcherInsideDecor", tGMT.getNetId(c, "Stretcher"), true)
                    else
                        DecorSetBool(c, "has_stretcher_inside", true)
                    end
                    if not NetworkHasControlOfEntity(d) then
                        TriggerServerEvent("GMT:updateStretcherLocation",tGMT.getNetId(d, "Stretchers"),tGMT.getNetId(c, "Stretchers"))
                    else
                        DecorSetInt(d, "inside_ambulance_netid", tGMT.getNetId(c, "Stretchers"))
                    end
...
RegisterCommand("stretcher",function()
    if globalNHSOnDuty then
        ...
        local g = tGMT.loadModel("trump-stretcher")
        local D = tGMT.spawnVehicle(g, v.x, v.y, v.z, GetEntityHeading(x), false, true, true)
...
function tGMT.isUsingStretcher()
    return b
end
