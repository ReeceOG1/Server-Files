RMenu.Add("policedog","main",RageUI.CreateMenu("Dog Support Unit", "Dog Management", tGMT.getRageUIMenuWidth(), tGMT.getRageUIMenuHeight()))
...
RegisterNetEvent("GMT:policeDogMenu",function()
    if globalOnPoliceDuty and tGMT.hasPoliceCallsign() then
        RMenu:Get("gmtscenemenu", "main"):SetSubtitle("MPD~w~: " .. tGMT.getPoliceCallsign() .. " - " .. tGMT.getPoliceRank() .. " - " .. tGMT.getPoliceName())
    end
    if globalOnPrisonDuty and tGMT.hasHmpCallsign() then
        RMenu:Get("gmtscenemenu", "main"):SetSubtitle("HMP~w~: " .. tGMT.getHmpCallsign() .. " - " .. tGMT.getHmpRank() .. " - " .. tGMT.getHmpName())
    end
...
function tpDog()
    local z = GetEntityCoords(tGMT.getPlayerPed(), true)
    ...
    while f.currentAction == d.SearchNearby and DoesEntityExist(f.handle) do
        ...
        for W, X in pairs(GetActivePlayers()) do
            ...
            if GetVehiclePedIsUsing(Y) == 0 and IsEntityVisible(Y) and #(GetEntityCoords(Y, true) - L) < 5.0 then
                ...
                local _ = tGMT.clientGetUserIdFromSource(Z)
                if tGMT.getJobType(_) == "" then
...
function newDog(q, a4)
    local a5 = tGMT.getPlayerPed()
    if not IsPedInAnyPoliceVehicle(a5) then
        local a6 = tGMT.loadModel(q)
...
function dogFollow()
    ...
    if f.currentAction ~= d.Follow then
        ClearPedTasks(f.handle)
        TaskFollowToOffsetOfEntity(f.handle, tGMT.getPlayerPed(), 0.0, 0.0, 0.0, 7.0, -1, 10.0, true)
        f.currentAction = d.Follow
    else
        ClearPedTasks(f.handle)
        f.currentAction = d.Follow
