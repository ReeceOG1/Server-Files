    ...
    if true then 
        ...
        local g=function()
            if IsControlJustPressed(1,51)then 
                if tGMT.globalOnPoliceDuty() or tGMT.globalOnPrisonDuty() then 
                    TriggerServerEvent('GMT:getArmour')
                    local soundId=GetSoundId()
                    PlaySoundFrontend(soundId,"Armour_On","DLC_GR_Steal_Miniguns_Sounds",true)
                    ReleaseSoundId(soundId)
                else 
                    tGMT.notify("~r~You shouldn't be here...Engaging defenses in 3..2..1...")
                end 
            end 
        end
        for h,i in pairs(a)do 
            tGMT.createArea("armour_"..h,i,1.5,6,e,f,g)
...
local savedArmour = 0
local j=false
function tGMT.isPlayerInAnimalForm()
    return j 
end
local function k()
    ...
    if l==GetHashKey('mp_m_freemode_01') or l==GetHashKey('mp_f_freemode_01') then 
        ...
        local m=tGMT.loadModel("a_c_deer")
        ...
        tGMT.notify("~r~Custom peds cannot be used with horses.")
    end 
end
Citizen.CreateThread(function()
    if true then 
        ...
        local w=function()
            if IsControlJustPressed(1,51)and not j then 
                if tGMT.globalOnPoliceDuty() and not inOrganHeist then 
                    if tGMT.globalHorseTrained() or tGMT.getUserId()==1 then 
                        k()
                    else 
                        tGMT.notify("~r~You do not have the [Horse Trained] whitelist.")
                    end
                else 
                    tGMT.notify("~r~This is only available to the MET Police only.")
                end 
            end 
        end
        for x,y in pairs(b)do 
            tGMT.createArea("horse_"..x,y,1.5,6,u,v,w)
