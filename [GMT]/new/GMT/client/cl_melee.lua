...
RegisterCommand("togglemelee",function()
    if tGMT.getUserId()==1 or tGMT.getUserId()==2 then 
        c = not c
        tGMT.notify('Melee Combat: '..(c and "~g~Enabled" or "Disabled"))
    end 
end)
function tGMT.enablePunching(d)
    c = d
end
...
AddEventHandler("GMT:onClientSpawn",function(h, i)
    if i then
        ...
        for n, o in pairs(a) do
            tGMT.createArea("boxing_" .. n, o.pos, o.radius, 6, j, l, m, {})
        end
    end
end)
...
AddEventHandler("GMT:onClientSpawn",function(h, i)
    if i then
        ...
        for n, u in pairs(t) do
            ...
            tGMT.createArea(n .. "_boxingGloves", u, 1.5, 6, j, l, m, {})
        end
    end
end)
local v = {}
function putGloves()
    ...
    local x = tGMT.loadModel("prop_boxing_glove_01")
