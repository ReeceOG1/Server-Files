...
function tGMT.isPlayerRappeling()
    return k
end
function tGMT.isPlayerInPoliceHeli()
    return l
end
local r = (a + b) * 0.5
local s = 0
...
local function X(Y)
    ...
    tGMT.DrawText(0.45, 0.9, "Model: " .. _ .. "\nPlate: " .. a0, 0.55)
end
local function a1()
    while true do
        if l then
            local a2 = tGMT.getPlayerPed()
            local heli = tGMT.getPlayerVehicle()
...
local av = {}
RegisterNetEvent("GMT:updateSpotlight",function(aw, ax)
    local ay = GetPlayerFromServerId(aw)
    if ay == -1 then
        return
    end
    local az = GetPlayerPed(ay)
    if az == 0 or az == tGMT.getPlayerPed() then
        return
    end
