...
Citizen.CreateThread(function()
	while true do
		...
		if GetVehiclePedIsTryingToEnter(m) ~= 0 or d then
			tGMT.setWeapon(m, "WEAPON_UNARMED", true)
		end
		if b then
		...
		if c then
			if IsControlJustPressed(0, 73) then
				...
				timeout(k,string.format("Do not press X to cancel the animation! Your weapons have been holstered for %ss",k / 1000))
			end
		end
					...
					if o ~= GetHashKey("WEAPON_UNARMED") then
						if table.has(i, o) then
							...
							if p ~= GetHashKey("WEAPON_UNARMED") and not table.has(i, p) then
								timeout(3500,"Do not change weapons to equip quicker! Your weapons have been holstered for 3.5s")
							else
								a = false
								if o == "WEAPON_PDGLOCK" and GetPedDrawableVariation(n, 7) == 187 then
									tGMT.loadCustomisationPreset("TaserInHolster")
								end
								if not d then
									a = false
									b = false
						...
						if table.has(i, f) then
							...
							if p ~= GetHashKey("WEAPON_UNARMED") and not table.has(i, p) then
								...
								if h and GetPedDrawableVariation(n, 7) == 161 then
									tGMT.loadCustomisationPreset("TaserOutHolster")
								end
							end
						else
							loadAnimDict("reaction@intimidation@1h")
							...
							RemoveAnimDict("reaction@intimidation@1h")
							b = true
							tGMT.setWeapon(n, GetHashKey(f), true)
							Citizen.Wait(1300)
							tGMT.setWeapon(n, "WEAPON_UNARMED", true)
							Citizen.Wait(700)
							ClearPedTasks(n)
							a = true
							b = false
