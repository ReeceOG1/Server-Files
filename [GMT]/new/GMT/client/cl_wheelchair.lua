RegisterCommand("wheelchair",function()
    if globalNHSOnDuty then
        local a = tGMT.loadModel("prop_wheelchair_01")
        local b = CreateObject(a, GetEntityCoords(tGMT.getPlayerPed()), true)
        SetModelAsNoLongerNeeded(a)
    end
end,false)
RegisterCommand("removewheelchair",function()
    if globalNHSOnDuty then
        local b = GetClosestObjectOfType(GetEntityCoords(tGMT.getPlayerPed()), 10.0, GetHashKey("prop_wheelchair_01"))
...
function SitOnWheelchair(x)
    ...
    AttachEntityToEntity(tGMT.getPlayerPed(), x, 0, 0, 0.0, 0.4, 0.0, 0.0, 180.0, 0.0, false, false, false, false, 2)
    local z = GetEntityHeading(x)
    while IsEntityAttachedToEntity(tGMT.getPlayerPed(), x) do
        Citizen.Wait(5)
        if IsPedDeadOrDying(tGMT.getPlayerPed()) then
            DetachEntity(tGMT.getPlayerPed(), true, true)
        end
        if not IsEntityPlayingAnim(tGMT.getPlayerPed(), "missfinale_c2leadinoutfin_c_int", "_leadin_loop2_lester", 3) then
            TaskPlayAnim(tGMT.getPlayerPed(),"missfinale_c2leadinoutfin_c_int","_leadin_loop2_lester",8.0,8.0,-1,69,1,false,false,false)
        end
        local A = false
        ...
        if IsControlJustPressed(0, 73) or IsControlJustPressed(0, 177) then
            ...
            SetEntityCoords(tGMT.getPlayerPed(), B, C, D)
            A = true
        end
...
function PickUpWheelchair(x)
    ...
    while not NetworkHasControlOfEntity(x) or not IsEntityAttachedToEntity(x, tGMT.getPlayerPed()) do
        Wait(0)
    end
    LoadAnim("anim@heists@box_carry@")
    Wait(0)
    while IsEntityAttachedToEntity(x, tGMT.getPlayerPed()) do
        Citizen.Wait(5)
        if not IsEntityPlayingAnim(tGMT.getPlayerPed(), "anim@heists@box_carry@", "idle", 3) then
            TaskPlayAnim(tGMT.getPlayerPed(), "anim@heists@box_carry@", "idle", 8.0, 8.0, -1, 50, 0, false, false, false)
        end
        if IsPedDeadOrDying(tGMT.getPlayerPed()) then
