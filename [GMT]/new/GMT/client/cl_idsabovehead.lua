...
Citizen.CreateThread(function()
    while true do
        ...
        for g, h in ipairs(GetActivePlayers()) do
            ...
            if l ~= f and (IsEntityVisible(l) or not tGMT.clientGetPlayerIsStaff(tGMT.clientGetUserIdFromSource(m))) then
...
function tGMT.setPlayerNameDistance(o)
    if o == -1 then
...
RegisterCommand("farids",function(q, r, s)
    if tGMT.getStaffLevel() > 2 and (tGMT.isStaffedOn() or tGMT.isInSpectate()) then
        local t = r[1]
        if t ~= nil and tonumber(t) then
            a = tonumber(t) + 000.1
            tGMT.setPlayerNameDistance(a)
        else
            tGMT.notify("Please enter a valid range! (/farids [range])")
        end
    end
end,false)
