...
local function c()
    if GetEntityHealth(tGMT.getPlayerPed()) <= 102 then
        tGMT.notify("~r~You try to move but you remember you're dead")
        return true
    elseif IsEntityDead(tGMT.getPlayerPed()) then
        tGMT.notify("~r~You try to move but you remember you're dead")
        return true
    else
        return false
...
RegisterCommand(
    "bag",
    function(f, g)
        local h = tGMT.getPlayerPed()
        if DoesEntityExist(h) and tGMT.canAnim() and not IsEntityDead(h) and d() then
...
function surrender()
    ...
    if DoesEntityExist(h) and tGMT.canAnim() and not IsEntityDead(h) and (d() or isPlayerNearPrison()) then
        tGMT.loadAnimDict("random@arrests")
        tGMT.loadAnimDict("random@arrests@busted")
...
function tGMT.isPlayerSurrendered()
...
function tGMT.isPlayerSurrenderedNoProgressBar()
    return j()
end
local u
RegisterNetEvent("GMT:startRobbingPlayer",function()
    u = GetGameTimer()
end)
RegisterNetEvent("GMT:endRobbingPlayer",function()
    u = nil
end)
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if B then
                ...
                tGMT.notify("Press ~r~[E]~w~ to take a picture!")
                B = true
            end
        end
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if y then
                ...
                Wait(120)
                tGMT.notify("Press ~r~[E]~w~ to take a toke!")
                y = true
            end
        end
        while y do
            ...
            if IsControlJustPressed(0, 38) then
                ...
                Wait(100)
                tGMT.notify("You take a huge rip!")
                Wait(7250)
            end
        end
    end,
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if v then
                ...
                Wait(120)
                tGMT.notify("Press ~r~[E]~w~ to wave the wand.")
                v = true
            end
        end
            ...
            if
             ...
             then
                if not Y and globalNHSOnDuty then
                    ...
                    Y = false
                    DetachEntity(tGMT.getPlayerPed(), false, false)
                    ClearPedSecondaryTask(tGMT.getPlayerPed())
                    local a5 = GetClosestPlayer(3)
...
RegisterNetEvent(
    ...
    function()
        ...
        Citizen.Wait(1000)
        ClearPedTasksImmediately(tGMT.getPlayerPed())
        SetTimecycleModifier("spectator5")
        SetPedMotionBlur(tGMT.getPlayerPed(), true)
        SetPedMovementClipset(tGMT.getPlayerPed(), "move_m@drunk@verydrunk", true)
        RemoveAnimSet("move_m@drunk@verydrunk")
        SetPedIsDrunk(tGMT.getPlayerPed(), true)
        DoScreenFadeIn(1000)
        ClearPedSecondaryTask(tGMT.getPlayerPed())
        DeleteObject(prop)
        Wait(60000)
        ...
        ResetPedMovementClipset(tGMT.getPlayerPed(), 0)
        SetPedIsDrunk(tGMT.getPlayerPed(), false)
        SetPedMotionBlur(tGMT.getPlayerPed(), false)
        RemoveAnimDict(H)
    end
)
RegisterNetEvent(
...
function tGMT.isPlayerKneeling()
...
RegisterNetEvent(
    ...
    function(ai)
        ...
        tGMT.loadAnimDict("misshair_shop@hair_dressers")
        ...
        tGMT.loadModel("prop_clippers_01")
