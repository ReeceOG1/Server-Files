...
local function f()
    ...
    g.position=tGMT.getPlayerCoords()
...
local function h()
    ...
    if i then 
        TriggerServerEvent("GMT:removeRPZone",i)
    end 
end
local inRPZone = false
RMenu.Add("rpzones","mainmenu",RageUI.CreateMenu("","Main Menu",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"gmt_adminui","gmt_adminui"))
RMenu.Add("rpzones","weapons",RageUI.CreateSubMenu(RMenu:Get("rpzones","mainmenu"),"","Weapons",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"gmt_adminui","gmt_adminui"))
RMenu.Add("rpzones","vehicles",RageUI.CreateSubMenu(RMenu:Get("rpzones","mainmenu"),"","Vehicles",tGMT.getRageUIMenuWidth(),tGMT.getRageUIMenuHeight(),"gmt_adminui","gmt_adminui"))
...
RegisterNetEvent("GMT:createRPZone",function(g)
...
RegisterNetEvent("GMT:removeRPZone",function(r)
...
local function G(g)
    local l,u=GetCurrentPedWeapon(tGMT.getPlayerPed())
    if u~=0 then 
        t(g,u)
    end
    local y,H=tGMT.getPlayerVehicle()
    if y~=0 and H then 
        A(g,y)
    end
...
tGMT.createThreadOnTick(I)

function tGMT.GetRPZoneInfo()
    local K=tGMT.getPlayerCoords()
    for l,g in ipairs(d)do 
        if#(K-g.position)<g.radius then
            return g
