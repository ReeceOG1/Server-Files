...
local l = 10.0
local m = {0, 0, 0, 255}
function tGMT.setMaxUnderWaterUITimenewTime(n)
    l = n
end
...
function tGMT.getCachedResolution()
    ...
    while true do
        if not globalHideUi then
            if not globalHideEmergencyCallUI then
                ...
                local E = tGMT.getShowHealthPercentageFlag()
                if C > 1.0 then
                    C = 1.0
                end
                ...
                if E then
                    tGMT.DrawText(A.leftX + A.Width / 2.0,A.bottomY - 0.0075,tostring(math.floor(B * 100)) .. "%",0.175,nil,nil,m)
                end
                ...
                elseif C > 0.0 then
                    ...
                    if E then
                        tGMT.DrawText(A.leftX + A.Width / 2.0,A.bottomY + 0.006,tostring(math.floor(C * 100)) .. "%",0.175,nil,nil,m)
                    end
                end
            end
        end
